# [Issue #26794] [Bug] update_weights_from_disk crashes for FusedMoE DeepseekV3.2 with 'narrow exceeds dim_size' (initial load OK)

source: https://github.com/sgl-project/sglang/issues/26794
state: closed | updated: 2026-09-23T00:27:00Z
labels: inactive
...
### ZhuohaoNi · 2026-07-24

I’d like to work on this. I’ll start with a focused regression test for reloading separate `gate_proj`/`up_proj` tensors, then fix the stacked-weight shard handling and run the relevant RL weight-update tests. I’ll keep the change scoped and ask for Ascend validation if the final check requires NPU hardware.

### ZhuohaoNi · 2026-07-24

Update: current `main` appears to have resolved the original layout mismatch through #26717 and #29503. I opened #32287 to add the missing regression coverage: a CPU test for separate gate/up/down reload after NPU MoE post-processing, plus a one-NPU `/update_weights_from_disk` integration test. The PR is a draft while CI validates both paths.
