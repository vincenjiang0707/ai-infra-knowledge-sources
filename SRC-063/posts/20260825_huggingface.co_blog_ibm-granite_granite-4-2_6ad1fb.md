...
Collection Efficient language models for multilingual generation, coding, RAG, and AI assistant workflows. • 15 items • Updated • 70

#
[
](https://huggingface.co#granite-42-llms-how-theyre-built)
Granite 4.2 LLMs: How They're Built
...
**Attention:**Grouped Query Attention (GQA) with 40 attention heads and 8 KV heads**Position Embedding:**Rotary Position Embedding (RoPE) with θ = 10,000,000**Feed-Forward:**MLP with SwiGLU activation**Normalization:**RMSNorm (ε = 1e-5)**Embeddings:**Separate input/output embeddings (not tied)**Precision:**bfloat16
...
###
[
](https://huggingface.co#data-quality-control)
...
Finally, we perform both local and global deduplication. Deduplication is based on SHA-256 hashes computed over the combination of the `tools`
...
The complete corpus is first globally shuffled to reduce ordering effects and ensure that samples from different domains are well mixed during training. The shuffled corpus is then partitioned into equally sized `.parquet`
...
###
[
](https://huggingface.co#phase-2-sft-for-the-30b-model)
...
##
[
](https://huggingface.co#reinforcement-learning-a-multi-stage-multi-environment-pipeline)
...
| SWE 2 | 32 | 16 | 128K | 128 |
0 | 5e-7 |
| Terminal | 8 | 32 | 64K | 64 |
0.01 | 1e-6 |
| Search | 32 | 16 | 128K | 64 |
0.01 | 5e-7 |
...
###
[
](https://huggingface.co#the-staged-curriculum)
...
###
[
](https://huggingface.co#reward-signals)
Reward Signals
...
|---|---|---|
Verifiable |
Exact-match, unit tests, format checkers, rule-based checkers on ground truth | RLVR · boosters · SWE |
Reward model / LLM judge |
Open-ended quality, preference, safety, answer correctness | RLVR · Search · RLHF |
Agentic outcome |
Did the model actually solve a task in a real environment? | SWE · Terminal · Search |
...
###
[
](https://huggingface.co#foundational-rl-build-the-skills)
Foundational RL: Build the Skills

####
[
](https://huggingface.co#rlvr-verifiable-reward-rl)
...
####
[
](https://huggingface.co#skill-boosters-targeted-lifts)
...
**Instruction following (IF):**multi-turn chat, inverse-IFEval, structured outputs**Code:**competitive coding only

Boosters are small, focused runs. A light KL penalty keeps the model close to its current behavior while nudging one skill.

###
[
](https://huggingface.co#agentic-rl-learning-to-act-8b--30b)
...
###
[
](https://huggingface.co#how-the-three-sizes-differ)
...
###
[
](https://huggingface.co#software-stack)
Software Stack

The training software stack is packaged into `.sqsh`
...
##
[
](https://huggingface.co#getting-started-transformers)
...
###
[
](https://huggingface.co#basic-inference-thinking-mode)
...
###
[
](https://huggingface.co#non-thinking-mode)
Non-Thinking Mode
...
###
[
](https://huggingface.co#low-effort-thinking)
Low-Effort Thinking
...
##
[
](https://huggingface.co#tool-calling)
Tool Calling
...
###
[
](https://huggingface.co#basic-tool-calling)
Basic Tool Calling
...
###
[
](https://huggingface.co#multi-turn-with-tool-response)
...
##
[
](https://huggingface.co#multi-turn-conversations)
Multi-Turn Conversations

###
[
](https://huggingface.co#history-thinking-truncation)
...
###
[
](https://huggingface.co#parsing-thinking-vs-final-answer)
...
##
[
](https://huggingface.co#using-with-agentic-coding-harnesses)
...
###
[
](https://huggingface.co#pi)
Pi

[Pi](https://pi.dev) is a minimal agent harness for AI-powered coding that runs in your terminal. It supports custom providers via a `models.json`
...
Then select the `granite-4.2-30b`
...
###
[
](https://huggingface.co#openhands)
OpenHands

[OpenHands](https://www.openhands.dev) is an AI software engineer that can plan, write code, and execute commands.

**Install and launch OpenHands**following the[official installation guide](https://docs.openhands.dev/openhands/usage/agent-canvas/setup).**Configure the LLM**in the OpenHands settings with:**Model:**`granite-4.2-30b`
...
prefix is required when connecting to OpenAI-compatible endpoints like vLLM. Refer to the[OpenHands local LLM documentation]for detailed setup instructions, troubleshooting, and alternative installation methods.

**Resources:**

[Granite 4.2 HF Collection](https://huggingface.co/collections/ibm-granite/granite-42-language-models)[GitHub: ibm-granite/granite-4.2-language-models](https://github.com/ibm-granite/granite-4.2-language-models)[Granite Documentation](https://www.ibm.com/granite/docs/)[Granite Community Resources](https://github.com/ibm-granite-community/)[Code Alchemy](https://research.ibm.com/blog/code-alchemy-for-synthetic-code)
