...
Any-to-Any • 12B • Updated • 148k • 749

#
[
](https://huggingface.co#welcome-gemma-4-frontier-multimodal-intelligence-on-device)
Welcome Gemma 4: Frontier multimodal intelligence on device

[Update on GitHub](https://github.com/huggingface/blog/blob/main/gemma4.md)
...
[What is New with Gemma 4?](https://huggingface.co#what-is-new-with-gemma-4)[Overview of Capabilities and Architecture](https://huggingface.co#overview-of-capabilities-and-architecture)[Multimodal Capabilities](https://huggingface.co#multimodal-capabilities)[Deploy Anywhere](https://huggingface.co#deploy-anywhere)[Multi-Token Prediction Drafters](https://huggingface.co#multi-token-prediction-drafters)[DiffusionGemma: Text Generation via Diffusion](https://huggingface.co#diffusiongemma-text-generation-via-diffusion)[Fine-tuning & Demos](https://huggingface.co#fine-tuning--demos)[Try Gemma 4](https://huggingface.co#try-gemma-4)[Benchmark Results](https://huggingface.co#benchmark-results)[Acknowledgements](https://huggingface.co#acknowledgements)

#
[
](https://huggingface.co#what-is-new-with-gemma-4)
...
[base](https://huggingface.co/google/gemma-4-E4B),[IT](https://huggingface.co/google/gemma-4-E4B-it)[base](https://huggingface.co/google/gemma-4-12B),[IT](https://huggingface.co/google/gemma-4-12B-it)[base](https://huggingface.co/google/gemma-4-31B),[IT](https://huggingface.co/google/gemma-4-31B-it)[base](https://huggingface.co/google/gemma-4-26B-A4B),[IT](https://huggingface.co/google/gemma-4-26B-A4B-it)##
[
...
- Alternating
**local sliding-window**and**global full-context**attention layers. Smaller dense models use sliding windows of 512 tokens while larger models use 1024 tokens. **Dual RoPE**configurations: standard RoPE for sliding layers, pruned RoPE for global layers, to enable longer context.**Per-Layer Embeddings (PLE)**: a second embedding table that feeds a small residual signal into every decoder layer.**Shared KV Cache**: the last N layers of the model reuse key-value states from earlier layers, eliminating redundant KV projections.**Vision encoder (except 12B)**: uses learned 2D positions and multidimensional RoPE. Preserves the original aspect ratios and can encode images to a few different token budgets (70, 140, 280, 560, 1120).**Audio encoder (E2B, E4B)**: USM-style conformer with the same base architecture as the one in Gemma-3n.**Unified encoder-free multimodality (12B only)**: the 12B Unified variant drops the separate vision and audio encoders and projects raw image patches and audio waveforms directly into the LLM's embedding space. See[Unified Multimodal (12B)](https://huggingface.co#unified-multimodal-12b).

####
[
](https://huggingface.co#per-layer-embeddings-ple)
...
####
[
](https://huggingface.co#shared-kv-cache)
Shared KV Cache
...
####
[
](https://huggingface.co#unified-multimodal-12b)
...
##
[
](https://huggingface.co#multimodal-capabilities)
...
###
[
](https://huggingface.co#multimodal-input-order)
Multimodal Input Order

Gemma 4 was trained with a specific convention to interleave input modalities:

**Image content goes**in your prompt.*before*the text**Audio content goes**in your prompt.*after*the text
...
###
[
](https://huggingface.co#object-detection-and-pointing)
Object Detection and Pointing

###
[
](https://huggingface.co#gui-detection)
GUI detection
...
###
[
](https://huggingface.co#object-detection)
Object Detection
...
###
[
](https://huggingface.co#multimodal-thinking-and-function-calling)
...
###
[
](https://huggingface.co#captioning)
Captioning
...
###
[
](https://huggingface.co#audio-question-answering)
...
###
[
](https://huggingface.co#multimodal-function-calling)
...
#
[
](https://huggingface.co#deploy-anywhere)
Deploy Anywhere
...
##
[
](https://huggingface.co#plug-in-your-local-agent)
...
###
[
](https://huggingface.co#mistralrs)
Mistral.rs
...
##
[
](https://huggingface.co#diffusiongemma-text-generation-via-diffusion)
...
**discrete diffusion**instead of token-by-token autoregression. It's built on the same 26B A4B Mixture-of-Experts foundation (25.2B total / 3.8B active parameters, 8 active experts out of 128 plus 1 shared), takes text and image inputs, generates text, and supports up to 256K context — all under the same Apache 2.0 license.
...
##
[
](https://huggingface.co#fine-tuning-with-trl)
...
###
[
](https://huggingface.co#fine-tuning-with-trl-on-vertex-ai)
...
##
[
](https://huggingface.co#fine-tuning-with-unsloth-studio)
...
##
[
](https://huggingface.co#try-gemma-4)
Try Gemma 4
...
[82.6%](https://huggingface.co/datasets/TIGER-Lab/MMLU-Pro?eval_result=google/gemma-4-26B-A4B-it)[69.4%](https://huggingface.co/datasets/TIGER-Lab/MMLU-Pro?eval_result=google/gemma-4-E4B-it)[60.0%](https://huggingface.co/datasets/TIGER-Lab/MMLU-Pro?eval_result=google/gemma-4-E2B-it)[89.2%](https://huggingface.co/datasets/MathArena/aime_2026?eval_result=google/gemma-4-31B-it)[88.3%](https://huggingface.co/datasets/MathArena/aime_2026?eval_result=google/gemma-4-26B-A4B-it)[42.5%](https://huggingface.co/datasets/MathArena/aime_2026?eval_result=google/gemma-4-E4B-it)[37.5%](https://huggingface.co/datasets/MathArena/aime_2026?eval_result=google/gemma-4-E2B-it)[84.3%](https://huggingface.co/datasets/Idavidrein/gpqa?eval_result=google/gemma-4-31B-it)[82.3%](https://huggingface.co/datasets/Idavidrein/gpqa?eval_result=google/gemma-4-26B-A4B-it)[58.6%](https://huggingface.co/datasets/Idavidrein/gpqa?eval_result=google/gemma-4-E4B-it)[43.4%](https://huggingface.co/datasets/Idavidrein/gpqa?eval_result=google/gemma-4-E2B-it)**Coding**[19.5%](https://huggingface.co/datasets/cais/hle?eval_result=google/gemma-4-31B-it)[8.7%](https://huggingface.co/datasets/cais/hle?eval_result=google/gemma-4-26B-A4B-it)[26.5%](https://huggingface.co/datasets/cais/hle?eval_result=google/gemma-4-31B-it)[17.2%](https://huggingface.co/datasets/cais/hle?eval_result=google/gemma-4-26B-A4B-it)**Vision****Audio******Long Context***Excluding Chinese language.
