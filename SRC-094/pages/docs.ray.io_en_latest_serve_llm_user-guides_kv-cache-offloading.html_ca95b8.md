source: https://docs.ray.io/en/latest/serve/llm/user-guides/kv-cache-offloading.html
lastmod: 

# KV cache offloading[#](https://docs.ray.io#kv-cache-offloading)
...
**Increased capacity**: Store more KV caches by using CPU RAM or local storage instead of relying solely on GPU memory**Cache reuse across requests**: Save and reuse previously computed KV caches for repeated or similar prompts, reducing prefill computation**Flexible storage backends**: Choose from multiple storage options including local CPU, disk, or distributed systems
...
## Deploy with LMCache[#](https://docs.ray.io#deploy-with-lmcache)
...
## Compose multiple KV transfer backends with MultiConnector[#](https://docs.ray.io#compose-multiple-kv-transfer-backends-with-multiconnector)

You can combine multiple KV transfer backends using `MultiConnector`

. This is useful when you want both local offloading and cross-instance transfer in disaggregated deployments.

### When to use MultiConnector[#](https://docs.ray.io#when-to-use-multiconnector)

Use `MultiConnector`

to combine multiple backends when you’re using prefill/decode disaggregation and want both cross-instance transfer (NIXL) and local offloading.
...
### LMCache environment variables[#](https://docs.ray.io#lmcache-environment-variables)

`LMCACHE_LOCAL_CPU`

: Set to`"True"`

to enable local CPU offloading`LMCACHE_CHUNK_SIZE`

: Size of KV cache chunks, in terms of tokens (default: 256)`LMCACHE_MAX_LOCAL_CPU_SIZE`

: Maximum CPU storage size in GB`LMCACHE_PD_BUFFER_DEVICE`

: Buffer device for prefill/decode scenarios (default: “cpu”)
...
### MultiConnector configuration[#](https://docs.ray.io#multiconnector-configuration)

`kv_connector`

: Set to`"MultiConnector"`

to compose multiple backends`kv_connector_extra_config.connectors`

: List of connector configurations to compose. Order matters—connectors earlier in the list take priority.Each connector in the list uses the same configuration format as standalone connectors


## Performance considerations[#](https://docs.ray.io#performance-considerations)
...
[Prefill/decode disaggregation](https://docs.ray.io/prefill-decode.html)- Deploy LLMs with separated prefill and decode phases[LMCache documentation](https://docs.lmcache.ai/)- LMCache configuration and features