...
When [Ian Buck](https://graphics.stanford.edu/papers/brookgpu/brookgpu.pdf) and others designed [CUDA C](https://modal.com/gpu-glossary/host-software/cuda-c), they were driven by a north star: can it be used to write a single precision vector addition (`saxpy`

) with respectable performance as a clean one-liner that’s easily understood by a C programmer? The core of the [CUDA programming model](https://modal.com/gpu-glossary/device-software/cuda-programming-model) laid down then and described in the [2008 Lindholm et al. paper](https://www.cs.cmu.edu/afs/cs/academic/class/15869-f11/www/readings/lindholm08_tesla.pdf) still persists today.
...
- A
[Load warp](https://github.com/Dao-AILab/flash-attention/blob/5c1627a7a1cda9c32cb9b937a053564e663f81bc/flash_attn/cute/flash_fwd_sm100.py#L837)to load query, key, and value tiles from global memory into shared memory - An
[MMA warp](https://github.com/Dao-AILab/flash-attention/blob/5c1627a7a1cda9c32cb9b937a053564e663f81bc/flash_attn/cute/flash_fwd_sm100.py#L953)to compute unnormalized attention scores from query and key tiles and accumulate score-weighted value tiles into the output tiles - Eight
[Softmax warps](https://github.com/Dao-AILab/flash-attention/blob/5c1627a7a1cda9c32cb9b937a053564e663f81bc/flash_attn/cute/flash_fwd_sm100.py#L1143-L1145)to compute normalized attention scores and track running stats (max, sum) - Four
[Correction warps](https://github.com/Dao-AILab/flash-attention/blob/5c1627a7a1cda9c32cb9b937a053564e663f81bc/flash_attn/cute/flash_fwd_sm100.py#L1413)to watch for updates to the normalization scale and re-normalize the output tiles - One or two
[Epilogue warps](https://github.com/Dao-AILab/flash-attention/blob/5c1627a7a1cda9c32cb9b937a053564e663f81bc/flash_attn/cute/flash_fwd_sm100.py#L1737)to store completed output tiles from shared memory into global memory
...
/[Compute Capability 10.0](https://modal.com/gpu-glossary/device-software/compute-capability). `cta_group::1`
...
issues the instruction. And we’re only working from a single warp. This is an important difference from performant Hopper MMAs, which were coordinated across an entire [warpgroup](https://modal.com/gpu-glossary/device-software/warpgroup).
...
CUDA PTX intrinsic, which is typically [mapped by the (closed-source) ptxas compiler to the MUFU.EX2 SASS instruction](https://godbolt.org/z/7e5jx9qcr).


The FA4 kernel does that too, but for [smaller attention head sizes](https://github.com/Dao-AILab/flash-attention/blob/add175637c5d54b74bc25372e49ce282d6f236fc/flash_attn/cute/flash_fwd_sm100.py#L1390-L1391) it [additionally mixes in a different exponentiation algorithm on some iterations with a tunable frequency](https://github.com/Dao-AILab/flash-attention/blob/0165c96fff7a7cd2e152aa9659f75c972a702f5d/flash_attn/cute/softmax.py#L234-L238). That implementation uses [this block of inline PTX](https://github.com/Dao-AILab/flash-attention/blob/add175637c5d54b74bc25372e49ce282d6f236fc/flash_attn/cute/utils.py#L501-L541) to compute `2 ** x`
...
on the unit interval (check out the approximation on Wolfram Alpha [here](https://www.wolframalpha.com/input?i=0.07711909*r%5E3%2B0.22756439*r%5E2%2B0.69514614*r%2B1.0+compared+to+2%5Er)). This approximation matches the output of the SFUs for `bf16`
...
means that we operate on a vector (as in [vector lanes](https://people.eecs.berkeley.edu/~pattrsn/252S98/Lec06-vector.pdf)) of two 32 bit values. You can read about a similar implementation for CPU vector instructions on Stack Overflow [here](https://stackoverflow.com/questions/47025373/fastest-implementation-of-the-natural-exponential-function-using-sse).
