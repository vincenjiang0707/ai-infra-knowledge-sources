source: https://docs.vllm.ai/projects/recipes/en/stable/CONTRIBUTING.html
lastmod: 2026-04-27

# Contributing a new recipe[¶](https://docs.vllm.ai#contributing-a-new-recipe)
...
command) and is exported as static JSON under `public/`

for
agents/tools to consume.

One recipe = one YAML file at `models/<hf_org>/<hf_repo>.yaml`

. The path
mirrors HuggingFace so the site URL `/<hf_org>/<hf_repo>`

matches
`huggingface.co/<hf_org>/<hf_repo>`
...
- Fork + clone the repo.
- Create
`models/<hf_org>/<hf_repo>.yaml`

using the schema below. - Validate:
`node scripts/build-recipes-api.mjs`

— must print`✓ JSON API: N models, 8 strategies`

with no errors. - Preview locally:
...
## Fetching HuggingFace metadata[¶](https://docs.vllm.ai#fetching-huggingface-metadata)

Before authoring, pull the model's HF config to get architecture, parameter count, and context length correct:

This dumps `config.json`

/ `params.json`

. Look at:

— names like`architectures`

`*MoE*`

,`*Mixtral*`

, presence of`num_experts`

/`num_local_experts`

/`moe.num_experts`

→ set`architecture: moe`
...
on the default variant (bf16, fp16, fp8, nvfp4, int4, etc.).

## YAML schema (authoritative)[¶](https://docs.vllm.ai#yaml-schema-authoritative)
...
## VRAM formula[¶](https://docs.vllm.ai#vram-formula)

`vram_minimum_gb = ceil(params × bytes_per_param × 1.2)`
...
-override variant points to a different base (e.g., a distilled
FP4 checkpoint), use **that** model's parameter count, not the base.

## Naming conventions[¶](https://docs.vllm.ai#naming-conventions)

- Use
for any speculative decoding feature (MTP / Eagle3 / ERNIE-MTP / qwen3_next_mtp / step3p5_mtp). Don't use`spec_decoding`

`mtp`

— the key was renamed across all recipes. - Dense models typically only get
`single_node_tp`
...
. MoE models can have all 8 strategies. - Quantized variants reuse the precision name (
`fp8`
...
). If the checkpoint is authored by someone else (e.g.`nvidia/*-NVFP4`

), set`model_id:`

inside the variant. - Tasks:
`omni`

means the model is served via vLLM-Omni (offline Python, not`vllm serve`

). The command builder hides the serve block for omni recipes and just surfaces the guide.

## Provider logo (new organizations only)[¶](https://docs.vllm.ai#provider-logo-new-organizations-only)

If `<hf_org>`

isn't already in `src/lib/providers.js`

, add an entry:

The build script fetches the logo automatically from the HuggingFace avatar at build time:

## Hardware support levels[¶](https://docs.vllm.ai#hardware-support-levels)
...
GPUs **not** in `meta.hardware`

render silently with no badge — the common
...
## Testing your recipe[¶](https://docs.vllm.ai#testing-your-recipe)
...
generated `vllm serve`

command matches what you'd expect. The Verify / Bench
popovers should open without clipping.

## Commit style[¶](https://docs.vllm.ai#commit-style)

Stage **only** the new recipe YAML (and `providers.js`
...
— generated at build time`node_modules/`

`site/`

— the legacy MkDocs output

## Questions[¶](https://docs.vllm.ai#questions)
