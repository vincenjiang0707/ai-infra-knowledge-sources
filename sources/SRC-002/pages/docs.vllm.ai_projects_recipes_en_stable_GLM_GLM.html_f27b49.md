source: https://docs.vllm.ai/projects/recipes/en/stable/GLM/GLM.html
lastmod: 2026-04-27

# GLM-4.X LLM Usage Guide[¶](https://docs.vllm.ai#glm-4x-llm-usage-guide)
...
For GLM-V series, check [here](https://docs.vllm.ai/GLM-V.html)

This guide describes how to run GLM-4.X Series with native FP8 and BF16. FP8 models have minimal accuracy loss. Unless you need strict reproducibility for benchmarking or similar scenarios, we recommend using FP8 to run at a lower cost. These models have MTP layers.
...
is usually good for prompt-heavy workloads. But you can reduce it to 16k and 8k to reduce activation memory usage and decrease latency. - vLLM conservatively use 90% of GPU memory, you can set
...
## Speculative Decoding with MTP[¶](https://docs.vllm.ai#speculative-decoding-with-mtp)
...
### Enabling MTP[¶](https://docs.vllm.ai#enabling-mtp)

To enable MTP speculative decoding, add the `--speculative-config`
...
We recommend using `--speculative-config.num_speculative_tokens 1`

for optimal performance. While higher values like 3 increase the mean acceptance length, the acceptance rate drops significantly, resulting in lower overall throughput.

### Performance Considerations[¶](https://docs.vllm.ai#performance-considerations)
...
### Step 1: Prepare Environment[¶](https://docs.vllm.ai#step-1-prepare-environment)

#### Option 1: Installation from pre-built wheels (For AMD ROCm: MI300x/MI325x/MI355x)[¶](https://docs.vllm.ai#option-1-installation-from-pre-built-wheels-for-amd-rocm-mi300xmi325xmi355x)
...
#### Option 2: Docker image[¶](https://docs.vllm.ai#option-2-docker-image)
...
### Step 2: Log in to Hugging Face[¶](https://docs.vllm.ai#step-2-log-in-to-hugging-face)

Hugging Face login:

### Step 3: Serving GLM Models[¶](https://docs.vllm.ai#step-3-serving-glm-models)

For the GLM model series, use `$model_name`

as the parameter in the serving and benchmark commands. Valid values for `$model_name`
...
### Step 4: Run Benchmark[¶](https://docs.vllm.ai#step-4-run-benchmark)
