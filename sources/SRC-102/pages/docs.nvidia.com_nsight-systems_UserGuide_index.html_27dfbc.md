source: https://docs.nvidia.com/nsight-systems/UserGuide/index.html

# User Guide[#](https://docs.nvidia.com#user-guide)

NVIDIA Nsight Systems user guide

## Preparing Your Application for Profiling[#](https://docs.nvidia.com#preparing-your-application-for-profiling)
...
### Focused Profiling[#](https://docs.nvidia.com#focused-profiling)
...
### Marking and Labeling Regions[#](https://docs.nvidia.com#marking-and-labeling-regions)

To understand what the application’s CPU threads are doing beyond CUDA function
calls, you can use the [NVIDIA Tools Extension API (NVTX)](https://github.com/NVIDIA/NVTX). When you add NVTX
...
## Profiling from the CLI[#](https://docs.nvidia.com#profiling-from-the-cli)

### Installing the CLI on Your Target[#](https://docs.nvidia.com#installing-the-cli-on-your-target)
...
#### CLI Global Options[#](https://docs.nvidia.com#cli-global-options)
...
### CLI Command Switches[#](https://docs.nvidia.com#cli-command-switches)
...
recipe |
Post process one or more existing Nsight Systems results to generate statistical
information and create various plots. See the |
sessions |
...
#### CLI Profile Command Switch Options[#](https://docs.nvidia.com#cli-profile-command-switch-options)
...
|
|
Collect DLA (Deep Learning inference Accelerator) workload trace. Only available on Windows on Arm. |
|
|
< command > |
...
process receives the following environment variables: Note NSYS_SESSION_NAME - the current session name NSYS_CALLBACK_NAME - the current callback name |
|
|
< command > |
...
process receives the following environment variables: Note NSYS_SESSION_NAME - the current session name NSYS_CALLBACK_NAME - the current callback name NSYS_REPORT_PATH - the path to the generated report file |
|
|
true, |
...
Select any combination of CPU core and uncore events, derived metrics, and CPU core single-pass metric
sets. Specify each item by name or alias. Use |
|
|
0x11,0x13,…,
|
...
selected events as
‘x,y’, |
Select the Uncore CPU Socket events to sample. Use the Note Deprecated. Use |
|
|
0,1,2,…, |
Collect Uncore metrics on the CPU socket. Multiple values can be selected, separated by commas only (no
spaces). Use the Note Deprecated. Use |
|
|
|
...
|
|
Set the CUDA graph trace granularity, launch origin, and NVTX projection mode. Syntax:
|
|
|
true, |
...
|
true, |
This switch tracks the page faults that occur when CPU code tries to access a memory page that resides on the device. Note that this feature may cause significant runtime overhead. |
|
|
true, |
This switch tracks the page faults that occur when GPU code tries to access a memory page that resides on the host. Note that this feature may cause significant runtime overhead. |
|
|
all, |
...
|
<directory paths> |
A list of directories with symbol files, separated by |
|
|
< seconds >, |
...
|
integer, |
Specify the DOCA Telemetry Service (DTS) API port. This is used for collecting high frequency NIC and Spectrum-X switch metrics. Allowed only when –nic-metrics is set to ‘hf-via-dts’ or –eth-switch-metrics-config is used. |
|
|
< seconds > |
...
|
<path> |
Path to a local JSON file that configures Spectrum-X switch metrics requirements. The file must contain
...
Linux: 1 to 1000
( |
The interval in milliseconds between each event sample collection. Not available in Nsight Systems Embedded Platforms Edition. On Linux, an integer from 1 to 1000 (default 10) that sets the actual sampling rate. On Windows on Arm, allowed values are 0 (best-effort) or from 0.022 (45 kHz) up to 1000 (1 Hz); the default is 0.5 (2 kHz). This is a limit on the maximum sampling rate. Actual rate is determined by hardware capabilities and sampled metric set. |
|
|
...
|
N/A |
WARNING: This switch is no longer supported. Please use the |
|
|
arrow, arrowdir, hdf,
jsonlines, sqlite,
...
all, cuda-visible,
|
Collect GPU Metrics from specified devices. Possible values are |
|
|
integer[,…],
|
Specify GPU Metrics sampling frequency. Accepts a single value (assigned to all selected GPUs) or a
comma-separated list with one value per GPU in the same order as |
|
|
help, alias[,…], file:<file name> |
Specify metric set for GPU Metrics. Accepts a single value (assigned to all selected GPUs) or a
comma-separated list with one value per GPU in the same order as |
|
|
help, <id1,id2,…>,
...
|
true, |
Trace GPU context switches. See the |
|
|
<tag>, |
...
|
|
Use detours for injection. If false, process injection will be performed by windows hooks which
allows it to bypass anti-cheat software. Equivalent to setting the |
|
|
true, |
...
lf, hf, hf-via-dts,
|
Collect metrics from NIC/HCA devices. The ‘hf’ option collects high frequency metrics but lacks RoCE, IPoIB, and ‘Send Waits’ metrics. The ‘hf’ option requires elevated user privileges. The ‘hf-via-dts’ option collects high frequency metrics using the DOCA Telemetry Service (DTS) API and does not require elevated privileges. The ‘lf’ option collects all available metrics but at a lower sampling frequency. The deprecated ‘true’ option is accepted for backwards compatibility and corresponds to ‘lf’. The ‘true’ option will be removed in a future release. System scope. Not available on Nsight Systems Embedded Platforms Edition. Note The |
|
|
range@domain, range,
range@*, |
...
|
true, |
Collect Reflex SDK ETW events. Available only on Windows. |
|
|
|
...
|
|
Show source file and line information, when available, for CPU sampling, CUDA API, OS runtime, and
...
|
|
Collect throughput and operations metrics from storage devices. See:
|
||
|
|
...
|
true, |
Perform system-wide injection using Windows hooks. Equivalent to setting the
|
|
|
|
...
true, false, batch,
none, |
Default is individual. If individual or true, trace each Vulkan SC workload’s GPU activity
individually. If batch, trace Vulkan SC workloads’ GPU activity in |
|
|
primary, |
If |
...
#### CLI Analyze Command Switch Options[#](https://docs.nvidia.com#cli-analyze-command-switch-options)
...
#### CLI Export Command Switch Options[#](https://docs.nvidia.com#cli-export-command-switch-options)
...
|
|
Controls export of dynamic NVTX binary payload tables (one relational table per NVTX payload schema). ‘none’ disables it; ‘all’ exports every schema; any other value is treated as a case-insensitive POSIX basic regular expression matched against schema names to export a subset. This affects SQLite, Arrow, and Arrow/Parquet directory exports only. This feature is experimental and may change in future releases. |
|
true, |
If true, overwrite all existing result files with same output filename (nsys-rep, SQLITE, HDF, JSONLINES, ARROW, ARROWDIR, PARQUETDIR). |
|
<tag> |
Print the help message. The option can take one optional argument that will be used as a tag. If a tag is provided, only options relevant to the tag will be printed. |
|
...
For example, the value By default, the time ranges have ```
...
#### CLI Finalize Command Switch Options[#](https://docs.nvidia.com#cli-finalize-command-switch-options)

After choosing the `finalize`
...
##### CLI Finalize List Subcommand[#](https://docs.nvidia.com#cli-finalize-list-subcommand)

The `finalize list`

subcommand displays pending deferred collections. Usage:
...
#### CLI Import Command Switch Options[#](https://docs.nvidia.com#cli-import-command-switch-options)

The `nsys import`

command converts a `.qdstrm`

intermediate result file into
a `.nsys-rep`

report file. A `.qdstrm`

file can be produced by the CLI before
report generation completes. Usually, Nsight Systems converts it automatically at
the end of a collection. Use `nsys import`

when automatic conversion did not
run or did not complete.

Use the same `nsys`

version that generated the `.qdstrm`

file to convert it.
The resulting `.nsys-rep`

file can be opened in the same version or a more
...
is not specified, the report is saved in the same directory
as the input file, using the input file name with the `.nsys-rep`

extension.
If the output file already exists, `nsys import`
...
|
<filename> |
Use this option to provide a different file name or path for the resulting report file. If this option
is not specified, the default output file is the input file path with the |
|
true, |
If true, overwrite the output file if it already exists. |

#### CLI Launch Command Switch Options[#](https://docs.nvidia.com#cli-launch-command-switch-options)
...
#### CLI Sessions Command Switch Subcommands[#](https://docs.nvidia.com#cli-sessions-command-switch-subcommands)
...
##### CLI Sessions List Command Switch Options[#](https://docs.nvidia.com#cli-sessions-list-command-switch-options)
...
#### CLI Shutdown Command Switch Options[#](https://docs.nvidia.com#cli-shutdown-command-switch-options)
...
#### CLI Start Command Switch Options[#](https://docs.nvidia.com#cli-start-command-switch-options)
...
#### CLI Stats Command Switch Options[#](https://docs.nvidia.com#cli-stats-command-switch-options)
...
on shipped scripts and formatters, see [Statistical Analysis](https://docs.nvidia.com/AnalysisGuide/index.html#statistical-analysis) .
...
#### CLI Status Command Switch Options[#](https://docs.nvidia.com#cli-status-command-switch-options)
...
#### CLI Stop Command Switch Options[#](https://docs.nvidia.com#cli-stop-command-switch-options)
...
### Example Single Command Lines[#](https://docs.nvidia.com#example-single-command-lines)
...
Effect: Launch the application using the legacy software-instrumented CUDA trace method instead of the default CUDA hardware trace method. Use this when CUDA hardware trace is unsupported for the workload or environment, or when comparing results against the previous CUDA trace method.
...
environment:nsys profile -c nvtx -w true -p profiler@service -e NSYS_NVTX_PROFILER_REGISTER_ONLY=0 ./app
...
(10 kHz). Profile any child processes. Generate the `report#.nsys-rep`
...
**Get list of available CPU core/uncore events, metrics, and metric sets**

```
nsys profile --cpu-metrics=help
```

Effect: Prints grammar describing how to list supported CPU core/uncore PMUs, their events and derived metrics, CPU core single-pass metric sets, and filters available for uncore events.

```
nsys profile --cpu-metrics=help:all
```

Effect: Lists the CPU core/uncore events, derived metrics, and CPU core single-pass metric sets available for sampling, the maximum number of events that can be sampled concurrently per core/uncore PMU, and a summary of supported PMUs and their filters (if applicable).
...
data for 20 seconds. Generate the `report#.nsys-rep`
...
### Example Interactive CLI Command Sequences[#](https://docs.nvidia.com#example-interactive-cli-command-sequences)
...
interest. Each `stop --defer-report`

returns quickly because report
generation is deferred, allowing the next `start`

to begin without delay.
After all collections are complete, use `nsys finalize list`

to see the
pending deferred collections and `nsys finalize`

to generate the
`.nsys-rep`

report files. Output name substitution patterns (`%h`
...
Option[#](https://docs.nvidia.com#example-output-from-stats-option)
...
### System Wide API Trace on Windows[#](https://docs.nvidia.com#system-wide-api-trace-on-windows)
...
### Symbol Locations on Linux and QNX[#](https://docs.nvidia.com#symbol-locations-on-linux-and-qnx)
...
CLI option, for example:--debug-symbols=/lib:/root/symbols

Alternatively,

`DbgFileSearchPath`

config option can be used, for example:NSYS_CONFIG_DIRECTIVES='DbgFileSearchPath="/lib:/root/symbols"' nsys profile <app>
...
environment variable to enable Debuginfod functionality.`DEBUGINFOD_URLS`

contains a list of Debuginfod servers (space separated URLs), example (local and public servers):export DEBUGINFOD_URLS="http://localhost:8002 https://debuginfod.ubuntu.com" nsys profile <app>
...
`$DEBUGINFOD_CACHE_PATH/debuginfod_client`
...
is set, then`$XDG_CACHE_HOME/debuginfod_client`

directory will be used.Otherwise, if neither DEBUGINFOD_CACHE_PATH nor XDG_CACHE_HOME are set,

`$HOME/.cache/debuginfod_client/`

directory is the default location for downloaded files.

Nsight Systems uses cached files only if Debuginfod functionality is enabled (

`DEBUGINFOD_URLS`

environment variable is set).Nsight Systems also support reading LLVM cache files (
...
### Opening Command Line Results Files for Visualization[#](https://docs.nvidia.com#opening-command-line-results-files-for-visualization)
...
### Handling Application Launchers (mpirun, deepspeed, etc)[#](https://docs.nvidia.com#handling-application-launchers-mpirun-deepspeed-etc)
...
#### Profile a Single Process or a Subset of Processes[#](https://docs.nvidia.com#profile-a-single-process-or-a-subset-of-processes)
...
#### DeepSpeed[#](https://docs.nvidia.com#deepspeed)
...
#### Torchrun/Pytorch[#](https://docs.nvidia.com#torchrun-pytorch)
...
The GUI launch script, `nsys-ui`

, is located in the host directory in your
Nsight Systems installation.

### Profiling Linux Targets from the GUI[#](https://docs.nvidia.com#profiling-linux-targets-from-the-gui)

#### Connecting to the Target Device[#](https://docs.nvidia.com#connecting-to-the-target-device)
...
passwordless login using `root`

username. To enable such a configuration, edit
...
#### System-Wide Profiling Options[#](https://docs.nvidia.com#system-wide-profiling-options)

#### Target Sampling Options[#](https://docs.nvidia.com#target-sampling-options)
...
#### Hotkey Trace Start/Stop[#](https://docs.nvidia.com#hotkey-trace-start-stop)
...
#### Launching Processes[#](https://docs.nvidia.com#launching-processes)
...
### Profiling Windows Targets from the GUI[#](https://docs.nvidia.com#profiling-windows-targets-from-the-gui)

Profiling on Windows devices is similar to the profiling on Linux devices.
Please refer to the [Profiling Linux Targets from the GUI](https://docs.nvidia.com/index.html#profiling-linux-targets-from-the-gui) section for the detailed
documentation and connection information. The major differences on the platforms
are listed below:

#### Remoting to a Windows Based Machine[#](https://docs.nvidia.com#remoting-to-a-windows-based-machine)
...
#### Hotkey Trace Start/Stop[#](https://docs.nvidia.com#id2)
...
#### Target Sampling Options on Windows[#](https://docs.nvidia.com#target-sampling-options-on-windows)
...
#### Symbol Locations[#](https://docs.nvidia.com#symbol-locations)
...
**Debugging Tools for Windows**, a part of the[Windows 10 SDK](https://developer.microsoft.com/en-us/windows/downloads/windows-10-sdk).Add the symbol server URL using the

**Add Server**button.Information about Microsoft’s public symbol server, which enables getting Windows operating system related debug symbols can be found

[here](https://docs.microsoft.com/en-us/windows-hardware/drivers/debugger/microsoft-public-symbols).

### Profiling QNX Targets from the GUI[#](https://docs.nvidia.com#profiling-qnx-targets-from-the-gui)

Profiling on QNX devices is similar to the profiling on Linux devices. Please
refer to the [Profiling Linux Targets from the GUI](https://docs.nvidia.com/index.html#profiling-linux-targets-from-the-gui)
section for the detailed documentation. The major differences on the platforms
are listed below:

Backtrace sampling is not supported. Instead backtraces are collected for long OS runtime libraries calls. Please refer to the

[OS Runtime Libraries Trace](https://docs.nvidia.com/index.html#os-runtime-libraries-trace)section for the detailed documentation.CUDA support is limited to CUDA 9.0+.
...
### Nsight Streamer for Nsight Systems[#](https://docs.nvidia.com#nsight-streamer-for-nsight-systems)
...
NGC](https://catalog.ngc.nvidia.com/orgs/nvidia/teams/devtools/containers/nsight-streamer-nsys)

### GUI VNC container[#](https://docs.nvidia.com#gui-vnc-container)
...
[Docker](https://www.docker.com), and Python 3.5 or later.
...
## Profiling within JupyterLab[#](https://docs.nvidia.com#profiling-within-jupyterlab)
...
extension on PyPI](https://pypi.org/project/jupyterlab-nvidia-nsight/)
...
at the top of the cell and a shell command to`nsys stop`

at the bottom of the cell. We recommend using the the absolute path to`nsys`

on your system to make sure it is found.Save the notebook.
...
is run, a new .nsys-rep file will be generated.Open the nsys-rep file in nsys-ui.


## Container, Scheduler, and Cloud Support[#](https://docs.nvidia.com#container-scheduler-and-cloud-support)

### Collecting Data Within a Container[#](https://docs.nvidia.com#collecting-data-within-a-container)
...
a shared folder between the containers. See section on [GUI VNC container](https://docs.nvidia.com#gui-vnc-container)
for more information.

#### Enable Docker Collection[#](https://docs.nvidia.com#enable-docker-collection)
...
#### Launch Docker Collection[#](https://docs.nvidia.com#launch-docker-collection)
...
### Profiling Services in the Cloud[#](https://docs.nvidia.com#profiling-services-in-the-cloud)
...
download see:
[NVIDIA Nsight Cloud](https://catalog.ngc.nvidia.com/orgs/nvidia/teams/devtools/collections/nsight-cloud).

#### Profiling Services Launched via Kubernetes (Nsight Operator)[#](https://docs.nvidia.com#profiling-services-launched-via-kubernetes-nsight-operator)
...
[NGC Nsight Operator](https://catalog.ngc.nvidia.com/orgs/nvidia/teams/devtools/helm-charts/nsight-operator).

#### Streaming GUI (Nsight Streamer)[#](https://docs.nvidia.com#streaming-gui-nsight-streamer)
...
### Profiling Slurm Jobs Running in Containers (Enroot/Pyxis or OCI)[#](https://docs.nvidia.com#profiling-slurm-jobs-running-in-containers-enroot-pyxis-or-oci)
...
to profile the workload, but the CLI (`nsys`

) itself must be made available
*inside* the container that Slurm launches, not just on the host.

The recommended pattern is **“inject, don’t rebuild”**: keep `nsys`

installed
once on the bare-metal compute node and bind-mount it into the container at
launch time, then wrap the in-container command with `nsys profile`
...
) on the bare-metal Slurm compute nodes.Bind-mount that install into the container at the same absolute path via the container runtime’s configuration.

Invoke

`nsys profile`

by absolute path (or, if you prefer, extend the container’s`PATH`

without replacing it). A wrapper script is only needed when you want per-rank logic.
...
output templates, and `perf_event_open`

apply here.

#### Install Nsight Systems on the Target Compute Nodes[#](https://docs.nvidia.com#install-nsight-systems-on-the-target-compute-nodes)
...
used to open a report should be the **same or newer** than the
CLI that produced it. Reports from a newer CLI opened in an older GUI are
the common failure mode. The `.qdrep`
...
##### Host kernel requirements[#](https://docs.nvidia.com#host-kernel-requirements)
...
is permissive enough on the host (typically`<= 2`

). You can verify readiness from inside the container with`nsys status --environment`

.The container has access to

`perf_event_open`

— see “Enable Docker Collection” above. For OCI runtimes this means`--cap-add=SYS_ADMIN`

,`--privileged`

, or a custom seccomp profile that allows`perf_event_open`

. Enroot launches containers unprivileged and, by default, does not install Docker’s restrictive seccomp profile on top of the host policy, so`perf_event_open`

is typically usable without extra flags — but verify with`nsys status --environment`

inside the container before assuming it.CUDA/NVIDIA driver libraries are already made available in the container by the runtime (NVIDIA Container Toolkit for OCI,

`--container-mounts`

+ the NVIDIA hook for Pyxis/Enroot). The Nsight Systems bind mount does not change that.

#### Make `nsys`

Visible Inside the Container[#](https://docs.nvidia.com#make-nsys-visible-inside-the-container)

##### Option A — Pyxis/Enroot via `srun --container-mounts`

[#](https://docs.nvidia.com#option-a-pyxis-enroot-via-srun-container-mounts)

Pyxis (the Slurm SPANK plugin for Enroot) exposes Enroot container launches as
`srun`

flags. The `--container-mounts`

flag accepts
`SRC:DST[:FLAGS][,SRC:DST...]`

(multiple flags are joined with `+`

, e.g.
`ro+rprivate`

). Bind-mount the host install into the container and invoke
`nsys`
...
`--container-mounts=<host>:<container>:ro`

bind-mounts the host install read-only.`ro`

is enough — the profiler does not write inside its own install tree.Calling

`nsys`

via its absolute in-container path avoids touching the image’s`PATH`

at all. If you prefer`nsys`

on`PATH`

(for example for interactive use), append it*inside*the container rather than replacing the image`PATH`

:srun \ --container-image=nvcr.io#nvidia/pytorch:<tag> \ --container-mounts=/opt/nvidia/nsight-systems/2026.2.1:/opt/nvidia/nsight-systems/2026.2.1:ro \ --export=ALL,NSYS_PATH=/opt/nvidia/nsight-systems/2026.2.1/bin \ --container-env=NSYS_PATH \ bash -lc 'PATH="$PATH:$NSYS_PATH"; nsys profile -o /reports/run_%q{SLURM_PROCID}_%p python train.py'

If you need

`nsys`

reports to land on a shared filesystem, also bind-mount the output directory (e.g.`--container-mounts=...,/scratch/$USER/reports:/reports:rw`
...
to the command line, do a quick container-runtime
preflight:
...
##### Option B — Enroot `mounts.d`

+ `environ.d`

(recommended, image-independent)[#](https://docs.nvidia.com#option-b-enroot-mounts-d-environ-d-recommended-image-independent)

When you do not control the `srun`

invocation (for example a fleet-wide
Slurm setup), Enroot can inject the mount and extend `PATH`

via its
standard configuration directories under `ENROOT_SYSCONF_PATH`

(default
`/etc/enroot`

). This is the idiomatic Enroot approach and does not require
...
##### Option C — Enroot pre-start hook (only if you need logic)[#](https://docs.nvidia.com#option-c-enroot-pre-start-hook-only-if-you-need-logic)
...
##### Option D — OCI runtime (Docker / Podman / containerd / CRI-O)[#](https://docs.nvidia.com#option-d-oci-runtime-docker-podman-containerd-cri-o)

For containers launched through an OCI runtime under Slurm (for example via
`srun --container-...`

wired to a Docker or container orchestrator, or via
a site-specific launcher), declare the bind mount in the container
configuration and invoke `nsys`

by its absolute in-container path so the
image’s own `PATH`

and other environment variables are preserved.
...
(for a container spec consumed by
`runc`
...
(or the custom seccomp profile described in “Enable Docker Collection”) so`perf_event_open`

is not blocked.Calling

`nsys`

by absolute path means the image’s own`PATH`

(and any other`ENV`

set by the image) is preserved. If you’d rather have`nsys`

on`PATH`

, append inside the container (`PATH="$PATH:/opt/nvidia/nsight-systems/2026.2.1/bin"`

) instead of replacing`PATH`

via`-e`

/`process.env`

.GPU exposure is still the runtime’s responsibility (

`--gpus all`

/ NVIDIA Container Toolkit hook /`nvidia.com/gpu`

device plugin). The Nsight Systems mount does not change that.If

`-o`

writes to a host directory, add a second bind mount for it (`-v /scratch/$USER/reports:/reports:rw`

or a corresponding`mounts`

entry) — the profiler cannot write through a read-only mount.

#### Wrap the In-Container Command with `nsys profile`

[#](https://docs.nvidia.com#wrap-the-in-container-command-with-nsys-profile)
...
runs outside the profiler and `nsys profile`
...
so concurrent ranks do not
clobber each other’s report file. `%h`
...
##### Profiling only a subset of ranks[#](https://docs.nvidia.com#profiling-only-a-subset-of-ranks)
...
binary. Example `nsys_profile.sh`
...
##### GPU and NIC metrics with multiple ranks per node[#](https://docs.nvidia.com#gpu-and-nic-metrics-with-multiple-ranks-per-node)
...
Nsight Systems ships a dedicated skill pack. For what it is,
how to point an agent at it, and the subjects it covers, see
[AI-Assisted Analysis](https://docs.nvidia.com/AnalysisGuide/index.html#ai-assisted-analysis) in the **Post-Collection Analysis Guide**.

## Custom ETW Trace[#](https://docs.nvidia.com#custom-etw-trace)
...
### D3D12 API Trace[#](https://docs.nvidia.com#d3d12-api-trace)
...
, and`ID3D12Device8::CreateCommittedResource2`

A warning will be reported if

`D3D12_HEAP_FLAG_CREATE_NOT_ZEROED`

is not set in the method’s`HeapFlags`
...
CPU page or from a`D3D12_HEAP_TYPE_UPLOAD`
...
CPU page or to a`D3D12_HEAP_TYPE_READBACK`
...
## WDDM Queues[#](https://docs.nvidia.com#wddm-queues)
...
Enabling the “Extensive trace” option will also capture extended DxgKrnl events from the `Microsoft-Windows-DxgKrnl`

provider, such as Hardware Scheduling queues, context status, allocations, sync wait, signal events, etc.
...
## WDDM HW Scheduler[#](https://docs.nvidia.com#wddm-hw-scheduler)
...
## Vulkan API Trace[#](https://docs.nvidia.com#vulkan-api-trace)

### Vulkan Overview[#](https://docs.nvidia.com#vulkan-overview)
...
### Pipeline Creation Feedback[#](https://docs.nvidia.com#pipeline-creation-feedback)
...
### Vulkan GPU Trace Notes[#](https://docs.nvidia.com#vulkan-gpu-trace-notes)
...
## Stutter Analysis[#](https://docs.nvidia.com#stutter-analysis)
...
### FPS Overview[#](https://docs.nvidia.com#fps-overview)
...
and D3D12 calls to`ID3D12CommandQueue::ExecuteCommandList`
...
### Frame Health[#](https://docs.nvidia.com#frame-health)
...
### Windows GPU Memory Utilization[#](https://docs.nvidia.com#windows-gpu-memory-utilization)
...
memory allocation graph, see [CUDA GPU Memory Allocation Graph](https://docs.nvidia.com#cuda-gpu-memory-allocation-graph) for that
functionality.
...
marker or selecting it in the [Events view](https://docs.nvidia.com/index.html#events-view) will display
...
### Vertical Synchronization[#](https://docs.nvidia.com#vertical-synchronization)

The VSYNC rows display when the monitor’s vertical synchronizations occur.

## CUDA Trace[#](https://docs.nvidia.com#cuda-trace)

### Basic CUDA trace[#](https://docs.nvidia.com#basic-cuda-trace)
...
#### CUDA trace methods[#](https://docs.nvidia.com#cuda-trace-methods)
...
selects this default trace method. Hardware trace usually
has lower overhead than the legacy software-instrumented trace, especially for
workloads that launch many short kernels.

If CUDA hardware trace cannot be collected, Nsight Systems may automatically use the legacy software-instrumented trace instead. The Diagnostics Summary page reports which CUDA trace method was collected and whether a fallback occurred.
...
to request software trace explicitly.

Additional configuration parameters are available:

**System-wide CUDA trace**- collect CUDA trace from eligible processes across the system that are launched after collection starts. By default, CUDA trace is captured from the target process and its descendants only. Select this option to switch to system-wide trace mode.System-wide CUDA trace requirements:

Processes running as a different user are not traced.

POSIX root Nsight Systems sessions trace CUDA processes running as root without additional setup.

POSIX non-root Nsight Systems sessions trace same-user CUDA processes only when

`CUDA_INJECTION_SHM_ALLOWED=TRUE`

is set in the target process environment before launch.Windows elevated Nsight Systems sessions trace same-user CUDA processes without additional setup.

Windows non-elevated Nsight Systems sessions trace non-elevated same-user CUDA processes only when

`CUDA_INJECTION_SHM_ALLOWED=TRUE`

is set in the target process environment before launch.

Only one session can collect system-wide CUDA trace at a time on the entire system; other sessions will fall back to process-tree scope.

**CUDA trace method**- by default, CUDA trace uses hardware trace on supported GPUs, beginning with Blackwell. Use the legacy software trace for MPS workloads, MIG partitions, vGPU or virtual machine environments, Confidential Compute systems, unsupported GPUs or drivers, or when you need to compare against the previous CUDA trace method. In the CLI, use`--trace=cuda-sw`

to request software trace explicitly.**Collect backtraces for API calls longer than X seconds**- turns on collection of CUDA API backtraces and sets the minimum time a CUDA API event must take before its backtraces are collected. Setting this value too low can cause high application overhead and seriously increase the size of your results file.**Flush data periodically**— specifies the period after which an attempt to flush CUDA trace data will be made. Normally, in order to collect full CUDA trace, the application needs to finalize the device used for CUDA work (call`cudaDeviceReset()`
...
**Skip some API calls**— avoids tracing insignificant CUDA Runtime API calls (namely,`cudaConfigureCall()`

,`cudaSetupArgument()`

,`cudaHostGetDevicePointers()`

). Not tracing these functions allows Nsight Systems to significantly reduce the profiling overhead, without losing any interesting data.**Collect GPU Memory Usage**- collects information used to generate a graph of CUDA allocated memory across time. Note that this will increase overhead. See[CUDA GPU Memory Allocation Graph](https://docs.nvidia.com#cuda-gpu-memory-allocation-graph).**Collect Unified Memory CPU page faults**- collects information on page faults that occur when CPU code tries to access a memory page that resides on the device. See[Unified Memory CPU Page Faults](https://docs.nvidia.com#unified-memory-cpu-page-faults).**Collect Unified Memory GPU page faults**- collects information on page faults that occur when GPU code tries to access a memory page that resides on the CPU. See[Unified Memory GPU Page Faults](https://docs.nvidia.com#unified-memory-gpu-page-faults).**Collect CUDA Graph trace**- by default, CUDA tracing will collect and expose information on a whole graph basis. The user can opt to collect on a node per node basis. See[CUDA Graph Trace](https://docs.nvidia.com#cuda-graph-trace).**Collect CUDA Event trace**- track device-side CUDA Event (the synchronization mechanism) completion, and get better correlation support among CUDA Event APIs.[CUDA Event Trace](https://docs.nvidia.com#cuda-event-trace).For Nsight Systems Workstation Edition,
...
### Launching NVIDIA Nsight Compute from a CUDA Kernel[#](https://docs.nvidia.com#launching-nvidia-nsight-compute-from-a-cuda-kernel)
...
### CUDA GPU Memory Allocation Graph[#](https://docs.nvidia.com#cuda-gpu-memory-allocation-graph)
...
Windows target. See [Windows GPU Memory Utilization](https://docs.nvidia.com#windows-gpu-memory-utilization).
...
profiled. See: [CUPTI documentation](https://docs.nvidia.com/cuda/cupti/index.html).
...
### Unified Memory Transfer Trace[#](https://docs.nvidia.com#unified-memory-transfer-trace)
...
**Coherence**Unified Memory migration occurred to guarantee data coherence. SMs (streaming multiprocessors) stop until the migration completes.

**Eviction**Unified Memory migrated to the CPU because it was evicted to make room for another block of memory on the GPU. This happens due to memory overcommitment which is available on Linux with Compute Capability ≥ 6.


#### Unified Memory CPU Page Faults[#](https://docs.nvidia.com#unified-memory-cpu-page-faults)
...
#### Unified Memory GPU Page Faults[#](https://docs.nvidia.com#unified-memory-gpu-page-faults)
...
### CUDA Graph Trace[#](https://docs.nvidia.com#cuda-graph-trace)
...
The optional launch origin and NVTX projection mode modifiers can appear in any order after the granularity.

When CUDA graph trace is set to `graph`

, the user sees each graph as one item
on the timeline:

When CUDA graph trace is set to `node`

, the user sees each graph as a set of
...
granularity is `graph`

. Otherwise, the default granularity is `node`

.

Use the `host-only`

launch origin to trace only CUDA graphs launched from host
code. Use `host-and-device`

to trace CUDA graphs launched from both host code
and device code. For graph granularity, `host-and-device`

requires CUDA driver
version 12.3 or higher. For node granularity, `host-and-device`

requires
hardware trace with CUDA driver version 13.0 or higher, and `--trace=cuda`

must
be enabled without `--trace=cuda-sw`

. Tracing graphs launched from device code
may cause significant runtime overhead.

If granularity is set to `graph`

and CUDA driver version 12.3 or higher is
available, the default launch origin is `host-and-device`

. For `node`

, the
default launch origin is `host-only`

unless hardware trace is enabled and CUDA
driver version 13.0 or higher is available.

Use `nvtx-live`

to project NVTX ranges for CUDA graphs constructed while
profiling is active. This is the default NVTX projection mode and adds no
overhead.

Use `nvtx-precapture`

to record CUDA API and NVTX events during graph
...
collection starts. This mode is experimental,
adds overhead during graph construction, and increases memory usage. It requires
`node`

granularity, CUDA tracing with `--trace=cuda`

or `-t cuda`

, and NVTX
tracing with `--trace=nvtx`

or `-t nvtx`

.

### CUDA Event Trace[#](https://docs.nvidia.com#cuda-event-trace)
...
option, or in the GUI by setting the appropriate drop
down.

When CUDA event trace is set to `true`

, users can see device-side CUDA event
...
### CUDA Python Backtrace[#](https://docs.nvidia.com#cuda-python-backtrace)
...
### CUDA Functions Skipped by Default[#](https://docs.nvidia.com#cuda-functions-skipped-by-default)
...
### cuDNN Function List for X86 CLI[#](https://docs.nvidia.com#cudnn-function-list-for-x86-cli)
...
## OpenMP Trace[#](https://docs.nvidia.com#openmp-trace)
...
## OS Runtime Libraries Trace[#](https://docs.nvidia.com#os-runtime-libraries-trace)
...
**Waiting**— the thread is not scheduled on a CPU, it is inside of an OS runtime libraries trace and is believed to be waiting on the firmware to complete a request.**In OS runtime library function**— the thread is scheduled on a CPU and inside of an OS runtime libraries trace. If the trace represents a system call, the process is likely running in kernel mode.
...
parameter. See
[Command Line Options](https://docs.nvidia.com/index.html#command-line-options) for more information.
...
### Locking a Resource[#](https://docs.nvidia.com#locking-a-resource)
...
### OS Runtime Libraries Trace Filters[#](https://docs.nvidia.com#os-runtime-libraries-trace-filters)
...
### OS Runtime Default Function List[#](https://docs.nvidia.com#os-runtime-default-function-list)
...
## Syscall Trace[#](https://docs.nvidia.com#syscall-trace)
...
process, see the capabilities [man page](https://man7.org/linux/man-pages/man7/capabilities.7.html) for details.

To enable this feature:

**CLI** — Add the `--syscall`

option to the `nsys start`

or `nsys profile`

commands (setting `syscall`

in the `--trace`

option is deprecated and will be ignored). The following values are supported:

`none`

— No syscall tracing [default].`process-tree`

— Collect syscalls for the profiled application process and its child processes.`pid-namespace`

— Collect syscalls made by all processes in the current PID namespace and its child namespaces. This is very close to how other features work in the`system-wide`

mode, e.g. inside a container, tracing will be limited to this container.

**GUI** — Select the **Collect syscall trace** checkbox. Currently, equivalent to the `--syscall=process-tree`
...
## NVTX Trace[#](https://docs.nvidia.com#nvtx-trace)
...
[NVTX GitHub repository](https://nvidia.github.io/NVTX/).
...
or the [NVTX documentation](https://nvidia.github.io/NVTX/).
...
Registration](https://docs.nvidia.com/cuda/profiler-users-guide/index.html#nvtx-string-registration).
...
in your source code. The nvtx3 directory is located in the Nsight Systems package in the`Target-<architecture>/nvtx/include`

directory and is available via github at[NVIDIA/NVTX](http://github.com/NVIDIA/NVTX).Add the following compiler flag:
...
In the project settings, select the

**Collect NVTX trace**checkbox.
...
and a nested range for each loop iteration.
...
A complete program that calls `some_function()`

does not require linking to
...
If the function returns or throws before calling `nvtxRangePop`
...
## OpenACC Trace[#](https://docs.nvidia.com#openacc-trace)
...
## OpenGL Trace[#](https://docs.nvidia.com#opengl-trace)
...
**CPU trace**: interception of API calls that an application does to APIs (such as OpenGL, OpenGL ES, EGL, GLX, WGL, etc.).**GPU trace**(or**workload trace**): trace of GPU workload (activity) triggered by use of OpenGL or OpenGL ES. Since draw calls are executed back-to-back, the GPU workload trace ranges include many OpenGL draw calls and operations in order to optimize performance overhead, rather than tracing each individual operation.
...
### OpenGL Trace Using Command Line[#](https://docs.nvidia.com#opengl-trace-using-command-line)

For general information on using the target CLI, see [CLI Profiling on Linux](https://docs.nvidia.com/index.html#cli-profiling). For the CLI, the functions that are traced are set to the following list:
...
## OpenXR API Trace[#](https://docs.nvidia.com#openxr-api-trace)
...
## GPU Hardware Profiling[#](https://docs.nvidia.com#gpu-hardware-profiling)

### GPU Context Switch[#](https://docs.nvidia.com#gpu-context-switch)
...
### GPU Metrics[#](https://docs.nvidia.com#gpu-metrics)
...
**IO throughputs:**PCIe, NVLink, and GPU memory bandwidth**SM utilization:**SMs activity, tensor core activity, instructions issued, warp occupancy, and unassigned warp slots
...
Counters](https://developer.nvidia.com/ERR_NVGPUCTRPERM) for more information.
...
#### Launching GPU Metrics from the CLI[#](https://docs.nvidia.com#launching-gpu-metrics-from-the-cli)

GPU Metrics feature is controlled with 3 CLI switches:

`--gpu-metrics-devices=[none|all|cuda-visible|(<index>[,...])]`

selects GPUs to sample (default is none).`--gpu-metrics-set=[<alias>|file:<file name>][,...]`

selects the metric set per GPU (default is the 1st suitable from the list). A single value is assigned to all selected GPUs; a comma-separated list assigns one metric set per GPU, in the same order as`--gpu-metrics-devices`

.`--gpu-metrics-frequency=[10..200000][,...]`

selects sampling frequency in Hz per GPU (default is 10000). A single value is assigned to all selected GPUs; a comma-separated list assigns one frequency per GPU, in the same order as`--gpu-metrics-devices`
...
order as `--gpu-metrics-devices`

). To see metric sets available for the
selected GPUs, use:
...
To assign different metric sets per GPU (GPU 0 with `tu10x`

, GPU 1 with `tu10x-gfxt`
...
#### Launching GPU Metrics from the GUI[#](https://docs.nvidia.com#launching-gpu-metrics-from-the-gui)

For commands to launch GPU Metrics from the CLI with examples, see
[Profiling from the CLI](https://docs.nvidia.com#cli-profiling).
...
Select the **Metric set:** dropdown to choose which available metric set you
...
#### Sampling frequency[#](https://docs.nvidia.com#sampling-frequency)
...
#### Available metrics[#](https://docs.nvidia.com#available-metrics)
...
**VRAM Write Bandwidth**-`FBPA.TriageA.dramc__write_throughput.avg.pct_of_peak_sustained_elapsed`
...
#### Exporting and Querying Data[#](https://docs.nvidia.com#exporting-and-querying-data)
...
#### Limitations[#](https://docs.nvidia.com#id10)
...
### NVML power and temperature metrics[#](https://docs.nvidia.com#nvml-power-and-temperature-metrics)
...
These metrics are provided by the NVML API calls `nvmlDeviceGetFieldValues`

and
`nvmlDeviceGetTemperature`

respectively. The power metrics are provided in Watts (W) and
...
For general information on Nsight Systems plugins please refer to [Nsight Systems Plugins](https://docs.nvidia.com#nsight-systems-plugins)
system.

### DCGM[#](https://docs.nvidia.com#dcgm)
...
information, see [DCGM Documentation](https://docs.nvidia.com/datacenter/dcgm/latest/index.html)
...
prefix selects preset metric groups defined in counter_groups.yaml;
available presets are `gpu`
...
## SoC Metrics[#](https://docs.nvidia.com#soc-metrics)

### Overview[#](https://docs.nvidia.com#id11)
...
### Available metrics[#](https://docs.nvidia.com#id12)

**CPU Read Throughput**`mcc__dram_throughput_srcnode_cpu_op_read.avg.pct_of_peak_sustained_elapsed`
...
**GPU Read Throughput**`mcc__dram_throughput_srcnode_gpu_op_read.avg.pct_of_peak_sustained_elapsed`
...
**DBB Read Throughput**`mcc__dram_throughput_srcnode_dbb_op_read.avg.pct_of_peak_sustained_elapsed`
...
**DRAM Read Throughput**`mcc__dram_throughput_op_read.avg.pct_of_peak_sustained_elapsed`

**DRAM Write Throughput**`mcc__dram_throughput_op_write.avg.pct_of_peak_sustained_elapsed`
...
**DLA0/DLA1 Active**`nvdla__cycles_active.avg.pct_of_peak_sustained_elapsed`
...
**DLA0/DLA1 Read Throughput**`nvdla__dbb2nvdla_read_throughput.avg.pct_of_peak_sustained_elapsed`

**DLA0/DLA1 Write Throughput**`nvdla__nvdla2dbb_write_throughput.avg.pct_of_peak_sustained_elapsed`
...
**NVENC Active**`nvenc__cycles_active.avg.pct_of_peak_sustained_elapsed`


The ratio of cycles the NVENC unit was actively processing a command to the number of cycles in the same sample period as a percentage.

**NVENC Read Throughput**`nvenc__memif2nvenc_read_throughput.avg.pct_of_peak_sustained_elapsed`
...
**PVA VPU Active**`pvavpu__vpu_cycles_active.avg.pct_of_peak_sustained_elapsed`
...
**PVA DMA Read Throughput**`pva__dbb2pvadma_read_throughput.avg.pct_of_peak_sustained_elapsed`
...
**OFA Active**`ofa_cycles_active.avg.pct_of_peak_sustained_elapsed`


The ratio of cycles the OFA (Optical Flow Accelerator) was actively processing a command to the number of cycles in the same sample period as a percentage.

**OFA Read Throughput**`ofa__memif2ofa_read_throughput.avg.pct_of_peak_sustained_elapsed`

**OFA Write Throughput**`ofa__ofa2memif_write_throughput.avg.pct_of_peak_sustained_elapsed`


The ratio of cycles the OFA (Optical Flow Accelerator) was actively processing read/write operations to the number of cycles in the same sample period as a percentage.

**VIC Active**`vic_cycles_active.avg.pct_of_peak_sustained_elapsed`
...
**VIC Read Throughput**`vic__dbb2vic_read_throughput.avg.pct_of_peak_sustained_elapsed`
...
### Launching SoC Metrics from the CLI[#](https://docs.nvidia.com#launching-soc-metrics-from-the-cli)
...
selects sampling frequency in Hz (default is 10000). Abbreviated suffixes`k`

and`M`

are accepted (e.g.`10k`
...
### Launching SoC Metrics from the GUI[#](https://docs.nvidia.com#launching-soc-metrics-from-the-gui)

When launching analysis in Nsight Systems, select **Collect SoC Metrics**.

The settings are similar to [GPU Metrics](https://docs.nvidia.com/index.html#launching-gpu-metrics-from-the-gui).

For commands to launch SoC Metrics from the CLI with examples, see the
[CLI documentation](https://docs.nvidia.com/index.html#profiling-from-the-cli).

## CPU Profiling on Linux[#](https://docs.nvidia.com#cpu-profiling-on-linux)
...
**CPU Context Switch Tracing**Nsight Systems can trace every time the OS schedules a thread on a logical CPU and every time the OS thread gets unscheduled from a logical CPU. The data is used to show CPU utilization and OS thread utilization within the Nsight Systems GUI. Context switch collections can be configured in process-tree or system-wide mode. In process-tree mode, Nsight Systems will trace the process, and any of its descendants, launched by Nsight Systems. In system-wide mode, Nsight Systems will trace all processes running on the system, including any processes launched by the Nsight Systems.

**CPU Event and Metric Sampling**Nsight Systems can periodically sample CPU core and uncore hardware event counts, OS event counts, and derive metrics. CPU core events and metrics can also be selected using single-pass metric sets. The data is collected system-wide across all CPUs and is not directly associated with individual processes or threads. Nsight Systems displays event rates and metric values over time in the GUI.

Use

`--cpu-metrics=help`

to list supported CPU core and uncore PMUs, their events and derived metrics, CPU core single-pass metric sets, and filters available for uncore events. Pass an event, metric, or metric set to`--cpu-metrics`

by name or alias, adding optional filters to uncore events. For tuning guidance, see the[Grace Performance Tuning Guide](https://docs.nvidia.com/grace-performance-tuning-guide.pdf).In this version of Nsight Systems,

`--cpu-metrics`

is available on Linux for NVIDIA Vera CPU, NVIDIA Grace CPU, NVIDIA GB10 Grace Blackwell Superchip (for example, NVIDIA DGX Spark), and NVIDIA Thor (for example, NVIDIA Jetson AGX Thor). Uncore event and metric support is more limited. On Windows on Arm,`--cpu-metrics`

is available for NVIDIA Arm-based CPUs (for example, on NVIDIA RTX Spark) without utilizing the perf subsystem.

### System Requirements[#](https://docs.nvidia.com#system-requirements)

**Paranoid Level**The

[system’s paranoid level](https://www.kernel.org/doc/Documentation/sysctl/kernel.txt)must be 2 or lower.
...
**Kernel Version**To support the CPU profiling features utilized by Nsight Systems, the kernel version must be greater than or equal to v4.3. RedHat has backported the required features to the v3.10.0-693 kernel. RedHat distros and their derivatives (e.g. CentOS) require a 3.10.0-693 or later kernel. Use the

`uname -r`

command to check the kernel’s version.**perf_event_open syscall**The perf_event_open syscall needs to be available. When running within a Docker container, the default seccomp settings will normally block the perf_event_open syscall. To workaround this issue, use the Docker

`run --privileged`

switch when launching the docker or modify the docker’s seccomp settings. Some VMs (virtual machines), e.g. AWS, may also block the perf_event_open syscall.**Sampling Trigger**In some rare case, a sampling trigger is not available. The sampling trigger is either a hardware or software event that causes a sample to be collected. Some VMs block hardware events from being accessed and therefore, prevent hardware events from being used as sampling triggers. In those cases, Nsight Systems will fall back to using a software trigger if possible.

**Checking Your Target System**Use the

`nsys status --environment`

command to check if a system meets the Nsight Systems CPU profiling requirements. Example output from this command is shown below. Note that this command does not check for Linux capability overrides - i.e., if the user or executable files have`CAP_SYS_ADMIN`

or`CAP_PERFMON`

capability. Also, note that this command does not indicate if system-wide mode can be used.

### Configuring a CPU Profiling Collection[#](https://docs.nvidia.com#configuring-a-cpu-profiling-collection)
...
#### Collecting Source File and Line Information[#](https://docs.nvidia.com#collecting-source-file-and-line-information)

Starting with Nsight Systems 2026.4, CPU sampling can collect source file and line information for the Top-Down and Bottom-Up function table modes. This adds the Source File and Source Line columns to those views, which can help identify the exact loop, conditional statement, or call site responsible for the most CPU usage.
...
To configure this information from the CLI, use `--show-source-info`
...
command. For example, to disable the default source
...
When this information is disabled, report generation is faster, but source file and line information is not resolved for CPU samples or supported backtraces.

### Visualizing CPU Profiling Results[#](https://docs.nvidia.com#visualizing-cpu-profiling-results)
...
[Timeline View in the Reading Your Report in the GUI section of the User Guide](https://docs.nvidia.com/index.html#timeline-view).
...
### CPU Events, Metrics, and Metric Sets[#](https://docs.nvidia.com#cpu-events-metrics-and-metric-sets)

#### Core Events, Metrics, and Metric Sets[#](https://docs.nvidia.com#core-events-metrics-and-metric-sets)
...
full - single-pass metric sets based on the Arm Topdown methodology for supported chips


The `--cpu-metrics=help`

command will print grammar describing how to list
supported core events, metrics, and metric sets for your CPU.
The selected events, metrics, and metric sets can be fed into the
`--cpu-metrics`

switch by name or by alias.

Future versions of Nsight Systems will provide full event and metric support for other x86 and Arm architectures.

#### Uncore Events and Metrics[#](https://docs.nvidia.com#uncore-events-and-metrics)

Nsight Systems can access and make available information about activities on the CPU uncore/SoC. What exact data is available varies by SoC.
...
#### Single-pass Metric Sets[#](https://docs.nvidia.com#single-pass-metric-sets)

A single-pass metric set is a predefined collection of CPU metrics whose
required programmable events fit within the programmable-counter capacity of
the associated PMU and can therefore be collected concurrently in one event
group. Nsight Systems provides CPU core single-pass metric sets based on the
[Arm Topdown methodology](https://developer.arm.com/documentation/109542/02/Arm-Topdown-methodology)
for supported CPUs. The available metric sets vary by CPU.

To list the CPU core single-pass metric sets available on the target, run:

```
nsys profile --cpu-metrics=help:core:metric-set
```

Select a listed metric set by its name or alias.

Use the metric sets progressively to narrow down the performance bottleneck:

Start with

`Topdown_L1`

to measure`frontend_bound`
...
is significant and`Frontend_Backend`

is reported by the help command, collect that set. Use its`frontend_core_bound`

,`frontend_mem_bound`

,`backend_core_bound`

, and`backend_mem_bound`

results to choose the corresponding`Frontend_Core`
...
drill-down set.Collect additional focused sets as needed, such as
...
. Available sets vary by CPU; use only sets reported by the help command for the target CPU.

The following example collects the `Operation_Mix_Ext_Compute`

metric set in
separate profiling runs for two NBody versions, `ver1`

and `ver2`

, to
evaluate vectorization enabled by the array-of-structures (AoS) to
structure-of-arrays (SoA) transformation.

The metric set includes `scalar_fp_percentage`

and `simd_percentage`

metrics,
which report the percentages of speculatively executed operations that are scalar
floating-point and Advanced SIMD (Neon) operations, respectively. A decrease in
`scalar_fp_percentage`

accompanied by an increase in `simd_percentage`

indicates the expected shift from scalar to vector operations.

```
nsys profile --event-sample=system-wide --cpu-metrics=Operation_Mix_Ext_Compute \
-- ver1/nbody.x
nsys profile --event-sample=system-wide --cpu-metrics=Operation_Mix_Ext_Compute \
-- ver2/nbody.x
```

These commands produce two `.nsys-rep`

files that can be compared directly or
processed with the `nvtx_cpu_topdown`

recipe.

On a CPU that supports both sets, the following example collects
`Topdown_L1`

and `Frontend_Backend`

during a single profiling session by
...
delimiter separates event groups. For more information, see
[Event Multiplexing](https://docs.nvidia.com#event-multiplexing).

#### Event Multiplexing[#](https://docs.nvidia.com#event-multiplexing)

There are hardware limitations on how many CPU counters can be collected at one time. If you need to collect more counters than are available, including when collecting multiple single-pass metric sets, you can either perform multiple runs (as in the Arm Topdown methodology below) or take advantage of Nsight Systems’s support for event multiplexing.

To multiplex events, you need to define event groups. An event group can be
defined in any of the following switches: `--cpu-metrics`
...
An event group is a group of events and metrics to be sampled concurrently. A
single-pass metric set fits within one event group. Use the `%`

delimiter to
separate event groups. For example:
...
3, etc. assuming the `--event-sampling-multiplex-interval`
...
switch also supports event multiplexing of events,
metrics, and single-pass metric sets using the same syntax. For
example, the following command line defines two event groups, each
...
#### Arm Topdown Analysis[#](https://docs.nvidia.com#arm-topdown-analysis)
...
. This script simplifies collecting all PMU core
...
switch is set to the default value of`sigterm`
...
) that will process the data from
...
[nvtx_cpu_topdown Recipe](https://docs.nvidia.com/AnalysisGuide/index.html#nvtx-cpu-topdown-recipe).
...
### Common Issues[#](https://docs.nvidia.com#common-issues)

**Reducing Overhead Caused By Sampling**There are several ways to reduce overhead caused by sampling.
...
**Sample intervals are irregular**My samples are not periodic - why? My samples are clumped up - why? There are gaps in between the samples - why? Likely reasons:
...
**No CPU profiling data is collected**There are a few common issues that cause CPU profiling data to not be collected:
...
switch or modify the Docker’s`seccomp`
...
command indicated that CPU profiling was supported. The host OS kernel version determines if CPU profiling is allowed and a CentOS host with a version < 3.10.0-693 is too old. In this case, the`nsys status --environment`

command is incorrect.


## CPU Metrics on Windows on Arm[#](https://docs.nvidia.com#cpu-metrics-on-windows-on-arm)

On Windows on Arm targets, Nsight Systems can periodically sample CPU core hardware event counts and derive metrics. The data is collected system-wide across all CPUs and is not directly associated with individual processes or threads. Nsight Systems displays the collected values over time in the report file.

In this version of Nsight Systems, `--cpu-metrics`

is available on Windows on
Arm for NVIDIA Arm-based systems (for example, NVIDIA RTX Spark). Use
`--cpu-metrics=help`

to list the available counter values.

## NVIDIA Video Profiling[#](https://docs.nvidia.com#nvidia-video-profiling)

### NVIDIA Video Hardware Profiling[#](https://docs.nvidia.com#nvidia-video-hardware-profiling)

#### Limitations/Requirements[#](https://docs.nvidia.com#limitations-requirements)

NVIDIA Video Hardware profiling requires:

Linux (x86_64 or Arm) and Windows (x86_64 or Arm)
...
following [link](https://download.nvidia.com/XFree86/Linux-x86_64/550.142/README/gsp.html).
...
The feature is enabled through the `--gpu-video-devices`
...
`--gpu-video-devices help`

- List supported devices and their IDs, List unsupported devices (if any) and the reason.`--gpu-video-devices none`

- Turn the feature off.`--gpu-video-devices all`

- Enable the feature on all supported devices. An error is returned if no devices support the feature.`--gpu-video-devices <id1,id2,...>`

- Enable the feature on the specified devices. The ID corresponds to what`help`
...
### NVIDIA Video Codec SDK Trace[#](https://docs.nvidia.com#nvidia-video-codec-sdk-trace)

Nsight Systems for x86 Linux and Windows targets can trace calls from the NVIDIA Video Codec SDK. This software trace can be launched from the GUI or using the `--trace nvvideo`

from the CLI

On the timeline, calls on the CPU to the NVIDIA Encoder API, NVIDIA Decoder API, and NVIDIA nvJPEG API will be shown.

#### NV Encoder API Functions Traced by Default[#](https://docs.nvidia.com#nv-encoder-api-functions-traced-by-default)
...
#### NV Decoder API Functions Traced by Default[#](https://docs.nvidia.com#nv-decoder-api-functions-traced-by-default)
...
#### NV JPEG API Functions Traced by Default[#](https://docs.nvidia.com#nv-jpeg-api-functions-traced-by-default)
...
## Network Communication Profiling[#](https://docs.nvidia.com#network-communication-profiling)
...
. For multi-node runs, please refer to the section on
[Handling Application Launchers (mpirun, deepspeed, etc)](https://docs.nvidia.com#handling-application-launchers).

### MPI API Trace[#](https://docs.nvidia.com#mpi-api-trace)
...
(see [MPI functions traced](https://docs.nvidia.com#mpi-functions-traced)).
...
[NVTX wrappers for MPI](https://github.com/NVIDIA/cuda-profiler/tree/master/nvtx_pmpi_wrappers)
...
#### MPI Communication Parameters[#](https://docs.nvidia.com#mpi-communication-parameters)
...
#### MPI functions traced[#](https://docs.nvidia.com#mpi-functions-traced)
...
### OpenSHMEM Library Trace[#](https://docs.nvidia.com#openshmem-library-trace)
...
### UCX API Trace[#](https://docs.nvidia.com#ucx-api-trace)
...
: (enabled by default) If set to`0`
...
#### UCX functions traced[#](https://docs.nvidia.com#ucx-functions-traced)
...
### NVIDIA NCCL Trace[#](https://docs.nvidia.com#nvidia-nccl-trace)

Nsight Systems provides two methods for tracing NVIDIA NCCL (NVIDIA Collective Communications Library) operations:

**Legacy NCCL tracing:**is based on NVTX annotations within the NCCL library itself.Enabled by default when NVTX tracing is active
...
**Advanced NCCL tracing:**A more detailed tracing mechanism introduced in Nsight Systems 2025.6.1.Requires NCCL version 2.28 or higher (with limited support for versions 2.27.4 and later)
...
#### NCCL Execution Concepts[#](https://docs.nvidia.com#nccl-execution-concepts)
...
function is projected to the fused CUDA kernel on the GPU.When no explicit groups are used, there is an implicit group as part of each API call. This implicit grouping is not shown in legacy NCCL tracing.
...
#### Advanced NCCL Tracing[#](https://docs.nvidia.com#advanced-nccl-tracing)
...
##### Trace Information[#](https://docs.nvidia.com#trace-information)
...
range that spans all API calls within the group. Technically, this range does not include`ncclGroupStart`

, but extends through the end of`ncclGroupEnd`

/`GroupLaunch`

.Individual API calls are shown below the
...
prefix is omitted from function names (e.g.,`ncclAllReduce`

appears as`AllReduce`

).The

`GroupLaunch`

range corresponds to the`ncclGroupEnd`

function call on the application thread invoking the NCCL group functions.For blocking communication, this range encompasses the preparation for kernel launches. There can be multiple launches per group, usually one per rank/device, and the actual CUDA kernel launches are collected as
...
ranges encompass the individual runtime scheduling for collective and point-to-point operations.Runtime scheduling can occur in different contexts:

Directly at the end of the

`API Group`

for blocking communicatorsIn a separate thread for non-blocking communicators
...
By default, for each API call to a CE collective, a corresponding range will be shown within the `GroupLaunch`

range.
In addition, you can configure Nsight Systems to add two `CE Sync`

ranges and one `CE Batch`

range below each CE collective range (see [Configuration Options](https://docs.nvidia.com#nccl-trace-options)).
...
They display the number of proxy steps in any given state, separated into `ProxyStepSend`
...
This thread is named `NCCL Progress [$rank/$nRanks]: $commHash`

to easily identify the context.
...
For complex applications, developers should assign names to communicators within the application by setting `commName`

in `ncclConfig_t`

.
Creation of communicators is indicated by a `CommInit`

marker in a dedicated initialization thread.
...
Events corresponding to individual operations are correlated by the `apiId`

, which for collective operations includes the operations of all participating ranks.
...
: All API calls and corresponding kernel launches in a specific (thread-local) group`group`
...
##### Report View[#](https://docs.nvidia.com#report-view)

The timeline view of a report with advanced NCCL tracing includes several types of `NCCL`
...
row directly under each device shows the GPU operations.CPU threads that call API functions include a

`NCCL`

row showing the respective API events.Internal threads that execute non-blocking or graph host functions also include a

`NCCL`

row showing runtime events (not shown in the example).An additional global

`NCCL`

row at the bottom combines the CPU-side NCCL events from all processes/threads in the report.Below the stream rows on the device,

`NCCL`

rows show projections of the respective internal NCCL ranges which were responsible for launching the corresponding CUDA activity.

Each `NCCL`

row can be expanded to show individual events for each communicator.

The screenshot shows two participating devices with a simple API group that includes two API calls (one for each rank).
Since this is a blocking communicator, the group directly executes the NCCL internal functions `GroupLaunch`
...
##### Exporting[#](https://docs.nvidia.com#exporting)

The data from advanced NCCL tracing can be exported as SQLite database for further processing.
See [SQLite Schema Reference](https://docs.nvidia.com/AnalysisGuide/index.html#sqlite-schema) for more details.
...
Add `--include-json true`

to include the metadata as JSON in the exported tables.
The specific schema of the exported tables may be subject to change in the future.

##### Limitations[#](https://docs.nvidia.com#id17)
...
markers.Single-rank communications, which are usually only buffer copies, are not shown at all. Their events are not captured.
...
option with`nsys profile`

or enable the “NCCL” section in the Network profiling options of the GUI:nsys profile -t nccl <application>
...
##### Configuration Options[#](https://docs.nvidia.com#configuration-options)

Configure advanced NCCL tracing using the `--nccl-trace`

option with `nsys profile`

. The following values are available and can be combined using commas (e.g., `--nccl-trace=api,rt`
...
: Collective API calls (e.g.,`AllReduce`

)`api-p2p`

: Point-to-point API calls (`Send`

and`Recv`

)`api`

: All of the three API ranges above (`api-group`
...
ranges for operation scheduling on the CPU`coll`

: Runtime scheduling of individual collective operations`p2p`

: Runtime scheduling of individual point-to-point operations`kernel-launch`

: Ranges around CUDA kernel launches`ce-coll`

: Launch of memory operations for Copy Engine (CE) -based collectives`ce-sync`

: Synchronization of CE collectives (within ce-coll)`ce-batch`

: Batch operations of CE collectives (within ce-coll)`rt`

: All runtime ranges (`group`
...
: Individual operations on the GPU`proxy-op`

: Proxy operation ranges (experimental)`proxy-step`

: Proxy step ranges including state changes (experimental, high overhead)`proxy-counters`

: Proxy counters (experimental)`default`

: The default set of events (`api`
...
: All possible events (except`proxy-op`

,`proxy-step`

)

The interaction between `--nccl-trace`
...
is explicitly set, it takes priority over any NCCL-related settings from`--trace`

. Any value other than`none`

implicitly includes`nccl`
...
is not set, the default behavior depends on`--trace`

: if`nccl`

is included in`--trace`

,`default`

is used for`--nccl-trace`

; otherwise,`none`

is used.

##### More Examples[#](https://docs.nvidia.com#more-examples)
...
### NVIDIA NVSHMEM Trace[#](https://docs.nvidia.com#nvidia-nvshmem-trace)

The NVIDIA network communication library NVSHMEM has been instrumented using NVTX annotations. To enable tracing this library in Nsight Systems, turn on NVTX tracing in the GUI or CLI. To enable the NVTX instrumentation of the NVSHMEM library, make sure that the environment variable `NVSHMEM_NVTX`

is set properly; e.g., `NVSHMEM_NVTX=common`

.

## Network Hardware Profiling[#](https://docs.nvidia.com#network-hardware-profiling)
...
### InfiniBand Network Information[#](https://docs.nvidia.com#infiniband-network-information)
...
command line switch. This command line switch can only be used together with the`ib-net-info-devices`
...
### Network Interface Controller (NIC) Profiling[#](https://docs.nvidia.com#network-interface-controller-nic-profiling)

#### NVIDIA NIC Metric Sampling[#](https://docs.nvidia.com#nvidia-nic-metric-sampling)
...
NIC metric sampling is supported on Linux x86_64 and Arm Server (SBSA) machines only, having minimum Linux kernel 4.12 and minimum MLNX_OFED 4.1. You can download the latest OFED driver through the DOCA-Host package as doca-ofed at

[NVIDIA DOCA Downloads](https://developer.nvidia.com/doca-downloads). For archived versions of the OFED driver you can visit[MLNX_OFED Download Center](https://network.nvidia.com/products/infiniband-drivers/linux/mlnx_ofed/). If collecting NIC metrics within a container, make sure that the container has access to the driver on the host machine. To check manually if OFED is installed and get its version you can run:`/usr/bin/ofed_info`
...
The DOCA telemetry (libdoca_telemetry.so.2) and common (libdoca_common.so.2) libraries must be installed. The libraries can be installed through DOCA SDK 2.9 or newer.
...
If collecting with the

`--nic-metrics=hf`

option additionally:The mlx5_fwctl driver module must be loaded.

The user must have elevated privileges.


If collecting through the DOCA Telemetry Service API (DTS) with the

`--nic-metrics=hf-via-dts`

option additionally:DTS must already be running with

`enable-http-api=true`

on the target system.The default DTS HTTP port is 9117 and can be overridden with
...
The high frequency option, `hf`

, collects samples at a higher frequency compared to the `lf`

option. `--nic-metrics=hf`

will not collect counters for RoCE, IPoIB traffic and the Send
waits metric. The `hf-via-dts`

option also collects high frequency NIC metrics, via the DOCA
Telemetry Service API.

**Available Metrics**

**Bytes sent**- Number of bytes sent through the NIC port.**Bytes received**- Number of bytes received by the NIC port.**Average sent packet size**- Average byte size of packets sent through the NIC port.**Average received packet size**- Average byte size of packets received by the NIC port.**CNPs sent**- Number of congestion notification packets sent by the NIC.**CNPs received**- Number of congestion notification packets received and handled by the NIC.**Send waits**- The number of ticks during which the port had data to transmit but no data was sent during the entire tick (either because of insufficient credits or because of lack of arbitration)
...
#### Amazon AWS EFA NIC Metrics[#](https://docs.nvidia.com#amazon-aws-efa-nic-metrics)
...
work on [AWS EC2 NVIDIA GPU accelerated compute instances](https://docs.aws.amazon.com/AWSEC2/latest/UserGuide/efa.html#efa-instance-types/) .
...
This collector is the first use case for the [Nsight Systems Plugins](https://docs.nvidia.com#nsight-systems-plugins)
system.

### Network Switch Profiling[#](https://docs.nvidia.com#network-switch-profiling)

#### InfiniBand Switch Metric Sampling[#](https://docs.nvidia.com#infiniband-switch-metric-sampling)
...
##### Limitations/Requirements[#](https://docs.nvidia.com#id18)
...
follow the directions at [RedHat Solutions](https://access.redhat.com/solutions/5929621).

To collect InfiniBand switch performance metric, using Nsight Systems CLI, add
the `--ib-switch-metrics-devices`
...
##### Available Metrics[#](https://docs.nvidia.com#id19)

**Bytes sent**- Number of bytes sent through all switch ports**Bytes received**- Number of bytes received by all switch ports**Send waits**- The number of ticks during which switch ports, selected by PortSelect, had data to transmit but no data was sent during the entire tick (either because of insufficient credits or of lack of arbitration)**Average sent packet size**- Average sent InfiniBand packet size**Average received packet size**- Average received InfiniBand packet size

#### Ethernet Switch Metric Sampling[#](https://docs.nvidia.com#ethernet-switch-metric-sampling)

##### Overview[#](https://docs.nvidia.com#id20)

Nsight Systems can collect High Frequency Telemetry (HFT) metrics from NVIDIA Spectrum-X Ethernet switches running NVIDIA Cumulus Linux. The metrics help identify short-lived traffic bursts, congestion, and changes in per-port network usage while an application is running.

During collection, Nsight Systems requests switch metrics from the DOCA Telemetry Service (DTS) running on the profiling target. DTS configures HFT and IP Flow Information Export (IPFIX) protocol export on each switch, receives the IPFIX records, and makes the metrics available on the Nsight Systems timeline. Collection is system wide and is not limited to the processes traced in the profiling session.

##### Limitations/Requirements[#](https://docs.nvidia.com#id21)

The following requirements must be met:

The profiling target must be a supported Linux x86_64 or Arm Server (SBSA) system.

DTS version 1.26.5 or later must be running on the profiling target with

`enable-http-api=true`

.The DTS API client dependency

`libcurl.so.4`

, must be installed on the profiling target.The switch must run NVIDIA Cumulus Linux 5.17 or later.

Network switch data requires clock synchronization to be actionable. Nsight Systems will use either NTP or PTP timing for synchronization. Note that PTP is strongly recommended especially when correlating samples from multiple systems because it provides more accurate synchronization. Refer to

[these instructions](https://docs.nvidia.com/networking-ethernet-software/cumulus-linux-518/System-Configuration/Date-and-Time/Network-Time-Protocol-NTP)to set the NTP synchronization, and to[these instructions](https://docs.nvidia.com/networking-ethernet-software/cumulus-linux-518/System-Configuration/Date-and-Time/Precision-Time-Protocol-PTP)to set the PTP synchronization.Initial switch configuration and IPFIX template exchange can delay the first metric sample by approximately 25 seconds. Use a profiling duration longer than 25 seconds.

Ethernet switch metrics are collected for up to 10 minutes. A profiling session can continue after that time, but no additional switch samples are collected.


###### Creating a Least-Privilege Switch Account[#](https://docs.nvidia.com#creating-a-least-privilege-switch-account)

DTS needs read and write access to the switch interfaces and the HFT and IPFIX configuration paths.
Create a custom class and role instead of assigning the `nvue-admin`

role. The commands in this
section require an existing switch administrator account. The following example creates an account
named `nsys-hft`

:

```
nv set system aaa class nsys-hft action allow
nv set system aaa class nsys-hft command-path '/interface/' permission rw
nv set system aaa class nsys-hft command-path '/system/telemetry/' permission rw
nv set system aaa role nsys-hft-role class nsys-hft
nv set system aaa role nsys-hft-role class nvapply
nv set system aaa role nsys-hft-role class nvshow
nv set system aaa user nsys-hft role nsys-hft-role
nv set system aaa user nsys-hft full-name "Nsight Systems HFT Collector"
nv set system aaa user nsys-hft password
nv set system aaa user nsys-hft state enabled
nv config apply
```

The password command prompts for the password and confirmation.

An administrator can verify the account and its permissions with the following commands:

```
nv show system aaa class nsys-hft
nv show system aaa role nsys-hft-role
nv show system aaa user nsys-hft
```

Create the same account with the same password on every switch listed in the configuration file. Nsight Systems currently supports one shared username and password for all switches in a profiling session.

##### Creating the Switch Configuration File[#](https://docs.nvidia.com#creating-the-switch-configuration-file)
...
|---|---|
|
Required JSON number. The only supported value is |
|
Required, non-empty array of IPv4 address strings. Hostnames and IPv6 addresses are not supported. |
|
Required, non-empty switch account name. The same account name is used for every listed switch. |
|
Required, non-empty password for the switch account. The same password is used for every listed switch. |

Field names are case-sensitive, all four fields are required, and additional fields are rejected.

##### Collecting Switch Metrics from the Command Line[#](https://docs.nvidia.com#collecting-switch-metrics-from-the-command-line)

The following command-line options control Ethernet switch metric collection:

Option |
Description |
|---|---|
|
Enables Ethernet switch metric collection through DTS and specifies the local JSON configuration file. |
|
Specifies the local DTS HTTP API port. The default is 9117. This option can only be used with
a DTS-backed collection feature, such as |

Before profiling, check the DTS API client dependency, the DTS API on the default port, and the DTS version:

```
nsys status --network
```

The status command checks DTS on port 9117. If DTS uses a different port, verify that DTS is
listening on that port and pass the port to the profiling command with `--dts-api-port`
...
##### Available Metrics[#](https://docs.nvidia.com#id22)

**Bytes sent**- Number of bytes sent per switch port**Bytes received**- Number of bytes received per switch port**Average sent packet size**- Average sent Ethernet packet size per switch port**Average received packet size**- Average received Ethernet packet size per switch port**Ethernet buffer occupancy**- Current shared egress-buffer occupancy for regular Ethernet traffic in traffic class 0**RoCE buffer occupancy**- Current shared egress-buffer occupancy for RoCE traffic in traffic class 3

#### InfiniBand Switch Congestion Events[#](https://docs.nvidia.com#infiniband-switch-congestion-events)

##### Overview[#](https://docs.nvidia.com#id23)
...
##### Limitations/Requirements[#](https://docs.nvidia.com#id24)
...
##### Using the Command Line[#](https://docs.nvidia.com#using-the-command-line)
...
### Network Interface Device Profiling[#](https://docs.nvidia.com#network-interface-device-profiling)
...
## Network Storage Profiling[#](https://docs.nvidia.com#network-storage-profiling)

Nsight Systems can profile several major storage / remote storage protocols. It
also ships with the `storage_util_map`

and `file_access_sum`

recipes for
post-collection analysis. See
[Post-Collection Analysis Guide](https://docs.nvidia.com/AnalysisGuide/index.html#id1)

Note

Storage metrics profiling requires additional kernel support and is tailored for HPC systems.
...
: enable NFS storage profiling for the specified volume(s) (specify`all`

to profile all volumes).`--lustre-volumes={all | volume1[,volume2][,volume3..]}`

: enable Lustre storage profiling for the specified volume(s) (specify`all`
...
: enable storage profiling of the specified local storage or NVMe-oF device(s) (specify`all`

to profile all devices).`--interval=<value>`

: sampling interval in milliseconds. Valid range is 1-60000 (default: 1).`--cache-samples=<value>`

: number of samples to cache before submitting the events. Valid range is 1-1000 (default: 100).
...
log files for the storage metrics collection
...
### NFS volumes counters[#](https://docs.nvidia.com#nfs-volumes-counters)
...
### Lustre volumes counters[#](https://docs.nvidia.com#lustre-volumes-counters)
...
command line argument should point to the `llite`
...
### Local and NVMe-oF volumes counters[#](https://docs.nvidia.com#local-and-nvme-of-volumes-counters)
...
### S3 Trace[#](https://docs.nvidia.com#s3-trace)
...
). Nsight Systems traces S3 upload and download request operations, and the individual HTTP transactions within each request.Note
...
**AWS CPP SDK**— C++ applications that use the AWS C++ SDK (`aws-cpp-sdk`

) for S3 operations can be traced. Nsight Systems traces both the S3 Client and the S3Crt Client.Note
...
**Boto3**— Python applications that use the`boto3`
...
**S3TorchConnector**— Python applications that use the`s3torchconnector`

library for PyTorch dataset access over S3 (e.g.,`S3IterableDataset`

,`S3MapDataset`

, Checkpoint and direct reader/writer operations).**Tensorflow-io**— Python applications that use the`tensorflow-io`

library to access S3 objects via`tf.io.gfile.GFile`

with`s3://`

paths.

#### Usage Example[#](https://docs.nvidia.com#usage-example)
...
parameter. See [Command Line Options](https://docs.nvidia.com/index.html#command-line-options) for more information.
...
— Collects S3 operation ranges, with core attributes, including bytes transferred, bucket name, key name, file path, and result status.`--trace=s3-verbose`

— In addition to everything collected by`s3`

, Nsight Systems will collect additional per-request metadata and breakdown of individual HTTP transactions. This additional data is intended for in-depth low-level analysis of transfer behavior, but may increase trace volume and processing overhead. This mode only affects tracing of AWS CRT, AWS CPP SDK, and Boto3 applications.

#### Process Statistics[#](https://docs.nvidia.com#process-statistics)
...
**Download Throughput**— Aggregate download throughput over time (bytes/s).**Upload Throughput**— Aggregate upload throughput over time (bytes/s).**Avg Download Size**— Average file/object size of currently active download operations (bytes).**Avg Upload Size**— Average file/object size of currently active upload operations (bytes).

These statistics can help identify periods of high or low S3 activity and reveal bottlenecks in data transfer patterns.

### GDS (GPUDirect Storage) Trace[#](https://docs.nvidia.com#gds-gpudirect-storage-trace)
...
Information about GDS can be found at [NVIDIA Magnum IO GPUDirect Storage](https://docs.nvidia.com/gpudirect-storage/).
...
[NVIDIA GPUDirect Storage Installation and Troubleshooting Guide](https://docs.nvidia.com/gpudirect-storage/troubleshooting-guide/index.html#nvidia-gpudirect-storage-installation-and-troubleshooting-guide/).
...
### GDS (GPUDirect Storage) Counters[#](https://docs.nvidia.com#gds-gpudirect-storage-counters)
...
: Enable GDS (GPUDirect Storage) user-space performance metrics collection.`--gds-libs-path=<path>`

: Specify a directory containing GPUDirect Storage libraries (must contain libcufile.so). Use this argument if the GDS libraries are located in a different path than the default. Default is`/usr/local/cuda/lib64`

. This argument is used together with`--gds-metrics`
...
### SCADA Metrics Profiling[#](https://docs.nvidia.com#scada-metrics-profiling)
...
#### Enabling SCADA Metrics Collection[#](https://docs.nvidia.com#enabling-scada-metrics-collection)
...
There are no spaces following the `scada_metrics`
...
a Unix Domain Socket at the path provided to `--socket-path`

cli option
(default path is `/tmp/scada_profiler_socket`
...
/tmp/scada_profiler_socket |
Filesystem path to the Unix Domain Socket used for communication with the SCADA server. |

**Usage Examples**

`nsys profile --enable scada_metrics <target-application>`

Collect SCADA metrics at the default sampling frequency of 1000 Hz, and default socket path of
...
Collect SCADA metrics at 100 Hz using custom socket path of
...
[Nsight Systems Plugins](https://docs.nvidia.com#nsight-systems-plugins) system.

#### Viewing SCADA Metrics in the Report[#](https://docs.nvidia.com#viewing-scada-metrics-in-the-report)
...
**Counters**— Scalar values sampled at each collection interval (e.g. total received buffers, average latency). Each counter is plotted as a time-series graph on the timeline.**Histograms**— Distribution metrics with predefined bins (e.g. latency distribution, commands-per-buffer distribution). Each histogram is displayed with its bucketed distribution over time.
...
## Python Profiling[#](https://docs.nvidia.com#python-profiling)
...
### Python Backtrace Sampling[#](https://docs.nvidia.com#python-backtrace-sampling)
...
See also [Visualizing CPU Profiling Results](https://docs.nvidia.com/index.html#visualizing-cpu-profiling-results).
...
### Python Functions Trace[#](https://docs.nvidia.com#python-functions-trace)
...
### Python GIL Tracing[#](https://docs.nvidia.com#python-gil-tracing)
...
This feature is not supported on Python that was compiled with `Py_GIL_DISABLED=1`

(See [Python documentation](https://docs.python.org/3/howto/free-threading-extensions.html#identifying-the-free-threaded-build-in-c) for details).
...
### PyTorch Profiling[#](https://docs.nvidia.com#pytorch-profiling)
...
To enable PyTorch [autograd nvtx](https://pytorch.org/docs/stable/autograd.html#torch.autograd.profiler.emit_nvtx), run Nsight Systems from the CLI using the `--pytorch`
...
Set `--pytorch=functions-trace`

for automatically annotating
PyTorch operations like [forward operations](https://docs.pytorch.org/docs/stable/generated/torch.nn.Module.html#torch.nn.Module.forward),
[backward operations](https://pytorch.org/docs/stable/autograd.html#torch.autograd.backward),
[step operations](https://docs.pytorch.org/docs/stable/optim.html#taking-an-optimization-step), etc.
with execution time ranges.

Set `--pytorch=functions-trace-shapes`

to attach additional data to each annotated range,
such as tensor shapes, training parameters, etc.
`functions-trace`

and `functions-trace-shapes`

are mutually exclusive.

Both `--pytorch=functions-trace`

and `--pytorch=functions-trace-shapes`

imply `--python-functions-trace=<nsys_install_dir>/<target-arch>/PythonFunctionsTrace/pytorch.json`

.

`autograd-nvtx`

and `autograd-shapes-nvtx`

options can be combined with `functions-trace`

or `functions-trace-shapes`

by adding them separated by a comma.

When profiling a [vLLM](https://docs.vllm.ai/en/stable/) application,
the `functions-trace`

option also automatically annotates vLLM-specific methods
such as `AsyncLLM.generate()`

, `Worker.execute_model()`

, and `Worker.sample_tokens()`
...
### Dask Profiling[#](https://docs.nvidia.com#dask-profiling)
...
Set `--dask=functions-trace`

for enabling Dask functions trace. This option sets `--python-functions-trace=<nsys_install_dir>/<target-arch>/PythonFunctionsTrace/dask.json`

and will rename relevant threads to ‘Dask Worker’ and ‘Dask Scheduler’.

`dask.json`

can be modified to include additional functions to be traced from any Python module.

Example screenshot:

## Profiling with DRIVE Hypervisor[#](https://docs.nvidia.com#profiling-with-drive-hypervisor)
...
- Enable XHV sampling, depends on`Collect HV Trace`

.`Event mask`

- Select XHV trace events, this option can be specified as`None`
...
## Adding Your Own Collection to a Report[#](https://docs.nvidia.com#adding-your-own-collection-to-a-report)
...
### Nsight Systems Plugins[#](https://docs.nvidia.com#product-name-plugins)

Nsight Systems plugins are tools that extend its data collection capabilities, available
via CLI with `--enable`

command option and via GUI.
There are multiple locations where the Nsight Systems searches for available plugins
described in the [Plugin discovery](https://docs.nvidia.com#plugin-discovery) section.
...
[Third Party Plugins List](https://nvidia.github.io/NsightSystemsPlugins/)
or [GitHub repository](https://github.com/NVIDIA/NsightSystemsPlugins).
...
#### How to launch a plugin[#](https://docs.nvidia.com#how-to-launch-a-plugin)

In CLI plugins are enabled with the `--enable`

command option that also allows
passing arguments to the plugin. It’s possible to launch multiple instances
of the same plugin by using multiple `--enable`

options.

Depending on the plugin type it may be available in `nsys profile`

, `nsys start`

or `nsys launch`

commands. For example, a minimal plugin that only sets some
environment variables will be applicable to the `nsys launch`
...
This default behavior can be amended, see the [Developing an Nsight Systems Plugin](https://docs.nvidia.com#developing-an-nsight-systems-plugin) section.
...
#### How to pass arguments to a plugin[#](https://docs.nvidia.com#how-to-pass-arguments-to-a-plugin)
...
, and the backslash itself can be escaped by
another backslash `\\\\`
...
for a literal comma), and `^^`

for a literal caret. To include
spaces in an argument, enclose the argument in double quotes `"`

.

See the section on the [Amazon AWS Elastic Fabric Adapter (EFA)
Network Counters](https://docs.nvidia.com#aws-efa-plugin) for an example.

#### Supported platforms[#](https://docs.nvidia.com#supported-platforms)

Nsight Systems plugins are supported on x86_64, arm64 Linux and x86_64, arm64 Windows.

#### Plugin discovery[#](https://docs.nvidia.com#plugin-discovery)
...
To list all available plugins use the `nsys plugins list`
...
### Developing an Nsight Systems Plugin[#](https://docs.nvidia.com#developing-an-product-name-plugin)

#### Plugin manifest file[#](https://docs.nvidia.com#plugin-manifest-file)
...
No |
Boolean |
If enabled, a standalone plugin must call the Collector
API method |
TerminateAfterCollection |
No |
Boolean |
If |

At least one of the `ExecutablePath`

, `LibraryPath`

, or `TargetEnvironment`

entries is required but all can be
...
#### Plugin types[#](https://docs.nvidia.com#plugin-types)

A plugin manifest may specify one or more actionable entries: `ExecutablePath`
...
. They are named as “standalone”, “in-process”, and “configuration” plugin types. When selected
with `--enable`

option Nsight Systems will perform all applicable actionable entries.
...
#### NsysDK Collector[#](https://docs.nvidia.com#nsysdk-collector)
...
#### Deploying plugins[#](https://docs.nvidia.com#deploying-plugins)
...
path. The easiest way to do this is to export the `NSYS_PLUGIN_SEARCH_DIRS`

environment variable
with the location of a folder that contains the plugin manifest.

#### Minimal standalone plugin[#](https://docs.nvidia.com#minimal-standalone-plugin)
...
The data collected from plugins is in the form of [NVTX events](https://docs.nvidia.com/index.html#nvtx-trace),
stdout and stderr streams.
The source code for the `network_interface`

plugin is deployed as an example in
`<profiler installation dir>/target-linux-x64/samples/NetworkPlugin.cpp`

.

#### Deferred events standalone plugin[#](https://docs.nvidia.com#deferred-events-standalone-plugin)
...
.
On POSIX platforms the NVTX library requires adding the `-ldl`

linker option. Refer to the
[NVTX documentation](https://nvidia.github.io/NVTX/) to learn how to use the deferred events feature.
...
#### In-process plugin process-exit handler[#](https://docs.nvidia.com#in-process-plugin-process-exit-handler)
...
handler runs during the dynamic loader’s library finalization, which happens *after* Nsight Systems has already
torn down its data collection. NVTX events emitted from such a handler are therefore lost.

To support this, the NsysDK Collector API provides a function that an in-process plugin can call to register a process-exit handler:
...
The plugin calls this from its `PluginLibraryInit`

function. Nsight Systems invokes the registered handler late during
process shutdown, while data collection is still active and just before it is torn down, so NVTX events emitted from the
handler are captured. The `userData`

pointer is passed back to the handler when it is invoked. As with all in-process
...
`nsysdkCollectorRegisterExitHandler`

may only be called from within an in-process plugin’s`PluginLibraryInit`

function. The profiler attributes the call to the plugin currently being initialized and the call is ignored with a diagnostic message if the function is called at any other time, such as from a thread the plugin spawned, after initialization has returned, or from a standalone plugin.The method only appends a handler. There’s no way to reset a previously registered handler.


When more than one in-process plugin registers a handler, the handlers run concurrently, each on its own thread that is created during plugin initialization (before the workload runs). Nsight Systems waits for all handlers to finish before proceeding with teardown, bounded by a timeout. A handler should therefore complete promptly: it delays process termination and the final flush of collected data. Once timeout is reached the process teardown proceeds regardless and any data emitted past this point may be lost.
...
, an uncaught signal, or being killed), since Nsight Systems has no opportunity to invoke it.The handler does not run in child processes created by

`fork`

without`exec`

; only the process in which the plugin was initialized invokes it.

#### Multiple plugin start requests[#](https://docs.nvidia.com#multiple-plugin-start-requests)

Nsight Systems supports plugins across several sub-commands, including `nsys profile`
...
lists.
One common case is the same plugin enabled in multiple `nsys start`

commands.
Another is a manifest that lists both standalone and in-process entries while you enable the plugin from `start`
...
Nsight Systems launches standalone plugins when report data collection begins. For a given standalone plugin, every configuration that shares the same arguments is merged into a single process launch. If you pass different argument sets for that plugin, each distinct set starts its own instance.

Once in-process plugins have been initialized, the set of plugins and their arguments is frozen so that every instance of the target application sub-processes receives a consistent plugins initialization sequence. Until a target application has been launched, newer sets of plugin arguments override the older ones, so that if the target process is started after a series of system-wide collections it uses the most recent set of plugin configurations.

### Import NVTXT[#](https://docs.nvidia.com#import-nvtxt)

**ImportNvtxt** is an utility which allows conversion of a
[NVTXT](https://docs.nvidia.com/gameworks/index.html#gameworkslibrary/nvtx/analysis_nvtxt_file_extension.htm)
...
**Manual**— timestamps are set using absolute values.**Relative**— timestamps are set using relative values with regards to report file which is being merged with the NVTXT file.**ClockMonotonicRaw**— timestamps values in the NVTXT file are considered to be gathered on the same target as the report file which is to be merged with NVTXT using`clock_gettime(CLOCK_MONOTONIC_RAW, ...)`

call.**CNTVCT**— timestamps values in the NVTXT file are considered to be gatheredon the same target as the report file which is to be merged with NVTXT using CNTVCT values.
...
- a nanoseconds value (greater than`ns_a`

)`nvtxt_a`

- an nvtxt file’s time unit value corresponding to`ns_a`

nanoseconds`nvtxt_b`

- an nvtxt file’s time unit value corresponding to`ns_b`
...
#### Commands[#](https://docs.nvidia.com#commands)
...
**lerp**— insert with linear interpolation.**lin**— insert with linear equation.
...
## Reading Your Report in GUI[#](https://docs.nvidia.com#reading-your-report-in-gui)

### Generating a New Report[#](https://docs.nvidia.com#generating-a-new-report)
...
### Opening an Existing Report[#](https://docs.nvidia.com#opening-an-existing-report)

An existing `.nsys-rep`

file can be opened using **File > Open…**.

### Report Tab[#](https://docs.nvidia.com#report-tab)
...
**View selector**— Allows switching between*Multi-report view*(absent for single reports),*Analysis Summary*,*Timeline View*,*Diagnostics Summary*, and*Symbol Resolution Logs*views.**Timeline**— This is where all charts are displayed.**Function table**— Located below the timeline, it displays statistical information about functions in the target application in multiple ways.

Additionally, the following controls are available:

**Zoom slider**— Allows you to vertically zoom the charts on the timeline.

### Analysis Summary View[#](https://docs.nvidia.com#analysis-summary-view)
...
**Profiling session information**- including information about the capture time, duration, report file, and host information.**Target information**- including OS and driver versions.**Process summary**- processes run during analysis, arguments, and CPU utilization.**Module summary**- modules used including name and CPU time (overall and per-process).**Note**- Module percentage in Analysis Summary page is calculated based on the cpu cycles logged in an IP sample for that module. When cpu cycles is inaccurate or absent (as is the case on x86_64 target), the module percentage is inaccurate.**Thread summary**- including process ID, name, and CPU utilization**CPU information**- information about all CPUs on the system**GPU information**- information about all GPUs on the system**Network hardware info**- information about NICs/Switches/Storage devices analyzed in the run.**Analysis options**- information about which Nsight Systems options were used when generating this report.**Nsight Systems**- information about the version used to collect and display the results.

Information from this view can be selected and copied using the mouse cursor.

### Diagnostics Summary View[#](https://docs.nvidia.com#diagnostics-summary-view)
...
### Symbol Resolution Logs View[#](https://docs.nvidia.com#symbol-resolution-logs-view)
...
### Timeline View[#](https://docs.nvidia.com#timeline-view)
...
#### Timeline[#](https://docs.nvidia.com#timeline)
...
), and OS Runtime trace data is present, thread states are estimated from OSRT events.
...
#### Zoom and Scroll[#](https://docs.nvidia.com#zoom-and-scroll)
...
#### Pinning Rows[#](https://docs.nvidia.com#pinning-rows)
...
them in the visual range. To select a row to pin, use `Ctrl+P`
...
#### Timeline Correlation[#](https://docs.nvidia.com#timeline-correlation)
...
#### Timeline/Events Correlation[#](https://docs.nvidia.com#timeline-events-correlation)
...
#### Row Height[#](https://docs.nvidia.com#row-height)
...
#### Row Percentage[#](https://docs.nvidia.com#row-percentage)
...
#### Timeline Options[#](https://docs.nvidia.com#timeline-options)

We strongly recommend using the OS/Desktop defaults for size and color, but if
you would like to set them for yourself, they are available using the
...
### Events View[#](https://docs.nvidia.com#events-view)
...
#### Function Table Modes[#](https://docs.nvidia.com#function-table-modes)

The function table can work in three modes:

**Top-Down View**— In this mode, expanding top-level functions provides information about the*callee*functions. One of the top-level functions is typically the main function of your application, or another entry point defined by the runtime libraries.**Bottom-Up View**— This is a reverse of the Top-Down view. On the top level, there are functions directly hit by the sampling profiler. To explore all possible call chains leading to these functions, you need to expand the subtrees of the top-level functions.**Flat View**— This view enumerates all functions ever observed by the profiler, even if they have never been directly hit, but just appeared somewhere on the call stack. This view typically provides a high-level overview of which parts of the code are CPU-intensive.
...
option or the `--show-source-info`

CLI option. This information can help
...
In Top-Down View, source information provides line-level Self % data, allowing more precise bottleneck identification.

In Bottom-Up View, a function can appear multiple times based on its occurrences in the samples. To prevent the Bottom-Up table from becoming unmanageable, Nsight Systems groups identical functions at the same level and aggregates their Self %. This allows you to analyze the performance of individual parts of a function while still viewing the total aggregate performance of the function as a whole. When rows for the same function and module are aggregated from different source lines, the aggregate row may retain source file information but does not show a single Source Line; expand the child rows to inspect each source line. This grouping logic scales recursively throughout the call tree.
...
**Per-thread filtering**— Each thread that has sampling information associated with it has a checkbox next to it on the timeline. Only threads with selected checkboxes are represented in the symbols table.**Time filtering**— A time filter can be setup on the timeline by pressing the left mouse button, dragging over a region of interest on the timeline, and then choosing**Filter by selection**in the dropdown menu. In this case, only sampling information collected during the selected time range will be used to build the symbols table.
...
#### Function Table Notes[#](https://docs.nvidia.com#function-table-notes)
...
### Multi-Report Timeline Views[#](https://docs.nvidia.com#multi-report-timeline-views)

#### Viewing Multiple Reports in Separate Panes[#](https://docs.nvidia.com#viewing-multiple-reports-in-separate-panes)
...
#### Viewing Multiple Reports in the Same Timeline[#](https://docs.nvidia.com#viewing-multiple-reports-in-the-same-timeline)

You can open several reports in a single timeline. This could be done using one of these methods:

**File > Open…**in the main menu, and select several report files.**File > New multi-report view**in the main menu, add report files that you want to open in the Multi-report view, and click the “Apply” button.
...
#### Time Synchronization[#](https://docs.nvidia.com#time-synchronization)
...
other more precise methods are not available. This time can be seen as ```
UTC
time at t=0
```
...
##### vClock Plugin[#](https://docs.nvidia.com#vclock-plugin)

When high-precision system-clock synchronization (such as PTP) is unavailable, the vClock plugin can improve cross-system report alignment. It correlates timestamps in profiling report files collected on different systems by measuring relative offsets between system clocks without changing them. vClock targets clock-correlation error on the order of tens of microseconds without requiring special operating system privileges. Correlation accuracy is affected by network quality between vClock providers.

The vClock plugin is available on the x86_64 and aarch64 Linux targets.

Note

The vClock plugin is experimental and is subject to change in future releases.

The plugin operates in two modes:

A

**provider**runs independently on each system and exchanges clock measurements with providers running on other systems.A

**consumer**is started by Nsight Systems during profiling. It connects to the provider on the same system and records vClock correlation data in the report.

**Set up the providers**

Before profiling, start a vClock provider on each host whose profiling report will be aligned, listing the other hosts as peers:

```
<nsys-target-dir>/plugins/vclock/vclock_plugin --peers <peer-ip>[,<peer-ip>,...]
```

The providers exchange clock measurements while they run. Leave them running until all profiling sessions are complete.

The providers communicate over UDP. Ensure that each provider’s listening port
is reachable from its peers. Use `--port`

to select the listening port.

After all providers are running, allow at least one second for them to exchange initial clock measurements.
...
If a peer port is omitted, the local |
|
|
Provider |
|
...
Shared correlation domain UUID. All participating providers and consumers must use the same UUID. |
|
Provider and consumer |
|
|
Directory for the vClock Unix domain sockets. The consumer and the provider it connects to must use the same directory. |
|
Show the help message. |

For general information about plugin discovery and argument syntax, refer to
[Nsight Systems Plugins](https://docs.nvidia.com#nsight-systems-plugins).
...
for report alignment, the `Report alignment source`
...
#### Timeline Hierarchy[#](https://docs.nvidia.com#timeline-hierarchy)
...
**Different hardware**— is used when reports are coming from different physical machines, and no hardware resources are shared in these reports. This mode is used when neither`NSYS_HW_ID`

or`NSYS_SYSTEM_ID`

is set and target hostnames are different or absent, and can be additionally signalled by specifying different`NSYS_HW_ID`

values.**Different systems, same hardware**— is used when reports are collected on different virtual machines (VMs) or containers on the same physical machine. To activate this mode, specify the same value of`NSYS_HW_ID`

when collecting the reports.**Same system**— is used when reports are collected within the same operating system (or container) environment. In this mode a process identifier (PID) 100 will refer to the same process in both reports. To manually activate this mode, specify the same value of`NSYS_SYSTEM_ID`

when collecting the reports. This mode is automatically selected when target hostnames are the same and neither`NSYS_HW_ID`
...
#### Example: MPI[#](https://docs.nvidia.com#example-mpi)
...
#### Limitations on Syncing Multiple Reports in Timeline[#](https://docs.nvidia.com#limitations-on-syncing-multiple-reports-in-timeline)
...
### Add-on Graphs - Flame Graph[#](https://docs.nvidia.com#add-on-graphs-flame-graph)
...
from[Brendan Gregg’s FlameGraph github](https://github.com/brendangregg/FlameGraph),Perl
...
## Visual Studio Integration[#](https://docs.nvidia.com#visual-studio-integration)
...
### General Troubleshooting[#](https://docs.nvidia.com#general-troubleshooting)
...
hotkey to start Nsight Systems data collection. Let the game continue running while Nsight Systems collects its profiling data (typically 10-60 seconds, or however long is relevant for you). Press`F12`

again to end the collection.
...
on Windows). You can override this
by setting the `NSYS_TMPDIR`

environment variable. This is the recommended
approach when:

`/tmp`

is read-only or has limited storage (e.g., containers, HPC nodes).You need Nsight Systems’s temporary files in a specific location for organizational or debugging purposes.
...
on Linux to store its temporary files and you do not want any conflicts with Nsight Systems’s temporary files.
...
environment variable (Linux) and `TEMP`

/`TMP`

(Windows)
are also respected as fallbacks when `NSYS_TMPDIR`

is not set.

Note

When using `--defer-report`

, make sure `NSYS_TMPDIR`

is set to the
same value for both the profiling run and the `nsys finalize`
...
#### CUDA Troubleshooting[#](https://docs.nvidia.com#cuda-troubleshooting)
...
before exit is not feasible, explicitly end analysis using `duration`

or
`nsys stop`

. The profiler will force a data flush just before the timeout.

### CLI Troubleshooting[#](https://docs.nvidia.com#cli-troubleshooting)
...
this happens automatically. If it does not, use `nsys import`
...
version that generated the .qdstrm file to convert it into
a .nsys-rep file. This .nsys-rep file can then be opened in the same version or
more recent versions of the GUI.

Run `nsys import`

on a system where the Nsight Systems CLI is installed. For
example:
...
the command line. If an `nsys`

argument contains shell metacharacters such as
`*`
...
For example, an NVTX capture range for any domain can be written as either of the following on Linux or macOS:
...
### Launch Processes in Stopped State[#](https://docs.nvidia.com#launch-processes-in-stopped-state)
...
#### LD_PRELOAD[#](https://docs.nvidia.com#ld-preload)
...
#### Launcher[#](https://docs.nvidia.com#launcher)
...
### GUI Troubleshooting[#](https://docs.nvidia.com#gui-troubleshooting)

#### Empty or Black Pages in Analysis or Diagnostics Summary[#](https://docs.nvidia.com#empty-or-black-pages-in-analysis-or-diagnostics-summary)
...
#### xcb-cursor0 or libxcb-cursor0 is needed to load the Qt xcb platform plugin[#](https://docs.nvidia.com#xcb-cursor0-or-libxcb-cursor0-is-needed-to-load-the-qt-xcb-platform-plugin)
...
**OpenSUSE**:sudo dnf install -y xcb-util-cursor

**Debian/Ubuntu**:sudo apt-get install -y libxcb-cursor0


#### Other Libraries Loading Errors[#](https://docs.nvidia.com#other-libraries-loading-errors)
...
##### Ubuntu and CentOS With Root Privileges[#](https://docs.nvidia.com#ubuntu-and-centos-with-root-privileges)
...
##### Ubuntu and CentOS Without Root Privileges[#](https://docs.nvidia.com#ubuntu-and-centos-without-root-privileges)
...
##### Other platforms, or if the previous steps did not help[#](https://docs.nvidia.com#other-platforms-or-if-the-previous-steps-did-not-help)
...
### Symbol Resolution[#](https://docs.nvidia.com#symbol-resolution)
...
directory. This utility works with ELF format files, with Linux Debuginfod cache directories and Linux/QNX symbol servers, with Windows PDB directories and symbol servers, or with files where each line is in the format `<start><length><name>`
...
#### Broken Backtraces on Tegra[#](https://docs.nvidia.com#broken-backtraces-on-tegra)
...
#### Debug Versions of ELF Files[#](https://docs.nvidia.com#debug-versions-of-elf-files)
...
For target devices with Ubuntu, see [Debug Symbol Packages](https://wiki.ubuntu.com/Debug_Symbol_packages). These packages typically install debug ELF files with `/usr/lib/debug`
...
suffix. For other packages, refer to the [Debug Symbol Packages](https://wiki.ubuntu.com/Debug_Symbol_packages) wiki page on how to add the debs package repository. After setting up the repository and running apt-get update, look for packages with `-dbgsym`
...
### Logging[#](https://docs.nvidia.com#logging)
...
#### Verbose Remote Logging on Linux Targets[#](https://docs.nvidia.com#verbose-remote-logging-on-linux-targets)
...
#### Verbose CLI Logging on Linux Targets[#](https://docs.nvidia.com#verbose-cli-logging-on-linux-targets)
...
directory will then include a file named`nsys-agent.log`
...
#### Verbose Logging on Windows Targets[#](https://docs.nvidia.com#verbose-logging-on-windows-targets)
...
```
C:\Program Files\NVIDIA Corporation\Nsight Systems <version>
```
...
Local Windows on Arm target:

C:\Program Files\NVIDIA Corporation\Nsight Systems <version>\target-windows-armv8
...
## Other Resources[#](https://docs.nvidia.com#other-resources)
...
### Training Seminars[#](https://docs.nvidia.com#training-seminars)

NVIDIA Deep Learning Institute Training - Self-Paced Online Course - [Nsight
Analysis System: Build Custom Python Analysis Scripts](https://learn.nvidia.com/courses/course-detail?course_id=course-v1:DLI+S-AC-13+V1)

NVIDIA Deep Learning Institute Training - Self-Paced Online Course - [Optimizing
CUDA Machine Learning Codes With Nsight Profiling Tools](https://learn.nvidia.com/courses/course-detail?course_id=course-v1:DLI+S-AC-03+V2)

CUDA Developer Tools YouTube Channel - [Intro to NVIDIA Nsight Systems](https://www.youtube.com/watch?v=dUDGO66IadU)

2018 NCSA Blue Waters Webinar - Video Only - [Introduction to NVIDIA Nsight
Systems](https://www.youtube.com/watch?v=WA8C48FJi3c)

### Blog Posts[#](https://docs.nvidia.com#blog-posts)
...
[Profiling LLM Training Workflows on NVIDIA Grace Hopper](https://developer.nvidia.com/blog/profiling-llm-training-workflows-on-nvidia-grace-hopper/)2025 :

[Advanced Optimization Strategies for LLM Training on NVIDIA Grace Hopper](https://developer.nvidia.com/blog/advanced-optimization-strategies-for-llm-training-on-nvidia-grace-hopper/)2023 :

[Optimizing CUDA Memory Transfers with NVIDIA Nsight Systems](https://developer.nvidia.com/blog/optimizing-cuda-memory-transfers-with-nsight-systems/)2023 :

[Accelerating Data Center and HPC Performance Analysis with NVIDIA Nsight Systems](https://developer.nvidia.com/blog/accelerating-data-center-and-hpc-performance-analysis-with-nvidia-nsight-systems/)2021 :

[NVIDIA Tools Extension API: An Annotation Tool for Profiling Code in Python and C/C++](https://developer.nvidia.com/blog/nvidia-tools-extension-api-nvtx-annotation-tool-for-profiling-code-in-python-and-c-c/)2021 :

[Optimizing DX12 Resource Uploads to the GPU Using CPU-Visible VRAM](https://developer.nvidia.com/blog/optimizing-dx12-resource-uploads-to-the-gpu-using-cpu-visible-vram/)2020 :

[Understanding the Visualization of Overhead and Latency in Nsight Systems](https://developer.nvidia.com/blog/understanding-the-visualization-of-overhead-and-latency-in-nsight-systems/)2019 :

[Migrating to NVIDIA Nsight Tools from NVVP and nvprof](https://devblogs.nvidia.com/migrating-nvidia-nsight-tools-nvvp-nvprof/)2019 :
...
### Feature Videos[#](https://docs.nvidia.com#feature-videos)

Short videos, only a minute or two, to introduce new features.

### Conference Presentations[#](https://docs.nvidia.com#conference-presentations)

GTC 2025 -

[Optimizing Multi-Language Scientific Simulations: A Grace Superchip Case Study](https://www.nvidia.com/en-us/on-demand/session/gtc25-s72537/)GTC 2024 -

[Achieving Higher Performance From Your Data Center and Cloud Application](https://www.nvidia.com/en-us/on-demand/session/gtc24-s62388/)Jetson Edge AI Developer Days 2023 -

[Getting the Most Out of Your Jetson Orin Using NVIDIA Nsight Developer Tools](https://www.nvidia.com/en-us/on-demand/session/gtcspring23-se52434/)GTC 2023 -

[Optimizing at Scale: Investigating Hidden Bottlenecks for Multi-Node Workloads](https://www.nvidia.com/en-us/on-demand/session/gtcspring23-s51421/)GTC 2023 -

[Optimize Multi-Node System Workloads With NVIDIA Nsight Systems](https://www.nvidia.com/en-us/on-demand/session/gtcspring23-d52660/)GTC 2023 -

[Ray-Tracing Development using NVIDIA Nsight Graphics and NVIDIA Nsight Systems](https://www.nvidia.com/en-us/on-demand/session/gtcspring23-dlit51580/)GTC 2022 -

[Optimizing Communication with Nsight Systems Network Profiling](https://www.nvidia.com/en-us/on-demand/session/gtcspring22-s41500/)GTC 2022 -

[Optimizing Vulkan 1.3 Applications with Nsight Graphics & Nsight Systems](https://www.nvidia.com/en-us/on-demand/session/other2021-vlk2201/)GTC 2021 -

[Tuning GPU Network and Memory Usage in Apache Spark](https://www.nvidia.com/en-us/on-demand/session/gtcspring21-s31566/)GTC 2020 -

[Scaling the Transformer Model Implementation in PyTorch Across Multiple Nodes](https://developer.nvidia.com/gtc/2020/video/s21351)GTC 2019 -

[Using Nsight Tools to Optimize the NAMD Molecular Dynamics Simulation Program](https://on-demand-gtc.gputechconf.com/gtcnew/sessionview.php?sessionName=s9503-using+nsight+tools+to+optimize+the+namd+molecular+dynamics+simulation+program)GTC 2018 -

[Optimizing HPC Simulation and Visualization Codes Using NVIDIA Nsight Systems](https://on-demand-gtc.gputechconf.com/gtcnew/sessionview.php?sessionName=s8718-optimizing+hpc+simulation+and+visualization+codes+using+the+nvidia+nsight+systems)GTC 2018 - Israel -

[Boost DNN Training Performance using NVIDIA Tools](https://on-demand-gtc.gputechconf.com/gtcnew/sessionview.php?sessionName=sil8105-boost+dnn+training+performance+using+nvidia+tools)Siggraph 2018 -

[Taming the Beast; Using NVIDIA Tools to Unlock Hidden GPU Performance](http://on-demand.gputechconf.com/siggraph/2018/video/sig1805-aurelio-reis-yaki-tebeka-taming-the-beast.html)

### For More Support[#](https://docs.nvidia.com#for-more-support)
