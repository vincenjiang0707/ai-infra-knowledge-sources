...
[Section 5.1.1](https://docs.nvidia.com/05-appendices/compute-capabilities.html#compute-capabilities-querying) shows how to query and determine the compute capability of the GPU(s) in a system.

## 1.3.2. CUDA Toolkit and NVIDIA Driver[#](https://docs.nvidia.com#cuda-toolkit-and-nvidia-driver)
...
## 1.3.3. Parallel Thread Execution (PTX)[#](https://docs.nvidia.com#parallel-thread-execution-ptx)
...
## 1.3.4. Cubins and Fatbins[#](https://docs.nvidia.com#cubins-and-fatbins)
...
Binary compatibility is promised only for binaries created by NVIDIA tools such as `nvcc`

. Manual editing or generating binary code for NVIDIA GPUs is not supported. Compatibility promises are invalidated if binaries are modified in any way.

### 1.3.4.2. PTX Compatibility[#](https://docs.nvidia.com#ptx-compatibility)

GPU code can be stored in executables in binary or PTX form, which is covered in [Cubins and Fatbins](https://docs.nvidia.com#cuda-platform-cubins-fatbins). When an application stores the PTX version of GPU code, that PTX can be JIT compiled at application runtime for any compute capability equal or higher to the compute capability of the PTX code. For example, if an application contains PTX for compute_80, that PTX code can be JIT compiled to later SM versions, such as sm_120 at application runtime. This enables forward compatibility with future GPUs without the need to rebuild applications or libraries.

### 1.3.4.3. Just-in-Time Compilation[#](https://docs.nvidia.com#just-in-time-compilation)
...
As with PTX JIT compilation, the NVIDIA driver may cache the results of off-target finalization of cubins to avoid repeating finalization work in subsequent invocations of the application. The cache is controlled by the same CUDA cache environment variables described in [Section 5.2.2](https://docs.nvidia.com/05-appendices/environment-variables.html#cuda-environment-variables-compilation).