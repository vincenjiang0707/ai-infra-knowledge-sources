source: https://docs.mthreads.com/mtstream/mtstream-doc-online/introduction
...
同时，MTStream SDK 深度集成以下 M1000 的硬件引擎，确保每一步都�由专用硬件完成，最大化吞吐量并降低 CPU 负载：
...
| DMABUF | Linux 内核提供的跨设备共享��内存机制。MTStream SDK 所有插件间通过 DMABUF 传递帧，无内存拷贝，极大提升吞吐效率 |
| Pipeline 控制器 | GStreamer 自动管理状态机、时钟同步、缓冲区生命周期，无需用户干预 |
