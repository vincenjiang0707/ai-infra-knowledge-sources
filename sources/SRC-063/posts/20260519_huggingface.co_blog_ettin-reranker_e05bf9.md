...
Text Ranking • 0.1B • Updated • 9.03k • 3

#
[
](https://huggingface.co#introducing-the-ettin-reranker-family)
Introducing the Ettin Reranker Family
...
[What is a reranker, and why pair one with an embedder?](https://huggingface.co#what-is-a-reranker-and-why-pair-one-with-an-embedder)[Usage](https://huggingface.co#usage)[Architecture Details](https://huggingface.co#architecture-details)[Results](https://huggingface.co#results)[Training](https://huggingface.co#training)[Conclusion](https://huggingface.co#conclusion)[Acknowledgements](https://huggingface.co#acknowledgements)

##
[
](https://huggingface.co#what-is-a-reranker-and-why-pair-one-with-an-embedder)
What is a reranker, and why pair one with an embedder?
...
for the highest throughput. See the [Speed](https://huggingface.co#speed)section below for more details, but in general you can expect a 1.7x-8.3x speedup over default loading depending on model size and sequence length.
...
###
[
](https://huggingface.co#end-to-end-retrieve-then-rerank-pipeline)
...
##
[
](https://huggingface.co#architecture-details)
...
, which lets us use sequence unpadding for variable-length inputs for Flash Attention 2. At medium-document sequence lengths this is a 1.7x-8.3x speedup over fp32+SDPA depending on model size (see [Speed](https://huggingface.co#speed) for the full benchmark):
...
###
[
](https://huggingface.co#mtebeng-v2-retrieval)
...
was set to during training (see [Evaluation](https://huggingface.co#evaluation)), and what I used to guide the experimentation.
...
). For most retrieve-then-rerank workloads, our 1B at a quarter of the parameters (see [Speed](https://huggingface.co#speed)) is a much more practical pick.

###
[
](https://huggingface.co#speed)
Speed
...
module (see [Architecture Details](https://huggingface.co#architecture-details) above) propagates unpadded inputs all the way through the model, so every layer only spends compute on real tokens. That's the difference between getting some of FA2's benefit and getting all of it.
...
**7517 (1.71x)****1.4 GB**`ettin-reranker-32m-v1`
...
**6602 (2.00x)****1.1 GB**`ettin-reranker-68m-v1`
...
**4913 (3.60x)****1.5 GB**`ettin-reranker-150m-v1`
...
**3237 (4.83x)****1.4 GB**`ettin-reranker-400m-v1`
...
**1738 (6.53x)****2.2 GB**`ettin-reranker-1b-v1`
...
module from the [Architecture Details](https://huggingface.co#architecture-details) section is set up for. The full snippet is the same as in the [Usage](https://huggingface.co#usage) section above:
...
###
[
](https://huggingface.co#distillation-recipe)
Distillation recipe
...
###
[
](https://huggingface.co#overall-training-script)
...
triples, kept as 39 named splits so the provenance of every row is visible.**Training script**: the ~150 lines in[Overall Training Script](https://huggingface.co#overall-training-script)above, which is the same script used for all six models.
