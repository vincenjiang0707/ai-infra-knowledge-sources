...
source: https://huggingface.co/blog/build-rocm-kernels
published: Mon, 17 Nov 2025 00:00:00 GMT

# Easily Build and Share ROCm Kernels with Hugging Face

[Update on GitHub](https://github.com/huggingface/blog/blob/main/build-rocm-kernels.md)


##
[
](https://huggingface.co#intoduction)
Intoduction
...
[) and share these kernels with the](https://github.com/huggingface/kernels/tree/main/builder)

**kernel-builder**[, with support for multiple GPU and accelerator backends, including CUDA, ROCm, Metal, and XPU. This ensures your kernels are fast, portable, and seamlessly integrated with PyTorch.](https://huggingface.co/kernels-community)

**kernels-community**In this guide, we focus exclusively on ROCm-compatible kernels and show how to build, test, and share them using [kernels](https://github.com/huggingface/kernels/tree/main). You’ll learn how to create kernels that run efficiently on AMD GPUs, along with best practices for reproducibility, packaging, and deployment.
...
##
[
](https://huggingface.co#build-steps)
Build Steps

We will use the GEMM kernel from [RadeonFlow_Kernels](https://github.com/RadeonFlow/RadeonFlow_Kernels) as an example. If you want to go straight to the guide, [click here](https://huggingface.co#step-1-project-structure).

###
[
](https://huggingface.co#about-the-kernel)
About the kernel

This section was written by the

RadeonFlow GEMMkernel authors to introduce the kernel.
...
is K × M in e4m3fnuz`b`

is K × N in e4m3fnuz`a_scale`

is (K // 128) × M in fp32`b_scale`

is (K // 128) × (N // 128) in fp32`c`
...
So now that we have a high-performance ROCm kernel, the natural question is: how do we integrate it into a real PyTorch workflow and share it with others? That’s exactly what we’ll cover next, using `kernels`
...
###
[
](https://huggingface.co#step-1-project-structure)
...
**build.toml**: The project manifest; it’s the brain of the build process.**gemm/**: Your raw CUDA source code where the GPU magic happens.**flake.nix**: The key to a perfectly reproducible build environment.**torch-ext/gemm/**: The Python wrapper for the raw PyTorch operators
...
is a HIP implementation file. This naming convention helps the kernel-builder ([ kernels/builder](https://github.com/huggingface/kernels/tree/main/builder)) correctly identify and compile each file type.

###
[
](https://huggingface.co#step-2-configuration-files-setup)
Step 2: Configuration Files Setup

####
[
](https://huggingface.co#the-buildtoml-manifest)
The `build.toml`
...
**name**(required): The name of your project. This should match your kernel name and will be used for the Python package.**universal**(optional): the kernel is a universal kernel when set to`true`
...
**backend**(required): The compute backend for the kernel. We use "rocm" for AMD GPU support.**rocm-archs**(required for ROCm): A list of ROCm architectures that the kernel should be compiled for. "gfx942" targets the MI300 series GPUs.**depends**(required): A list of dependencies. We depend on "torch" to use PyTorch's tensor operations.**include**(optional): Include directories relative to the project root. This helps the compiler find header files.

####
[
](https://huggingface.co#the-flakenix-reproducibility-file)
The `flake.nix`
...
####
[
](https://huggingface.co#writing-the-kernel)
Writing the Kernel
...
####
[
](https://huggingface.co#registering-a-native-pytorch-operator)
...
####
[
](https://huggingface.co#setting-up-the-__init__py-wrapper)
...
###
[
](https://huggingface.co#step-3-building-the-kernel)
...
**Linux**: use the[official Nix installer](https://nixos.org/download/).**macOS**: use the[Determinate Nix installer](https://docs.determinate.systems/determinate-nix/). In addition, Xcode 16.x is currently required to build kernels.

####
[
](https://huggingface.co#getting-started-with-nix)
...
####
[
](https://huggingface.co#building-kernels-with-nix)
...
####
[
](https://huggingface.co#development-shell-for-local-development)
...
###
[
](https://huggingface.co#step-4-uploading-the-kernel-to-the-hub)
...
####
[
](https://huggingface.co#building-the-kernel-for-all-pytorch-and-rocm-versions)
...
####
[
](https://huggingface.co#pushing-to-the-hugging-face-hub)
Pushing to the Hugging Face Hub

Pushing the build artifacts to the Hub will make it straightforward for other developers to use your kernel. We can use the `kernels upload`
...
###
[
](https://huggingface.co#step-5-lets-use-it-)
Step 5: Let's use it :)
...
##
[
](https://huggingface.co#related-libraries--hub)
Related Libraries & Hub

[kernels](https://github.com/huggingface/kernels)– Library to build, manage and load kernels from the Hub. It contains the[kernel-builder](https://github.com/huggingface/kernels/tree/main/builder)tooling.[Kernels Community Hub](https://huggingface.co/kernels-community)– Share and discover kernels from the community.