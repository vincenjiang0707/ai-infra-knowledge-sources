...
Viewer • Updated • 228k • 926 • 4

#
[
](https://huggingface.co#vision-language-model-alignment-in-trl-⚡️)
Vision Language Model Alignment in TRL ⚡️
...
Vision Language Models (VLMs) are getting stronger, but *aligning* them to human preferences still matters. In TRL, we already showed how to post-train VLMs with [ Supervised Fine-Tuning (SFT)](https://huggingface.co/docs/trl/main/en/training_vlm_sft) and

[. This time, we’re going further.](https://huggingface.co/learn/cookbook/fine_tuning_vlm_dpo_smolvlm_instruct)

**Direct Preference Optimization (DPO)****tl;dr** Here’s what’s new in TRL:

**Mixed Preference Optimization (MPO)****Group Relative Policy Optimization (GRPO)****Group Sequence Policy Optimization (GSPO)**(a variant of GRPO)
...
**Reinforce Leave One Out (RLOO)****Online Direct Preference Optimization (Online DPO)**

This enables more efficient and scalable multimodal alignment.

Finally:

**Native Supervised Fine-tuning support for Vision Language Models****Training scripts and demo notebooks**to help you get started quickly
...
[Multimodal Alignment for VLMs in TRL ⚡️](https://huggingface.co#multimodal-alignment-for-vlms-in-trl-%EF%B8%8F)

##
[
](https://huggingface.co#alignment-for-vision-language-models)
...
###
[
](https://huggingface.co#mixed-preference-optimization-mpo)
...
Since this is only modifying the loss, we added combined loss support to TRL's `DPOTrainer`

class. To use it, you can initialize the `DPOConfig`
...
###
[
](https://huggingface.co#multimodal-group-relative-policy-optimization-grpo)
...
###
[
](https://huggingface.co#group-sequence-policy-optimization-gspo)
...
###
[
](https://huggingface.co#comparison)
Comparison
...
###
[
](https://huggingface.co#further-extensions-for-vlms)
...
####
[
](https://huggingface.co#reinforce-leave-one-out-rloo)
...
####
[
](https://huggingface.co#online-direct-preference-optimization-online-dpo)
...
To train a VLM, you need to provide a dataset with an additional `images`
...
##
[
](https://huggingface.co#vllm-integration-in-trl)
...
There’s mainly two modes: `colocate`

and `server`

. [ colocate](https://huggingface.co/blog/vllm-colocate) runs vLLM in the same process as the training loop, sharing the same GPU between training and generation, creating a vLLM LLM instance inside the

`GRPOTrainer`

. Meanwhile `server`

requires you to serve vLLM separately in a different process where you can hit the server. You can start this server with the command:```
...
One more tip: we have added support for using vLLM with transformers backend in TRL. You can enable it when running a script with colocate or when serving the model by passing the `--vllm_model_impl transformers`
...
##
[
](https://huggingface.co#useful-resources)
Useful Resources

Below, you can find a compilation of resources to explore the alignment of VLMs in detail. Enjoy!

**Vision Language Models (Better, Faster, Stronger)**(**Enhancing the Reasoning Ability of Multimodal Large Language Models via Mixed Preference Optimization****MPO paper**)(**DeepSeekMath: Pushing the Limits of Mathematical Reasoning in Open Language Model****GRPO paper**)**Open-R1****repository**and**Open-R1 reward functions**and**TRL documentation****TRL repository****MPO VLM recipe****GRPO VLM recipe****More multimodal alignment recipes****TRL multimodal training scripts****vLLM Integration in trl docs****Transformers backend integration in vLLM**