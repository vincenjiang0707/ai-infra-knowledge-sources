...
#
[
](https://huggingface.co#from-zero-to-gpu-a-guide-to-building-and-scaling-production-ready-cuda-kernels)
From Zero to GPU: A Guide to Building and Scaling Production-Ready CUDA Kernels

[Update on GitHub](https://github.com/huggingface/blog/blob/main/kernel-builder.md)
...
##
[
](https://huggingface.co#what-youll-learn)
What You’ll Learn
...
##
[
](https://huggingface.co#part-1-anatomy-of-a-modern-cuda-kernel)
...
: The project manifest; it’s the brain of the build process.`build.toml`

: Your raw CUDA source code where the GPU magic happens.`csrc/`

: The key to a perfectly reproducible* build environment.`flake.nix`

: The Python wrapper for the raw PyTorch operators.`torch-ext/img2gray/`


###
[
](https://huggingface.co#step-2-the-buildtoml-manifest)
Step 2: The `build.toml`

Manifest

This file orchestrates the entire build. It tells the `kernel-builder`

what to compile and how everything connects.
...
###
[
](https://huggingface.co#step-3-the-flakenix-reproducibility-file)
Step 3: The `flake.nix`

Reproducibility File

To ensure anyone can build your kernel on any machine, we use a `flake.nix`

file. It locks the exact version of the `kernel-builder`

and its dependencies, eliminating "it works on my machine" issues.
...
###
[
](https://huggingface.co#step-4-writing-the-cuda-kernel)
...
###
[
](https://huggingface.co#step-5-registering-a-native-pytorch-operator)
Step 5: Registering a Native PyTorch Operator

This is the most important step. We're not just binding to Python; we're registering our function as a native PyTorch operator. This makes it a first-class citizen in the PyTorch ecosystem, visible under the `torch.ops`
...
**Compatibility with**: By registering our kernel this way,`torch.compile`

`torch.compile`

can "see" it. This allows PyTorch to fuse your custom operator into larger computation graphs, minimizing overhead and maximizing performance. It's the key to making your custom code work seamlessly with PyTorch's broader performance ecosystem.**Hardware-Specific Implementations**: This system allows you to provide different backends for the same operator. You could add another`TORCH_LIBRARY_IMPL(img2gray, CPU, ...)`

block pointing to a C++ CPU function. PyTorch's dispatcher would then automatically call the correct implementation (CUDA or CPU) based on the input tensor's device, making your code powerful and portable.**Setting up the**`__init__.py`
...
###
[
](https://huggingface.co#step-6-building-the-kernel)
...
####
[
](https://huggingface.co#drop-into-a-nix-shell)
...
####
[
](https://huggingface.co#set-up-build-artifacts)
...
####
[
](https://huggingface.co#create-a-python-virtual-environment)
...
####
[
](https://huggingface.co#compile-the-kernel-and-install-the-python-package)
...
####
[
](https://huggingface.co#development-cycle)
Development Cycle
...
###
[
](https://huggingface.co#step-7-sharing-with-the-world)
...
####
[
](https://huggingface.co#building-the-kernel-for-all-pytorch-and-cuda-versions)
...
[supported build variants], keeping them current with the latest PyTorch and CUDA releases while also supporting trailing versions for broader compatibility.
...
Pushing the build artifacts to the Hub will make it straightforward for other developers to use your kernel, as we saw in [our previous post](https://huggingface.co/blog/hello-hf-kernels). We can use the `kernels upload`
...
library. Our kernel and all of its build variants are now available at [drbh/img2gray](https://huggingface.co/drbh/img2gray/tree/main/build).

###
[
](https://huggingface.co#step-8-loading-and-testing-your-custom-op)
...
##
[
](https://huggingface.co#part-2-from-one-kernel-to-many-solving-production-challenges)
...
###
[
](https://huggingface.co#kernel-versions)
Kernel Versions
...
series. The version bound can be a Python-style [version specifier](https://packaging.pypa.io/en/stable/specifiers.html).
...
####
[
](https://huggingface.co#locking-kernels)
Locking Kernels
...
###
[
](https://huggingface.co#pre-downloading-locked-kernels)
...
[HF_HOME]caching rules apply.

###
[
](https://huggingface.co#creating-legacy-python-wheels)
...
. Whether you’re a seasoned developer or just starting out, the community is here to support you.
