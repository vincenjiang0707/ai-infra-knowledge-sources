...
source: https://huggingface.co/blog/nvidia-reachy-mini
published: Mon, 05 Jan 2026 00:00:00 GMT

Robotics • 3B • Updated • 32.4k • 91

#
[
](https://huggingface.co#nvidia-brings-agents-to-life-with-dgx-spark-and-reachy-mini)
NVIDIA brings agents to life with DGX Spark and Reachy Mini

[Update on GitHub](https://github.com/huggingface/blog/blob/main/nvidia-reachy-mini.md)
...
##
[
](https://huggingface.co#ingredients)
Ingredients
...
- A reasoning model: demo uses
[NVIDIA Nemotron 3 Nano](https://huggingface.co/nvidia/NVIDIA-Nemotron-3-Nano-30B-A3B-BF16) - A vision model: demo uses
[NVIDIA Nemotron Nano 2 VL](https://huggingface.co/nvidia/NVIDIA-Nemotron-Nano-12B-v2-VL-BF16) - A text-to-speech model: demo uses
[ElevenLabs](https://elevenlabs.io) [Reachy Mini](https://www.pollen-robotics.com/reachy-mini/)(or[Reachy Mini Simulation](https://github.com/pollen-robotics/reachy_mini/blob/develop/docs/platforms/simulation/get_started.md))- Python v3.10+ environment, with
...
[DGX Spark](https://www.nvidia.com/en-us/products/workstations/dgx-spark/)or a GPU with sufficient VRAM). Our implementation requires ~65GB disk space for the reasoning model, and ~28GB for the vision model. - Cloud deployment– Deploy the models on cloud GPUs e.g. through
[NVIDIA Brev](http://build.nvidia.com/gpu)or[Hugging Face Inference Endpoints](https://endpoints.huggingface.co/). - Serverless model endpoints – Send requests to
[NVIDIA](https://build.nvidia.com/nvidia/nemotron-3-nano-30b-a3b/deploy)or[Hugging Face Inference Providers](https://huggingface.co/docs/inference-providers/en/index).

##
[
](https://huggingface.co#giving-agentic-powers-to-reachy)
...
##
[
](https://huggingface.co#building-the-agent)
Building the agent
...
##
[
](https://huggingface.co#step-0-set-up-and-get-access-to-models-and-services)
...
##
[
](https://huggingface.co#step-1-build-a-chat-interface)
...
##
[
](https://huggingface.co#step-2-add-nemo-agent-toolkits-built-in-react-agent-for-tool-calling)
...
- Put a hard cap on steps (max_tool_calls) so the agent can’t spiral.
...
##
[
](https://huggingface.co#step-3-add-a-router-to-direct-queries-to-different-models)
...
##
[
](https://huggingface.co#step-4-add-a-pipecat-bot-for-real-time-voice--vision)
...
##
[
](https://huggingface.co#step-5-hook-everything-up-to-reachy-hardware-or-simulation)
...
###
[
](https://huggingface.co#run-the-full-system)
Run the full system
...
####
[
](https://huggingface.co#terminal-1-reachy-daemon)
...
####
[
](https://huggingface.co#terminal-2-bot-service)
...
####
[
](https://huggingface.co#terminal-3-nemo-agent-toolkit-service)
...
**Reachy Sim**– This window appears automatically when you start the simulator daemon in Terminal 1. This is applicable if you’re running Reachy mini simulation in place of the physical device.**Pipecat Playground**– This is the client-side UI where you can connect to the agent, enable microphone and camera inputs, and view live transcripts. In Terminal 2, open the URL exposed by the bot service:[http://localhost:7860/](http://localhost:7860/client/). Click “CONNECT” in your browser. It may take a few seconds to initialize, and you’ll be prompted to grant microphone (and optionally camera) access.Once both windows are up and running:
...
##
[
](https://huggingface.co#try-these-example-prompts)
...
##
[
](https://huggingface.co#where-to-go-next)
Where to go next
...
[LLM Router developer example](https://github.com/NVIDIA-AI-Blueprints/llm-router/tree/experimental)to balance cost, latency, and quality by intelligently directing queries between different models. - Check out the tutorial for building a voice-powered RAG agent with guardrails using Nemotron open models.
...
[apps](https://huggingface.co/spaces/pollen-robotics/Reachy_Mini#apps)built by the community for Reachy.
