...
#
[
](https://huggingface.co#multi-vector-late-interaction-embedding-models-with-sentence-transformers)
Multi-Vector (Late Interaction) Embedding Models with Sentence Transformers
...
This blog is about using multi-vector models. If you want to learn how to

trainthem, see the companion[Training and Finetuning Multi-Vector Embedding Models with Sentence Transformers]blogpost.
...
[What are Multi-Vector Models?](https://huggingface.co#what-are-multi-vector-models)[Installation](https://huggingface.co#installation)[Loading a Model](https://huggingface.co#loading-a-model)[Encoding Queries and Documents](https://huggingface.co#encoding-queries-and-documents)[Scoring with MaxSim](https://huggingface.co#scoring-with-maxsim)[Semantic Search](https://huggingface.co#semantic-search)[Retrieve and Rerank](https://huggingface.co#retrieve-and-rerank)[Indexing](https://huggingface.co#indexing)[Visual Document Retrieval](https://huggingface.co#visual-document-retrieval)[Audio Retrieval](https://huggingface.co#audio-retrieval)[Video Retrieval](https://huggingface.co#video-retrieval)[Interpretability](https://huggingface.co#interpretability)[Token Pooling](https://huggingface.co#token-pooling)[Speeding Up Inference](https://huggingface.co#speeding-up-inference)[Evaluating a Model](https://huggingface.co#evaluating-a-model)[Coming from PyLate or colpali-engine](https://huggingface.co#coming-from-pylate-or-colpali-engine)[Supported Models](https://huggingface.co#supported-models)[Acknowledgements](https://huggingface.co#acknowledgements)[Additional Resources](https://huggingface.co#additional-resources)
...
###
[
](https://huggingface.co#the-maxsim-operator)
The MaxSim Operator
...
[
](https://huggingface.co#what-you-gain-and-what-it-costs)
...
###
[
](https://huggingface.co#inspecting-what-a-checkpoint-configured)
...
What you get back is a *list* of 2D tensors, one per input, each of shape `(num_tokens, embedding_dim)`
...
##
[
](https://huggingface.co#scoring-with-maxsim)
Scoring with MaxSim
...
###
[
](https://huggingface.co#score-magnitude-and-meanmaxsim)
...
##
[
](https://huggingface.co#semantic-search)
Semantic Search
...
Past that size you want a real late-interaction index, which Sentence Transformers doesn't ship. It doesn't need to, because these indexes store whatever `encode_document`

produced, so you encode here and hand the token embeddings to something built for them. [Indexing](https://huggingface.co#indexing) has working snippets for four of the options, and the section directly below covers how to skip the index entirely.
...
##
[
](https://huggingface.co#visual-document-retrieval)
...
pinning the open pull request that adds this one's Sentence Transformers configuration ([Supported Models](https://huggingface.co#visual-document-retrieval-models) has the full list). Image documents are passed as URLs, local paths, or PIL images:
...
##
[
](https://huggingface.co#audio-retrieval)
Audio Retrieval
...
##
[
](https://huggingface.co#video-retrieval)
Video Retrieval
...
##
[
](https://huggingface.co#interpretability)
Interpretability
...
. Three special tokens contribute 22.7% of the score while carrying none of the query's content. Below this table the script prints the passage itself, with the winning tokens highlighted in place.

##
[
](https://huggingface.co#token-pooling)
Token Pooling

If the index footprint worries you, the most effective knob is to store fewer token vectors. `HierarchicalTokenPooling`

implements the [token pooling](https://arxiv.org/abs/2409.14683v1) technique from Clavié, Chaffin, and Adams by clustering each document's token vectors with Ward linkage on cosine distance and replacing each cluster with its mean, keeping roughly `1 / pool_factor`
...
A cluster mean is a worse match for a query token than the best of its members was, and the coarser the clusters, the more that shows. The [original experiments](https://arxiv.org/abs/2409.14683v1) measured that cost on BEIR and found very little of it, at 100.6% of the unpooled retrieval performance on average at `pool_factor=2`

, and 99.0% at `pool_factor=3`

. Halving your index for free is a good deal, so 2 is a reasonable place to start. How much it costs on your data is corpus-specific though, so measure it with an [evaluator](https://huggingface.co#evaluating-a-model) before you settle on a factor. The runnable comparison is [token_pooling.py](https://github.com/huggingface/sentence-transformers/blob/main/examples/multi_vector_encoder/compression/token_pooling.py).
...
##
[
](https://huggingface.co#speeding-up-inference)
...
##
[
](https://huggingface.co#evaluating-a-model)
Evaluating a Model
...
##
[
](https://huggingface.co#coming-from-pylate-or-colpali-engine)
Coming from PyLate or colpali-engine

`MultiVectorEncoder`

absorbs the modeling, inference, training, and evaluation of both libraries. Every PyLate checkpoint loads directly, and [Supported Models](https://huggingface.co#supported-models) lists the colpali-engine checkpoints along with the `revision`
...
The difference worth knowing about is on bare (non-ColBERT) checkpoints, where PyLate's `ColBERT("bert-base-uncased")`

applies the classic recipe by default while `MultiVectorEncoder("bert-base-uncased")`

builds a plain stack, leaving the prefixes, query expansion, and skiplist as explicit choices. The training loss and evaluator equivalents, and the data-handling differences, are in the [Migration Guide](https://sbert.net/docs/migration_guide.html#migrating-from-pylate).

Save compatibility is one-way in every case. PyLate, Stanford-NLP ColBERT, and colpali-engine checkpoints all load into `MultiVectorEncoder`
...
###
[
](https://huggingface.co#text-retrieval-models)
...
[vultr/VultronRetrieverPrime-Qwen3.5-8B](https://huggingface.co/vultr/VultronRetrieverPrime-Qwen3.5-8B)`revision="refs/pr/2"`

[vultr/VultronRetrieverCore-Qwen3.5-4.5B](https://huggingface.co/vultr/VultronRetrieverCore-Qwen3.5-4.5B)`revision="refs/pr/1"`

[tencent/EVIE-Preview-4.5B](https://huggingface.co/tencent/EVIE-Preview-4.5B)[nvidia/nemotron-colembed-vl-8b-v2](https://huggingface.co/nvidia/nemotron-colembed-vl-8b-v2)`revision="refs/pr/4"`

, needs `trust_remote_code=True`

[athrael-soju/colqwen3.5-4.5B-v3](https://huggingface.co/athrael-soju/colqwen3.5-4.5B-v3)[TomoroAI/tomoro-colqwen3-embed-8b](https://huggingface.co/TomoroAI/tomoro-colqwen3-embed-8b)`trust_remote_code=True`

[nvidia/nemotron-colembed-vl-4b-v2](https://huggingface.co/nvidia/nemotron-colembed-vl-4b-v2)`revision="refs/pr/7"`

, needs `trust_remote_code=True`

[OpenSearch-AI/Ops-Colqwen3-4B](https://huggingface.co/OpenSearch-AI/Ops-Colqwen3-4B)`revision="refs/pr/5"`

, needs `trust_remote_code=True`

[nvidia/llama-nemotron-colembed-vl-3b-v2](https://huggingface.co/nvidia/llama-nemotron-colembed-vl-3b-v2)`revision="refs/pr/3"`

, needs `trust_remote_code=True`

[tsystems/colqwen2.5-3b-multilingual-v1.0](https://huggingface.co/tsystems/colqwen2.5-3b-multilingual-v1.0)`revision="refs/pr/3"`

[tsystems/colqwen2.5-3b-multilingual-v1.0-merged](https://huggingface.co/tsystems/colqwen2.5-3b-multilingual-v1.0-merged)`revision="refs/pr/1"`

[TomoroAI/tomoro-colqwen3-embed-4b](https://huggingface.co/TomoroAI/tomoro-colqwen3-embed-4b)`trust_remote_code=True`

[nomic-ai/colnomic-embed-multimodal-7b](https://huggingface.co/nomic-ai/colnomic-embed-multimodal-7b)`revision="refs/pr/3"`

[nomic-ai/colnomic-embed-multimodal-3b](https://huggingface.co/nomic-ai/colnomic-embed-multimodal-3b)`revision="refs/pr/6"`

[VAGOsolutions/SauerkrautLM-ColQwen3-8b-v0.1](https://huggingface.co/VAGOsolutions/SauerkrautLM-ColQwen3-8b-v0.1)`revision="refs/pr/1"`

[Metric-AI/ColQwen2.5-3b-multilingual-v1.0](https://huggingface.co/Metric-AI/ColQwen2.5-3b-multilingual-v1.0)`revision="refs/pr/2"`

[vultr/VultronRetrieverFlash-Qwen3.5-0.8B](https://huggingface.co/vultr/VultronRetrieverFlash-Qwen3.5-0.8B)`revision="refs/pr/2"`

[VAGOsolutions/SauerkrautLM-ColQwen3-4b-v0.1](https://huggingface.co/VAGOsolutions/SauerkrautLM-ColQwen3-4b-v0.1)`revision="refs/pr/1"`

[VAGOsolutions/SauerkrautLM-ColQwen3-2b-v0.1](https://huggingface.co/VAGOsolutions/SauerkrautLM-ColQwen3-2b-v0.1)`revision="refs/pr/1"`

[Verm1ion/ColTurk-VDR-Qwen3VL-4B-v1.0](https://huggingface.co/Verm1ion/ColTurk-VDR-Qwen3VL-4B-v1.0)`revision="refs/pr/1"`

[vidore/colqwen2.5-v0.2](https://huggingface.co/vidore/colqwen2.5-v0.2)[vidore/colqwen2.5-v0.1](https://huggingface.co/vidore/colqwen2.5-v0.1)[vidore/colqwen-omni-v0.1](https://huggingface.co/vidore/colqwen-omni-v0.1)[VAGOsolutions/SauerkrautLM-ColQwen3-1.7b-Turbo-v0.1](https://huggingface.co/VAGOsolutions/SauerkrautLM-ColQwen3-1.7b-Turbo-v0.1)`revision="refs/pr/1"`

[vidore/colpali-v1.3](https://huggingface.co/vidore/colpali-v1.3)[vidore/colpali-v1.3-hf](https://huggingface.co/vidore/colpali-v1.3-hf)[vidore/colpali-v1.2](https://huggingface.co/vidore/colpali-v1.2)[vidore/colqwen2-v1.0](https://huggingface.co/vidore/colqwen2-v1.0)[vidore/colqwen2-v0.1](https://huggingface.co/vidore/colqwen2-v0.1)[vidore/colpali](https://huggingface.co/vidore/colpali)[vidore/colpali-v1.1](https://huggingface.co/vidore/colpali-v1.1)[VAGOsolutions/SauerkrautLM-ColLFM2-450M-v0.1](https://huggingface.co/VAGOsolutions/SauerkrautLM-ColLFM2-450M-v0.1)`revision="refs/pr/1"`

[vidore/colsmolvlm-v0.1](https://huggingface.co/vidore/colsmolvlm-v0.1)[VAGOsolutions/SauerkrautLM-ColMinistral3-3b-v0.1](https://huggingface.co/VAGOsolutions/SauerkrautLM-ColMinistral3-3b-v0.1)`revision="refs/pr/1"`

[vidore/colpali-hard-v1.1](https://huggingface.co/vidore/colpali-hard-v1.1)[vidore/colSmol-500M](https://huggingface.co/vidore/colSmol-500M)[vidore/colSmol-256M](https://huggingface.co/vidore/colSmol-256M)[ModernVBERT/colmodernvbert](https://huggingface.co/ModernVBERT/colmodernvbert)[vidore/colpali-v1.2-hf](https://huggingface.co/vidore/colpali-v1.2-hf)[vidore/colqwen2-v1.0-hf](https://huggingface.co/vidore/colqwen2-v1.0-hf)Most of these are LoRA adapter repositories, with the adapter applied directly onto its base at load time. Some also have a `-merged`
...
###
[
](https://huggingface.co#example-scripts)
Example Scripts
