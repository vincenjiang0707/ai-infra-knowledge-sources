...
source: https://huggingface.co/blog/open-responses
...
[Update on GitHub](https://github.com/huggingface/blog/blob/main/open-responses.md)
...
##
[
](https://huggingface.co#what-is-open-responses)
...
##
[
](https://huggingface.co#what-do-we-need-to-know-to-build-with-open-responses)
...
###
[
](https://huggingface.co#client-requests-to-open-responses)
...
###
[
](https://huggingface.co#changes-for-inference-clients-and-providers)
...
- Expanded reasoning visibility: Open Responses formalizes three optional fields for reasoning items:
`content`

(raw reasoning traces),`encrypted_content`

(provider-specific protected content), and`summary`

(sanitized from raw traces).

OpenAI models used to only expose `summary`
...
- Implementing richer state changes and payloads, including more detailed observability—for example, a hosted Code Interpreter can send a specific
`interpreting`

state to improve agent and user visibility during long-running operations.
...
###
[
](https://huggingface.co#open-responses-for-routing)
...
###
[
](https://huggingface.co#sub-agent-loops)
Sub Agent Loops
...
Clients control loop behavior via `max_tool_calls`

to cap iterations and `tool_choice`
