...
Image-to-Video • 2B • Updated • 739k • 2.32k

#
[
](https://huggingface.co#custom-kernels-for-all-from-codex-and-claude)
Custom Kernels for All from Codex and Claude

[Update on GitHub](https://github.com/huggingface/blog/blob/main/custom-cuda-kernels-agent-skills.md)
...
Writing CUDA kernels is hard. Writing CUDA kernels that correctly integrate with `transformers`
...
##
[
](https://huggingface.co#why-a-skill-for-kernels)
Why a skill for kernels?

The [Kernel Hub](https://huggingface.co/blog/hello-hf-kernels) solved the distribution of custom hardware kernels. You can load pre-compiled kernels from the Hub with a single `get_kernel`

call. No builds, no flags. However, someone still needs to **write the kernels**. That is the gap this skill fills.
...
have different module hierarchies, normalization conventions, and integration patterns. Custom kernels need to be registered in PyTorch for`torch.compile`

to recognize. - For distribution, kernels can depend on CUDA, Pytorch, and Python versions creating massive environment matrices.
...
The skill ships with the `kernels`
...
This drops the skill into `.claude/skills/cuda-kernels/`
...
The agent can read the skill, select the right architecture parameters, generate the CUDA source, write the PyTorch bindings, set up `build.toml`
...
##
[
](https://huggingface.co#what-is-in-the-skill)
...
, including the pitfalls specific to each library - Kernel templates with vectorized memory access patterns for BF16, FP16, and FP32
...
The agent is instructed to generate kernels that conform to the templates in `references/kernel-templates.md`
...
##
[
](https://huggingface.co#benchmarking-the-kernels-diffusers-ltx-video-on-h100)
Benchmarking the kernels: Diffusers (LTX-Video on H100)

The agent built RMSNorm, RoPE 3D, GEGLU, and AdaLN kernels for [LTX-Video](https://huggingface.co/Lightricks/LTX-Video), a video generation pipeline from `diffusers`

. The full example is at `examples/ltx_video/`
...
###
[
](https://huggingface.co#isolated-rmsnorm-benchmark)
...
###
[
](https://huggingface.co#end-to-end-video-generation-49-frames-30-steps-h100-80gb)
End-to-end video generation (49 frames, 30 steps, H100 80GB)

Next, we compare the end-to-end video generation performance of the optimized kernels against the baseline (no compile) and the `torch.compile`
...
##
[
](https://huggingface.co#benchmarking-the-kernels-transformers-qwen3-8b-on-h100)
Benchmarking the kernels: Transformers (Qwen3-8B on H100)

The agent built an RMSNorm kernel for [Qwen3-8B](https://huggingface.co/Qwen/Qwen3-8B), a large language model from `transformers`

with 65 RMSNorm modules across 32 layers. The full example is at `examples/qwen3_8b/`
...
###
[
](https://huggingface.co#isolated-rmsnorm-benchmark-1)
Isolated RMSNorm benchmark
...
##
[
](https://huggingface.co#publishing-your-kernel-to-the-hub)
...
###
[
](https://huggingface.co#1-verify-the-project-structure)
...
tells `kernel-builder`

what to build. The agent generates this for you, including the correct `cuda-capabilities`
...
###
[
](https://huggingface.co#2-build-all-variants-with-nix)
...
This builds the kernel for every required PyTorch/CUDA variant and places the results in `build/`
...
###
[
](https://huggingface.co#3-create-a-hub-repo-and-push)
...
###
[
](https://huggingface.co#4-others-load-it-in-one-line)
