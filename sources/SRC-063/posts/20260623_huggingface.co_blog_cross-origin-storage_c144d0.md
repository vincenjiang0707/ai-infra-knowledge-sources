...
Text Classification • Updated • 26.1k • 14

#
[
](https://huggingface.co#experimenting-with-the-proposed-cross-origin-storage-api-in-transformersjs)
Experimenting with the proposed Cross-Origin Storage API in Transformers.js
...
##
[
](https://huggingface.co#the-cache-challenge)
The cache challenge
...
###
[
](https://huggingface.co#model-resources)
Model resources
...
###
[
](https://huggingface.co#wasm-runtime-resources)
...
###
[
](https://huggingface.co#cache-isolation)
Cache isolation

####
[
](https://huggingface.co#ai-model-resources-serving)
...
####
[
](https://huggingface.co#wasm-runtime-resources-serving)
...
####
[
](https://huggingface.co#chromes-implementation)
...
##
[
](https://huggingface.co#enter-the-cross-origin-storage-api)
...
[
](https://huggingface.co#control-who-can-read-what)
...
###
[
](https://huggingface.co#integrity-by-design)
Integrity by design
...
###
[
](https://huggingface.co#privacy-without-sacrificing-utility)
...
###
[
](https://huggingface.co#what-this-means-for-the-transformersjs-example)
...
###
[
](https://huggingface.co#model-flexibility)
Model flexibility

The toy example works just fine with `Xenova/whisper-tiny.en`

, but of course you possibly wouldn't say no if the user already has [any of the other Whisper variants](https://huggingface.co/Xenova/models?search=whisper) in their COS cache. For example, the user might already have [ Xenova/whisper-large-v3](https://huggingface.co/Xenova/whisper-large-v3), which, as the name suggests, is a lot larger than the tiny variant. Transformers.js's

[Model Registry](https://huggingface.co/docs/transformers.js/en/api/utils/model_registry)makes being flexible about your models possible. If you know your app's needs can be addressed by, for example, any of

`Xenova/whisper-tiny.en`

, `whisper-medium.en`

, or `Xenova/whisper-large-v3`

, you can check the registry for the associated files for each model, probe for their existence in the COS cache (which may partially or completely contain the model resources you need), and then take a decision what model to choose eventually. The [API directly integrates with COS (and of course the Cache API), so this operation is really ergonomic.](https://huggingface.co/docs/transformers.js/en/api/utils/model_registry#modelregistryispipelinecachedtask-modelid-options--promise--boolean-)
...
polyfill on all pages and test the complete flow. Check out the [source code of the extension](https://github.com/web-ai-community/cross-origin-storage-extension) and follow the [usage instructions](https://github.com/web-ai-community/cross-origin-storage-extension?tab=readme-ov-file#usage) to get started.

With the extension installed, try the full end-to-end experience right now: open the first [toy example with COS enabled](https://googlechrome.github.io/samples/transformersjs-automatic-speech-recognition/index3.html), let it load `Xenova/whisper-tiny.en`
...
##
[
](https://huggingface.co#call-to-action)
Call to action
