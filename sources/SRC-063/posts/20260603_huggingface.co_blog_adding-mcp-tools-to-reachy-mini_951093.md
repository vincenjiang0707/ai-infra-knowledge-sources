...
Great work on making Reachy even easier to customize!

How can I add these MCP tools on a wireless Reachy Mini with a preinstalled conversation app?

The `tools.txt`

approach makes the MCP integration especially clear. I like how remote tools are kept separate from the built-in robot tools while still being enabled per profile. The distinction between installing a Space and actually activating its tool IDs in `tools.txt`

also seems important, particularly when multiple remote tools are being used together.

I’m also using an MCP server on my site, [https://tinid.ph/](https://tinid.ph/), to work with AI agents, and it has been really useful for me. In particular, it has helped a lot with identifying and fixing technical issues on the site. The idea of keeping tools accessible to AI agents through MCP, rather than building every capability directly into the application, makes a lot of sense.