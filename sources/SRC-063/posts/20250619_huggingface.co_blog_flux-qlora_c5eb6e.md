...
published: Thu, 19 Jun 2025 00:00:00 GMT

Viewer • Updated • 13 • 35 • 1
...
[Dataset](https://huggingface.co#dataset)[FLUX Architecture](https://huggingface.co#flux-architecture)[QLoRA Fine-tuning FLUX.1-dev with diffusers](https://huggingface.co#qlora-fine-tuning-flux1-dev-with-diffusers)[FP8 Fine-tuning with](https://huggingface.co#fp8-fine-tuning-with-torchao)`torchao`

[Inference with Trained LoRA Adapters](https://huggingface.co#inference-with-trained-lora-adapters)[Running on Google Colab](https://huggingface.co#running-on-google-colab)[Conclusion](https://huggingface.co#conclusion)
...
to adopt the artistic style of Alphonse Mucha, using a small [dataset](https://huggingface.co/datasets/derekl35/alphonse-mucha-style).
...
**QLoRA: The Efficiency Powerhouse:** [QLoRA](https://huggingface.co/docs/peft/main/en/developer_guides/quantization) enhances LoRA by first loading the pre-trained base model in a quantized format (typically 4-bit via `bitsandbytes`
...
**QLoRA:**Peak VRAM usage for QLoRA fine-tuning was approximately 9GB.**BF16 LoRA:**Running standard LoRA (with the base FLUX.1-dev in FP16) on the same setup consumed 26 GB VRAM.**BF16 full finetuning:**An estimate would be ~120 GB VRAM with no memory optimizations.
...
We fine-tuned FLUX.1-dev LoRA on an H100 SXM GPU slightly modified`diffusers-torchao`

[training scripts](https://gist.github.com/sayakpaul/f0358dd4f4bcedf14211eba5704df25a#file-train_dreambooth_lora_flux-py). The following command was used:
...
[ ](https://huggingface.co/datasets/huggingface/documentation-images/resolve/main/blog/quantization-backends-diffusers2/alphonse_mucha_fp8_combined.png)

Key steps to enable FP8 training with `torchao`

involve:

**Injecting FP8 layers**into the model using`convert_to_float8_training`
...
**Flexibility:**Easily switch between different LoRA adapters without reloading the base model**Experimentation:**Test multiple artistic styles or concepts by swapping adapters**Modularity:**Combine multiple LoRA adapters using`set_adapters()`
...
**VRAM efficiency:**No additional memory overhead from adapter weights during inference**Speed:**Slightly faster inference as there's no need to apply adapter computations**Quantization compatibility:**Can re-quantize the merged model for maximum memory efficiency
