# 2025-12-13-vllm-router-release
...
- It supports discovery and routing for both
**NIXL**and**NCCL-based (with ZMQ discovery)**disaggregation backends.
...
**Kubernetes Service Discovery:**The router can operate in a Kubernetes-native mode, automatically discovering, monitoring, and routing to vLLM worker pods using label selectors.**Fault Tolerance:**It includes configurable**retry logic**(with exponential backoff and jitter) and**circuit breakers**. If a worker fails health checks, the router immediately removes it from the routing pool and will retry requests, preventing cascading failures.**Observability:**A built-in Prometheus endpoint (`/metrics`

) exports detailed metrics on request volume, latency, error rates, and the health of individual workers, providing complete visibility into the serving fleet.
...
A Kubernetes-native routing framework that utilizes default queue-aware load balancing.[llm-d](https://github.com/llm-d/llm-d):**vLLM-native:**The standard[K8s native load balancer](https://kubernetes.io/docs/concepts/services-networking/), which employs a basic round-robin strategy. Crucially, this option is*not*aware of Prefill/Decode states, treating all pods as identical vLLM replicas.
