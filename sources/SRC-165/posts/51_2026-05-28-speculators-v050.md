...
[Gemma 4 31B DFlash speculator](https://huggingface.co/RedHatAI/gemma-4-31B-it-speculator.dflash)was trained and acceptance rates were evaluated across diverse task types. The results demonstrate strong performance particularly on reasoning and code generation tasks:
...
Similar to the Eagle 3 models, DFlash models contain a `speculators_config`

in their config.json which contain details on the target model, speculative tokens, the name of the speculative algorithm, etc. With this config, models can be served using a basic `vllm serve`
...
- vLLM server initializes with the base model (and some special configuration)
- Training prompts are sent to vLLM for inference
- Hidden states are extracted and temporarily written to disk (or ram disk)
- The training process loads the extracted hidden states and deletes the file
- Speculator model trains on extracted states
