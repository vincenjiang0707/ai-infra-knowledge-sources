# 2026-01-08-kv-offloading-connector
...
**Improving request latency**(assuming reading from the cache is faster than re-calculating the KV data)**Increasing per-node throughput**(as the load on the GPU cores is reduced, thus allowing to process more concurrent requests).
...
Combining this with the previous point, this makes CPU offloading**ideal for efficiently handling preemptions**of requests. - CPU RAM is also a
**convenient staging area**for further offloading to external storage.
...
To use the offloading connector for CPU offloading, simply add the following CLI flag to the `vllm serve`
...
- Enabling preempted requests to be loaded back from the CPU (
[PR #29870](https://github.com/vllm-project/vllm/pull/29870)) - Fix a race condition between offloading and model computation (
[PR #31341](https://github.com/vllm-project/vllm/pull/31341))
...
- Copying using
**DMA**- via cudaMemcpyAsync. - Copying using a
**custom CUDA kernel**which utilizes**GPU cores**to copy 16-byte words using raw pointers. This approach is effective as it uses the massive parallelism offered by the GPU cores. On the other hand, it creates greater interference with the main tasks of the GPU cores.
...
This depends on the model being served, and the vLLM configuration. In the next section, we will answer this question for some of today’s commonly used models.**How does both approaches affect the GPU model computation performance?**
