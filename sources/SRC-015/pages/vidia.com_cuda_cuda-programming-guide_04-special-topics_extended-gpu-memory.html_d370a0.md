source: https://docs.nvidia.com/cuda/cuda-programming-guide/04-special-topics/extended-gpu-memory.html

# 4.19. Extended GPU Memory[#](https://docs.nvidia.com#extended-gpu-memory)
...
## 4.19.1. Preliminaries[#](https://docs.nvidia.com#preliminaries)
...
### 4.19.1.1. EGM Platforms: System topology[#](https://docs.nvidia.com#egm-platforms-system-topology)
...
and cause performance issues. Use `CUDA_VISIBLE_DEVICES`

instead.

### 4.19.1.2. Socket Identifiers: What are they? How to access them?[#](https://docs.nvidia.com#socket-identifiers-what-are-they-how-to-access-them)
...
### 4.19.1.3. Allocators and EGM support[#](https://docs.nvidia.com#allocators-and-egm-support)
...
NVLinks. Currently, `cuMemCreate`

and `cudaMemPoolCreate`

allocators are
supported with appropriate location type and NUMA identifiers.

### 4.19.1.4. Memory management extensions to current APIs[#](https://docs.nvidia.com#memory-management-extensions-to-current-apis)

Currently, EGM memory can be mapped with Virtual Memory (`cuMemCreate`

) or
Stream Ordered Memory (`cudaMemPoolCreate`
...
communication. Therefore we encourage the reader to see [Chapter 4.16](https://docs.nvidia.com/inter-process-communication.html#interprocess-communication).

Note

We encourage readers to read CUDA Programming Guide’s [Chapter 4.17](https://docs.nvidia.com/virtual-memory-management.html#virtual-memory-management) and [Chapter 4.3](https://docs.nvidia.com/stream-ordered-memory-allocation.html#stream-ordered-memory-allocator) for a better understanding.
...
## 4.19.2. Using the EGM Interface[#](https://docs.nvidia.com#using-the-egm-interface)

### 4.19.2.1. Single-Node, Single-GPU[#](https://docs.nvidia.com#single-node-single-gpu)
...
### 4.19.2.2. Single-Node, Multi-GPU[#](https://docs.nvidia.com#single-node-multi-gpu)
...
approach. Therefore, using the `cuDeviceGetAttribute`

function the
user should be able to learn the closest NUMA node id. (See [Socket Identifiers: What are they? How to access them?](https://docs.nvidia.com#socket-identifiers-what-are-they-how-to-access-them)).
Then the user can allocate and manage EGM memory using VMM (Virtual
Memory Management) API or CUDA Memory Pool.

#### 4.19.2.2.1. Using VMM APIs[#](https://docs.nvidia.com#using-vmm-apis)
...
allocation. See CUDA Programming Guide’s [Virtual Memory Management section](https://docs.nvidia.com#virtual-memory-management)
...
#### 4.19.2.2.2. Using CUDA Memory Pool[#](https://docs.nvidia.com#using-cuda-memory-pool)
...
as the location type and numaId
...
the existing peer access API, `cudaMemPoolSetAccess`
...
### 4.19.2.3. Multi-Node, Multi-GPU[#](https://docs.nvidia.com#multi-node-multi-gpu)
...
The user should allocate memory using `cuMemCreate`

and again the
user has to explicitly provide `CU_MEM_LOCATION_TYPE_HOST_NUMA`

as
the location type and numaID as the location identifier. In
addition `CU_MEM_HANDLE_TYPE_FABRIC`
...
After creating allocation handle using `cuMemCreate`
...
[Compute fabric transport](https://docs.nvidia.com/compute-fabric-transport.html#compute-fabric-transport) provides a
