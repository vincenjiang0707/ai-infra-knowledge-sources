source: https://docs.nvidia.com/cuda/cuda-programming-guide/05-appendices/cuda-cpp-execution-model.html

# 5.8. CUDA C++ Execution model[#](https://docs.nvidia.com#cuda-c-execution-model)
...
## 5.8.1. Host threads[#](https://docs.nvidia.com#host-threads)
...
## 5.8.2. Device threads[#](https://docs.nvidia.com#device-threads)

Once a device thread makes progress:

If it is part of a

[Cooperative Grid](https://docs.nvidia.com/cuda/cuda-runtime-api/group__CUDART__EXECUTION.html#group__CUDART__EXECUTION_1g504b94170f83285c71031be6d5d15f73), all device threads in its grid shall eventually make progress.Otherwise, all device threads in its

[thread-block cluster](https://docs.nvidia.com/02-basics/intro-to-cuda-cpp.html#thread-block-clusters)shall eventually make progress.[Note: Threads in other thread-block clusters are not guaranteed to eventually make progress. - end note.]
...
[std::this_thread::yield]([[thread.thread.this]]),make a call to a library I/O function,

perform an access through a volatile glvalue,

perform a synchronization operation or an atomic operation, or

continue execution of a trivial infinite loop (
...
## 5.8.3. CUDA APIs[#](https://docs.nvidia.com#cuda-apis)
...
`CUDA_DEVICE_MAX_CONNECTIONS=1 CUDA_LAUNCH_BLOCKING=1`

, and then check that the program still terminates. If it does not, the program has a bug. This method is not sufficient because it does not catch all Forward Progress bugs, but it does catch many such bugs. - end note.]
...
### 5.8.3.1. Dependencies[#](https://docs.nvidia.com#dependencies)
...
[CUDA Stream Commands]. These may include dependencies on the completion of, among others,[CUDA Events]and[CUDA Kernels]. - end note.]
