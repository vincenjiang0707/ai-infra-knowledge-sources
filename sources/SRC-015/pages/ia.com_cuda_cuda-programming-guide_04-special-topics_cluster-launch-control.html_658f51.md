...
# 4.13. Work Stealing with Cluster Launch Control[#](https://docs.nvidia.com#work-stealing-with-cluster-launch-control)
...
[figure](https://docs.nvidia.com#cluster-launch-control-diagram) above presents the execution
...
## 4.13.1. API Details[#](https://docs.nvidia.com#api-details)
...
to [asynchronous data copies](https://docs.nvidia.com/03-advanced/advanced-kernel-programming.html#advanced-kernels-async-copies).
...
variable.Decoding instructions that extract success/failure status and the cancelled thread block index.


Note that cluster launch control operations are modeled as async proxy operations (see [Async Thread and Async Proxy](https://docs.nvidia.com/03-advanced/advanced-kernel-programming.html#advanced-kernels-hardware-implementation-asynchronous-execution-features-async-thread-proxy)).

### 4.13.1.1. Thread Block Cancellation[#](https://docs.nvidia.com#thread-block-cancellation)
...
**Setup Phase**(Steps 1-2): Declare and initialize cancellation result and synchronization variables.**Work-Stealing Loop**(Steps 3-5): Execute repeatedly to request, synchronize, and process cancellation results.
...
Since thread block cancellation is a uniform instruction, it is recommended to submit it inside

[invoke_one](https://docs.nvidia.com/cooperative-groups.html#cooperative-groups-invoke-one)thread selector. This allows the compiler to optimize out the peeling loop.Synchronize (complete) asynchronous cancellation request:
...
[proxies](https://docs.nvidia.com/cuda/parallel-thread-execution/index.html#proxies), and protect against data races between iterations of the work-stealing loop.

### 4.13.1.2. Constraints on Thread Block Cancellation[#](https://docs.nvidia.com#constraints-on-thread-block-cancellation)

The constraints are related to failed cancellation requests:

Submitting another cancellation request after

**observing**a previously failed request is*undefined behavior*.In the two code examples below, assuming the first cancellation request fails, only the first example exhibits undefined behavior. The second example is correct because there is no observation between the cancellation requests:

**Invalid code:**// First request: ptx::clusterlaunchcontrol_try_cancel(&result0, &bar0); // First request query: [Synchronize bar0 code here.] bool success0 = ptx::clusterlaunchcontrol_query_cancel_is_canceled(result0); assert(!success0); // Observed failure; second cancellation will be invalid. // Second request - next line is Undefined Behavior: ptx::clusterlaunchcontrol_try_cancel(&result1, &bar1);

**Valid code:**// First request: ptx::clusterlaunchcontrol_try_cancel(&result0, &bar0); // Second request: ptx::clusterlaunchcontrol_try_cancel(&result1, &bar1); // First request query: [Synchronize bar0 code here.] bool success0 = ptx::clusterlaunchcontrol_query_cancel_is_canceled(result0); assert(!success0); // Observed failure; second cancellation was valid.
...
result pointer to avoid data races.If the same barrier is used for synchronization, the arrival and transaction counts must be adjusted accordingly.



## 4.13.2. Example: Vector-Scalar Multiplication[#](https://docs.nvidia.com#example-vector-scalar-multiplication)
...
### 4.13.2.1. Use-case: Thread Blocks[#](https://docs.nvidia.com#use-case-thread-blocks)
...
__global__ void kernel_fixed_work (float* data, int n) { // Prologue: float alpha = compute_scalar(); // Computation: int i = blockIdx.x * blockDim.x + threadIdx.x; if (i < n) data[i] *= alpha; } // Launch: kernel_fixed_work<<<(n + 1023) / 1024, 1024>>>(data, n);

Fixed Number of Thread Blocks:

__global__ void kernel_fixed_blocks (float* data, int n) { // Prologue: float alpha = compute_scalar(); // Computation: int i = blockIdx.x * blockDim.x + threadIdx.x; while (i < n) { data[i] *= alpha; i += gridDim.x * blockDim.x; } } // Launch: kernel_fixed_blocks<<<SM_COUNT, 1024>>>(data, n);
...
### 4.13.2.2. Use-case: Thread Block Clusters[#](https://docs.nvidia.com#use-case-thread-block-clusters)

In the case of a [thread block clusters](https://docs.nvidia.com/02-basics/intro-to-cuda-cpp.html#thread-block-clusters),
...
within a cluster. Therefore, thread blocks within the cluster need to add the local block index.Synchronization is performed by each cluster’s thread block using a local

`__shared__`

memory barrier. Barrier operations must be performed with the`ptx::scope_cluster`

scope.Cancelling in the cluster case requires all the thread blocks to exist. A user can guarantee that all thread blocks are running by using

`cg::cluster_group::sync()`

from[sync](https://docs.nvidia.com/05-appendices/device-callable-apis.html#cg-api-sync-function)API.
