source: https://docs.mthreads.com/torchmusa/torchmusa-doc-online/optimization

# 性能优化

## profiler 工具[](https://docs.mthreads.com#profiler-工具)
...
注意，在当前适配的 profiler 版本中，`use_musa`

和 `activities`

参数无效，这两个参数的设定不会影响 profiler 的配置。
...
## 使能 TensorCore 优化[](https://docs.mthreads.com#使能-tensorcore-优化)

**s4000** 支持 **TensorFloat32(TF32)** tensor cores。利用tensor cores，可以使得 **矩阵乘** 和 **卷积** 计算得到加速。torch_musa中TF32的使用方式和CUDA中一致，可以参考 [PyTorch官方示例](https://pytorch.org/docs/stable/notes/cuda.html#tensorfloat-32-tf32-on-ampere-devices) 。在快速入门章节中，我们也提供了示例代码，见[使能 TensorCore 示例代码](https://docs.mthreads.com#tensorcore)。需要注意的是，CUDA中矩阵乘和卷积计算是分开控制的，而在torch_musa中是统一控制的，代码差异如下所示。在torch_musa中 **allow_tf32** 默认值是False。
