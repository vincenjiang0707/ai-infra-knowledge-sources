source: https://docs.mthreads.com/torchmusa/torchmusa-doc-online/ops_dev
...
## 算子实现分类[](https://docs.mthreads.com#算子实现分类)
...
## 是否需要适配算子[](https://docs.mthreads.com#是否需要适配算子)
...
- 尝试调用 functional 规则的算子，即内部创建 result ，计算结果写入后返回，发现该实现缺失。
- 先创建 result，作为 out 参数传入 out 规则的算子调用，发现该实现也缺失。
- 没有其他可调用的算子，报错返回异常。

此时需要我们手动实现 tril 算子。考虑计算逻辑的完备性，建议把 tril group 内所有调用规则对应的 C++ 算子全都实现。

## 实现Structured算子[](https://docs.mthreads.com#实现structured算子)
...
- 以 functional 算子名创建 meta::structured_tril 类（简称 meta 类，继承官方的 MetaBase 基类），新增 meta 函数实现预处理。
- 以 out 算子绑定的后端实现名创建 native::structured_tril_cuda 类（简称 impl 类，继承 meta 类），新增 impl 函数实现计算。
- 创建 structured_tril_cuda_functional 类（简称 functional 类，继承 impl 类），覆写基类方法实现结果 tensor 的创建。
- 创建 structured_tril_cuda_inplace 类（简称 inplace 类，继承 impl 类），覆写基类方法实现可读写 tensor 的校验。
- 创建 structured_tril_cuda_out 类（简称 out 类，继承 impl 类），覆写基类方法实现结果 tensor 的校验和尺寸变化。
- functional/inplace/out 算子实现为分别实例化 3/4/5 中创建的子类，依次调用 meta 和 impl 函数，最后返回结果。
...
- out 算子声明除了后端名外，需要增加 structured 实现方式的标记，支持扩展字段描述基类信息，以及�传递预处理的中间变量映射关系。
...
### 复用meta/impl函数（Legacy）[](https://docs.mthreads.com#复用metaimpl函数legacy)
...
!!! note 注解： DispatchStub 的原理可参考 pytorch/aten/src/ATen/native/DispatchStub.h 源码文件

由于 MUSA 的编程模型实现了 CUDA 兼容，MUSA kernel 的一种实现方法是通过 CUDA-Porting 工具完成，主要流程如下：

- 新建目录 build/generated_cuda_compatible，保存 Porting 的 kernels 文件和依赖头文件。
- 把 PyTorch 仓库中的 CUDA kernels 和安装目录中的头文件复制到新建目录内（维持相对路径）。
- Porting 工具实施文本替换，如将 cudaMalloc 替换成 musaMalloc，cuda_fp16.h 替换成 musa_fp16.h 等。
...
### 只复用meta函数（LegacyMeta)[](https://docs.mthreads.com#只复用meta函数legacymeta)
...
### 自定义meta/impl函数（Customized）[](https://docs.mthreads.com#自定义metaimpl函数customized)

如果在实现MUSA structured算子时遇到如下情况：

- 预处理逻辑和 CPU/CUDA 有区别。
- meta 类需要继承不同的基类。
- meta 函数需要传给 impl 函数自定义的中间值，与 CPU/CUDA 不同或 CPU/CUDA 不传中间值。
...
计算函数的第二个参��数是中间变量 var，非算子调用时传入的原始参数 diagonal，剩下的实现过程与 LegacyMeta 方式类似。
...
## 实现UnStructured算子[](https://docs.mthreads.com#实现unstructured算子)
...
# structured 默认为 false，标记为 unstructured 实现方�式
...
### 复用公共函数（Legacy）[](https://docs.mthreads.com#复用公共函数legacy)
...
### 接入MUDNN（Customized）[](https://docs.mthreads.com#接入mudnncustomized)

如果算子的 MUSA 实现涉及数据计算，正好 MUDNN 库提供了相应的能力时，我们可以在实现逻辑内部直接调用 MUDNN 接口完成计算。使用 MUDNN 库的主要步骤如下：

- 参数校验，主要检查 device/dtype/defined_tensor 等。
- 添加 DeviceGuard。
- 参数转换，比如 conv 算子只支持连续 tensors，需要提前把输入/输出 tensors 转换为连续的。
- 创建输入/输出 MUTensors，以及 MUDNN 的计算实例，配置计算参数。
- 调用实例接口完成计算，返回计算结果。
...
通过 mudnn*.h 头文件我们可以查询 MUDNN 库的算子支持情况和接口定义，默认地址为 /usr/local/musa/include 目录。如果 MUDNN 不支持，我们也可以通过 CUDA-Porting 等方式手动实现 MUSA kernels，在��函数内部手工调用完成计算。

### CPU计算（Customized）[](https://docs.mthreads.com#cpu计算customized)
