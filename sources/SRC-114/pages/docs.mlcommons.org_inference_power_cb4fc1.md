source: https://docs.mlcommons.org/inference/power/

# Power Measurement[¶](https://docs.mlcommons.org#power-measurement)

*Originally Prepared by the MLCommons taskforce on automation and reproducibility and OctoML*.

## Requirements[¶](https://docs.mlcommons.org#requirements)

-
Power analyzer (anyone

[certified by SPEC PTDaemon](https://www.spec.org/power/docs/SPECpower-Device_List.html)). Yokogawa is the one that most submitters have submitted with and a new single-channel model like 310E can cost around 3000$. -
SPEC PTDaemon (can be downloaded from

[here](https://github.com/mlcommons/power)after signing the EULA which can be requested by sending an email to`support@mlcommons.org`
...
[MLCommons power-dev](https://github.com/mlcommons/power-dev)repository which has the`server.py`

to be run on the director node and`client.py`

to be run on the SUT node. This repository being public will be automatically pulled by the MLC workflow.

## Connecting power analyzer to the computer[¶](https://docs.mlcommons.org#connecting-power-analyzer-to-the-computer)
...
## Ranging mode and Testing mode[¶](https://docs.mlcommons.org#ranging-mode-and-testing-mode)
...
## Start Power Server (Power analyzer should be connected to this computer and PTDaemon runs here)[¶](https://docs.mlcommons.org#start-power-server-power-analyzer-should-be-connected-to-this-computer-and-ptdaemon-runs-here)
...
--device_port=1```
(can change as per the USB slot used for connecting) can be used on Windows for USB connection
*
```

--device_type=49`corresponds to Yokogawa 310E and`

ptd -h`should list the device_type for all supported devices. The location of`

ptd```
can be found using the below command
*
```
...
### Running the power server inside a docker container[¶](https://docs.mlcommons.org#running-the-power-server-inside-a-docker-container)
...
## Running a dummy workload with power (on host machine)[¶](https://docs.mlcommons.org#running-a-dummy-workload-with-power-on-host-machine)
...
### Run a dummy workload with power inside a docker container[¶](https://docs.mlcommons.org#run-a-dummy-workload-with-power-inside-a-docker-container)
...
## Running MLPerf Image Classification with power[¶](https://docs.mlcommons.org#running-mlperf-image-classification-with-power)
...
### Running MLPerf Image Classification with power inside a docker container[¶](https://docs.mlcommons.org#running-mlperf-image-classification-with-power-inside-a-docker-container)
