...
For benchmarking production vLLM servers, we recommend [GuideLLM](https://github.com/vllm-project/guidellm), an established performance benchmarking framework with live progress updates and automatic report generation. It is also more flexible than `vllm bench serve`

in terms of dataset loading, request formatting, and workload patterns.

## Dataset Overview[¶](https://docs.vllm.ai#dataset-overview)
...
### 🚀 Online Benchmark[¶](https://docs.vllm.ai#online-benchmark)
...
Repeating `vllm bench serve`

against the same server can reuse prompts left in the prefix cache and inflate throughput. This can affect any reproducible dataset; the synthetic random dataset is reproducible for a fixed `--seed`

, which defaults to `0`

. Prefix cache hits can also come from shared prefixes within the run, so interpret cache metrics in the context of the workload. If cache reuse is not intended, vary `--seed`

, reset or restart the server, or use `vllm bench sweep serve`

, which resets server caches between runs.

#### Understanding the Latency Metrics[¶](https://docs.vllm.ai#understanding-the-latency-metrics)

`vllm bench serve`

measures latency at the benchmark client:
...
**Time to first token (TTFT)**is the time from sending a request to receiving its first streamed output.**Inter-token latency (ITL)**records the time between consecutive streamed outputs. The reported ITL statistics aggregate these individual gaps across all successful requests.-
**Time per output token (TPOT)**is calculated once per request, excluding the first token, and then aggregated across requests:\[ \text{TPOT} = \frac{\text{end-to-end latency} - \text{TTFT}} {\text{number of output tokens} - 1} \]

With standard decoding, each streamed output usually contains one token, so ITL and TPOT are typically similar.

With speculative decoding, one streamed output can contain multiple tokens, such as several accepted draft tokens within a single engine step. ITL records only the gaps between streamed outputs; it does not add zero-duration gaps for tokens in the same output. TPOT instead amortizes the request's decoding time over every output token.
...
#### Results Visualization[¶](https://docs.vllm.ai#results-visualization)
...
##### Dataset statistics[¶](https://docs.vllm.ai#dataset-statistics)

The generated figure shows the input prompt and output tokens distribution.

#### Custom Dataset[¶](https://docs.vllm.ai#custom-dataset)

If the dataset you want to benchmark is not supported yet in vLLM, even then you can benchmark on it using [ CustomDataset](https://docs.vllm.ai/api/vllm/benchmarks/datasets/datasets/#vllm.benchmarks.datasets.datasets.CustomDataset). At inference time, use the option

`--dataset-name custom`

. Your data needs to be in the `.jsonl`

format and needs to have "prompt" field per entry, e.g., data.jsonl{"prompt": "What is the capital of India?"}
...
#### Custom Audio Dataset[¶](https://docs.vllm.ai#custom-audio-dataset)

If the audio dataset you want to benchmark is not supported yet in vLLM, then you can benchmark on it using [ CustomAudioDataset](https://docs.vllm.ai/api/vllm/benchmarks/datasets/datasets/#vllm.benchmarks.datasets.datasets.CustomAudioDataset). At inference time, use the option
...
-
**Supported models:**Theclass supports two types of audio models: ASR models (e.g. Whisper) which do not require a "prompt" field; and multimodal audio-text chat models (e.g. Qwen2-Audio). Since these model types require different arguments at inference, we are giving two examples.`CustomAudioDataset`
...
If the image dataset you want to benchmark is not supported yet in vLLM, then you can benchmark on it using [ CustomImageDataset](https://docs.vllm.ai/api/vllm/benchmarks/datasets/datasets/#vllm.benchmarks.datasets.datasets.CustomImageDataset). At inference time, use the option
...
This dataset is governed by the [NVIDIA Evaluation Dataset License Agreement](https://huggingface.co/datasets/nvidia/SPEED-Bench/blob/main/License.pdf). For each dataset a user elects to use, the user is responsible for checking if the dataset license is fit for the intended purpose. The `prepare.py`
...
, where high entropy data contains unstructured data such as creative writing while low entropy data contains more structured data such as coding, more details are in the dataset card.

#### BFCL (Tool-Calling) Benchmark[¶](https://docs.vllm.ai#bfcl-tool-calling-benchmark)

The Berkeley Function Calling Leaderboard (BFCL) dataset measures serving latency and throughput on realistic tool-calling traffic. Each request carries a per-sample `tools`

schema and chat history, so the server must expose `/v1/chat/completions`

with an auto-tool-choice parser enabled. The benchmark client always uses the `openai-chat`
...
is a comma-separated list of BFCL v3 category names (without the `BFCL_v3_`

prefix or `.json`

suffix). Defaults to `simple,live_simple,multiple`

. Other supported non-multi-turn categories include `parallel`
...
. Multi-turn categories are not yet supported.
...
) so modern grammar backends accept the translated tool definitions.

#### Other HuggingFaceDataset Examples[¶](https://docs.vllm.ai#other-huggingfacedataset-examples)
...
#### Probe Requests[¶](https://docs.vllm.ai#probe-requests)
...
Setting `--probe-request-rate`

to a positive value sends single-token text-only probe requests at that rate (requests per second) alongside the main workload. Probes bypass `--max-concurrency`
...
##### Load Pattern Control Parameters[¶](https://docs.vllm.ai#load-pattern-control-parameters)
...
: Limits concurrent outstanding requests. If this argument is not provided, concurrency is unlimited. Set a value to simulate backpressure. When set, include`client_queue_time`

in`--percentile-metrics`

to report time spent waiting for the benchmark client's concurrency limit. With a finite`--request-rate`

,`e2el_including_client_queue`

reports schedule-relative end-to-end latency; it is omitted for`--request-rate=inf`

, where all requests arrive at benchmark start.
...
### 📈 Offline Throughput Benchmark[¶](https://docs.vllm.ai#offline-throughput-benchmark)
...
#### Other HuggingFaceDataset Examples[¶](https://docs.vllm.ai#other-huggingfacedataset-examples_1)
...
#### Server Setup[¶](https://docs.vllm.ai#server-setup)
...
### 📚 Long Document QA Benchmark[¶](https://docs.vllm.ai#long-document-qa-benchmark)
...
### 🗂️ Prefix Caching Benchmark[¶](https://docs.vllm.ai#prefix-caching-benchmark)
...
##### Prefix Repetition Dataset[¶](https://docs.vllm.ai#prefix-repetition-dataset)

### Replay Timed Traces[¶](https://docs.vllm.ai#replay-timed-traces)

## Show more

Example of how to run traces which have timing information with them.

#### Running MoonshotAI traces[¶](https://docs.vllm.ai#running-moonshotai-traces)
...
### 🧪 Hashing Benchmarks[¶](https://docs.vllm.ai#hashing-benchmarks)
...
### ⚡ Request Prioritization Benchmark[¶](https://docs.vllm.ai#request-prioritization-benchmark)
...
### 👁️ Multi-Modal Benchmark[¶](https://docs.vllm.ai#multi-modal-benchmark)

## Show more

Benchmark the performance of multi-modal requests in vLLM.

#### Images (ShareGPT4V)[¶](https://docs.vllm.ai#images-sharegpt4v)
...
(see[Offline Throughput Benchmark](https://docs.vllm.ai#-offline-throughput-benchmark)for an example).
...
### 🔬 Multimodal Processor Benchmark[¶](https://docs.vllm.ai#multimodal-processor-benchmark)
...
### Embedding Benchmark[¶](https://docs.vllm.ai#embedding-benchmark)

Benchmark the performance of embedding requests in vLLM.

## Show more

#### Text Embeddings[¶](https://docs.vllm.ai#text-embeddings)
...
#### Multi-modal Embeddings[¶](https://docs.vllm.ai#multi-modal-embeddings)
...
For other models, please add your own implementation inside [ vllm/benchmarks/lib/endpoint_request_func.py](https://github.com/vllm-project/vllm/blob/main/vllm/benchmarks/lib/endpoint_request_func.py) to match the expected instruction format.
...
### Reranker Benchmark[¶](https://docs.vllm.ai#reranker-benchmark)
