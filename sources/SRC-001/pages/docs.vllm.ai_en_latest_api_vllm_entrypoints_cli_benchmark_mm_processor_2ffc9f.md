...
[¶](https://docs.vllm.ai#vllm.entrypoints.cli.benchmark.mm_processor)

Classes:

-
–[BenchmarkMMProcessorSubcommand](https://docs.vllm.ai#vllm.entrypoints.cli.benchmark.mm_processor.BenchmarkMMProcessorSubcommand)`vllm bench mm-processor`

profiles the multimodal input processor pipeline of

##

`BenchmarkMMProcessorSubcommand`

[¶](https://docs.vllm.ai#vllm.entrypoints.cli.benchmark.mm_processor.BenchmarkMMProcessorSubcommand)

Bases: [BenchmarkSubcommandBase](https://docs.vllm.ai/base/#vllm.entrypoints.cli.benchmark.base.BenchmarkSubcommandBase)

`vllm bench mm-processor`

profiles the multimodal input processor pipeline of vision-language models. It measures per-stage latency from the HuggingFace processor through to the encoder forward pass, helping you identify preprocessing bottlenecks and understand how different image resolutions or item counts affect end-to-end request time.

The benchmark supports two data sources: synthetic random multimodal inputs (`random-mm`

) and HuggingFace datasets (`hf`

). Warmup requests are run before measurement to ensure stable results.
...
For more examples (HF datasets, warmup, JSON output), see the [multimodal processor benchmark guide](https://docs.vllm.ai/benchmarking/cli/#multimodal-processor-benchmark).