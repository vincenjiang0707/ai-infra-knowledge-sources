source: https://docs.vllm.ai/en/latest/features/sleep_mode/
lastmod: 2026-09-24

# Sleep Mode[¶](https://docs.vllm.ai#sleep-mode)
...
## Sleep levels[¶](https://docs.vllm.ai#sleep-levels)
...
Enable sleep mode by passing `enable_sleep_mode=True`

to the [ LLM](https://docs.vllm.ai/api/vllm/entrypoints/llm/#vllm.entrypoints.llm.LLM) class.
...
#### RLHF weight updates[¶](https://docs.vllm.ai#rlhf-weight-updates)
...
to control which resources are restored, useful for RLHF and weight updates. **Note** that `is_sleeping`

will report `true`

until all components are awake.
...
#### Retaining frozen weights during RLHF updates[¶](https://docs.vllm.ai#retaining-frozen-weights-during-rlhf-updates)

Set `sleep_preserve_parameter_names`

(CLI: `--sleep-preserve-parameter-names`

) to runtime parameter-name glob patterns for weights that stay frozen during training. Each pattern must match `model.named_parameters()`

; checkpoint names and `requires_grad`

are not used.

Level-2 sleep backs up selected GPU parameters to pageable CPU memory; weights wake-up restores them in place and releases the backups. CPU parameters need no copy. Allow enough host memory for backups. Level-1 behavior is unchanged.

The trainer must omit these parameters from updates, and reload/post-processing must preserve their values and storage (including avoiding replacement with meta tensors). This option does not filter updates and applies only to the target model.

#### Release only KV cache memory[¶](https://docs.vllm.ai#release-only-kv-cache-memory)

`LLM.release_kv_cache_memory()`

discards KV cache physical memory while keeping model weights resident. It requires a completed pause and all executor memory to be resident: full sleep, partial wake-up, and repeated release without restoring memory are rejected. Requests retained with `mode="keep"`

are recomputed after wake-up.

Use the default `cumem`

backend with managed allocations: `enable_cumem_allocator=True`

on CUDA/ROCm (also enabled by `enable_sleep_mode=True`

), or `enable_sleep_mode=True`

on XPU. Other backends must implement selective discard; CPU and unmanaged allocations are unsupported.
...
To enable sleep mode in a vLLM server you need to initialize it with the flag `VLLM_SERVER_DEV_MODE=1`

and pass `--enable-sleep-mode`

to the vLLM server.

#### Server in development mode[¶](https://docs.vllm.ai#server-in-development-mode)

When using the flag `VLLM_SERVER_DEV_MODE=1`

you enable development endpoints, and these endpoints should not be exposed to users.
...
— Put the model to sleep (`level=1`

).`POST /release_kv_cache_memory`

— Discard KV cache memory after a completed pause, while all executor memory is resident.`POST /wake_up`

— Wake up the model. Supports optional`tags`

query parameters for partial wake-up (e.g.,`?tags=weights`

).`POST /collective_rpc`

— Perform a collective remote procedure call (RPC).`GET /is_sleeping`
...
## Limitation[¶](https://docs.vllm.ai#limitation)
