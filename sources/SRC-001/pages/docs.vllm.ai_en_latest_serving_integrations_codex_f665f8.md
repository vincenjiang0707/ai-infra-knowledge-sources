...
Codex requires a model with strong tool calling capabilities. The model must support the OpenAI-Responses tool calling API. See [Tool Calling](https://docs.vllm.ai/features/tool_calling/) for details on enabling tool calling for your model.
...
## Configuring Codex[¶](https://docs.vllm.ai#configuring-codex)

Codex is configured via a TOML file located at `~/.codex/config.toml`
...
You can set the `env_key`

to any dummy environment variable since vLLM doesn't require authentication by default:

Warning

When using the `responses`

API, ensure your vLLM version supports the OpenAI Responses API.
...
**Connection refused**: Ensure vLLM is running and accessible at the specified URL. Check that the port matches and that `base_url`
...
**Model not found**: Ensure the `model`

field in `~/.codex/config.toml`
