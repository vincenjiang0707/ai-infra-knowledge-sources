...
To run it as an OpenAI-compatible server, add the `--load-format runai_streamer`
...
, which supports `az login`

, managed identity, environment variables (`AZURE_CLIENT_ID`
...
## Sharded Model Loading[¶](https://docs.vllm.ai#sharded-model-loading)

vLLM also supports loading sharded models using Run:ai Model Streamer. This is particularly useful for large models that are split across multiple files. To use this feature, use the `--load-format runai_streamer_sharded`

flag:

The sharded loader expects model files to follow the same naming pattern as the regular sharded state loader: `model-rank-{rank}-part-{part}.safetensors`

. You can customize this pattern using the `pattern`

parameter in `--model-loader-extra-config`
