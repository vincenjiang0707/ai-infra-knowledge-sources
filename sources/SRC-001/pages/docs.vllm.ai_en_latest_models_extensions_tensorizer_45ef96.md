...
## Installing Tensorizer[¶](https://docs.vllm.ai#installing-tensorizer)
...
## The basics[¶](https://docs.vllm.ai#the-basics)

To load a model using Tensorizer, the model first needs to be serialized by Tensorizer. [ The example script](https://github.com/vllm-project/vllm/blob/main/examples/features/tensorize_vllm_model.py) takes care of this process.

Let's walk through a basic example by serializing `facebook/opt-125m`

using the script, and then loading it for inference.

## Serializing a vLLM model with Tensorizer[¶](https://docs.vllm.ai#serializing-a-vllm-model-with-tensorizer)
...
or the [ LLM](https://docs.vllm.ai/api/vllm/entrypoints/llm/#vllm.entrypoints.llm.LLM) entrypoint. You can pass the directory where you saved the model to the
...
. For example, to serve the tensorized model saved previously with the LoRA adapter, you'd do:Or, with `LLM()`
...
## Options for configuring Tensorizer[¶](https://docs.vllm.ai#options-for-configuring-tensorizer)
