...
## ColBERT Late Interaction Models[¶](https://docs.vllm.ai#colbert-late-interaction-models)
...
For **non-BERT backbones**, use `--hf-overrides`

to set the correct architecture:
...
## ColQwen3 Multi-Modal Late Interaction Models[¶](https://docs.vllm.ai#colqwen3-multi-modal-late-interaction-models)
...
### Text-only scoring and reranking[¶](https://docs.vllm.ai#text-only-scoring-and-reranking)
...
APIs also accept multi-modal inputs directly. Pass image documents using the `data_1`
...
) fields with a `content`

list containing `image_url`

and `text`

parts — the same format used by the OpenAI chat completion API:
...
You can also get the raw token embeddings using the `/pooling`

API with `token_embed`
...
[examples/pooling/token_embed/colqwen3_token_embed_online.py](https://github.com/vllm-project/vllm/blob/main/examples/pooling/token_embed/colqwen3_token_embed_online.py) - Reranking (text + multi-modal):
[examples/pooling/score/colqwen3_rerank_online.py](https://github.com/vllm-project/vllm/blob/main/examples/pooling/score/colqwen3_rerank_online.py)

## ColQwen3.5 Multi-Modal Late Interaction Models[¶](https://docs.vllm.ai#colqwen35-multi-modal-late-interaction-models)
...
## Llama Nemotron Multimodal[¶](https://docs.vllm.ai#llama-nemotron-multimodal)

### Embedding Model[¶](https://docs.vllm.ai#embedding-model)

Llama Nemotron VL Embedding models combine the bidirectional Llama embedding backbone (from `nvidia/llama-nemotron-embed-1b-v2`

) with SigLIP as the vision encoder to produce single-vector embeddings from text and/or images.
...
The chat template bundled with this model's tokenizer is not suitable for the embeddings API. Use the provided override template above when serving with the `messages`

-based (chat-style) embeddings API.

The override template uses the message `role`

to automatically prepend the appropriate prefix: set `role`

to `"query"`

for queries (prepends `query:`

) or `"document"`

for passages (prepends `passage:`

). Any other role omits the prefix.
...
supports dense retrieval, lexical matching, and ColBERT-style multi-vector retrieval. Its `config.json`

declares `XLMRobertaModel`

, so vLLM otherwise loads it as a vanilla RoBERTa model without the extra sparse and ColBERT weights. The examples below therefore override the architecture with [ BgeM3EmbeddingModel](https://docs.vllm.ai/api/vllm/model_executor/models/roberta/#vllm.model_executor.models.roberta.BgeM3EmbeddingModel).
...
as well to pair token IDs with their scores. See [ test_bge_m3.py](https://github.com/vllm-project/vllm/blob/main/tests/models/language/pooling/test_bge_m3.py) for a complete example that also combines repeated token IDs.

For ColBERT vectors, replace `<task>`
...
task so the model computes dense and lexical outputs together. Public requests must use task `plugin`

and put the plugin-specific fields under `data`
...
. The combined `embed&token_classify`

task is an internal execution contract for this plugin, not a generic Pooling API response format. Without the plugin, select one of the three concrete tasks above.