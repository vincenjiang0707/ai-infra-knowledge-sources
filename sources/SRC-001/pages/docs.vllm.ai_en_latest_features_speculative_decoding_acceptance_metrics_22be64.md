...
When speculative decoding is enabled, vLLM can report per-request acceptance metrics in the response, under `metrics.speculative_decoding`

. This lets a client compute the mean acceptance length and the accepted-draft-length distribution for an individual request, as a complement to the server-aggregated spec-decode metrics exposed at `/metrics`
...
is experimental and its shape may change in a future release. Pin to a vLLM version if you depend on it.

## Enabling[¶](https://docs.vllm.ai#enabling)

Start the server with `--per-request-spec-decode-metrics`
...
Collection is gated at the source: with `none`

, nothing is accumulated.

## Response Format[¶](https://docs.vllm.ai#response-format)

Acceptance metrics share the top-level `metrics`

object with the timing [per-request metrics](https://docs.vllm.ai/per_request_metrics/) — `metrics.speculative_decoding`

sits alongside the timing fields. Like timing, they describe a single generation stream, so they are reported only for single-sequence requests and are `null`
...
, two ordered arrays are added, one entry per verification step:
...
is present whenever `--per-request-spec-decode-metrics`

is `summary`

/`detailed`

, speculative decoding is enabled, and `n == 1`

(with an all-zero histogram if the request drafted nothing). It is `null`

otherwise.

## Streaming[¶](https://docs.vllm.ai#streaming)

In streaming responses, `metrics`

(including `speculative_decoding`

) rides the final usage chunk, which is only emitted when usage reporting is enabled — set `stream_options.include_usage: true`

or start the server with `--enable-force-include-usage`

.

## Relationship to Prometheus metrics[¶](https://docs.vllm.ai#relationship-to-prometheus-metrics)

The per-request fields are the individual-request counterpart of the server-aggregated spec-decode counters at `/metrics`

. Summed across the single-sequence requests that report them, they reconcile with the aggregate counters (which also count `n > 1`

requests, so the totals match only for all-`n == 1`
