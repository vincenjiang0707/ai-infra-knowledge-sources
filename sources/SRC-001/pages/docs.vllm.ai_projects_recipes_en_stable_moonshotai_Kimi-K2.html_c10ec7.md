source: https://docs.vllm.ai/projects/recipes/en/stable/moonshotai/Kimi-K2.html
lastmod: 2026-04-27

# moonshotai/Kimi-K2 Usage Guide[¶](https://docs.vllm.ai#moonshotaikimi-k2-usage-guide)

This guide describes how to run Kimi-K2 with native FP8.

**Note:** This guide is partially referenced and adapted from the official [Kimi-K2-Instruct Deployment Guidance](https://huggingface.co/moonshotai/Kimi-K2-Instruct/blob/main/docs/deploy_guidance.md) provided by Moonshot AI. We would like to express our gratitude to the original authors.[¶](https://docs.vllm.ai#note-this-guide-is-partially-referenced-and-adapted-from-the-official-kimi-k2-instruct-deployment-guidance-provided-by-moonshot-ai-we-would-like-to-express-our-gratitude-to-the-original-authors)

## Installing vLLM[¶](https://docs.vllm.ai#installing-vllm)

## Running Kimi-K2 with FP8 on 16xH800[¶](https://docs.vllm.ai#running-kimi-k2-with-fp8-on-16xh800)

The smallest deployment unit for Kimi-K2 FP8 weights with 128k seqlen on mainstream H800 platform is a cluster with 16 GPUs with either Tensor Parallel (TP) or "data parallel + expert parallel" (DP+EP). Running parameters for this environment are provided below. You may scale up to more nodes and increase expert-parallelism to enlarge the inference batch size and overall throughput.

### Tensor Parallelism + pipeline-parallelism[¶](https://docs.vllm.ai#tensor-parallelism-pipeline-parallelism)
...
### Data Parallelism + Expert Parallelism[¶](https://docs.vllm.ai#data-parallelism-expert-parallelism)
...
### FP8 Benchmark on 16xH800[¶](https://docs.vllm.ai#fp8-benchmark-on-16xh800)
...
### FP8 Benchmark on 16xH200[¶](https://docs.vllm.ai#fp8-benchmark-on-16xh200)
