source: https://docs.vllm.ai/en/latest/api/vllm/distributed/weight_transfer/clients/
lastmod: 2026-09-24

Built-in `VLLMWeightSyncClient`

implementations.

These adapt the inference engine's weight-sync control plane to concrete transports. A `TrainerWeightTransferEngine`

takes one of these (or any object with the same four methods — the protocol is structural) and drives the full handshake through it.

Imports of `ray`

/ `requests`

are deferred to call time so this module is importable without those packages installed.
...
CUDA IPC handles (`ipc_handles`

) are tuples of non-JSON-native objects, so over HTTP they are pickled+base64-encoded into `ipc_handles_pickled`

(which the worker auto-deserializes when `VLLM_ALLOW_INSECURE_SERIALIZATION=1`

). Other backends (NCCL) carry only JSON-native metadata and pass through unchanged. Mirrors the old IPC `_do_send`
