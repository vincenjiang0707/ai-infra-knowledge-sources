...
vLLM IR is a **functional intermediate representation (IR)** that fills the gap between low-level `torch`

ops and vLLM layers like [ RMSNorm](https://docs.vllm.ai/api/vllm/model_executor/layers/layernorm/#vllm.model_executor.layers.layernorm.RMSNorm) and quantization operators, By separating operator

**semantics**from the

**implementation**and

**dispatching**, vLLM IR simplifies both compilation and kernel registration & dispatching simultaneously. It operates as a

**dialect**in the torch FX representation, allowing full interoperability with “regular” torch ops & custom torch ops/kernels, as well as a piecewise migration from the previous

[approach.](https://docs.vllm.ai/api/vllm/model_executor/custom_op/#vllm.model_executor.custom_op.CustomOp)

`CustomOp`

Key design principles:

**Eager-compile consistency**: identical behavior (barring minor numerics) in eager and compiled modes**Simple, transparent, yet powerful kernel selection**: good visibility and control allowing easy debugging**Convention over configuration**: near-zero boilerplate required to register ops and implementations**Extensibility**: ops and implementations can be registered anywhere, in-tree or out-of-tree**Interoperability**: fully compatible with “regular” torch ops & custom torch ops/kernels, reducing developer friction and allowing piecewise migration
...
## Quick Overview[¶](https://docs.vllm.ai#quick-overview)

### Declaring an IR Operation[¶](https://docs.vllm.ai#declaring-an-ir-operation)

IR operations are declared using the `@register_op`
...
**Semantic definition**: Specifies the exact semantics of the operation, including shapes and strides**Default implementation**: Used when no other (better) implementation is available**Reference for testing**: Other implementations must match these semantics

### Registering Implementations[¶](https://docs.vllm.ai#registering-implementations)

Kernel implementations are registered using the `register_impl`
...
: Static boolean indicating if this implementation is available`supports_args`

: Function checking if the implementation supports specific arguments`inplace`

: Whether this implementation reuses input memory for outputs

### Using IR Operations in Models[¶](https://docs.vllm.ai#using-ir-operations-in-models)
...
Kernel selection is controlled via priority lists in the configuration. Priority lists specify the order in which implementations are considered, with the first supported implementation being selected. This includes the static support check (`supported=...`

) and the dynamic arg support check (`supports_args=...`

).

#### Command Line Configuration[¶](https://docs.vllm.ai#command-line-configuration)
...
## Compilation Pipeline[¶](https://docs.vllm.ai#compilation-pipeline)

vLLM IR heavily customizes the `torch.compile`

-based compilation process to allow custom compile passes to operate on high-level IR while still producing efficient low-level code at the end. The compilation pipeline consists of several stages:

### 1. Dynamo Tracing[¶](https://docs.vllm.ai#1-dynamo-tracing)

When `torch.compile`

traces the model's forward pass, vLLM IR operations appear as custom operations in the `vllm_ir`
...
AOTAutograd functionalizes the graph, converting any mutating operations to functional equivalents. For vLLM IR operations with `maybe_inplace`

overloads, we perform this manually before AOTAutograd, converting them to the functional `default`
...
), storing this information in vLLM's `PassContext`

for later use in clone elimination.

### 3. IR Fusion and Transformation Passes[¶](https://docs.vllm.ai#3-ir-fusion-and-transformation-passes)
...
### 4. IR Lowering[¶](https://docs.vllm.ai#4-ir-lowering)

The lowering pass ([ VllmIRLoweringPass](https://docs.vllm.ai/api/vllm/compilation/passes/ir/lowering_pass/#vllm.compilation.passes.ir.lowering_pass.VllmIRLoweringPass)) replaces each vLLM IR operation with its selected implementation. The implementation is chosen based on the priority list and support predicates, using the
...
### 5. Clone Cleanup[¶](https://docs.vllm.ai#5-clone-cleanup)

After lowering, the clone elimination pass ([ UnsafeCloneEliminationPass](https://docs.vllm.ai/api/vllm/compilation/passes/ir/clone_elimination/#vllm.compilation.passes.ir.clone_elimination.UnsafeCloneEliminationPass)) removes unnecessary clones introduced during lowering. This pass is essential for achieving zero-copy behavior when using in-place kernels with

`maybe_inplace`

. The pass removes a clone if:- the cloned input is created in the graph and not used again in the graph
...
### 6. Inductor Optimization and Codegen[¶](https://docs.vllm.ai#6-inductor-optimization-and-codegen)
...
**Inductor lowering and pointwise fusion**: Fusing element-wise operations, reductions, etc.**Memory planning**: Determining buffer allocation and reuse**Kernel generation**: Generating Triton or C++ code for fused operations**Autotuning**: Selecting the best kernel configurations

### Pipeline Summary[¶](https://docs.vllm.ai#pipeline-summary)
...
## Core vLLM IR Concepts[¶](https://docs.vllm.ai#core-vllm-ir-concepts)

### Operation Declaration[¶](https://docs.vllm.ai#operation-declaration)

Operations are declared with the `@register_op`

decorator, which creates an `IrOp`
...
: List of parameter names considered "activations" (typically consumed by`maybe_inplace`

). Defaults to parameters starting with`x`

.`allow_inplace`

: Creates a`maybe_inplace`

overload for memory-efficient execution (see below).

### The `maybe_inplace`

Overload[¶](https://docs.vllm.ai#the-maybe_inplace-overload)

The `maybe_inplace`

overload is a critical feature for memory efficiency in LLM inference. It signals that the caller doesn't need to preserve the activation inputs after the operation, allowing in-place implementations to reuse input memory for outputs.

#### Semantics and Usage[¶](https://docs.vllm.ai#semantics-and-usage)
...
Using an activation input after passing it to `maybe_inplace`
...
#### Compilation Behavior[¶](https://docs.vllm.ai#compilation-behavior)

During compilation, the inplace functionalization pass validates that donated inputs are not used again and converts `maybe_inplace`

to the functional `default`
...
#### Eager Mode Behavior[¶](https://docs.vllm.ai#eager-mode-behavior)

In eager mode (without `torch.compile`
...
in model code and in-place kernel implementations provides optimal memory efficiency in both eager and compiled modes, with identical semantics in both cases.

#### Memory Savings Example[¶](https://docs.vllm.ai#memory-savings-example)
...
### Implementation Registration[¶](https://docs.vllm.ai#implementation-registration)

Implementations are registered using the `register_impl`
...
: Reserved for the native torch implementation (declared with`@register_op`

)`vllm_c`

: C++/CUDA kernels via`torch.ops._C`

`aiter`

: AMD AITER library`xpu_kernels`

: SYCL/SYCLTLA kernels implemented in`vllm-xpu-kernels`

`triton_*`

: Triton kernels- Platform/library names for other implementations

**Support checking:**

`supported`

: Static boolean, checked once at import time (e.g.,`HAS_TRITON`

,`is_cuda_alike()`

)`supports_args`

: Function`(*args, **kwargs) -> bool`

checking argument compatibility- Called with
**fake tensors**during compilation for zero-cost checking - Called with
**real tensors**during eager mode dispatch - Should NOT check batch sizes or add guards based on values
...
Batch-invariant kernels are automatically selected when `VLLM_BATCH_INVARIANT=1`

is set.

### Eager Mode vs Compile Mode[¶](https://docs.vllm.ai#eager-mode-vs-compile-mode)
...
- IR ops appear in FX graph as
`torch.ops.vllm_ir.*`

custom ops - Lowering selects implementation using fake tensors
...
## Other Topics[¶](https://docs.vllm.ai#other-topics)

### Out-of-Tree Implementations[¶](https://docs.vllm.ai#out-of-tree-implementations)
...
## Migration from CustomOp[¶](https://docs.vllm.ai#migration-from-customop)

vLLM IR is designed to coexist with and gradually replace [ CustomOp](https://docs.vllm.ai/api/vllm/model_executor/custom_op/#vllm.model_executor.custom_op.CustomOp):

**Op declaration**: Convertclass`CustomOp`

and move`PluggableLayer`

`forward_native`

to`@register_op`

function**Implementation registration**: Use`@ir.ops.op_name.register_impl`

instead of overriding methods**Layer usage**: Replace`self.op(...)`

with`ir.ops.op_name(...)`

**Configuration**: Migrate`--compilation-config.custom-ops`
...
[torch.compile Integration](https://docs.vllm.ai/torch_compile/)- General compilation infrastructure[Fusions](https://docs.vllm.ai/fusions/)- Custom fusion and transformation passes in vLLM[Custom Operations](https://docs.vllm.ai/custom_op/)- Legacy custom op system