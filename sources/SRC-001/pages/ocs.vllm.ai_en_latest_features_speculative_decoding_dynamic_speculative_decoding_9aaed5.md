source: https://docs.vllm.ai/en/latest/features/speculative_decoding/dynamic_speculative_decoding/
lastmod: 2026-09-24

# Dynamic Speculative Decoding[¶](https://docs.vllm.ai#dynamic-speculative-decoding)

## Why is Dynamic SD needed?[¶](https://docs.vllm.ai#why-is-dynamic-sd-needed)
...
## Use cases[¶](https://docs.vllm.ai#use-cases)
...
To use Dynamic SD, add `num_speculative_tokens_per_batch_size`

to the config of an SD method which is a list of list. Here, an entry is `[start_bs, end_bs, optimal_K]`

which means when the concurrency is within range `[start_bs, end_bs]`

then `optimal_K`

number of draft tokens are used. For e.g.,
...
## Online Examples[¶](https://docs.vllm.ai#online-examples)

### Dynamic SD Eagle Drafter[¶](https://docs.vllm.ai#dynamic-sd-eagle-drafter)
...
### Dynamic SD Eagle3 Drafter[¶](https://docs.vllm.ai#dynamic-sd-eagle3-drafter)
...
- Not compatible with data parallelism (
`--data-parallel-size > 1`

). Each DP rank schedules independently, so ranks can pick different K values, causing DP collective divergence and deadlocks. When DP is enabled, vLLM automatically disables`num_speculative_tokens_per_batch_size`

and falls back to the static`num_speculative_tokens`
