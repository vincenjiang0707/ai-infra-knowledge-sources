source: https://docs.vllm.ai/projects/recipes/en/stable/Google/Gemma4.html
lastmod: 2026-04-27

# Gemma 4 Usage Guide[¶](https://docs.vllm.ai#gemma-4-usage-guide)
...
### Dense Models[¶](https://docs.vllm.ai#dense-models)

| Model | Parameters | Min NVIDIA GPUs (BF16) | Min AMD GPUs (BF16) | Min TPUs | HuggingFace |
|---|---|---|---|---|---|
| Gemma 4 E2B IT | effective 2B | 1× (24 GB+) | 1× MI300X/MI325X/MI350X/MI355X | - |
|

[google/gemma-4-E4B-it](https://huggingface.co/google/gemma-4-E4B-it)[google/gemma-4-31B-it](https://huggingface.co/google/gemma-4-31B-it)### Mixture-of-Experts (MoE) Models[¶](https://docs.vllm.ai#mixture-of-experts-moe-models)

| Model | Total / Active Params | Min NVIDIA GPUs (BF16) | Min AMD GPUs (BF16) | Min TPUs | HuggingFace |
|---|---|---|---|---|---|
| Gemma 4 26B-A4B IT | 26B / 4B active | 1× (80 GB) | 1× MI300X/MI325X/MI350X/MI355X | 4× Trillium / 1× Ironwood |
|

### Key Architecture Features[¶](https://docs.vllm.ai#key-architecture-features)

**Multimodal**: Natively processes text and images (video supported via a custom vLLM processing pipeline that extracts frames; smaller gemma4-E2B and gemma-4-E4B also support audio).**MoE**: 128 fine-grained experts with top-8 routing and custom GELU-activated FFN**Dual Attention**: Alternating sliding-window (local) and global attention with different head dimensions**Thinking Mode**: Structured reasoning via`<|channel>thought\n...<channel|>`
...
### pip (NVIDIA CUDA)[¶](https://docs.vllm.ai#pip-nvidia-cuda)
...
### pip (AMD ROCm: MI300X, MI325X, MI350X, MI355X)[¶](https://docs.vllm.ai#pip-amd-rocm-mi300x-mi325x-mi350x-mi355x)
...
### Quick Start (Single GPU)[¶](https://docs.vllm.ai#quick-start-single-gpu)

### Multi-GPU Deployment[¶](https://docs.vllm.ai#multi-gpu-deployment)
...
where MODEL is any of the gemma4 models.### Docker Deployment[¶](https://docs.vllm.ai#docker-deployment)
...
### Cloud TPU Deployment via Docker[¶](https://docs.vllm.ai#cloud-tpu-deployment-via-docker)
...
### AMD GPU Deployment (MI300X, MI325X, MI350X, MI355X) via Docker[¶](https://docs.vllm.ai#amd-gpu-deployment-mi300x-mi325x-mi350x-mi355x-via-docker)
...
to skip audio encoder memory allocation. - For text-only workloads, pass
...
### Launch Server with Audio Support[¶](https://docs.vllm.ai#launch-server-with-audio-support)
...
## Video Understanding[¶](https://docs.vllm.ai#video-understanding)
...
### Launch Server with Video Support[¶](https://docs.vllm.ai#launch-server-with-video-support)
...
### Launch Server with Thinking Support[¶](https://docs.vllm.ai#launch-server-with-thinking-support)
...
## Function Calling / Tool Use[¶](https://docs.vllm.ai#function-calling-tool-use)
...
### Launch Server with Tool Calling[¶](https://docs.vllm.ai#launch-server-with-tool-calling)
...
### Structured Outputs with Thinking[¶](https://docs.vllm.ai#structured-outputs-with-thinking)
...
### Launch Server for Benchmarking[¶](https://docs.vllm.ai#launch-server-for-benchmarking)
...
### Interpreting Results[¶](https://docs.vllm.ai#interpreting-results)
...
### Throughput vs. Latency Tuning[¶](https://docs.vllm.ai#throughput-vs-latency-tuning)
...
### Memory Optimization[¶](https://docs.vllm.ai#memory-optimization)
...
### Server Flags Reference[¶](https://docs.vllm.ai#server-flags-reference)
...
### Full-Featured Server Launch[¶](https://docs.vllm.ai#full-featured-server-launch)
...
## Deploy on Modal[¶](https://docs.vllm.ai#deploy-on-modal)

[Modal](https://modal.com) lets you run this recipe on cloud GPUs with a single command — no infrastructure setup required.

The deployment script is [ gemma4-modal.py](https://docs.vllm.ai/gemma4-modal.py) in this directory.