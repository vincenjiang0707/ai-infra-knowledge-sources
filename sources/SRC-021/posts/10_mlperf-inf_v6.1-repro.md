...
## Workloads Summary[#](https://rocm.blogs.amd.com#workloads-summary)
...
**AMD Instinct MI355X, MI350X, or MI350P**GPUs. Eight GPUs are required for all single-node submissions. The multi-node GPT-OSS 120B submission requires 72 GPUs across 9 nodes.ECC must be enabled on all GPU devices. Verify with:
...
**OS:**Ubuntu 22.04 LTS or later (recommended).**ROCm:**[PLACEHOLDER: ROCm version, e.g., ROCm 6.4] — install via the[ROCm Quick Start Guide](https://rocm.docs.amd.com/en/latest/deploy/linux/quick_start.html).**Docker:**Docker Engine with AMD GPU passthrough configured. Verify your installation:docker run --rm --device=/dev/kfd --device=/dev/dri rocm/rocm-terminal rocminfo | grep "gfx"
...
## Llama 2 70B[#](https://rocm.blogs.amd.com#llama-2-70b)
...
### Step 1: Prepare the Docker Container - Llama 2 70B[#](https://rocm.blogs.amd.com#step-1-prepare-the-docker-container-llama-2-70b)
...
### Step 2: Download the Reference Model and Dataset - Llama 2 70B[#](https://rocm.blogs.amd.com#step-2-download-the-reference-model-and-dataset-llama-2-70b)
...
The llama2-70b model on Hugging Face is gated. You must accept the model license at `https://huggingface.co/meta-llama/Llama-2-70b-hf`

before the download will succeed with your token.

### Step 3: Run the Benchmark Harness - Llama 2 70B[#](https://rocm.blogs.amd.com#step-3-run-the-benchmark-harness-llama-2-70b)

Note: The commands below are for MI355X. To execute on a different GPU, change the config file as appropriate.

#### Offline Scenario Performance Benchmark - Llama 2 70B[#](https://rocm.blogs.amd.com#offline-scenario-performance-benchmark-llama-2-70b)
...
The `mlperf_log_accuracy.json`

file is processed to verify the accuracy of the offline scenario:
...
#### Server Scenario Performance Benchmark - Llama 2 70B[#](https://rocm.blogs.amd.com#server-scenario-performance-benchmark-llama-2-70b)
...
Run the server scenario accuracy test to generate the `mlperf_log_accuracy.json`
...
is processed to verify the accuracy of the server scenario:
...
#### Interactive Scenario Performance Benchmark - Llama 2 70B[#](https://rocm.blogs.amd.com#interactive-scenario-performance-benchmark-llama-2-70b)
...
is processed to verify the accuracy of the interactive scenario:
...
### Step 1: Prepare the Docker Container - GPT-OSS 120B[#](https://rocm.blogs.amd.com#step-1-prepare-the-docker-container-gpt-oss-120b)
...
### Step 2: Download the Reference Model and Dataset - GPT-OSS 120B[#](https://rocm.blogs.amd.com#step-2-download-the-reference-model-and-dataset-gpt-oss-120b)
...
### Step 3: Run the Benchmark Harness - GPT-OSS 120B[#](https://rocm.blogs.amd.com#step-3-run-the-benchmark-harness-gpt-oss-120b)

Note: The commands below are for MI355X. To execute on a different GPU, change the config file as appropriate.

#### Offline Scenario Performance Benchmark - GPT-OSS-120B[#](https://rocm.blogs.amd.com#offline-scenario-performance-benchmark-gpt-oss-120b)
...
#### Server Scenario Performance Benchmark - GPT-OSS-120B[#](https://rocm.blogs.amd.com#server-scenario-performance-benchmark-gpt-oss-120b)
...
file is processed to verify the accuracy of the server scenario:
...
### Multi-Node Submission[#](https://rocm.blogs.amd.com#multi-node-submission)
...
**Cluster:**9 nodes × 8 AMD Instinct MI355X GPUs =**72 total GPUs****Interconnect:**[PLACEHOLDER: InfiniBand / Ethernet spec]**MPI / launcher:**[PLACEHOLDER: mpirun / torchrun / custom launcher]

#### Step 1: Prepare the Docker Container - GPT-OSS-120B Multi-Node[#](https://rocm.blogs.amd.com#step-1-prepare-the-docker-container-gpt-oss-120b-multi-node)
...
#### Step 2: Download the Reference Model and Dataset - GPT-OSS-120B Multi-Node[#](https://rocm.blogs.amd.com#step-2-download-the-reference-model-and-dataset-gpt-oss-120b-multi-node)
...
#### Step 3: Run the Benchmark Harness - GPT-OSS-120B Multi-Node[#](https://rocm.blogs.amd.com#step-3-run-the-benchmark-harness-gpt-oss-120b-multi-node)

##### Running the Benchmark Under the Server Scenario[#](https://rocm.blogs.amd.com#running-the-benchmark-under-the-server-scenario)

To run the benchmark under the server scenario, use the run_harness.sh script to start the SUT client. Make sure `device_count`

is set to the number of all the healthy GPUs across all the nodes in the cluster.
...
Use the `distributed_async_server.py`

script to start a worker on each node, including the Head node. Use the IP of the Head node for the Head node IP:
...
script with `config-name`

set to `offline_mi355x`

is used to start the SUT client:
...
## DeepSeek-R1[#](https://rocm.blogs.amd.com#deepseek-r1)

**DeepSeek-R1** is a reasoning-focused large language model. AMD submission runs the quantized `S3_sq_a05_v2`

checkpoint on MI355X with the **SGLang** inference backend. Scenarios: **Offline** and **Server**.

### Step 1: Prepare the Docker Container - DeepSeek-R1[#](https://rocm.blogs.amd.com#step-1-prepare-the-docker-container-deepseek-r1)
...
You start in `/lab-mlperf-inference/code`

, where `main.py`

lives. The `-v $(pwd)/model:/model`

and `-v $(pwd)/data:/data`

mounts keep the downloaded model and dataset on the host so they persist across containers.

### Step 2: Download the Reference Model and Dataset - DeepSeek-R1[#](https://rocm.blogs.amd.com#step-2-download-the-reference-model-and-dataset-deepseek-r1)
...
The configs read `/data/deepseek-r1/mlperf_deepseek_r1_dataset_4388_fp8_eval.pkl`

, so make sure the eval file ends up there (move or symlink it if the downloader nests it under a subfolder).

### Step 3: Run the Benchmark Harness - DeepSeek-R1[#](https://rocm.blogs.amd.com#step-3-run-the-benchmark-harness-deepseek-r1)

Run from `/lab-mlperf-inference/code`

. Every run must pass `--backend sglang`
...
`setup_deepseek_accuracy_env.sh`

is a **one-time** step. It installs the PRM800K (math) and LiveCodeBench graders the scorer needs. Run it before your first `check_deepseek_accuracy_scores.sh`

, otherwise scoring aborts with a missing-module error.

#### Offline Scenario - DeepSeek-R1[#](https://rocm.blogs.amd.com#offline-scenario-deepseek-r1)
...
#### Server Scenario - DeepSeek-R1[#](https://rocm.blogs.amd.com#server-scenario-deepseek-r1)
...
DLRM-v3’s sparse embedding engine maps peer GPU memory over AMD Infinity Fabric, which depends on the host stack as well as the container. Use Ubuntu 24.04 with Linux kernel 6.8 or newer, `amdgpu`

6.16.6 or newer, and host ROCm 7.2 or newer. On older host stacks the identical container fails during embedding-engine initialization.

Unlike the other workloads in this blog, DLRM-v3 is driven by a standalone runner repository rather than a single pre-built image. The runner provisions the container, builds the two components that must be compiled for the `gfx950`

target, stages the model and dataset, and runs the performance, accuracy, and compliance tests. This is the procedure AMD used to produce the submission results.

### Step 1: Prepare the Docker Container - DLRM-v3[#](https://rocm.blogs.amd.com#step-1-prepare-the-docker-container-dlrm-v3)
...
### Step 2: Download the Reference Model and Dataset - DLRM-v3[#](https://rocm.blogs.amd.com#step-2-download-the-reference-model-and-dataset-dlrm-v3)
...
**Checkpoint:**download from[https://inference.mlcommons-storage.org/metadata/dlrm-v3-checkpoint.uri](https://inference.mlcommons-storage.org/metadata/dlrm-v3-checkpoint.uri)(~964 GB, sharded)**Dataset:**`dlrmv3_preprocessed_full`
...
**Host requirements** (qualified MI355X 8-GPU box — `run_gold.sh`

checks most of these before launch):

≥ 220 GB free VRAM per GPU (export

`MIN_FREE_GB=220`

;`run_gold`

default is 210)SMT enabled and expected CPUs online (≥ 256 online CPUs)

CPU governor set to

`performance`

on all online CPUs (or export`SKIP_CPU_CHECK=1`
...
### Step 3: Run the Benchmark Harness - DLRM-v3[#](https://rocm.blogs.amd.com#step-3-run-the-benchmark-harness-dlrm-v3)

All runs go through the `run_image.sh`

launcher, which provisions the container with GPU device access and the capabilities the sparse embedding engine needs for peer memory over the fabric. The harness shards the embedding table across all eight GPUs on its own, so no GPU-count flag is required.
...
#### Server Scenario Performance Benchmark - DLRM-v3[#](https://rocm.blogs.amd.com#server-scenario-performance-benchmark-dlrm-v3)
...
#### Offline Scenario Performance Benchmark - DLRM-v3[#](https://rocm.blogs.amd.com#offline-scenario-performance-benchmark-dlrm-v3)
...
#### Accuracy Benchmark - DLRM-v3[#](https://rocm.blogs.amd.com#accuracy-benchmark-dlrm-v3)
...
and then scores GAUC against the reference:
...
#### Compliance Test (TEST08) - DLRM-v3[#](https://rocm.blogs.amd.com#compliance-test-test08-dlrm-v3)
...
AMD submission reports `TEST PASS`

with zero unmatched entries on both systems.

#### Read the Verdicts - DLRM-v3[#](https://rocm.blogs.amd.com#read-the-verdicts-dlrm-v3)
...
## Wan 2.2-t2v[#](https://rocm.blogs.amd.com#wan-2-2-t2v)
...
### Step 1: Prepare the Docker Container - Wan 2.2-t2v[#](https://rocm.blogs.amd.com#step-1-prepare-the-docker-container-wan-2-2-t2v)
...
### Step 2: Download the Reference Model and Dataset - Wan 2.2-t2v[#](https://rocm.blogs.amd.com#step-2-download-the-reference-model-and-dataset-wan-2-2-t2v)
...
### Step 3: Run the Benchmark Harness - Wan 2.2-t2v[#](https://rocm.blogs.amd.com#step-3-run-the-benchmark-harness-wan-2-2-t2v)
...
Switch to the `/app/mlperf/mlperf_inference/text_to_video/wan-2.2-t2v-a14b/`

folder inside the Docker container. Use `run_scenarios.sh`

to run the accuracy, performance, compliance, and, optionally, VBench benchmarks for both Single Stream and Offline scenarios:
...
. If you used the bind-mount above, that tree appears on the host as `./mlperf_outputs/`

.

## Llama 3.1 8B[#](https://rocm.blogs.amd.com#llama-3-1-8b)
...
### Step 1: Prepare the Docker Container - Llama 3.1 8B[#](https://rocm.blogs.amd.com#step-1-prepare-the-docker-container-llama-3-1-8b)
...
### Step 2: Download the Reference Model and Dataset - Llama 3.1 8B[#](https://rocm.blogs.amd.com#step-2-download-the-reference-model-and-dataset-llama-3-1-8b)
...
The configs read `/data/cnn_eval.json`

, so make sure the eval file ends up there (move or symlink it if the downloader nests it under a subfolder).

### Step 3: Run the Benchmark Harness - Llama 3.1 8B[#](https://rocm.blogs.amd.com#step-3-run-the-benchmark-harness-llama-3-1-8b)

Run from `/lab-mlperf-inference/code`

. Every run must pass `--backend vllm`

.

Note: The commands below are for MI355X. To execute on a different GPU, change the config file as appropriate.

#### Offline Scenario - Llama 3.1 8B[#](https://rocm.blogs.amd.com#offline-scenario-llama-3-1-8b)
...
#### Server Scenario - Llama 3.1 8B[#](https://rocm.blogs.amd.com#server-scenario-llama-3-1-8b)
...
#### Interactive Scenario - Llama 3.1 8B[#](https://rocm.blogs.amd.com#interactive-scenario-llama-3-1-8b)
...
The `check_llama3_1_8b_accuracy_scores.sh`

step reads the dataset from `/data/cnn_eval.json`

and the model from `/model/`

by default, and writes `accuracy.txt`

(ROUGE scores) next to each `mlperf_log_accuracy.json`

.

## Expected Results[#](https://rocm.blogs.amd.com#expected-results)
...
**MLCommons inference GitHub:**[mlcommons/inference](https://github.com/mlcommons/inference)**ROCm GitHub Discussions:**[ROCm/ROCm#discussions](https://github.com/ROCm/ROCm/discussions)**AMD ROCm blogs:**[https://rocm.blogs.amd.com](https://rocm.blogs.amd.com)
