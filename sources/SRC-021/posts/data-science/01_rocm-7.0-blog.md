...
source: https://rocm.blogs.amd.com/ecosystems-and-partners/rocm-7.0-blog/README.html

# ROCm 7.0: An AI-Ready Powerhouse for Performance, Efficiency, and Productivity[#](https://rocm.blogs.amd.com#rocm-7-0-an-ai-ready-powerhouse-for-performance-efficiency-and-productivity)
...
## AI Ecosystem Highlights[#](https://rocm.blogs.amd.com#ai-ecosystem-highlights)
...
### AI Frameworks[#](https://rocm.blogs.amd.com#ai-frameworks)
...
### Inference Engines[#](https://rocm.blogs.amd.com#inference-engines)
...
### AI Model Optimization[#](https://rocm.blogs.amd.com#ai-model-optimization)
...
### AI Performance Enablement[#](https://rocm.blogs.amd.com#ai-performance-enablement)
...
### Training[#](https://rocm.blogs.amd.com#training)
...
## Hardware Enablement: AMD Instinct MI350 Series GPUs[#](https://rocm.blogs.amd.com#hardware-enablement-amd-instinct-mi350-series-gpus)
...
## The ROCm 7.0 SDK[#](https://rocm.blogs.amd.com#the-rocm-7-0-sdk)
...
### Runtime & Compiler[#](https://rocm.blogs.amd.com#runtime-compiler)
...
### Math & Compute Libraries[#](https://rocm.blogs.amd.com#math-compute-libraries)
...
[hipBLASLt](https://github.com/ROCm/rocm-libraries/tree/develop/projects/hipblaslt/CHANGELOG.md)library now supports a wide range of data formats, including FP32, FP16, BF16, FP8, and BF8—alongside fused activation functions like Swish, SiLU, and GELU, enabling more efficient matrix multiplication on the MI300A APU. Memory and performance tuning improvements further optimize GEMM operations for diverse tensor shapes.[hipBLAS](https://github.com/ROCm/rocm-libraries/tree/develop/projects/hipblas/CHANGELOG.md)adds better workspace memory control and streamlines its API under HIPBLAS_V2, improving usability and reducing memory stalls.[rocBLAS](https://github.com/ROCm/rocm-libraries/tree/develop/projects/rocblas/CHANGELOG.md)benefits from broader hardware support, modernized build tools, and targeted optimizations for GEMM and GEMV routines, especially in large batch sizes and high-dimensional workloads.[hipSPARSE](https://github.com/ROCm/rocm-libraries/tree/develop/projects/hipsparse/CHANGELOG.md)expands support for low-precision data types and introduces mixed precision across key routines, improving performance with minimal accuracy trade-offs.[hipSPARSELt](https://github.com/ROCm/rocm-libraries/tree/develop/projects/hipsparselt/CHANGELOG.md)enhances block-sparse operations with FP8/BF8 support and kernel tuning for MI300A, while integrating profiling tools like ROC-TX.[rocSPARSE](https://github.com/ROCm/rocm-libraries/tree/develop/projects/rocsparse/CHANGELOG.md)introduces new routines like SpGEAM and v2_SpMV, expands half-float precision, and improves runtime efficiency through smarter memory usage and build optimizations. Solver libraries rocSOLVER and hipSOLVER also evolve.[rocSOLVER](https://github.com/ROCm/rocm-libraries/tree/develop/projects/rocsolver)adds hybrid CPU-GPU execution and support for SVD via Cuppen’s algorithm. Performance has been boosted for routines like`BDSQR`
...
, while memory usage has been reduced for eigensolvers such as`STEDC`

,`SYEVD/HEEVD`

, and`SYGVD/HEGVD`

.[hipSOLVER](https://github.com/ROCm/rocm-libraries/tree/develop/projects/hipsolver)improves CUDA compatibility for sparse matrix workflows. Together, these updates make ROCm 7.0 a more powerful and developer-friendly platform for scientific computing, machine learning, and high-performance AI applications.[hipCUB](https://github.com/ROCm/rocm-libraries/tree/develop/projects/hipcub/CHANGELOG.md)updates enable GPU binary compression by default, reducing build sizes and improving multi-target build reliability. It also adds efficient single-pass scan operators and inclusive scans with custom initial values, aligning more closely with upstream CUB functionality and improving integration with rocPRIM.[rocThrust](https://github.com/ROCm/rocm-libraries/tree/develop/projects/rocthrust/CHANGELOG.md)sees better modularity and testing, with upgraded benchmarking tools and compatibility improvements through CCCL/thrust 2.6.0 integration.[rocRAND](https://github.com/ROCm/rocm-libraries/tree/develop/projects/rocrand/CHANGELOG.md)strengthens reliability with expanded unit tests, refined error handling, and optimized tuning for MI300 GPUs, making it more robust for statistical simulations.[rocPRIM](https://github.com/ROCm/rocm-libraries/tree/develop/projects/rocprim/CHANGELOG.md)introduces new utilities and APIs for improved numerical stability and flexible data transformations, while optimizing multi-stream workloads and scan operations.[rocFFT](https://github.com/ROCm/rocm-libraries/tree/develop/projects/rocfft/CHANGELOG.md)enhances fast Fourier transform capabilities with new single-precision kernels and optimized plans for large 1D transforms, improving throughput and efficiency for high-performance computing tasks.
...
[Composable Kernel](https://github.com/ROCm/rocm-libraries/blob/develop/projects/composablekernel/CHANGELOG.md)now supports a broader range of data types and delivers higher throughput and efficiency for large-scale model deployment.[MIOpen](https://github.com/ROCm/rocm-libraries/tree/develop/projects/miopen/CHANGELOG.md)improves convolution performance by fusing multiple operations into single, optimized kernels, significantly reducing memory traffic and latency.[rocWMMA](https://github.com/ROCm/rocWMMA/CHANGELOG.md)expands matrix-multiply-accumulate capabilities with support for interleaved layouts and mixed precision formats like BF8 and FP8. The new Fragment Scheduler API and static loop unrolling enhance control and reduce overhead, while unified fragment APIs and wave-level optimizations improve kernel occupancy and parallelism.
...
### Communication Libraries[#](https://rocm.blogs.amd.com#communication-libraries)
...
Beyond communication efficiency, ROCm 7.0 also brings greater reliability and deployment flexibility. A dedicated RAS (Reliability, Availability, and Serviceability) thread and a new `rcclras`
...
### Computer Vision and Image Processing[#](https://rocm.blogs.amd.com#computer-vision-and-image-processing)
...
## AMD Datacenter Infrastructure and Enterprise AI Tools[#](https://rocm.blogs.amd.com#amd-datacenter-infrastructure-and-enterprise-ai-tools)
...
### Enterprise AI: Open, Scalable, and Ready for Deployment[#](https://rocm.blogs.amd.com#enterprise-ai-open-scalable-and-ready-for-deployment)
...
### AMD GPU Drivers & Operating System Support[#](https://rocm.blogs.amd.com#amd-gpu-drivers-operating-system-support)

[Installation docs](https://rocm.docs.amd.com/projects/install-on-linux/en/latest/index.html)and release notes are now split between the AMD GPU Driver and ROCm SDK, letting users choose combinations of the two during installation. This allows users to update the AMD GPU Drivers and the ROCm toolkit independently.[AMD GPU driver 30.10.0](https://advanced-micro-devices-dcgpu-documentation--35.com.readthedocs.build/projects/amdgpu-docs/en/35/documentation/change-logs/30.10.0.html)adds support for the MI350X and MI355X.enables bare-metal SPX+NPS1 and DPX + NPS2 partitioning modes on the MI350X and MI355X.
...
### AMD GPU Virtualization Support & Drivers[#](https://rocm.blogs.amd.com#amd-gpu-virtualization-support-drivers)
...
### AMD GPU Operator[#](https://rocm.blogs.amd.com#amd-gpu-operator)
