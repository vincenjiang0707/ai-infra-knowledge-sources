source: https://docs.ray.io/en/latest/serve/llm/user-guides/deployment-initialization.html
lastmod: 

# Deployment Initialization[#](https://docs.ray.io#deployment-initialization)
...
### Load gated models[#](https://docs.ray.io#load-gated-models)
...
### Fast download from Hugging Face[#](https://docs.ray.io#fast-download-from-hugging-face)

Enable fast downloads with Hugging Face’s `hf_transfer`
...
## Model Loading from remote storage[#](https://docs.ray.io#model-loading-from-remote-storage)
...
parameter in `model_loading_config`
...
For Google Cloud Storage, use the `gs://`
...
protocol. The URI must embed the container and storage account as
`container@account.<domain>`

:

`abfss://<container>@<account>.dfs.core.windows.net/path/to/model`

(ADLS Gen2)`azure://<container>@<account>.blob.core.windows.net/path/to/model`
...
Azure loading requires the `adlfs`

and `azure-identity`

packages on every node
that loads the model. Ship them through `runtime_env`

so Ray installs them on
...
resolves either one automatically, so the deployment
...
## RunAI Streamer[#](https://docs.ray.io#runai-streamer)
...
### S3 and RunAI Streamer[#](https://docs.ray.io#s3-and-runai-streamer)
...
### RunAI Streamer from a local path[#](https://docs.ray.io#runai-streamer-from-a-local-path)

When `load_format`

is `runai_streamer`

, Ray Serve LLM doesn’t download the model. It passes `model_source`

to the streamer, which reads it directly. The streamer supports a local path on each node in addition to remote object stores, and the set of supported remote schemes depends on your `runai-model-streamer`

and vLLM versions. Use a local path when the weights are already staged on a volume mounted on every node or copied to local disk, such as weights pulled from another source before serving. Point `model_source`

at the path, set `load_format`

to `runai_streamer`

, and tune the number of concurrent read streams with the `RUNAI_STREAMER_CONCURRENCY`
...
### Model Sharding[#](https://docs.ray.io#model-sharding)
...
### Azure Blob streaming with RunAI Streamer[#](https://docs.ray.io#azure-blob-streaming-with-runai-streamer)

RunAI Streamer reads Azure Blob Storage natively through the `az://`

scheme, streaming weights into GPU memory without staging them on disk first. This requires versions of `runai-model-streamer`

and vLLM that support `az://`

, so confirm the versions bundled in your image.

Set `model_source`

to an `az://<container>/<model>`
...
environment variable tells the streamer which storage account to read from. On AKS, bind the pod to a workload identity that holds the **Storage Blob Data Reader** role so the streamer authenticates with a Microsoft Entra ID token instead of a key.
...
schemes, which download the model to disk before loading, the `az://`

scheme streams directly from Blob into GPU memory.
...
## Additional Optimizations[#](https://docs.ray.io#additional-optimizations)

### Torch Compile Cache[#](https://docs.ray.io#torch-compile-cache)
...
. Upload this directory to your S3 bucket. The cache folder can now be retrieved at startup. We provide a custom utility to download the compile cache from cloud storage. Specify the `CloudDownloader`

callback in `LLMConfig`

and supply the relevant arguments. Make sure to set the `cache_dir`

in compilation_config correctly.
...
### Custom Initialization Behaviors[#](https://docs.ray.io#custom-initialization-behaviors)
...
[which overrides the](https://github.com/ray-project/ray/blob/master/python/ray/llm/_internal/common/callbacks/cloud_downloader.py)

`CloudDownloader`

`on_before_download_model_files_distributed`
...
### Model source selection[#](https://docs.ray.io#model-source-selection)

**Use Hugging Face**for publicly available models and quick prototyping**Use remote storage**for private models, custom fine-tunes, or when co-located with compute**Enable fast downloads**when downloading large models from Hugging Face

### Security[#](https://docs.ray.io#security)

**Never commit tokens**to version control. Use environment variables or secrets management.**Use IAM roles**instead of access keys for production deployments on AWS.**Scope permissions**to read-only access for model loading.

### Performance[#](https://docs.ray.io#performance)

**Co-locate storage and compute**in the same cloud region to reduce latency and egress costs.**Use fast download**(`HF_HUB_ENABLE_HF_TRANSFER`

) for models larger than 10GB.**Cache models**locally if you’re repeatedly deploying the same model.**See benchmarks**[here](https://docs.ray.io/benchmarks.html)for detailed information about optimizations

## Troubleshooting[#](https://docs.ray.io#troubleshooting)

### Slow downloads from Hugging Face[#](https://docs.ray.io#slow-downloads-from-hugging-face)
...
### Cloud storage access errors[#](https://docs.ray.io#cloud-storage-access-errors)
...
, or the**Storage Blob Data Reader**role)For Azure, confirm the

`adlfs`

and`azure-identity`

Python packages are installed on every nodeVerify the bucket or container exists and is accessible from your deployment region


### Model files not found[#](https://docs.ray.io#model-files-not-found)

Verify the model structure matches Hugging Face format (must include

`config.json`

, tokenizer files, and model weights)Check that all required files are present in the bucket
