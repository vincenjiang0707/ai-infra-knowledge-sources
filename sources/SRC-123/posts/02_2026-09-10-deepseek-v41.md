...
Both modes are opt-in (`--enable-encoder-swa-bounded-replay`

, `--enable-decoder-swa-bounded-replay`

) and can be combined. Encoder replay excludes speculative decoding. Decoder tail-only computation does not support input logprobs or full prompt hidden-state capture.
