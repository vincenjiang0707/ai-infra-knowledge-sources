...
**Fused kernels**cut the fixed cost of every step without changing its math.**Cache-DiT**reuses results between denoising steps, so some steps never run.**SubBlock sparse attention**reduces the cost of the steps that do run, by skipping attention blocks whose contribution is below a threshold.
...
the cache is refreshed. All cache modes use `Fn=1`
...
, maximum consecutive cached steps`1`

; - stride: shared packed-stack RDT
`0.08`

, maximum consecutive cached steps`3`

.

MiniMax-H3 has one `MiniMaxH3DiTModel`

whose block stack carries packed video
...
; the first ten denoising steps use dense attention, and SubBlock
is enabled afterward. The minimum sequence length is `4096`
...
; the latter is faster but has lower SSIM on several
T2VA cases.
...
); “49-step trace” refers to these model
evaluations.
...
SubBlock 0.80 + stride. The worker recorded the actual `cached_steps`
...
buffer. QK RMSNorm and 3D RoPE are fused into a single kernel
...
16-token sub-blocks on both sides (`n_q=n_k=4`

). A lightweight pooling and
...
value is the fraction of key blocks allowed to be dropped, not
the fraction retained. Thus `sparsity=0.75`

keeps roughly 25% of key blocks per
query block. The more aggressive `0.80`

setting is faster but has a larger
...
is included as a diagnostic reference; the benchmark profiles
use `0.75`

and `0.80`

. Because the router ranks key blocks independently for
each query block and head, `sparsity=0.50`

and `0.75`

retain roughly the top
half and top quarter of that row's available key blocks, subject to 8-block
budget rounding. Across these workloads, the `0.75`

budget retains most of the
...
**Prompt 1**· T2VA · 5 s · three cats carrying brass instruments and playing beside a sleeping owner;**Prompt 2**· T2VA · 10 s · a rainy cyberpunk city at night;**Prompt 3**· FL2VA · 5 s · a clay fox continuation.
...
— wrote the first version of this post, drives the SGLang kernel work these results build on, and provides the diffusion runtime, the fused kernels, and the parallelism measured here.[SGLang Diffusion Team](https://github.com/sgl-project/sglang)— ran the H200 benchmark, brought SubBlock sparse attention into SGLang Diffusion, and revised this post.[Ji Huang (@IPostYellow), Ant Group](https://github.com/IPostYellow)— @DefTruth and the vipshop.com team, for Cache-DiT and for support integrating its cache profiles into SGLang Diffusion.[Cache-DiT Team](https://github.com/vipshop/cache-dit)— for open-sourcing MiniMax-H3, the model every measurement here runs on.[MiniMax](https://github.com/MiniMax-AI)**NVIDIA**— for the underlying SubBlock sparse attention support, including the block-sparse attention forward these results depend on.
