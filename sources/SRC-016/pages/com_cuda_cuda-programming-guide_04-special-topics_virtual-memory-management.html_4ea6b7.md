source: https://docs.nvidia.com/cuda/cuda-programming-guide/04-special-topics/virtual-memory-management.html

# 4.17. Virtual Memory Management[#](https://docs.nvidia.com#virtual-memory-management)

In the CUDA programming model, memory allocation calls (such as
`cudaMalloc()`

) return a memory address in GPU memory.
The address can be used with any CUDA API or inside a device
kernel.
Developers can enable peer device access to that memory
allocations by using `cudaEnablePeerAccess`
...
mapping all `cudaMalloc`

allocations to peer devices. In most
...
## 4.17.1. Preliminaries[#](https://docs.nvidia.com#preliminaries)

### 4.17.1.1. Definitions[#](https://docs.nvidia.com#definitions)
...
. On supported platforms and provided the NVIDIA
...
stores. [Compute fabric transport](https://docs.nvidia.com/compute-fabric-transport.html#compute-fabric-transport) provides a
...
### 4.17.1.2. Query for Support[#](https://docs.nvidia.com#query-for-support)
...
Aside from using `CU_MEM_HANDLE_TYPE_FABRIC`

as the handle type and not
...
character device, `nvidia-caps-imex-channels`

. To use fabric handle-based
...
## 4.17.2. API Overview[#](https://docs.nvidia.com#api-overview)

The VMM API provides developers
with granular control over virtual memory management. VMM, being a very low-level API,
requires use of the [CUDA Driver API](https://docs.nvidia.com/03-advanced/driver-api.html#driver-api) directly.
...
## 4.17.3. Unicast Memory Sharing[#](https://docs.nvidia.com#unicast-memory-sharing)
...
### 4.17.3.1. Allocate and Export[#](https://docs.nvidia.com#allocate-and-export)
...
API. The allocation created by this function does not have any
device or host mappings. The function argument
`CUmemGenericAllocationHandle`

describes the properties of the memory to
...
The memory allocated by `cuMemCreate`

is referenced by the
`CUmemGenericAllocationHandle`

it returns. Note that this reference is
...
To allocate device memory in a specific locality domain, set `CUmemAllocationProp::location.type`

to `CU_MEM_LOCATION_TYPE_DEVICE_LOCALITY_DOMAIN`

. Set both members of `CUmemAllocationProp::location.localized`

: `deviceId`

identifies the GPU, and `localityDomainId`

identifies a valid locality domain on that GPU. Query the number of valid locality domains with `cuDeviceGetAttribute`
...
On a device with one locality domain, requesting locality domain 0 is equivalent to a full-device allocation created with `CU_MEM_LOCATION_TYPE_DEVICE`

; the resulting allocation reports its location type as `CU_MEM_LOCATION_TYPE_DEVICE`
...
### 4.17.3.3. Reserve and Map[#](https://docs.nvidia.com#reserve-and-map)
...
memory allocations made by `cuMemCreate`

. The address range reserved must be
...
parameters to `cuMemAddressReserve`

. The address range obtained will not
...
. These functions are conceptually similar to `mmap`

and `munmap`

on Linux or `VirtualAlloc`
...
address range obtained from `cuMemAddressReserve`

and the physical
allocation obtained from `cuMemCreate`

or `cuMemImportFromShareableHandle`
...
unmap the address of the mapping with `cuMemUnmap`
...
### 4.17.3.4. Access Rights[#](https://docs.nvidia.com#access-rights)
...
region of the address range using `cuMemMap`

does not make the address
...
forces all prior and future allocations made with `cudaMalloc`
...
### 4.17.3.5. Releasing the Memory[#](https://docs.nvidia.com#releasing-the-memory)

To release the allocated memory and address space, both the source and target
processes should use `cuMemUnmap`

, `cuMemRelease`

, and `cuMemAddressFree`
...
frees a virtual address range that
...
## 4.17.4. Multicast Memory Sharing[#](https://docs.nvidia.com#multicast-memory-sharing)
...
The [multimem PTX instructions](https://docs.nvidia.com/cuda/parallel-thread-execution/index.html#data-movement-and-conversion-instructions-multimem)
...
as described above.Add all GPUs that should participate in the multicast team with
...
as described above to the multicast handle. All devices need to be added to the multicast team before binding memory on any device.Reserve an address range, map the multicast handle and set access rights as described above for regular unicast mappings. Unicast and multicast mappings to the same physical memory are possible. See the

[Virtual Aliasing Support](https://docs.nvidia.com#virtual-aliasing-support)section on how to ensure consistency between multiple mappings to the same physical memory.Use the

[multimem PTX instructions](https://docs.nvidia.com/cuda/parallel-thread-execution/index.html#data-movement-and-conversion-instructions-multimem)with the multicast mappings.

The `multi_node_p2p`

example in the [Multi GPU Programming Models](https://github.com/NVIDIA/multi-gpu-programming-models/) GitHub
...
address space and operates on it with `multimem`

instructions — the *address-centric*
model. [Compute fabric transport](https://docs.nvidia.com/compute-fabric-transport.html#compute-fabric-transport) provides a
...
### 4.17.4.1. Allocating Multicast Objects[#](https://docs.nvidia.com#allocating-multicast-objects)
...
### 4.17.4.2. Add Devices to Multicast Objects[#](https://docs.nvidia.com#add-devices-to-multicast-objects)
...
### 4.17.4.3. Bind Memory to Multicast Objects[#](https://docs.nvidia.com#bind-memory-to-multicast-objects)
...
physical memory allocated with `cuMemCreate`
...
### 4.17.4.4. Use Multicast Mappings[#](https://docs.nvidia.com#use-multicast-mappings)

To use multicast mappings in CUDA C++, it is necessary to use the [multimem PTX instructions](https://docs.nvidia.com/cuda/parallel-thread-execution/index.html#data-movement-and-conversion-instructions-multimem)
with inline PTX:
...
## 4.17.5. Advanced Configuration[#](https://docs.nvidia.com#advanced-configuration)

### 4.17.5.1. Memory Type[#](https://docs.nvidia.com#memory-type)
...
to opt-in to specific memory features.
Applications must ensure that the requested memory type is supported by
the device.

### 4.17.5.2. Compressible Memory[#](https://docs.nvidia.com#compressible-memory)
...
. Users must query if device supports
...
### 4.17.5.3. Virtual Aliasing Support[#](https://docs.nvidia.com#virtual-aliasing-support)
...
with different virtual addresses. This is called virtual aliasing.
...
same kernel, a `fence.proxy.alias`

can be used between the two accesses. The
...
### 4.17.5.4. OS-Specific Handle Details for IPC[#](https://docs.nvidia.com#os-specific-handle-details-for-ipc)

With `cuMemCreate`

, users can indicate at
...
by setting `CUmemAllocationProp::requestedHandleTypes`
...
attempting to export memory allocated with `cuMemCreate`
...
Users should set the `CUmemAllocationProp::requestedHandleTypes`
