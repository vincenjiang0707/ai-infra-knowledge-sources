source: https://docs.nvidia.com/cuda/cuda-programming-guide/04-special-topics/lazy-loading.html

# 4.8. Lazy Loading[#](https://docs.nvidia.com#lazy-loading)

## 4.8.1. Introduction[#](https://docs.nvidia.com#introduction)

Lazy loading reduces program initialization time by waiting to load CUDA modules until they are needed. Lazy loading is particularly effective for programs that only use a small number of the kernels they include, as is common when using libraries. Lazy loading is designed to be invisible to the user when the CUDA programming model is followed. [Potential Hazards](https://docs.nvidia.com#lazy-loading-potential-hazards) explains this in detail. As of CUDA 12.3 lazy Loading is enabled by default on all platforms, but can be controlled via the `CUDA_MODULE_LOADING`

environment variable.

## 4.8.2. Change History[#](https://docs.nvidia.com#change-history)
...
## 4.8.3. Requirements for Lazy Loading[#](https://docs.nvidia.com#requirements-for-lazy-loading)
...
### 4.8.3.1. CUDA Runtime Version Requirement[#](https://docs.nvidia.com#cuda-runtime-version-requirement)
...
### 4.8.3.2. CUDA Driver Version Requirement[#](https://docs.nvidia.com#cuda-driver-version-requirement)
...
### 4.8.3.3. Compiler Requirements[#](https://docs.nvidia.com#compiler-requirements)
...
### 4.8.3.4. Kernel Requirements[#](https://docs.nvidia.com#kernel-requirements)

Lazy loading does not affect modules containing managed variables, which will still be loaded eagerly.

## 4.8.4. Usage[#](https://docs.nvidia.com#usage)

### 4.8.4.1. Enabling & Disabling[#](https://docs.nvidia.com#enabling-disabling)

Lazy loading is enabled by setting the `CUDA_MODULE_LOADING`

environment variable to `LAZY`

. Lazy loading can be disabled by setting the `CUDA_MODULE_LOADING`

environment variable to `EAGER`

. As of CUDA 12.3, lazy loading is enabled by default on all platforms.

### 4.8.4.2. Checking if Lazy Loading is Enabled at Runtime[#](https://docs.nvidia.com#checking-if-lazy-loading-is-enabled-at-runtime)

The `cuModuleGetLoadingMode`

API in the CUDA driver API can be used to determine if lazy loading is enabled. Note that CUDA must be initialized before running this function. Sample usage is shown in the snippet below.
...
### 4.8.4.3. Forcing a Module to Load Eagerly at Runtime[#](https://docs.nvidia.com#forcing-a-module-to-load-eagerly-at-runtime)
...
function will cause a module to be loaded into device memoryThe

`cudaFuncGetAttributes()`

function will cause a kernel to be loaded into device memory

Note

`cuModuleLoad()`

does not guarantee that a module will be loaded immediately.

## 4.8.5. Potential Hazards[#](https://docs.nvidia.com#potential-hazards)
...
### 4.8.5.1. Impact on Concurrent Kernel Execution[#](https://docs.nvidia.com#impact-on-concurrent-kernel-execution)
...
### 4.8.5.2. Large Memory Allocations[#](https://docs.nvidia.com#large-memory-allocations)
...
instead of an allocator that allocates the entire VRAM on startupadd some buffer to compensate for the delayed loading of kernels

preload all kernels that will be used in the program before trying to initialize the allocator


### 4.8.5.3. Impact on Performance Measurements[#](https://docs.nvidia.com#impact-on-performance-measurements)
