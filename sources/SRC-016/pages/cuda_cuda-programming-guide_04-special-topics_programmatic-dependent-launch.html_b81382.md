source: https://docs.nvidia.com/cuda/cuda-programming-guide/04-special-topics/programmatic-dependent-launch.html

# 4.5. Programmatic Dependent Launch and Synchronization[#](https://docs.nvidia.com#programmatic-dependent-launch-and-synchronization)
...
## 4.5.1. Background[#](https://docs.nvidia.com#background)

A CUDA application utilizes the GPU by launching and executing multiple kernels on it.
A typical GPU activity timeline is shown in [Figure 42](https://docs.nvidia.com#gpu-activity).

Here, `secondary_kernel`

is launched after `primary_kernel`

finishes its execution.
Serialized execution is usually necessary because `secondary_kernel`

depends on result data
produced by `primary_kernel`

. If `secondary_kernel`

has no dependency on `primary_kernel`

,
both of them can be launched concurrently by using [CUDA Streams](https://docs.nvidia.com/02-basics/asynchronous-execution.html#cuda-streams).
Even if `secondary_kernel`

is dependent on `primary_kernel`
...
[Figure 43](https://docs.nvidia.com#secondary-kernel-preamble) demonstrates the portion of `secondary_kernel`

that could
be executed concurrently without impacting the application.
Note that concurrent launch also allows us to hide the launch latency of `secondary_kernel`
...
shown in [Figure 44](https://docs.nvidia.com#preamble-overlap) is
...
## 4.5.2. API Description[#](https://docs.nvidia.com#api-description)

In Programmatic Dependent Launch, a primary and a secondary kernel are launched in the same CUDA stream.
The primary kernel should execute `cudaTriggerProgrammaticLaunchCompletion`
...
When the secondary kernel is launched using the `cudaLaunchAttributeProgrammaticStreamSerialization`
...
## 4.5.3. Use in CUDA Graphs[#](https://docs.nvidia.com#use-in-cuda-graphs)

Programmatic Dependent Launch can be used in [CUDA Graphs](https://docs.nvidia.com/cuda-graphs.html#cuda-graphs) via [stream capture](https://docs.nvidia.com/cuda-graphs.html#cuda-graphs-creating-a-graph-using-stream-capture) or
directly via [edge data](https://docs.nvidia.com/cuda-graphs.html#cuda-graphs-edge-data). To program this feature in a CUDA Graph with edge data, use a `cudaGraphDependencyType`

value of `cudaGraphDependencyTypeProgrammatic`

on an edge connecting two kernel nodes. This edge type makes the upstream kernel
visible to a `cudaGridDependencySynchronize()`

in the downstream kernel. This type must be used with an outgoing port of
either `cudaGraphKernelNodePortLaunchCompletion`

or `cudaGraphKernelNodePortProgrammatic`
