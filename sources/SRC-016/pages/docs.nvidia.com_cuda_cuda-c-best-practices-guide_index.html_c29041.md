source: http://docs.nvidia.com/cuda/cuda-c-best-practices-guide/index.html

CUDA C++ Best Practices Guide

#
1. Overview[](http://docs.nvidia.com#overview)
...
#
2. Preface[](http://docs.nvidia.com#preface)
...
##
2.1. Who Should Read This Guide?[](http://docs.nvidia.com#who-should-read-this-guide)
...
##
2.2. Assess, Parallelize, Optimize, Deploy[](http://docs.nvidia.com#assess-parallelize-optimize-deploy)
...
###
2.2.1. Assess[](http://docs.nvidia.com#assess)
...
###
2.2.2. Parallelize[](http://docs.nvidia.com#parallelize)
...
###
2.2.3. Optimize[](http://docs.nvidia.com#optimize)
...
###
2.2.4. Deploy[](http://docs.nvidia.com#deploy)
...
##
2.3. Recommendations and Best Practices[](http://docs.nvidia.com#recommendations-and-best-practices)
...
##
2.4. Assessing Your Application[](http://docs.nvidia.com#assessing-your-application)
...
#
3. Heterogeneous Computing[](http://docs.nvidia.com#heterogeneous-computing)
...
##
3.1. Differences between Host and Device[](http://docs.nvidia.com#differences-between-host-and-device)
...
[1](http://docs.nvidia.com#fn1). As the host and device memories are separated, items in the host memory must occasionally be communicated between device memory and host memory as described in[What Runs on a CUDA-Enabled Device?](http://docs.nvidia.com#what-runs-on-cuda-enabled-device).
...
##
3.2. What Runs on a CUDA-Enabled Device?[](http://docs.nvidia.com#what-runs-on-a-cuda-enabled-device)
...
[Data Transfer Between Host and Device](http://docs.nvidia.com#data-transfer-between-host-and-device).) This cost has several ramifications:-
...
[Data Transfer Between Host and Device](http://docs.nvidia.com#data-transfer-between-host-and-device)provides further details, including the measurements of bandwidth between the host and the device versus within the device proper.
...
#
4. Application Profiling[](http://docs.nvidia.com#application-profiling)

##
4.1. Profile[](http://docs.nvidia.com#profile)
...
###
4.1.1. Creating the Profile[](http://docs.nvidia.com#creating-the-profile)
...
###
4.1.2. Identifying Hotspots[](http://docs.nvidia.com#identifying-hotspots)

In the example above, we can clearly see that the function `genTimeStep()`

takes one-third of the total running time of the application. This should be our first candidate function for parallelization. [Understanding Scaling](http://docs.nvidia.com#understanding-scaling) discusses the potential benefit we might expect from such parallelization.
...
###
4.1.3. Understanding Scaling[](http://docs.nvidia.com#understanding-scaling)
...
####
4.1.3.2. Weak Scaling and Gustafson’s Law[](http://docs.nvidia.com#weak-scaling-and-gustafson-s-law)
...
####
4.1.3.3. Applying Strong and Weak Scaling[](http://docs.nvidia.com#applying-strong-and-weak-scaling)
...
#
5. Parallelizing Your Application[](http://docs.nvidia.com#parallelizing-your-application)
...
#
6. Getting Started[](http://docs.nvidia.com#getting-started)
...
##
6.1. Parallel Libraries[](http://docs.nvidia.com#parallel-libraries)
...
##
6.2. Parallelizing Compilers[](http://docs.nvidia.com#parallelizing-compilers)
...
##
6.3. Coding to Expose Parallelism[](http://docs.nvidia.com#coding-to-expose-parallelism)
...
#
7. Getting the Right Answer[](http://docs.nvidia.com#getting-the-right-answer)
...
##
7.1. Verification[](http://docs.nvidia.com#verification)

###
7.1.1. Reference Comparison[](http://docs.nvidia.com#reference-comparison)

A key aspect of correctness verification for modifications to any existing program is to establish some mechanism whereby previous known-good reference outputs from representative inputs can be compared to new results. After each change is made, ensure that the results match using whatever criteria apply to the particular algorithm. Some will expect bitwise identical results, which is not always possible, especially where floating-point arithmetic is concerned; see [Numerical Accuracy and Precision](http://docs.nvidia.com#numerical-accuracy-and-precision) regarding numerical accuracy. For other algorithms, implementations may be considered correct if they match the reference within some small epsilon.
...
###
7.1.2. Unit Testing[](http://docs.nvidia.com#unit-testing)
...
##
7.2. Debugging[](http://docs.nvidia.com#debugging)
...
##
7.3. Numerical Accuracy and Precision[](http://docs.nvidia.com#numerical-accuracy-and-precision)
...
###
7.3.1. Single vs. Double Precision[](http://docs.nvidia.com#single-vs-double-precision)

Devices of [CUDA Compute Capability](http://docs.nvidia.com#cuda-compute-capability) 1.3 and higher provide native support for double-precision floating-point values (that is, values 64 bits wide). Results obtained using double-precision arithmetic will frequently differ from the same operation performed via single-precision arithmetic due to the greater precision of the former and due to rounding issues. Therefore, it is important to be sure to compare values of like precision and to express the results within a certain tolerance rather than expecting them to be exact.

###
7.3.2. Floating Point Math Is Not Associative[](http://docs.nvidia.com#floating-point-math-is-not-associative)
...
###
7.3.3. IEEE 754 Compliance[](http://docs.nvidia.com#ieee-754-compliance)
...
###
7.3.4. x86 80-bit Computations[](http://docs.nvidia.com#x86-80-bit-computations)
...
#
8. Optimizing CUDA Applications[](http://docs.nvidia.com#optimizing-cuda-applications)
...
#
9. Performance Metrics[](http://docs.nvidia.com#performance-metrics)
...
##
9.1. Timing[](http://docs.nvidia.com#timing)
...
###
9.1.1. Using CPU Timers[](http://docs.nvidia.com#using-cpu-timers)
...
###
9.1.2. Using CUDA GPU Timers[](http://docs.nvidia.com#using-cuda-gpu-timers)

The CUDA event API provides calls that create and destroy events, record events (including a timestamp), and convert timestamp differences into a floating-point value in milliseconds. [How to time code using CUDA events](http://docs.nvidia.com#how-to-time-code-using-cuda-events-figure) illustrates their use.
...
###
9.2.1. Theoretical Bandwidth Calculation[](http://docs.nvidia.com#theoretical-bandwidth-calculation)
...
###
9.2.3. Throughput Reported by Visual Profiler[](http://docs.nvidia.com#throughput-reported-by-visual-profiler)

For devices with [compute capability](http://docs.nvidia.com#cuda-compute-capability) of 2.0 or greater, the Visual Profiler can be used to collect several different memory throughput measures. The following throughput metrics can be displayed in the Details or Detail Graphs view:
...
The Requested Global Load Throughput and Requested Global Store Throughput values indicate the global memory throughput requested by the kernel and therefore correspond to the effective bandwidth obtained by the calculation shown under [Effective Bandwidth Calculation](http://docs.nvidia.com#effective-bandwidth-calculation).
...
It’s important to note that both numbers are useful. The actual memory throughput shows how close the code is to the hardware limit, and a comparison of the effective or requested bandwidth to the actual bandwidth presents a good estimate of how much bandwidth is wasted by suboptimal coalescing of memory accesses (see [Coalesced Access to Global Memory](http://docs.nvidia.com#coalesced-access-to-global-memory)). For global memory accesses, this comparison of requested memory bandwidth to actual memory bandwidth is reported by the Global Memory Load Efficiency and Global Memory Store Efficiency metrics.

[2](http://docs.nvidia.com#id24)-
...
#
10. Memory Optimizations[](http://docs.nvidia.com#memory-optimizations)
...
##
10.1. Data Transfer Between Host and Device[](http://docs.nvidia.com#data-transfer-between-host-and-device)
...
Finally, higher bandwidth between the host and the device is achieved when using *page-locked* (or *pinned*) memory, as discussed in the CUDA Programming Guide and the [Pinned Memory](http://docs.nvidia.com#pinned-memory) section of this document.

###
10.1.1. Pinned Memory[](http://docs.nvidia.com#pinned-memory)
...
###
10.1.2. Asynchronous and Overlapping Transfers with Computation[](http://docs.nvidia.com#asynchronous-and-overlapping-transfers-with-computation)
...
[Asynchronous and Overlapping Transfers with Computation](http://docs.nvidia.com#asynchronous-transfers-and-overlapping-transfers-with-computation) illustrates the basic technique.
...
[Staged concurrent copy and execute](http://docs.nvidia.com#staged-concurrent-copy-and-execute) shows how the transfer and kernel execution can be broken up into nStreams stages. This approach permits some overlapping of the data transfer and execution.
...
(In [Staged concurrent copy and execute](http://docs.nvidia.com#staged-concurrent-copy-and-execute), it is assumed that *N* is evenly divisible by `nThreads*nStreams`
...
in [Figure 1](http://docs.nvidia.com#timeline-comparison-for-copy-and-kernel-execution-figure), and `nStreams`

is equal to 4
for [Staged concurrent copy and execute](http://docs.nvidia.com#staged-concurrent-copy-and-execute) in the bottom half of the figure.
...
###
10.1.3. Zero Copy[](http://docs.nvidia.com#zero-copy)
...
The host code in [Zero-copy host code](http://docs.nvidia.com#zero-copy-host-code) shows how zero copy is typically set up.
...
. In the code
in [Zero-copy host code](http://docs.nvidia.com#zero-copy-host-code), `kernel()`
...
###
10.1.4. Unified Virtual Addressing[](http://docs.nvidia.com#unified-virtual-addressing)

Devices of [compute capability](http://docs.nvidia.com#cuda-compute-capability) 2.0 and later support a special addressing mode called *Unified Virtual Addressing* (UVA) on 64-bit Linux and Windows. With UVA, the host memory and the device memories of all installed supported devices share a single virtual address space.
...
##
10.2. Device Memory Spaces[](http://docs.nvidia.com#device-memory-spaces)
...
memory spaces include global, local, shared, texture, and registers, as shown in [Figure 2](http://docs.nvidia.com#memory-spaces-cuda-device-figure).

Of these different memory spaces, global memory is the most plentiful; see Features and Technical Specifications of the CUDA Programming Guide for the amounts of memory available in each memory space at each [compute capability](http://docs.nvidia.com#cuda-compute-capability) level. Global, local, and texture memory have the greatest access latency, followed by constant memory, shared memory, and the register file.

The various principal traits of the memory types are shown in [Table 1](http://docs.nvidia.com#salient-features-device-memory-table).
...
###
10.2.1. Coalesced Access to Global Memory[](http://docs.nvidia.com#coalesced-access-to-global-memory)
...
####
10.2.1.1. A Simple Access Pattern[](http://docs.nvidia.com#a-simple-access-pattern)
...
have been performed by a device with [compute capability](http://docs.nvidia.com#cuda-compute-capability) 6.0 or higher.

####
10.2.1.2. A Sequential but Misaligned Access Pattern[](http://docs.nvidia.com#a-sequential-but-misaligned-access-pattern)
...
will be requested, as shown in [Figure 4](http://docs.nvidia.com#misaligned-sequential-addresses-fall-5-32-byte-l2-cache-seqments).
...
####
10.2.1.3. Effects of Misaligned Accesses[](http://docs.nvidia.com#effects-of-misaligned-accesses)
...
in [A copy kernel that illustrates misaligned accesses](http://docs.nvidia.com#a-copy-kernel-that-illustrates-misaligned-accesses).
...
In [A copy kernel that illustrates misaligned accesses](http://docs.nvidia.com#a-copy-kernel-that-illustrates-misaligned-accesses), data is copied from the input array `idata`
...
####
10.2.1.4. Strided Accesses[](http://docs.nvidia.com#strided-accesses)
...
in [A kernel to illustrate non-unit stride data copy](http://docs.nvidia.com#a-kernel-to-illustrate-non-unit-stride-data-copy), which copies data with a stride of stride elements between threads from `idata`
...
[Figure 6](http://docs.nvidia.com#adjacent-threads-accessing-memory-with-stride-of-2-figure) illustrates such a situation; in this case, threads within a warp access words in memory with a stride of 2. This action leads to a load of eight L2 cache segments per warp on the Tesla V100 (compute capability 7.0).
...
for the 32 threads in a warp, as indicated in [Figure 7](http://docs.nvidia.com#performance-of-stridecopy-kernel-figure).

As illustrated in [Figure 7](http://docs.nvidia.com#performance-of-stridecopy-kernel-figure), non-unit-stride global memory accesses should be avoided whenever possible. One method for doing so utilizes shared memory, which is discussed in the next section.

###
10.2.2. L2 Cache[](http://docs.nvidia.com#l2-cache)
...
####
10.2.2.1. L2 Cache Access Window[](http://docs.nvidia.com#l2-cache-access-window)
...
####
10.2.2.2. Tuning the Access Window Hit-Ratio[](http://docs.nvidia.com#tuning-the-access-window-hit-ratio)
...
###
10.2.4. Local Memory[](http://docs.nvidia.com#local-memory)
...
###
10.2.5. Texture Memory[](http://docs.nvidia.com#texture-memory)
...
####
10.2.5.1. Additional Texture Capabilities[](http://docs.nvidia.com#additional-texture-capabilities)
...
, the hardware provides other capabilities that might be useful for some applications such as image processing, as shown in [Table 4](http://docs.nvidia.com#useful-features-for-tex1d-tex2d-tex3d-fetches-table).
...
###
10.2.6. Constant Memory[](http://docs.nvidia.com#constant-memory)
...
###
10.2.7. Registers[](http://docs.nvidia.com#registers)
...
####
10.2.7.1. Register Pressure[](http://docs.nvidia.com#register-pressure)
...
##
10.3. Allocation[](http://docs.nvidia.com#allocation)
...
##
10.4. NUMA Best Practices[](http://docs.nvidia.com#numa-best-practices)
...
#
11. Execution Configuration Optimizations[](http://docs.nvidia.com#execution-configuration-optimizations)
...
##
11.1. Occupancy[](http://docs.nvidia.com#occupancy)
...
###
11.1.1. Calculating Occupancy[](http://docs.nvidia.com#calculating-occupancy)
...
##
11.2. Hiding Register Dependencies[](http://docs.nvidia.com#hiding-register-dependencies)
...
##
11.6. Multiple contexts[](http://docs.nvidia.com#multiple-contexts)
...
#
12. Instruction Optimization[](http://docs.nvidia.com#instruction-optimization)
...
##
12.1. Arithmetic Instructions[](http://docs.nvidia.com#arithmetic-instructions)
...
[Control Flow Instructions](http://docs.nvidia.com#control-flow-instructions)Reduce the number of instructions, for example, by optimizing out synchronization points whenever possible as described in

[Synchronization Instruction](http://docs.nvidia.com#synchronization-instruction)or by using restricted pointers as described in the[CUDA Programming Guide](https://docs.nvidia.com/cuda/cuda-programming-guide/05-appendices/cpp-language-extensions.html#restrict-pointers).
...
###
12.1.1. Throughput of Native Arithmetic Instructions[](http://docs.nvidia.com#throughput-of-native-arithmetic-instructions)
...
[3](http://docs.nvidia.com#id58)-
multiple instructions for __nv_bfloat16

[4](http://docs.nvidia.com#id59)-
64 for __nv_bfloat16

[5](http://docs.nvidia.com#id60)-
for reciprocal: approximate using

`__fdividef(1.F, x)`

[6](http://docs.nvidia.com#id61)-
32 for extended-precision

[7](http://docs.nvidia.com#id62)-
assuming converged threads

[8](http://docs.nvidia.com#id63)-
assuming converged threads

-
9(
[1](http://docs.nvidia.com#id64),[2](http://docs.nvidia.com#id65)) -
22 for signed version

[10](http://docs.nvidia.com#id66)-
...
###
12.1.2. Control Flow Instructions[](http://docs.nvidia.com#control-flow-instructions)
...
###
12.1.3. Synchronization Instruction[](http://docs.nvidia.com#synchronization-instruction)
...
###
12.1.4. Division Modulo Operations[](http://docs.nvidia.com#division-modulo-operations)
...
###
12.1.5. Loop Counters Signed vs. Unsigned[](http://docs.nvidia.com#loop-counters-signed-vs-unsigned)
...
###
12.1.6. Reciprocal Square Root[](http://docs.nvidia.com#reciprocal-square-root)
...
###
12.1.7. Other Arithmetic Instructions[](http://docs.nvidia.com#other-arithmetic-instructions)
...
###
12.1.8. Exponentiation With Small Fractional Arguments[](http://docs.nvidia.com#exponentiation-with-small-fractional-arguments)
...
###
12.1.9. Math Libraries[](http://docs.nvidia.com#math-libraries)
...
##
12.2. Memory Instructions[](http://docs.nvidia.com#memory-instructions)
...
#
13. Control Flow[](http://docs.nvidia.com#control-flow)

##
13.1. Branching and Divergence[](http://docs.nvidia.com#branching-and-divergence)
...
##
13.2. Branch Predication[](http://docs.nvidia.com#branch-predication)
...
#
14. Deploying CUDA Applications[](http://docs.nvidia.com#deploying-cuda-applications)
...
#
15. Understanding the Programming Environment[](http://docs.nvidia.com#understanding-the-programming-environment)
...
Programmers should be aware of two version numbers. The first is the [compute capability](http://docs.nvidia.com#cuda-compute-capability), and the second is the version number of the CUDA Runtime and CUDA Driver APIs.

##
15.1. CUDA Compute Capability[](http://docs.nvidia.com#cuda-compute-capability)
...
CUDA Sample. The output for that program is shown in [Figure 16](http://docs.nvidia.com#sample-cuda-configuration-data-reported-devicequery-figure). This information is obtained by calling `cudaGetDeviceProperties()`

and accessing the information in the structure it returns.

The major and minor revision numbers of the compute capability are shown on the seventh line of [Figure 16](http://docs.nvidia.com#sample-cuda-configuration-data-reported-devicequery-figure). Device 0 of this system has compute capability 7.0.
...
##
15.2. Additional Hardware Data[](http://docs.nvidia.com#additional-hardware-data)
...
##
15.3. Which Compute Capability Target[](http://docs.nvidia.com#which-compute-capability-target)
...
See [Building for Maximum Compatibility](http://docs.nvidia.com#building-for-maximum-compatibility) for further discussion of the flags used for building code for multiple generations of CUDA-capable device simultaneously.

##
15.4. CUDA Runtime[](http://docs.nvidia.com#cuda-runtime)
...
#
16. CUDA Compatibility Developer’s Guide[](http://docs.nvidia.com#cuda-compatibility-developer-s-guide)
...
##
16.1. CUDA Toolkit Versioning[](http://docs.nvidia.com#cuda-toolkit-versioning)
...
##
16.2. Source Compatibility[](http://docs.nvidia.com#source-compatibility)
...
##
16.3. Binary Compatibility[](http://docs.nvidia.com#binary-compatibility)
...
###
16.3.1. CUDA Binary (cubin) Compatibility[](http://docs.nvidia.com#cuda-binary-cubin-compatibility)
...
##
16.4. CUDA Compatibility Across Minor Releases[](http://docs.nvidia.com#cuda-compatibility-across-minor-releases)
...
###
16.4.1. Existing CUDA Applications within Minor Versions of CUDA[](http://docs.nvidia.com#existing-cuda-applications-within-minor-versions-of-cuda)
...
####
16.4.1.1. Handling New CUDA Features and Driver APIs[](http://docs.nvidia.com#handling-new-cuda-features-and-driver-apis)
...
####
16.4.1.2. Using PTX[](http://docs.nvidia.com#using-ptx)
...
####
16.4.1.3. Dynamic Code Generation[](http://docs.nvidia.com#dynamic-code-generation)
...
####
16.4.1.4. Recommendations for building a minor-version compatible library[](http://docs.nvidia.com#recommendations-for-building-a-minor-version-compatible-library)
...
####
16.4.1.5. Recommendations for taking advantage of minor version compatibility in your application[](http://docs.nvidia.com#recommendations-for-taking-advantage-of-minor-version-compatibility-in-your-application)
...
#
17. Preparing for Deployment[](http://docs.nvidia.com#preparing-for-deployment)

##
17.1. Testing for CUDA Availability[](http://docs.nvidia.com#testing-for-cuda-availability)
...
function reports various features of the available devices, including the [CUDA Compute Capability](http://docs.nvidia.com#cuda-compute-capability) of the device (see also the Compute Capabilities section of the CUDA Programming Guide). See [Version Management](https://docs.nvidia.com/cuda/cuda-runtime-api/group__CUDART____VERSION.html#group__CUDART____VERSION) for details on how to query the available CUDA software API versions.

##
17.2. Error Handling[](http://docs.nvidia.com#error-handling)
...
##
17.3. Building for Maximum Compatibility[](http://docs.nvidia.com#building-for-maximum-compatibility)

Each generation of CUDA-capable device has an associated *compute capability* version that indicates the feature set supported by the device (see [CUDA Compute Capability](http://docs.nvidia.com#cuda-compute-capability)). One or more compute capability versions can be specified to the nvcc compiler while building a file; compiling for the native compute capability for the target GPU(s) of the application is important to ensure that application kernels achieve the best possible performance and are able to use the features that are available on a given generation of GPU.
...
is available, then the kernel will be compiled *Just In Time* (JIT) (see [Compiler JIT Cache Management Tools](http://docs.nvidia.com#compiler-jit-cache-management))
...
After the application is dynamically linked against the CUDA Runtime, this version of the runtime library should be [bundled with](http://docs.nvidia.com#redistribution) the application. It can be copied into the same directory as the application executable or into a subdirectory of that installation path.
...
the [dynamically-linked version of the CUDA Runtime library](http://docs.nvidia.com#dynamically-linked-cuda-runtime), these libraries should
be [bundled with](http://docs.nvidia.com#redistribution) the application executable when distributing that application.

###
17.4.1. CUDA Toolkit Library Redistribution[](http://docs.nvidia.com#cuda-toolkit-library-redistribution)

The CUDA Toolkit’s End-User License Agreement (EULA) allows for redistribution of many of the CUDA libraries under certain terms and conditions. This allows applications that depend on these libraries [to redistribute the exact versions](http://docs.nvidia.com#redistribution-which-files) of the libraries against which they were built and tested, thereby avoiding any trouble for end users who might have a different version of the CUDA Toolkit (or perhaps none at all) installed on their machines. Please refer to the EULA for details.
...
####
17.4.1.1. Which Files to Redistribute[](http://docs.nvidia.com#which-files-to-redistribute)
...
of the application; see [Where to Install Redistributed CUDA Libraries](http://docs.nvidia.com#redistribution-where-to-install).
...
####
17.4.1.2. Where to Install Redistributed CUDA Libraries[](http://docs.nvidia.com#where-to-install-redistributed-cuda-libraries)
...
#
18. Deployment Infrastructure Tools[](http://docs.nvidia.com#deployment-infrastructure-tools)

##
18.1. Nvidia-SMI[](http://docs.nvidia.com#nvidia-smi)
...
###
18.1.1. Queryable state[](http://docs.nvidia.com#queryable-state)
...
###
18.1.2. Modifiable state[](http://docs.nvidia.com#modifiable-state)
...
##
18.2. NVML[](http://docs.nvidia.com#nvml)
...
##
18.3. Cluster Management Tools[](http://docs.nvidia.com#cluster-management-tools)
...
##
18.4. Compiler JIT Cache Management Tools[](http://docs.nvidia.com#compiler-jit-cache-management-tools)
...
##
18.5. CUDA_VISIBLE_DEVICES[](http://docs.nvidia.com#cuda-visible-devices)
...
#
19. Recommendations and Best Practices[](http://docs.nvidia.com#id98)

This chapter contains a summary of the recommendations for optimization that are explained in this document.

##
19.1. Overall Performance Optimization Strategies[](http://docs.nvidia.com#overall-performance-optimization-strategies)
...
#
20. nvcc Compiler Switches[](http://docs.nvidia.com#nvcc-compiler-switches)

##
20.1. nvcc[](http://docs.nvidia.com#nvcc)
...
specifies the maximum number of registers kernels can use at a per-file level. See[Register Pressure](http://docs.nvidia.com#register-pressure). (See also the`__launch_bounds__`
...
call. This makes the code run faster at the cost of diminished precision and accuracy. See[Math Libraries](http://docs.nvidia.com#math-libraries).

#
21. Notices[](http://docs.nvidia.com#notices)

##
21.1. Notice[](http://docs.nvidia.com#notice)
...
##
21.2. OpenCL[](http://docs.nvidia.com#opencl)

OpenCL is a trademark of Apple Inc. used under license to the Khronos Group Inc.

##
21.3. Trademarks[](http://docs.nvidia.com#trademarks)
