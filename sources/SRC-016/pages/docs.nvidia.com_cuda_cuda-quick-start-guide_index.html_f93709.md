source: https://docs.nvidia.com/cuda/cuda-quick-start-guide/index.html
...
#
3. Windows[](https://docs.nvidia.com#windows)
...
##
3.1. Network Installer[](https://docs.nvidia.com#network-installer)
...
**next**to download and install all components.Once the download completes, the installation will begin automatically.

Once the installation completes, click “next” to acknowledge the Nsight Visual Studio Edition installation summary.

Click

**close**to close the installer.Navigate to the Samples’

`nbody`

directory in[https://github.com/NVIDIA/cuda-samples/tree/master/Samples/5_Domain_Specific/nbody](https://github.com/NVIDIA/cuda-samples/tree/master/Samples/5_Domain_Specific/nbody).-
Open the
...
**Build**menu within Visual Studio and click**Build Solution**. -
...
##
3.2. Local Installer[](https://docs.nvidia.com#local-installer)
...
**next**to install all components.Once the installation completes, click

**next**to acknowledge the Nsight Visual Studio Edition installation summary.Click
...
##
3.3. Pip Wheels - Windows[](https://docs.nvidia.com#pip-wheels-windows)
...
##
3.4. Conda[](https://docs.nvidia.com#conda)
...
###
4.1.1. Redhat / CentOS[](https://docs.nvidia.com#redhat-centos)
...
####
4.1.1.1. RPM Installer[](https://docs.nvidia.com#rpm-installer)

Perform the following steps to install CUDA and verify the installation.

Install EPEL to satisfy the DKMS dependency by following the instructions at

[EPEL’s website](https://fedoraproject.org/wiki/EPEL).-
**Enable optional repos**:On

**RHEL 8 Linux**only, execute the following steps to enable optional repositories.-
**On x86_64 workstation:**subscription-manager repos --enable=rhel-8-for-x86_64-appstream-rpms subscription-manager repos --enable=rhel-8-for-x86_64-baseos-rpms subscription-manager repos --enable=codeready-builder-for-rhel-8-x86_64-rpms
...
Starting with CUDA 13.4, the NVIDIA Linux driver is no longer bundled with the CUDA Toolkit and must be installed separately. Install the driver by following the instructions in the

[NVIDIA Driver Installation Guide](https://docs.nvidia.com/datacenter/tesla/driver-installation-guide/index.html). -
Install the repository meta-data, clean the yum cache, and install the CUDA Toolkit:

sudo rpm --install cuda-repo-<distro>-<version>.<architecture>.rpm sudo rpm --erase gpg-pubkey-7fa2af80* sudo yum clean expire-cache sudo yum install cuda-toolkit
...
variables:export PATH=/usr/local/cuda-13.4/bin${PATH:+:${PATH}} export LD_LIBRARY_PATH=/usr/local/cuda-13.4/lib64\ ${LD_LIBRARY_PATH:+:${LD_LIBRARY_PATH}}

-
Install a writable copy of the samples from

[https://github.com/nvidia/cuda-samples](https://github.com/nvidia/cuda-samples), then build and run the nbody sample using the Linux instructions in[https://github.com/NVIDIA/cuda-samples/tree/master/Samples/5_Domain_Specific/nbody](https://github.com/NVIDIA/cuda-samples/tree/master/Samples/5_Domain_Specific/nbody).Note

Run samples by navigating to the executable’s location, otherwise it will fail to locate dependent resources.


####
4.1.1.2. Runfile Installer[](https://docs.nvidia.com#runfile-installer)
...
export PATH=/usr/local/cuda-13.4/bin${PATH:+:${PATH}} export LD_LIBRARY_PATH=/usr/local/cuda-13.4/lib64\ ${LD_LIBRARY_PATH:+:${LD_LIBRARY_PATH}}
...
###
4.1.2. Fedora[](https://docs.nvidia.com#fedora)
...
####
4.1.2.1. RPM Installer[](https://docs.nvidia.com#fedora-x86-64-rpm)
...
[NVIDIA Driver Installation Guide](https://docs.nvidia.com/datacenter/tesla/driver-installation-guide/index.html). -
Install the repository meta-data, clean the dnf cache, and install the CUDA Toolkit:

sudo rpm --install cuda-repo-<distro>-<version>.<architecture>.rpm sudo rpm --erase gpg-pubkey-7fa2af80* sudo dnf clean expire-cache sudo dnf install cuda-toolkit
...
####
4.1.2.2. Runfile Installer[](https://docs.nvidia.com#fedora-x86-64-run)
...
###
4.1.3. SUSE Linux Enterprise Server[](https://docs.nvidia.com#suse-linux-enterprise-server)
...
####
4.1.3.1. RPM Installer[](https://docs.nvidia.com#sles-x86-64-rpm)
...
[NVIDIA Driver Installation Guide](https://docs.nvidia.com/datacenter/tesla/driver-installation-guide/index.html). -
Install the repository meta-data, refresh the Zypper cache, update the GPG key, and install the CUDA Toolkit:
...
[https://github.com/nvidia/cuda-samples](https://github.com/nvidia/cuda-samples), then build and run the vectorAdd sample using the Linux instructions in[https://github.com/NVIDIA/cuda-samples/tree/master/Samples/0_Introduction/vectorAdd](https://github.com/NVIDIA/cuda-samples/tree/master/Samples/0_Introduction/vectorAdd).Note

Run samples by navigating to the executable’s location, otherwise it will fail to locate dependent resources.


####
4.1.3.2. Runfile Installer[](https://docs.nvidia.com#sles-x86-64-run)
...
###
4.1.4. OpenSUSE[](https://docs.nvidia.com#opensuse)
...
####
4.1.4.1. RPM Installer[](https://docs.nvidia.com#suse-x86-64-rpm)
...
[NVIDIA Driver Installation Guide](https://docs.nvidia.com/datacenter/tesla/driver-installation-guide/index.html). -
Install the repository meta-data, refresh the Zypper cache, and install the CUDA Toolkit:

sudo rpm --install cuda-repo-<distro>-<version>.<architecture>.rpm sudo rpm --erase gpg-pubkey-7fa2af80* sudo zypper refresh sudo zypper install cuda-toolkit
...
####
4.1.4.2. Runfile Installer[](https://docs.nvidia.com#suse-x86-64-run)
...
###
4.1.5. Amazon Linux 2023[](https://docs.nvidia.com#amazon-linux-2023)

####
4.1.5.1. Prepare Amazon Linux 2023[](https://docs.nvidia.com#prepare-amazon-linux-2023)

Perform the

[pre-installation actions.](https://docs.nvidia.com/index.html#pre-installation-actions)-
...
[local repo](https://docs.nvidia.com/index.html#local-repo-installation-for-amazon)or[network repo](https://docs.nvidia.com/index.html#network-repo-installation-for-amazon).

####
4.1.5.2. Local Repo Installation for Amazon Linux[](https://docs.nvidia.com#local-repo-installation-for-amazon-linux)

-
**Install local repository on file system:**sudo rpm --install cuda-repo-amzn2023-X-Y-local-<version>*.x86_64.rpm


####
4.1.5.3. Network Repo Installation for Amazon Linux[](https://docs.nvidia.com#network-repo-installation-for-amazon-linux)

-
**Enable the network repository and clean the DNF cache:**sudo dnf config-manager --add-repo https://developer.download.nvidia.com/compute/cuda/repos/amzn2023/x86_64/cuda-amzn2023.repo sudo dnf clean expire-cache


####
4.1.5.4. Common Installation Instructions for Amazon Linux[](https://docs.nvidia.com#common-installation-instructions-for-amazon-linux)

These instructions apply to both local and network installation for Amazon Linux.

-
**Install CUDA SDK:**sudo dnf module install nvidia-driver:latest-dkms sudo dnf install cuda-toolkit

-
**Install GPUDirect Filesystem:**sudo dnf install nvidia-gds

-
**Add libcuda.so symbolic link, if necessary:**The

`libcuda.so`

library is installed in the`/usr/lib{,64}/nvidia`

directory. For pre-existing projects which use`libcuda.so`

, it may be useful to add a symbolic link from`libcuda.so`
...
[post-installation actions.](https://docs.nvidia.com/index.html#post-installation-actions)

###
4.1.6. Pip Wheels - Linux[](https://docs.nvidia.com#pip-wheels-linux)
...
###
4.1.7. Conda[](https://docs.nvidia.com#x86-64-conda)
...
###
4.1.8. WSL[](https://docs.nvidia.com#wsl)
...
-
**Install repository meta-data**sudo dpkg -i cuda-repo-<distro>_<version>_<architecture>.deb

-
**Update the CUDA public GPG key**sudo apt-key del 7fa2af80

When installing using the local repo:

sudo cp /var/cuda-repo-wsl-ubuntu-13-4-local/cuda-*-keyring.gpg /usr/share/keyrings/
...
-
**Update the Apt repository cache and install the CUDA Toolkit**sudo apt-get update sudo apt-get install cuda-toolkit

Note

Do not install a Linux NVIDIA driver inside the WSL environment. In WSL, the GPU driver is provided by the Windows host, and installing a Linux driver inside the WSL distribution will overwrite the driver provided by Windows and break CUDA.


###
4.1.9. Ubuntu[](https://docs.nvidia.com#ubuntu)
...
####
4.1.9.1. Debian Installer[](https://docs.nvidia.com#debian-installer)
...
[NVIDIA Driver Installation Guide](https://docs.nvidia.com/datacenter/tesla/driver-installation-guide/index.html). -
Install the repository meta-data, update the GPG key, update the apt-get cache, and install the CUDA Toolkit:
...
####
4.1.9.2. Runfile Installer[](https://docs.nvidia.com#ubuntu-x86-64-run)
...
###
4.1.10. Debian[](https://docs.nvidia.com#debian)
...
####
4.1.10.1. Debian Installer[](https://docs.nvidia.com#debian-x86-64-deb)
...
[NVIDIA Driver Installation Guide](https://docs.nvidia.com/datacenter/tesla/driver-installation-guide/index.html). -
Install the repository meta-data, remove old GPG key, install GPG key, update the apt-get cache, and install the CUDA Toolkit:
...
####
4.1.10.2. Runfile Installer[](https://docs.nvidia.com#debian-x86-64-run)
