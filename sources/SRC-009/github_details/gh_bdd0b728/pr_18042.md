...
state: open | updated: 2026-09-24T03:03:14Z
...
- Added and registered `NemotronHConfig` to preserve Nemotron-H hybrid patterns containing `-`. The config validates pattern characters, derives block types and layer count, and delegates patterns that the native Transformers config can represent.
- The config also reconstructs patterns from `layers_block_type`, handles MTP pattern precedence, and preserves `use_cache`. Verify these compatibility paths against existing checkpoints.
- Imported TRT-LLM config registrations before tokenizer loading. Updated `NemotronHForCausalLM.__init__` to accept factory keyword arguments.
- Removed the Nemotron Nano NVFP4 integration waiver.

## QA Engineer Review

- Added tests for dense pattern preservation, native config delegation, invalid and missing patterns, block-type handling, MTP behavior, `use_cache`, tokenizer registration, and model-config loading.
- Added a CPU-only test for `_from_config(..., use_cache=False)`.
- Added `tests/unittest/_torch/test_custom_config_registration.py` to `tests/integration/test_lists/test-db/l0_cpu.yml`.
- Removed the Nemotron Nano NVFP4 waiver. The supplied comments report a later successful CI run with automatic retries; they do not establish whether the unwaived integration case passed without retries.
- Coverage verdict: **needs follow-up**.

## Per-File QA Perspective

- `tensorrt_llm/_torch/auto_deploy/models/custom/modeling_nemotron_h.py`: Verify that accepting factory keyword arguments does not change model initialization.
- `tensorrt_llm/_torch/configs/__init__.py`: Verify that `NemotronHConfig` is exported and registered for `nemotron_h`.
- `tensorrt_llm/_torch/configs/nemotron_h.py`: Verify pattern preservation and validation, derived layer settings, checkpoint reconstruction, MTP behavior, and cache settings.
- `tensorrt_llm/tokenizer/tokenizer.py`: Verify that TRT-LLM config registration occurs before `AutoTokenizer` loading.
- `tests/unittest/_torch/test_custom_config_registration.py`: Covers config dispatch, Nemotron-H patterns and error paths, tokenizer registration, and model-config loading. Listed in `tests/integration/test_lists/test-db/l0_cpu.yml`.
- `tests/unittest/auto_deploy/singlegpu/models/test_modeling_nemotron_h.py`: Covers `_from_config(..., use_cache=False)` and preservation of the supplied config. No test-list entry is identified in the supplied evidence.
- `tests/integration/test_lists/waives.txt`: Removes the Nemotron Nano NVFP4 waiver. Verify that the integration case runs.
- `tests/integration/test_lists/test-db/l0_cpu.yml`: Adds the config-registration test file to the L0 CPU CI list.
...
**Run ID**: `d43f0930-3e47-422b-ab63-9e21f6324fc1`
...
Reviewing files that changed from the base of the PR and between eb06366cf36ac49eab8539b42a968e9dea094846 and c7713cef4de0fee279137d6d03d99edea69a1352.
...
|---|---|
|**Nemotron-H configuration contract** <br> `tensorrt_llm/_torch/configs/nemotron_h.py`, `tensorrt_llm/_torch/configs/__init__.py`|Adds `NemotronHConfig`, hybrid-pattern validation, derived block types, configuration defaults, MTP pattern handling, and explicit `nemotron_h` registration.|
...
<!-- change_assessment_commit:"c7713cef4de0fee279137d6d03d99edea69a1352" -->
**Change:** Bug fix
...
**Merge Risk:** _⚪ Minimal_ · up to `c7713`
<!-- final_review_risk_coverage:{"sourceCommitId":"c7713cef4de0fee279137d6d03d99edea69a1352","coveredCommitId":"c7713cef4de0fee279137d6d03d99edea69a1352","kind":"reviewed"} -->

Nemotron-H configuration and tokenizer loading preserve the intended pattern handling, and AutoDeploy applies its cache setting through the config. No actionable merge-blocking risk is established; merge after normal checks.
...
| :------------------------: | :------- | :-------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- |
|         Title check        | ✅ Passed | The title follows the required NVBugs and type format and clearly identifies the main fix: preserving Nemotron-H hybrid_override_pattern values that contain MLP layers.                                  |
|      Description check     | ✅ Passed | The description explains the loading failure, the compatibility-shim solution, and the relevant CPU-only and end-to-end test coverage. It includes the required Description, Test Coverage, and PR Check… |
...
Docstring coverage is 4.00% which is insufficient. The required threshold is 80.00%. Docstring coverage is scoped to functions touched by this diff. Analyzed 25 functions across 6 files. (1 skipped: 1 unsupported.)
...
[PR_Github #75300](https://nv/trt-llm-cicd/job/helpers/job/PR_Github/75300/) [ run ] triggered by Bot. Commit: `c7713ce` [Link to invocation](https://github.com/NVIDIA/TensorRT-LLM/pull/18042#issuecomment-5799860817)
...
[PR_Github #75300](https://nv/trt-llm-cicd/job/helpers/job/PR_Github/75300/) [ run ] completed with state `SUCCESS`. Commit: `c7713ce` 
[/LLM/main/L0_MergeRequest_PR pipeline #62047](https://nv/trt-llm-cicd/job/main/job/L0_MergeRequest_PR/62047/) completed with status: 'FAILURE'

[CI Report](http://tensorrt-llm.tensorrt-llm-ci-report.sc2-paas.nvidia.com/?job=%2FLLM%2Fmain%2FL0_MergeRequest_PR&build=62047)
...
[CI Agent Failure Analysis](https://pbss.s8k.io/v1/AUTH_svc_tensorrt/sw-tensorrt-ci-analysis/LLM/main/L0_MergeRequest_PR/62047/failure_analysis.html)


[Link to invocation](https://github.com/NVIDIA/TensorRT-LLM/pull/18042#issuecomment-5799860817)
...
[PR_Github #75318](https://nv/trt-llm-cicd/job/helpers/job/PR_Github/75318/) [ run ] triggered by Bot. Commit: `8c42fdd` [Link to invocation](https://github.com/NVIDIA/TensorRT-LLM/pull/18042#issuecomment-5802582706)

### tensorrt-cicd · 2026-09-24

[PR_Github #75318](https://nv/trt-llm-cicd/job/helpers/job/PR_Github/75318/) [ run ] completed with state `SUCCESS`. Commit: `8c42fdd` 
[/LLM/main/L0_MergeRequest_PR pipeline #62065](https://nv/trt-llm-cicd/job/main/job/L0_MergeRequest_PR/62065/) completed with status: 'FAILURE'

[CI Report](http://tensorrt-llm.tensorrt-llm-ci-report.sc2-paas.nvidia.com/?job=%2FLLM%2Fmain%2FL0_MergeRequest_PR&build=62065)
...
[CI Agent Failure Analysis](https://pbss.s8k.io/v1/AUTH_svc_tensorrt/sw-tensorrt-ci-analysis/LLM/main/L0_MergeRequest_PR/62065/failure_analysis.html)


[Link to invocation](https://github.com/NVIDIA/TensorRT-LLM/pull/18042#issuecomment-5802582706)

### pranav-nvidia · 2026-09-24

/bot run

### tensorrt-cicd · 2026-09-24

[PR_Github #75336](https://nv/trt-llm-cicd/job/helpers/job/PR_Github/75336/) [ run ] triggered by Bot. Commit: `8c42fdd` [Link to invocation](https://github.com/NVIDIA/TensorRT-LLM/pull/18042#issuecomment-5805052078)

### tensorrt-cicd · 2026-09-24

[PR_Github #75336](https://nv/trt-llm-cicd/job/helpers/job/PR_Github/75336/) [ run ] completed with state `SUCCESS`. Commit: `8c42fdd` 
[/LLM/main/L0_MergeRequest_PR pipeline #62082](https://nv/trt-llm-cicd/job/main/job/L0_MergeRequest_PR/62082/) completed with status: 'SUCCESS'

[CI Report](http://tensorrt-llm.tensorrt-llm-ci-report.sc2-paas.nvidia.com/?job=%2FLLM%2Fmain%2FL0_MergeRequest_PR&build=62082)


[Link to invocation](https://github.com/NVIDIA/TensorRT-LLM/pull/18042#issuecomment-5805052078)
