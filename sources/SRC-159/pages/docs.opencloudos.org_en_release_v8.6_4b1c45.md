source: https://docs.opencloudos.org/en/release/v8.6/
...
- SELinux, the fapolicyd framework, and Policy-Based Decryption (PBD) for automated unlocking of LUKS-encrypted drives support the SAP HANA database management system.
...
- Fixed the use after free problem of tcp free skb, fixed the problem of virtio length, fixed the problem of virtio-net XDP queues, the acs problem of Feiteng architecture, and the failure of interrupt distribution.
- The bpf supports the nettrace function, which allows developers to track the cause of network packet loss through bpf (the kernel also turns related packet loss trace points)
