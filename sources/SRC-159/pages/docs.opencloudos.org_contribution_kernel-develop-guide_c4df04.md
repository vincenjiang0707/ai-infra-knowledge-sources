source: https://docs.opencloudos.org/contribution/kernel-develop-guide/
...
必须build通过外， 还必须编译通过代码树中默认的`defconfig`
...
。 -
**最小要求是不能增加build error/warning.**一个自测方法如下：`make allmodconfig modules all -s ; make allyesconfig noocconfig all -s`
