source: https://docs.opencloudos.org/en/contribution/kernel-develop-guide/
...
Provide kernel default configuration.

`make noocconfig`

The OC-specific configuration item is deleted in the current configuration as a build test case of the OC kernel.
...
-
The person in charge of feature submission needs to

`signed-off`

, and the reviewer needs to add`Reviewed-by：`
...
-
When submitting self-developed features(commit log)，require to include
...
。 -
When turning on the characteristics of other code trees, it is necessary to act as an independent branch and:
...
of the person who took the turn: (recommended to use`git cherry-pick -s`

) -
The solution to the round conflict should be indicated in the commit log:
...
-
The checkpatch.pl and each config compilation for each commit:

-
The newly added kernel config needs to be closed and compiled in the kernel/configs/nooc.config file ；

-
As an upstream kernel code, downstream users may have different requirements for using OC Kernel. In ARM and X86, in addition to the default
...
must be built and passed, they must also compile and pass the default`defconfig`
...
in the code tree. -
**The minimum requirement is not to addbuild error/warning.**A self testing method is as follows:`make allmodconfig modules all -s ; make allyesconfig noocconfig all -s`

-
The recommendations for kernel benchmark self-testing under each config are as follows:
...
-
Provide test cases for features, such as kunit/kselftest or other use cases

-
Pass basic kernel testing a. LTP related testing does not increase failure. b. Stress-ng testing - if it can be covered.

-
Gcov or kcov code coverage>90% when executing tests.
