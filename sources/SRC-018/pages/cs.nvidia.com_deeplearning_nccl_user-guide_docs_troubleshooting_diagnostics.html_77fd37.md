source: https://docs.nvidia.com/deeplearning/nccl/user-guide/docs/troubleshooting/diagnostics.html

# Diagnostics[](https://docs.nvidia.com#diagnostics)
...
**RAS diagnostics**passively collect and compare state information (GPU, CUDA driver, and NCCL configuration) across ranks, without exercising NCCL data paths.**Active diagnostics**exercise actual communication paths between GPUs to verify that they function correctly.
...
documented in [RAS Diagnostics](https://docs.nvidia.com/ras.html#ras-diagnostics).

## Active Diagnostics[](https://docs.nvidia.com#active-diagnostics)
...
### Running Active Diagnostics[](https://docs.nvidia.com#running-active-diagnostics)

Active diagnostics are disabled by default. To run them, set `NCCL_RUN_DIAGNOSTICS=1`
...
. A run with no reported issues might look like:
...
identifies a condition that may require review, such as a failed verification or a check that could not be completed.

For a partial P2P result, the summary reports the number of GPU-to-GPU peer accesses that passed verification out of the total number tested:
...
#### P2P Check[](https://docs.nvidia.com#p2p-check)
...
than direct peer access are excluded from the summary; their `reason=indirect`

skip records are
available with `NCCL_DEBUG=INFO`

and `NCCL_DEBUG_SUBSYS=INIT`
...
#### Network Bandwidth Check[](https://docs.nvidia.com#network-bandwidth-check)

The network bandwidth check runs `ib_write_bw`
...
between the nodes of the communicator. See [Networking Troubleshooting](https://docs.nvidia.com/networking_troubleshooting.html) for instructions on
...
communication (`NCCL_CROSS_NIC`

), phase 0 instead checks against the client’s next device after
...
tested (e.g. a merged device), a single `[INFO]`

line from rank 0 notes it.
...
ranks, and rank 0 prints the minimum, median, and maximum for each mode (tagged `[OK]`

, or
`[INFO]`

when the mode has outliers or not every scheduled measurement produced a result) and
emits an `[INFO]`
...
guards each individual `ib_write_bw`

run. When that time runs out, all nodes skip the remaining
measurements together and a `test unable to complete in allocated time`
