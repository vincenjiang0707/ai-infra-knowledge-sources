source: https://docs.nvidia.com/deeplearning/nccl/archives/nccl_214/nccl-developer-guide/index.html
...
For `ncclReduce` and `ncclAllreduce` functions, this means that
passing `sendBuff == recvBuff` will perform in place operations, storing final
results at the same place as initial data was read from.
...
`NCCL_RINGS``NCCL_RINGS`variable overrides the rings that NCCL forms by default. Rings are sequences of ranks. They can be any permutations of ranks.NCCL filters out any rings that do not contain the number of ranks in the NCCL communicator. In general, the ring formation is dependent on the hardware topology connecting the GPUs in your system.

`0`to`n-1`, where`n`is the number of GPUs in your communicator.The ranks can be separated by any non-digit character, for example, " ", "-", except "|".

Multiple rings can be specified separated by the pipe character "|".

For example, if you have 4 GPUs in a
communicator, you can form communication rings as
such:`0 1 2 3 | 3 2 1 0`.

This will form two rings, one in each direction.

`NCCL_MAX_NRINGS`(since 2.0.5)

`NCCL_MAX_NRINGS`variable limits the number of rings NCCL can use. Reducing the number of rings also reduces the number of CUDA blocks used for communication, hence the impact on GPU computing resources.`1`.`NCCL_CHECKS_DISABLE`(since 2.0.5)

`0`. Set the value to`1`to disable checks.`NCCL_LAUNCH_MODE`(since 2.1.0)

Setting it to
`PARALLEL` uses the previous launch
system which can be faster but is prone to deadlocks.

`NCCL_IB_TIMEOUT``NCCL_IB_TIMEOUT`variable controls the InfiniBand Verbs Timeout. For more information, see[InfiniBand](https://docs.nvidia.com/index.html#infiniband).`14`.The value depends on the size of your InfiniBand network.

`NCCL_IB_GID_INDEX`(since 2.1.4)

`show_gids`command to set this value. For more information, see[InfiniBand](https://docs.nvidia.com/index.html#infiniband).`0`.`NCCL_IB_SL`(since 2.1.4)

[InfiniBand](https://docs.nvidia.com/index.html#infiniband).`1`.`NCCL_IB_CUDA_SUPPORT``NCCL_IB_CUDA_SUPPORT`variable is used to disable GPU Direct RDMA.Define and set to `0` to disable
GPU Direct RDMA.
