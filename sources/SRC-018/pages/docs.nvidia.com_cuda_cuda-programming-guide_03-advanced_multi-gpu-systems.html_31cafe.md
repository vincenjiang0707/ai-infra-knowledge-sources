source: https://docs.nvidia.com/cuda/cuda-programming-guide/03-advanced/multi-gpu-systems.html

# 3.4. Programming Systems with Multiple GPUs[#](https://docs.nvidia.com#programming-systems-with-multiple-gpus)
...
Higher level abstractions and supporting system software such as CUDA interprocess communication, parallel reductions using

[NCCL](https://developer.nvidia.com/nccl), and communication using NVLink and/or GPU-Direct RDMA with APIs such as[NVSHMEM](https://developer.nvidia.com/nvshmem)and MPI
...
are discussed in the [Interprocess Communication](https://docs.nvidia.com/04-special-topics/inter-process-communication.html#interprocess-communication) section.
Advanced Virtual Memory Management (VMM) APIs support both intra-node and
...
[Virtual Memory Management](https://docs.nvidia.com/04-special-topics/virtual-memory-management.html#virtual-memory-management).

Compute fabric transport takes a different approach for GPUs on an NVLink fabric: rather
than reaching a peer’s memory through a mapped virtual address, as the approaches above
do, it names a group of resources using a logical endpoint id, a particular resource within that group via an offset, and accesses the resource with explicit,
asynchronous fabric operations. See [Compute Fabric Transport](https://docs.nvidia.com#multi-gpu-compute-fabric-transport) below.
...
## 3.4.1. Multi-Device Context and Execution Management[#](https://docs.nvidia.com#multi-device-context-and-execution-management)
...
### 3.4.1.1. Device Enumeration[#](https://docs.nvidia.com#device-enumeration)
...
### 3.4.1.2. Device Selection[#](https://docs.nvidia.com#device-selection)
...
Until a call to `cudaSetDevice()`

is made by the host thread, the current device defaults to device 0.
...
### 3.4.1.3. Multi-Device Stream, Event, and Memory Copy Behavior[#](https://docs.nvidia.com#multi-device-stream-event-and-memory-copy-behavior)
...
will succeed even if the input stream and input event are associated to different devices. `cudaStreamWaitEvent()`

can therefore be used to synchronize multiple devices with each other.

Each device has its own [default stream](https://docs.nvidia.com/02-basics/asynchronous-execution.html#async-execution-blocking-non-blocking-default-stream), so commands issued to the default stream of a device may execute out of order or concurrently with respect to commands issued to the default stream of any other device.

## 3.4.2. Multi-Device Peer-to-Peer Transfers and Memory Access[#](https://docs.nvidia.com#multi-device-peer-to-peer-transfers-and-memory-access)

### 3.4.2.1. Peer-to-Peer Memory Transfers[#](https://docs.nvidia.com#peer-to-peer-memory-transfers)
...
can be used with the copy type `cudaMemcpyDeviceToDevice`

or `cudaMemcpyDefault`

.

Otherwise, copies must be performed using `cudaMemcpyPeer()`
...
[Asynchronous Execution](https://docs.nvidia.com/02-basics/asynchronous-execution.html#asynchronous-execution)) issued after the copy to either device can start.
...
If peer-to-peer access is enabled between two devices, e.g., as described in [Peer-to-Peer Memory Access](https://docs.nvidia.com#multi-gpu-peer-to-peer-memory-access), peer-to-peer memory copies between these two devices no longer need to be staged through the host and are therefore faster.

### 3.4.2.2. Peer-to-Peer Memory Access[#](https://docs.nvidia.com#peer-to-peer-memory-access)
...
Peer-to-peer memory access is supported between two devices if `cudaDeviceCanAccessPeer()`

returns true for the specified devices.

Peer-to-peer memory access must be enabled between two devices by calling `cudaDeviceEnablePeerAccess()`
...
A unified virtual address space is used for both devices (see [Unified Virtual Address Space](https://docs.nvidia.com/02-basics/understanding-memory.html#memory-unified-virtual-address-space)), so the same pointer can be used to address memory from both devices as shown in the code sample below.
...
The use of `cudaDeviceEnablePeerAccess()`

to enable peer memory access operates globally on all previous and subsequent GPU memory allocations on the peer device.
Enabling peer access to a device via `cudaDeviceEnablePeerAccess()`

adds runtime cost to device memory allocation operations on that peer due to the need make the allocations immediately accessible to the current device and any other peers that also have access, adding multiplicative overhead that scales with the number of peer devices.
...
### 3.4.2.3. Peer-to-Peer Memory Consistency[#](https://docs.nvidia.com#peer-to-peer-memory-consistency)
...
[synchronization scope](https://nvidia.github.io/cccl/unstable/libcudacxx/extended_api/memory_model.html#thread-scopes).
Similarly, memory operations fall within
the `thread_scope_system`

[memory synchronization domain](https://docs.nvidia.com/cuda/cuda-c-programming-guide/#memory-synchronization-domains).

CUDA [Atomic Functions](https://docs.nvidia.com/05-appendices/cpp-language-extensions.html#atomic-functions) can perform read-modify-write operations
on an object in peer device memory when only a single GPU is
accessing that object.
The requirements and limitations for peer atomicity are described in the CUDA memory model [atomicity requirements](https://nvidia.github.io/cccl/unstable/libcudacxx/extended_api/memory_model.html#atomicity) discussion.

### 3.4.2.4. Multi-Device Managed Memory[#](https://docs.nvidia.com#multi-device-managed-memory)
...
and APIs for GPU-exclusive access to managed memory are described in [Multi-GPU](https://docs.nvidia.com/04-special-topics/unified-memory.html#um-legacy-multi-gpu).

### 3.4.2.5. Host IOMMU Hardware, PCI Access Control Services, and VMs[#](https://docs.nvidia.com#host-iommu-hardware-pci-access-control-services-and-vms)
...
### 3.4.2.6. Compute Fabric Transport[#](https://docs.nvidia.com#compute-fabric-transport)

Peer-to-peer memory access reaches a peer allocation through a virtual address
that the issuing process has reserved and mapped. Compute fabric transport
provides an alternative for large NVLink fabrics that names the peer by a logical
endpoint id and the resource accessed by offset rather than a mapped address.
It is intended for
communication libraries and advanced applications and is described in
[Compute Fabric Transport](https://docs.nvidia.com/04-special-topics/compute-fabric-transport.html#compute-fabric-transport).