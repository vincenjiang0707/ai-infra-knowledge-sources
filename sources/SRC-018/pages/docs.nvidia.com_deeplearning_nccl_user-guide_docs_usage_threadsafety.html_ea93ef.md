...
in parallel to independent communicators located on the same device with multiple threads (see [Using multiple NCCL communicators concurrently](https://docs.nvidia.com/communicators.html#multi-thread-concurrent-usage)).
If the child communicator shares the resources with the parent communicator (i.e., [ncclConfig_t](https://docs.nvidia.com/api/types.html#ncclconfig) by splitShare), it is not
