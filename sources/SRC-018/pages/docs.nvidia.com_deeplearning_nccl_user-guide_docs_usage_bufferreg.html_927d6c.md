...
# User Buffer Registration[](https://docs.nvidia.com#user-buffer-registration)
...
## NVLink Sharp Buffer Registration[](https://docs.nvidia.com#nvlink-sharp-buffer-registration)
...
[or a qualified allocator (see]`ncclMemAlloc()`

[Memory Allocator]).The NCCL operation is launched on a stream captured by a CUDA graph for each rank.
...
[Memory Allocator]).Register buffer with

[before calling collectives for each rank.]`ncclCommRegister()`

Call NCCL collectives as usual but similarly keep the offset to the head address of the buffer the same for each rank.


Registered buffers will be deregistered when users explicitly call [ ncclCommDeregister()](https://docs.nvidia.com/api/comms.html#c.ncclCommDeregister). Here is a local based buffer registration example:
...
## IB Sharp Buffer Registration[](https://docs.nvidia.com#ib-sharp-buffer-registration)
...
## General Buffer Registration[](https://docs.nvidia.com#general-buffer-registration)
...
The user buffers can be allocated through VMM API (i.e., cuMem*), any VMM-based allocators ([Memory Allocator](https://docs.nvidia.com#mem-allocator)) or ncclMemAlloc will work.
...
## Buffer Registration, GPU Direct RDMA, and MPS with MLOPart[](https://docs.nvidia.com#buffer-registration-gpu-direct-rdma-and-mps-with-mlopart)
...
## Buffer Registration and PXN[](https://docs.nvidia.com#buffer-registration-and-pxn)
...
## Memory Allocator[](https://docs.nvidia.com#memory-allocator)
...
See the description of [ ncclCommWindowRegister()](https://docs.nvidia.com/api/comms.html#c.ncclCommWindowRegister) and

[for additional details.](https://docs.nvidia.com/api/comms.html#c.ncclCommWindowDeregister)

`ncclCommWindowDeregister()`

## Zero-CTA Optimization[](https://docs.nvidia.com#zero-cta-optimization)
...
The communicator is configured with the

`NCCL_CTA_POLICY_ZERO`

flag (please see[NCCL Communicator CTA Policy Flags])Supported collectives:
