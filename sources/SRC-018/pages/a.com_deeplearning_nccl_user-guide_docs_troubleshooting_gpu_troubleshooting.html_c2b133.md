...
# GPU troubleshooting[](https://docs.nvidia.com#gpu-troubleshooting)

## GPU Direct[](https://docs.nvidia.com#gpu-direct)
...
### GPU-to-GPU communication[](https://docs.nvidia.com#gpu-to-gpu-communication)
...
**Checking peer-to-peer GPU memory access.** You can use `nvidia-smi topo -p2p <capability>`

to print a matrix of P2P status between GPU pairs. The `<capability>`
...
For example, on a healthy PCI Express-based 8-GPU system with full peer-to-peer GPU memory access, the matrix from `nvidia-smi topo -p2p p`
...
for peer access on the GPU pairs you use,
common causes include Linux bare-metal IOMMU settings and **PCI Access Control
Services (ACS)**; see [Linux bare-metal IOMMU and PCIe peer-to-peer](https://docs.nvidia.com#troubleshooting-iommu-p2p) and
[PCI Access Control Services (ACS)](https://docs.nvidia.com#troubleshooting-acs).
...
**simpleP2P** (`Samples/0_Introduction/simpleP2P`

) checks the transferred data after each transfer to confirm that the copy completed successfully. A successful run prints additional lines earlier in the log; the excerpt below is truncated and shows only the final lines:
...
To measure the available bandwidth between GPUs, we recommend using `nvbandwidth`

rather than the CUDA sample above. Download and build it following the instructions at [https://github.com/NVIDIA/nvbandwidth](https://github.com/NVIDIA/nvbandwidth).

**Disabling peer-to-peer GPU memory access for testing.** To see whether a problem is related to the P2P transport, compare runs with peer-to-peer GPU memory access disabled or restricted using [NCCL_P2P_DISABLE](https://docs.nvidia.com/env.html#env-nccl-p2p-disable) and [NCCL_P2P_LEVEL](https://docs.nvidia.com/env.html#env-nccl-p2p-level) (see the [Environment Variables](https://docs.nvidia.com/env.html) chapter).

### Linux bare-metal IOMMU and PCIe peer-to-peer[](https://docs.nvidia.com#linux-bare-metal-iommu-and-pcie-peer-to-peer)
...
Programming Guide]([https://docs.nvidia.com/cuda/cuda-programming-guide/03-advanced/multi-gpu-systems.html#host-iommu-hardware-pci-access-control-services-and-vms](https://docs.nvidia.com/cuda/cuda-programming-guide/03-advanced/multi-gpu-systems.html#host-iommu-hardware-pci-access-control-services-and-vms))
states that the IOMMU must be disabled on Linux bare-metal
systems to prevent silent device memory corruption.

This can affect NCCL because NCCL uses CUDA peer-to-peer access for GPU-to-GPU traffic when CUDA reports that the path is available. On affected systems, symptoms can look like application or framework bugs rather than a clean NCCL error.

As described in [the peer-to-peer GPU memory access check](https://docs.nvidia.com#troubleshooting-p2p-gpu-memory-access), a positive `nvidia-smi topo -p2p p`
...
Kernel messages vary, but output like the following indicates translated IOMMU mode and is concerning for Linux bare-metal PCIe P2P:
...
On affected bare-metal systems, avoid translated IOMMU mode for GPU PCIe P2P. Disable IOMMU translation for the GPU path.

### GPU-to-NIC communication[](https://docs.nvidia.com#gpu-to-nic-communication)
...
module is now supplied with the CUDA drivers, however it must be loaded on each node boot with:
...
is not accessible an alternative way to verify the module has been loaded is to run:
...
### PCI Access Control Services (ACS)[](https://docs.nvidia.com#pci-access-control-services-acs)
...
## Topology detection[](https://docs.nvidia.com#topology-detection)
