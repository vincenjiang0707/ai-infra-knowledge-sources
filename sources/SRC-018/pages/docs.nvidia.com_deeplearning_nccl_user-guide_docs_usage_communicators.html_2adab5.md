...
# Creating a Communicator[](https://docs.nvidia.com#creating-a-communicator)
...
Given a static mapping of ranks to CUDA devices, the [ ncclCommInitRank()](https://docs.nvidia.com/api/comms.html#c.ncclCommInitRank),

[and](https://docs.nvidia.com/api/comms.html#c.ncclCommInitRankConfig)

`ncclCommInitRankConfig()`

[functions will create communicator objects, each communicator object being associated to a fixed rank and CUDA device. Those objects will then be used to launch communication operations.](https://docs.nvidia.com/api/comms.html#c.ncclCommInitAll)

`ncclCommInitAll()`

Before calling [ ncclCommInitRank()](https://docs.nvidia.com/api/comms.html#c.ncclCommInitRank), you need to first create a unique object which will be used by all processes
and threads to synchronize and understand they are part of the same communicator. This is done by calling the

[function.](https://docs.nvidia.com/api/comms.html#c.ncclGetUniqueId)

`ncclGetUniqueId()`

The [ ncclGetUniqueId()](https://docs.nvidia.com/api/comms.html#c.ncclGetUniqueId) function returns an ID which has to be broadcast to all participating threads and
...
## Using MIG instances[](https://docs.nvidia.com#using-mig-instances)
...
7 MIG instances on each GPU and running the typical `nccl-tests`
...
Guide](https://docs.nvidia.com/datacenter/tesla/mig-user-guide/getting-started-with-mig.html).

## Creating a communicator with options[](https://docs.nvidia.com#creating-a-communicator-with-options)

The [ ncclCommInitRankConfig()](https://docs.nvidia.com/api/comms.html#c.ncclCommInitRankConfig) function allows creating a NCCL communicator with specific options.

The config parameters NCCL supports are listed here [ncclConfig_t](https://docs.nvidia.com/api/types.html#ncclconfig).
...
## Creating a communicator using multiple ncclUniqueIds[](https://docs.nvidia.com#creating-a-communicator-using-multiple-nccluniqueids)

The [ ncclCommInitRankScalable()](https://docs.nvidia.com/api/comms.html#c.ncclCommInitRankScalable) function enables the creation of a NCCL communicator using many ncclUniqueIds.
...
## Shrinking a communicator[](https://docs.nvidia.com#shrinking-a-communicator)

The [ ncclCommShrink()](https://docs.nvidia.com/api/comms.html#c.ncclCommShrink) function allows you to create a new communicator by removing specific ranks from an existing one.
...
Only ranks that will be part of the new communicator should call
...
## Growing a communicator[](https://docs.nvidia.com#growing-a-communicator)

The [ ncclCommGrow()](https://docs.nvidia.com/api/comms.html#c.ncclCommGrow) function allows you to create a new communicator by adding new ranks to an existing one.
...
The process requires a coordinator rank from the existing communicator to generate a unique identifier using [ ncclCommGetUniqueId()](https://docs.nvidia.com/api/comms.html#c.ncclCommGetUniqueId),
...
**Coordinator selection**: Any rank from the existing communicator can be the coordinator. The coordinator callsto generate the grow identifier.`ncclCommGetUniqueId()`

**Rank assignment**: Existing ranks retain their original rank numbers in the new communicator. New ranks must be assigned ranks starting from the size of the parent communicator.**Out-of-band communication**: The grow identifier must be distributed from the coordinator to all new ranks using a communication mechanism outside of NCCL (e.g., MPI, sockets, shared files).**Parent communicator cleanup**: After the grow operation completes successfully, existing ranks should destroy the parent communicator usingto free resources.`ncclCommDestroy()`

**No outstanding operations**: There should not be any outstanding NCCL operations on the parent communicator when callingto avoid potential deadlocks.`ncclCommGrow()`
...
## Creating more communicators[](https://docs.nvidia.com#creating-more-communicators)
...
## Finalizing a communicator[](https://docs.nvidia.com#finalizing-a-communicator)
...
## Destroying a communicator[](https://docs.nvidia.com#destroying-a-communicator)
...
# Error handling and communicator abort[](https://docs.nvidia.com#error-handling-and-communicator-abort)
...
## Asynchronous errors and error handling[](https://docs.nvidia.com#asynchronous-errors-and-error-handling)
...
# Fault Tolerance[](https://docs.nvidia.com#fault-tolerance)
...
# Quality of Service[](https://docs.nvidia.com#quality-of-service)
...
TC is specified during communicator creation using [ncclConfig_t](https://docs.nvidia.com/api/types.html#ncclconfig).
