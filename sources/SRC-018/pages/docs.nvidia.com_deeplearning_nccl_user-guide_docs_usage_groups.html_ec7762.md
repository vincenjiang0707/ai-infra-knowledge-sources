...
to improve performance, or merging multiple send/receive point-to-point operations (see [Point-to-point communication](https://docs.nvidia.com/p2p.html#point-to-point)
section). All three usages can be combined together, with one exception: calls to [ ncclCommInitRank()](https://docs.nvidia.com/api/comms.html#c.ncclCommInitRank)
cannot be merged with others.

## Management Of Multiple GPUs From One Thread[](https://docs.nvidia.com#management-of-multiple-gpus-from-one-thread)
...
We do not recommed using CUDA graph capture when managing multiple GPUs from one thread. In some cases `cudaGraphLaunch`

may block, preventing the launch across all GPUs. See [Using NCCL with CUDA Graphs](https://docs.nvidia.com/cudagraph.html#using-nccl-with-cuda-graphs) for details.
...
## Aggregated Operations (2.2 and later)[](https://docs.nvidia.com#aggregated-operations-2-2-and-later)
...
## Group Operation Ordering Semantics[](https://docs.nvidia.com#group-operation-ordering-semantics)
...
## Nonblocking Group Operation[](https://docs.nvidia.com#nonblocking-group-operation)
