...
## System configuration[](https://docs.nvidia.com#system-configuration)

### NCCL_SOCKET_IFNAME[](https://docs.nvidia.com#nccl-socket-ifname)

The `NCCL_SOCKET_IFNAME`

variable specifies which IP interfaces to use for communication.

#### Values accepted[](https://docs.nvidia.com#values-accepted)

Define to a list of prefixes to filter interfaces to be used by NCCL.

Multiple prefixes can be provided, separated by the `,`

symbol.

Using the `^`

symbol, NCCL will exclude interfaces starting with any prefix in that list.

To match (or not) an exact interface name, begin the prefix string with the `=`
...
: Use all interfaces starting with `eth`
...
: Do not use any interface starting with `docker`


`^=docker0`

: Do not use interface `docker0`

.

Note: By default, the loopback interface (`lo`

) and docker interfaces (`docker*`

) would not be selected unless there are no other interfaces available. If you prefer to use `lo`

or `docker*`

over other interfaces, you would need to explicitly select them using `NCCL_SOCKET_IFNAME`

. The default algorithm will also favor interfaces starting with `ib`

over others. Setting `NCCL_SOCKET_IFNAME`

will bypass the automatic interface selection algorithm and may use all interfaces matching the manual selection.

### NCCL_SOCKET_FAMILY[](https://docs.nvidia.com#nccl-socket-family)

The `NCCL_SOCKET_FAMILY`

variable allows users to force NCCL to use only IPv4 or IPv6 interface.

#### Values accepted[](https://docs.nvidia.com#id2)

Set to `AF_INET`

to force the use of IPv4, or `AF_INET6`

to force IPv6 usage.

### NCCL_SOCKET_MAGIC[](https://docs.nvidia.com#nccl-socket-magic)

The `NCCL_SOCKET_MAGIC`

variable overrides the 64-bit magic value used in NCCL’s internal TCP socket handshake for transports that rely on it (for example the Socket network plugin and InfiniBand connection bootstrap sockets). It does **not** change bootstrap communicators that use `NCCL_COMM_ID`

or [ ncclGetUniqueId()](https://docs.nvidia.com/api/comms.html#c.ncclGetUniqueId); those use a separate per-communicator magic.

Setting the same value on every process in a job (for example `0x$(printf '%x' "$SLURM_JOB_ID")`

under Slurm) can reduce accidental cross-talk when unrelated workloads share nodes and ports.

#### Values accepted[](https://docs.nvidia.com#id4)

Unset or empty: NCCL uses the historical built-in default.

Otherwise: a non-negative integer in decimal or hexadecimal (with optional `0x`

prefix), parsed as for `strtoull(..., 0)`

. Invalid strings fall back to the built-in default with a warning.

### NCCL_SOCKET_RETRY_CNT[](https://docs.nvidia.com#nccl-socket-retry-cnt)

(since 2.24)

The `NCCL_SOCKET_RETRY_CNT`

variable specifies the number of times NCCL retries to establish a socket connection after an `ETIMEDOUT`
...
#### Values accepted[](https://docs.nvidia.com#id5)

The default value is 34, any positive value is valid.

### NCCL_SOCKET_RETRY_SLEEP_MSEC[](https://docs.nvidia.com#nccl-socket-retry-sleep-msec)

(since 2.24)

The `NCCL_SOCKET_RETRY_SLEEP_MSEC`

variable specifies the number of milliseconds NCCL waits before retrying to establish a socket connection after the first `ETIMEDOUT`
...
error.
For subsequent errors, the waiting time scales linearly with the error count. The total time will therefore be (N+1) * N/2 * `NCCL_SOCKET_RETRY_SLEEP_MSEC`

, where N is given by `NCCL_SOCKET_RETRY_CNT`

.
With the default values of `NCCL_SOCKET_RETRY_CNT`

and `NCCL_SOCKET_RETRY_SLEEP_MSEC`

, the total retry time will be approx. 60 seconds.

#### Values accepted[](https://docs.nvidia.com#id6)

The default value is 100 milliseconds, any positive value is valid.

### NCCL_SOCKET_POLL_TIMEOUT_MSEC[](https://docs.nvidia.com#nccl-socket-poll-timeout-msec)

(since 2.28)

The `NCCL_SOCKET_POLL_TIMEOUT_MSEC`

variable specifies a timeout in
...
#### Values accepted[](https://docs.nvidia.com#id7)
...
### NCCL_SOCKET_NTHREADS[](https://docs.nvidia.com#nccl-socket-nthreads)

(since 2.4.8)

The `NCCL_SOCKET_NTHREADS`

variable specifies the number of CPU helper threads used per network connection for socket transport. Increasing this value may increase the socket transport performance, at the cost of a higher CPU usage.

#### Values accepted[](https://docs.nvidia.com#id8)
...
For generic 100G networks, this value can be manually set to 4. However, the product of `NCCL_SOCKET_NTHREADS`

and `NCCL_NSOCKS_PERTHREAD`

cannot exceed 64. See also `NCCL_NSOCKS_PERTHREAD`

.

### NCCL_NSOCKS_PERTHREAD[](https://docs.nvidia.com#nccl-nsocks-perthread)
...
#### Values accepted[](https://docs.nvidia.com#id9)
...
cannot exceed 64. See also `NCCL_SOCKET_NTHREADS`

.

### NCCL_CROSS_NIC[](https://docs.nvidia.com#nccl-cross-nic)

The `NCCL_CROSS_NIC`

variable controls whether NCCL should allow rings/trees to use different NICs,
...
setting is therefore dependent on the network topology, and in particular
on whether the network fabric is rail-optimized or not.

This has no effect on systems with only one NIC.

#### Values accepted[](https://docs.nvidia.com#id10)
...
### NCCL_IB_HCA[](https://docs.nvidia.com#nccl-ib-hca)

The `NCCL_IB_HCA`

variable specifies which Host Channel Adapter (RDMA) interfaces to use for communication. On Windows,
NetworkDirect reuses it to select adapters by Windows interface alias.

#### Values accepted[](https://docs.nvidia.com#id11)

Define to filter IB Verbs interfaces to be used by NCCL. The list is comma-separated; each entry follows the form
`<hca>[:<port>[:<rail>[:<plane>]]]`

, where fields after `<hca>`

are optional and separated by the `:`

symbol.
An optional prefix `^`

indicates the list is an exclude list. A second optional prefix `=`
...
is omitted, all ports on the HCA are used. If `<rail>`
...
field must still be present as an empty field to preserve field positions.
For example, `mlx5_0::0:0`

uses all ports on `mlx5_0`

with rail 0 and plane 0.

Note: The optional `<rail>`

and `<plane>`

fields assign a rail and plane identity to the associated device. Both default to `-1`

(unassigned) when omitted.
...
: Use ports 1 of cards `mlx5_0`

and `mlx5_1`

. Rail and plane assignment are both undefined.

`^=mlx5_1,mlx5_4`

: Do not use cards `mlx5_1`
...
: Use port 1 on each of `mlx5_0`

and `mlx5_1`

, assigning both to rail 0 with `mlx5_0`

on plane 0 and `mlx5_1`

on plane 1.

`=mlx5_0::0:0,mlx5_1::0:1`

: Use all ports on each of `mlx5_0`
...
, if they exist.
It is therefore always recommended to add the `=`

prefix to ensure an exact match.

For NetworkDirect, specify comma-separated Windows interface aliases without IB port, rail, or plane fields. For example,
`=Ethernet 4,Ethernet 10`

selects exactly those two interfaces.

Note: There is a fixed upper limit of 32 Host Channel Adapter (HCA) devices supported in NCCL.

### NCCL_IB_RAIL_POLICY[](https://docs.nvidia.com#nccl-ib-rail-policy)

(since 2.30.5)

Controls automatic rail and plane assignment for supported ConnectX SuperNIC devices. This can be used with
`NCCL_NET_MERGE_POLICY=RAIL`

to fuse only ports that belong to the same auto-detected rail.
The automatic rail and plane assignment will not overwrite the rails or planes given by the user (see `NCCL_IB_HCA`

).

#### Values accepted[](https://docs.nvidia.com#id12)

`NONE`

: Disable automatic rail and plane assignment.

`CX9`

(default) : Currently only supported on aarch64 systems with CX9 HCAs, NCCL will assign the rails and planes assuming the reference architecture configuration.

The per-socket rail layout can be selected explicitly with a suffix:

`CX9:FLIP`

: Flipped layout `[0 1 | 1 0]`
...
: Alternate layout `[0 1 0 1]`

.

`CX9:BLOCK`

: Block layout `[0 0 1 1]`

.

### NCCL_IB_TIMEOUT[](https://docs.nvidia.com#nccl-ib-timeout)

The `NCCL_IB_TIMEOUT`

variable controls the InfiniBand Verbs Timeout.
...
#### Values accepted[](https://docs.nvidia.com#id13)
...
### NCCL_IB_RETRY_CNT[](https://docs.nvidia.com#nccl-ib-retry-cnt)

(since 2.1.15)

The `NCCL_IB_RETRY_CNT`

variable controls the InfiniBand retry count. Total
time spent retrying is determined by the product of the retry count and the
timeout. For example, the default values of `NCCL_IB_TIMEOUT=20`

and
`NCCL_IB_RETRY_CNT=7`

result in approximately 30 seconds of waiting (7
...
#### Values accepted[](https://docs.nvidia.com#id14)

The default value is 7. Valid values are 0-7.

### NCCL_IB_GID_INDEX[](https://docs.nvidia.com#nccl-ib-gid-index)

(since 2.1.4)

The `NCCL_IB_GID_INDEX`

variable defines the Global ID index used in RoCE mode.
...
#### Values accepted[](https://docs.nvidia.com#id15)

The default value is -1.

### NCCL_IB_ADDR_FAMILY[](https://docs.nvidia.com#nccl-ib-addr-family)

(since 2.21)

The `NCCL_IB_ADDR_FAMILY`

variable defines the IP address family associated to
the infiniband GID dynamically selected by NCCL when `NCCL_IB_GID_INDEX`

is left
unset.

#### Values accepted[](https://docs.nvidia.com#id16)

The default value is “AF_INET”.

### NCCL_IB_ADDR_RANGE[](https://docs.nvidia.com#nccl-ib-addr-range)

(since 2.21)

The `NCCL_IB_ADDR_RANGE`

variable defines the range of valid GIDs dynamically
selected by NCCL when `NCCL_IB_GID_INDEX`

is left unset.

#### Values accepted[](https://docs.nvidia.com#id17)
...
### NCCL_IB_ROCE_VERSION_NUM[](https://docs.nvidia.com#nccl-ib-roce-version-num)

(since 2.21)

The `NCCL_IB_ROCE_VERSION_NUM`

variable defines the RoCE version associated to
the infiniband GID dynamically selected by NCCL when `NCCL_IB_GID_INDEX`

is left
unset.

#### Values accepted[](https://docs.nvidia.com#id18)

The default value is 2.

### NCCL_IB_PKEY[](https://docs.nvidia.com#nccl-ib-pkey)

(since 2.1.4)

The `NCCL_IB_PKEY`

variable selects the InfiniBand partition key (PKey) used by
...
#### Values accepted[](https://docs.nvidia.com#id19)

The default value is 0 (the first entry of the PKey table).

### NCCL_IB_PKEY_VALUE[](https://docs.nvidia.com#nccl-ib-pkey-value)

(since 2.31)

The `NCCL_IB_PKEY_VALUE`

variable selects the InfiniBand partition key (PKey)
by its **value** (for example `0x8111`

) rather than by its table index. At
...
The membership bit (`0x8000`

) is masked out during the comparison, so full and
limited members of a partition both match.

When both `NCCL_IB_PKEY_VALUE`
...
takes precedence. If the requested value is not present in
the local PKey table, NCCL fails the connection with a warning.

#### Values accepted[](https://docs.nvidia.com#id20)

The default value is -1 (unset), in which case the `NCCL_IB_PKEY`

index is used.

### NCCL_IB_SL[](https://docs.nvidia.com#nccl-ib-sl)
...
#### Values accepted[](https://docs.nvidia.com#id21)

The default value is 0.

### NCCL_IB_TC[](https://docs.nvidia.com#nccl-ib-tc)
...
#### Values accepted[](https://docs.nvidia.com#id22)

The default value is 0.

### NCCL_IB_FIFO_TC[](https://docs.nvidia.com#nccl-ib-fifo-tc)
...
#### Values accepted[](https://docs.nvidia.com#id23)

The default value is the traffic class set by NCCL_IB_TC, which defaults to 0 if not set.

### NCCL_IB_RETURN_ASYNC_EVENTS[](https://docs.nvidia.com#nccl-ib-return-async-events)
...
#### Values accepted[](https://docs.nvidia.com#id24)

The default value is 1, set to 0 to disable

### NCCL_IB_EVENT_BASED_LB[](https://docs.nvidia.com#nccl-ib-event-based-lb)
...
queried with the `ibv_query_port_speed`

API whenever that device reports an `IBV_EVENT_DEVICE_SPEED_CHANGE`

asynchronous event, and the distribution is recomputed at runtime from the new speeds.

This feature is intended for NICs that expose a single interface with multiple planes underneath, where the interface can continue to operate at a reduced speed when a subset of its planes goes down. A full interface or NIC going down is considered a failure and is handled by the failover-recovery feature.

Detecting speed changes requires `IBV_EVENT_DEVICE_SPEED_CHANGE`

and `ibv_query_port_speed`

support in the
InfiniBand verbs library (`IBVERBS_1.16`

or newer). This variable has no effect on non-fused devices, since all data
is sent on their single physical device regardless of its speed.

#### Values accepted[](https://docs.nvidia.com#id25)

The default value is 0 (data is split equally across devices), set to 1 to enable.

### NCCL_OOB_NET_ENABLE[](https://docs.nvidia.com#nccl-oob-net-enable)

(since 2.23)
The variable `NCCL_OOB_NET_ENABLE`

enables the use of NCCL net for out-of-band communications.
...
#### Values accepted[](https://docs.nvidia.com#id26)

Set the variable to 0 to disable, and to 1 to enable.

### NCCL_OOB_NET_IFNAME[](https://docs.nvidia.com#nccl-oob-net-ifname)

(since 2.23)
If NCCL net is enabled for out-of-band communication (see `NCCL_OOB_NET_ENABLE`

), the `NCCL_OOB_NET_IFNAME`

variable specifies which network interfaces to use.

#### Values accepted[](https://docs.nvidia.com#id27)
...
The list is comma-separated; port numbers can be specified using the `:`

symbol.
An optional prefix `^`

indicates the list is an exclude list.
A second optional prefix `=`

indicates that the tokens are exact names, otherwise by default NCCL would treat each token as a prefix.
...
will use the Infiniband NET, with the interface `mlx5_1`


`NCCL_NET="IB" NCCL_OOB_NET_ENABLE=1 NCCL_OOB_NET_IFNAME="mlx5_1"`

will use the Infiniband NET, with the first interface found in the list of `mlx5_1`
...
will use the socket NET, with the first interface found in the list of `ens1f0`

, `ens1f1`

, etc.

### NCCL_UID_STAGGER_THRESHOLD[](https://docs.nvidia.com#nccl-uid-stagger-threshold)

(since 2.23)
The `NCCL_UID_STAGGER_THRESHOLD`

variable is used to trigger staggering of communications between NCCL ranks and the ncclUniqueId in order to avoid overflowing the ncclUniqueId.
...
#### Values accepted[](https://docs.nvidia.com#id28)

The value of `NCCL_UID_STAGGER_THRESHOLD`

must be a strictly positive integer.
If unspecified, the default value is 256.

### NCCL_UID_STAGGER_RATE[](https://docs.nvidia.com#nccl-uid-stagger-rate)

(since 2.23)

The `NCCL_UID_STAGGER_RATE`

variable is used to define the message rate targeted when staggering the communications between NCCL ranks and the ncclUniqueId.
...
#### Values accepted[](https://docs.nvidia.com#id29)

The value of `NCCL_UID_STAGGER_RATE`

must be a strictly positive integer, expressed in messages/second.
If unspecified, the default value is 7000.

### NCCL_NET[](https://docs.nvidia.com#nccl-net)

(since 2.10)

Forces NCCL to use a specific network, for example to make sure NCCL uses an external plugin and doesn’t automatically fall back on the internal IB or Socket implementation. Setting this environment variable will override the `netName`

configuration in all communicators (see [ncclConfig_t](https://docs.nvidia.com/api/types.html#ncclconfig)); if not set (undefined), the network module will be determined by the configuration; if not passing configuration, NCCL will automatically choose the best network module.

#### Values accepted[](https://docs.nvidia.com#id30)
...
### NCCL_NET_PLUGIN[](https://docs.nvidia.com#nccl-net-plugin)
...
For example, setting `NCCL_NET_PLUGIN=foo`

will cause NCCL to try to load `foo`

and, if `foo`

cannot be found, `libnccl-net-foo.so`

(provided that it exists on the system).

#### Values accepted[](https://docs.nvidia.com#id31)

Plugin suffix, plugin file name, or “none”.

### NCCL_GIN_PLUGIN[](https://docs.nvidia.com#nccl-gin-plugin)
...
For example, setting `NCCL_GIN_PLUGIN=foo`

will cause NCCL to try to load `foo`

and, if `foo`

cannot be found, `libnccl-gin-foo.so`
...
#### Values accepted[](https://docs.nvidia.com#id33)

Comma-separated list of plugin suffixes and/or plugin file names, or “none”.

### NCCL_RMA_PLUGIN[](https://docs.nvidia.com#nccl-rma-plugin)
...
For example, setting `NCCL_RMA_PLUGIN=foo`

will cause NCCL to try to load `foo`

and, if `foo`

cannot be found, `libnccl-rma-foo.so`
...
#### Values accepted[](https://docs.nvidia.com#id35)

Comma-separated list of plugin suffixes and/or plugin file names, or “none”.

### NCCL_TUNER_PLUGIN[](https://docs.nvidia.com#nccl-tuner-plugin)
...
If no plugin was found look for the tuner symbols in the net plugin (refer to
...
For example, setting `NCCL_TUNER_PLUGIN=foo`

will cause NCCL to try to load `foo`

and, if `foo`

cannot be found, `libnccl-tuner-foo.so`

(provided that it exists on the system).

#### Values accepted[](https://docs.nvidia.com#id36)

Plugin suffix, plugin file name, or “none”.

### NCCL_PROFILER_PLUGIN[](https://docs.nvidia.com#nccl-profiler-plugin)
...
If NCCL_PROFILER_PLUGIN is set to

`STATIC_PLUGIN`

, the plugin symbols are searched in the program binary.


For example, setting `NCCL_PROFILER_PLUGIN=foo`

will cause NCCL to try to load `foo`

and, if `foo`

cannot be found, `libnccl-profiler-foo.so`

(provided that it exists on the system).

#### Values accepted[](https://docs.nvidia.com#id37)

Plugin suffix, plugin file name, or “none”.

### NCCL_ENV_PLUGIN[](https://docs.nvidia.com#nccl-env-plugin)
...
variable can be used to let NCCL load an external environment plugin. Set it to either a library name or a suffix string to choose among multiple NCCL environment plugins. This setting will cause NCCL to look for the environment plugin library using the following strategy: If

`NCCL_ENV_PLUGIN`

is set to a library name, attempt loading that library (e.g.`NCCL_ENV_PLUGIN=/path/to/library/libfoo.so`

will cause NCCL to try to load`/path/to/library/libfoo.so`

);If

`NCCL_ENV_PLUGIN`

is set to a suffix string, attempt loading`libnccl-env-<NCCL_ENV_PLUGIN>.so`

(e.g.`NCCL_ENV_PLUGIN=foo`

will cause NCCL to try to load`libnccl-env-foo.so`

from the system library path);If

`NCCL_ENV_PLUGIN`

is not set, attempt loading the default`libnccl-env.so`

library from the system library path;If

`NCCL_ENV_PLUGIN`

is set to “none”, explicitly disable the external plugin and use the internal one;If no plugin was found (neither user defined nor default) or the variable is set to “none”, use the internal environment plugin.



#### Values accepted[](https://docs.nvidia.com#id38)

Plugin library name (e.g., `/path/to/library/libfoo.so`

), suffix (e.g., `foo`

), or “none”.

### NCCL_IGNORE_CPU_AFFINITY[](https://docs.nvidia.com#nccl-ignore-cpu-affinity)

(since 2.4.6)

The `NCCL_IGNORE_CPU_AFFINITY`

variable controls whether NCCL honors the CPU affinity inherited from the launcher or parent process.
...
See [CPU and memory affinity](https://docs.nvidia.com/troubleshooting/performance_and_tuning.html#cpu-memory-affinity) for placement guidance.

#### Values accepted[](https://docs.nvidia.com#id40)

The default is 0, set to 1 to cause NCCL to ignore the job’s supplied CPU affinity.

### NCCL_CONF_FILE[](https://docs.nvidia.com#nccl-conf-file)

(since 2.23)

The `NCCL_CONF_FILE`

variable allows the user to specify a file with the static configuration.
This does not accept the `~`

character as part of the path; please convert to a relative or absolute path first.

#### Values accepted[](https://docs.nvidia.com#id41)

If unset or if the version is prior to 2.23, NCCL uses .nccl.conf in the home directory if available.

### NCCL_DEBUG[](https://docs.nvidia.com#nccl-debug)

The `NCCL_DEBUG`

variable controls the debug information that is displayed from NCCL. This variable is commonly used for debugging.

#### Values accepted[](https://docs.nvidia.com#id43)
...
### NCCL_DEBUG_LEVELS[](https://docs.nvidia.com#nccl-debug-levels)

(since 2.32)

The `NCCL_DEBUG_LEVELS`

variable adds individual levels to the inclusive
selection made by `NCCL_DEBUG`

. The effective selection is the logical union
...
For example, the following maintains `WARN`

as the baseline while adding
`ATTN`

on NCCL versions that support this variable. `NCCL_DEBUG=WARN`
...
Older NCCL versions ignore `NCCL_DEBUG_LEVELS`
...
Do not set `NCCL_DEBUG=ATTN`

as a site-wide setting in a mixed-version
deployment. NCCL versions before 2.32 do not recognize `ATTN`

and fall back
to no debug logging. Use `NCCL_DEBUG=WARN NCCL_DEBUG_LEVELS=ATTN`

instead.

### NCCL_DEBUG_FILE[](https://docs.nvidia.com#nccl-debug-file)

(since 2.2.12)

The `NCCL_DEBUG_FILE`

variable directs the NCCL debug logging output to a file.
The filename format can be set to *filename.%h.%p* where *%h* is replaced with the
hostname and *%p* is replaced with the process PID. This does not accept the `~`

character as part of the path, please convert to a relative or absolute path first.

#### Values accepted[](https://docs.nvidia.com#id46)

The default output file is *stdout* unless this environment variable is set.
The filename can also be set to `/dev/stdout`

or `/dev/stderr`

to direct NCCL debug logging
output to those predefined I/O streams. This also has the effect of making the output line buffered.

Setting `NCCL_DEBUG_FILE`

will cause NCCL to create and overwrite any previous files of that name.

Note: If the filename is not unique across all the job processes, then the output may be lost or corrupted.

### NCCL_DEBUG_SUBSYS[](https://docs.nvidia.com#nccl-debug-subsys)

(since 2.3.4)

The `NCCL_DEBUG_SUBSYS`

variable allows the user to filter the `NCCL_DEBUG=INFO`
...
#### Values accepted[](https://docs.nvidia.com#id48)
...
### NCCL_DEBUG_TIMESTAMP_FORMAT[](https://docs.nvidia.com#nccl-debug-timestamp-format)

(since 2.26)

The `NCCL_DEBUG_TIMESTAMP_FORMAT`

variable allows the user to change
the format used when printing debug log messages.

The time is printed as a local
time. This can be changed by setting the `TZ`

environment variable. UTC
is available by setting `TZ=UTC`

. Valid values for TZ look like:
`US/Pacific`
...
Note that the non-call `TRACE`

level of logs continues to print the
microseconds since the NCCL debug subsystem was initialized. The
`TRACE`

logs can also print the strftime formatted timestamp at the
...
#### Value accepted[](https://docs.nvidia.com#value-accepted)
...
can be specified, where `X`

is a single numerical digit from 1-9.
This will print fractions of a second. The value of `X`

indicates how many digits will be printed. For example, `%3f`

will
print milliseconds. The value is zero padded. For example:
`[%F %T.%9f] `

. (Note that this can only be used once in the format
string.)

### NCCL_DEBUG_TIMESTAMP_LEVELS[](https://docs.nvidia.com#nccl-debug-timestamp-levels)

(since 2.26)

The `NCCL_DEBUG_TIMESTAMP_LEVELS`

variable allows the user to set
which log lines get a timestamp depending upon the level of the log.

#### Value accepted[](https://docs.nvidia.com#id51)

The value should be a comma separated list of the levels which should
have the timestamp. Valid levels are: `VERSION`
...
all levels. If the value is prefixed with a caret (`^`

) then the
listed levels will NOT log a timestamp, and the rest will.
The default is to enable timestamps for `WARN`
...
For example, `NCCL_DEBUG_TIMESTAMP_LEVELS=WARN,INFO,TRACE`

will turn
it on for warnings, info logs, and traces. Or,
`NCCL_DEBUG_TIMESTAMP_LEVELS=^TRACE`

will turn them on for everything
but traces, which (except call traces) have their own type of timestamp
(microseconds since nccl debug initialization).

### NCCL_COLLNET_ENABLE[](https://docs.nvidia.com#nccl-collnet-enable)

(since 2.6)

Enable the use of the CollNet plugin.

#### Value accepted[](https://docs.nvidia.com#id52)

Default is 0, define and set to 1 to use the CollNet plugin.

### NCCL_COLLNET_NODE_THRESHOLD[](https://docs.nvidia.com#nccl-collnet-node-threshold)

(since 2.9.9)

A threshold for the number of nodes below which CollNet will not be enabled.

#### Value accepted[](https://docs.nvidia.com#id53)

Default is 2, define and set to an integer.

### NCCL_CTA_POLICY[](https://docs.nvidia.com#nccl-cta-policy)

(since 2.29, legacy values since 2.27)

The `NCCL_CTA_POLICY`

variable allows the user to set the policy for the NCCL communicator.

#### Value accepted[](https://docs.nvidia.com#id54)

Set to `DEFAULT`

(or `0`

, legacy) to use `NCCL_CTA_POLICY_DEFAULT`
...
, legacy) to use `NCCL_CTA_POLICY_EFFICIENCY`
...
, legacy) to use `NCCL_CTA_POLICY_ZERO`

policy.

Set multiple non-legacy policies with the `|`

operator.

For more explanation about NCCL policies, please see [NCCL Communicator CTA Policy Flags](https://docs.nvidia.com/api/flags.html#cta-policy-flags).

### NCCL_NETDEVS_POLICY[](https://docs.nvidia.com#nccl-netdevs-policy)

(since 2.28)

The `NCCL_NETDEVS_POLICY`

variable allows the user to set the policy for the assignment of network devices to the GPUs.
...
#### Value accepted[](https://docs.nvidia.com#id55)

By default, NCCL assigns each GPU to network devices by considering the set of available devices, as well as other GPUs in the same communicator. In specific scenarios, this policy might lead to different GPUs from different communicators sharing the same network devices, and therefore impacts performance.

If set to `MAX:N`

, NCCL uses up to N of the network devices available to each GPU.

If set to `ALL`

, NCCL will use all the available network devices for each GPU, disregarding other GPUs.

### NCCL_MULTI_RANK_GPU_ENABLE[](https://docs.nvidia.com#nccl-multi-rank-gpu-enable)

(since 2.30) (experimental)

The `NCCL_MULTI_RANK_GPU_ENABLE`

variable allows NCCL to run with multiple ranks using the same
...
the users to use `NCCL_MAX_CTAS`

to limit the number of channels per rank to prevent resource
exhaustion.

Finally, NVLS is currently not compatible with multiple ranks within the same communicator using
the same GPU. If `NCCL_NVLS_ENABLE`

is set to 1, communicator initialization will fail when
multiple ranks per GPU are detected. If `NCCL_NVLS_ENABLE`

is set to 2 (the default), NVLS will
...
#### Values accepted[](https://docs.nvidia.com#id56)
...
### NCCL_TOPO_FILE[](https://docs.nvidia.com#nccl-topo-file)

(since 2.6)

Path to an XML file to load before detecting the topology. By default, NCCL will load `/var/run/nvidia-topologyd/virtualTopology.xml`

if present.

#### Value accepted[](https://docs.nvidia.com#id57)

A path to an accessible file describing part or all of the topology.

The topology file must describe the topology of a single host (even for multi-node NVLink systems).

`NCCL_TOPO_DUMP_FILE`

produces a file containing the topology of the NVLink domain.
For multi-node NVLink systems, such a file should not be used directly as input to `NCCL_TOPO_FILE`
...
#### Value accepted[](https://docs.nvidia.com#id58)
...
### NCCL_SET_THREAD_NAME[](https://docs.nvidia.com#nccl-set-thread-name)

(since 2.12)

Give more meaningful names to NCCL CPU threads to ease debugging and analysis.

#### Value accepted[](https://docs.nvidia.com#id59)
...
### NCCL_P2P_DISABLE[](https://docs.nvidia.com#nccl-p2p-disable)

The `NCCL_P2P_DISABLE`

variable disables the peer to peer (P2P) transport, which uses CUDA direct access between GPUs, using NVLink or PCI.

#### Values accepted[](https://docs.nvidia.com#id60)

Define and set to 1 to disable direct GPU-to-GPU (P2P) communication.

### NCCL_P2P_LEVEL[](https://docs.nvidia.com#nccl-p2p-level)

(since 2.3.4)

The `NCCL_P2P_LEVEL`

variable allows the user to finely control when to use the peer to peer (P2P) transport between GPUs.
...
#### Values accepted[](https://docs.nvidia.com#id61)
...
#### Integer Values (Legacy)[](https://docs.nvidia.com#integer-values-legacy)

There is also the option to declare `NCCL_P2P_LEVEL`

as an integer corresponding to the path type. These numerical values were kept for retro-compatibility, for those who used numerical values before strings were allowed.
...
### NCCL_P2P_DIRECT_DISABLE[](https://docs.nvidia.com#nccl-p2p-direct-disable)

The `NCCL_P2P_DIRECT_DISABLE`

variable forbids NCCL to directly access user buffers through P2P between GPUs of the same process. This is useful when user buffers are allocated with APIs which do not automatically make them accessible to other GPUs managed by the same process and with P2P access.

#### Values accepted[](https://docs.nvidia.com#id62)

Define and set to 1 to disable direct user buffer access across GPUs.

### NCCL_SHM_DISABLE[](https://docs.nvidia.com#nccl-shm-disable)

The `NCCL_SHM_DISABLE`

variable disables the Shared Memory (SHM) transports. SHM is used between devices when peer-to-peer cannot happen, therefore, host memory is used. NCCL will use the network (i.e. InfiniBand or IP sockets) to communicate between the CPU sockets when SHM is disabled.

#### Values accepted[](https://docs.nvidia.com#id63)

Define and set to 1 to disable communication through shared memory (SHM).

### NCCL_BUFFSIZE[](https://docs.nvidia.com#nccl-buffsize)

The `NCCL_BUFFSIZE`

variable controls the size of the buffer used by NCCL when communicating data between pairs of GPUs.
...
#### Values accepted[](https://docs.nvidia.com#id64)
...
### NCCL_NTHREADS[](https://docs.nvidia.com#nccl-nthreads)

The `NCCL_NTHREADS`

variable sets the number of CUDA threads per CUDA block. NCCL will launch one CUDA block per communication channel.
...
#### Values accepted[](https://docs.nvidia.com#id65)
...
### NCCL_MAX_NCHANNELS[](https://docs.nvidia.com#nccl-max-nchannels)

(NCCL_MAX_NRINGS since 2.0.5, NCCL_MAX_NCHANNELS since 2.5.0, deprecated in 2.17)

The `NCCL_MAX_NCHANNELS`

variable limits the number of channels NCCL can use. Reducing the number of channels also reduces the
number of CUDA blocks used for communication, hence the impact on GPU computing resources.

The old `NCCL_MAX_NRINGS`

variable (used until 2.4) still works as an alias in newer versions but is ignored if `NCCL_MAX_NCHANNELS`

is set.

This environment variable has been deprecated in favor of `NCCL_MAX_CTAS`

which can also be set programmatically using [ncclCommInitRankConfig](https://docs.nvidia.com/api/comms.html#ncclcomminitrankconfig). If
both are set then the most restrictive value (i.e., the lower value) is used.

#### Values accepted[](https://docs.nvidia.com#id66)

Any value above or equal to 1.

### NCCL_MIN_NCHANNELS[](https://docs.nvidia.com#nccl-min-nchannels)

(NCCL_MIN_NRINGS since 2.2.0, NCCL_MIN_NCHANNELS since 2.5.0, deprecated in 2.17)

The `NCCL_MIN_NCHANNELS`

variable controls the minimum number of channels you want NCCL to use.
...
The old `NCCL_MIN_NRINGS`

variable (used until 2.4) still works as an alias in newer versions, but is ignored if `NCCL_MIN_NCHANNELS`
...
#### Values accepted[](https://docs.nvidia.com#id67)

The default is platform dependent. Set to an integer value, up to 12 (up to 2.2), 16 (2.3 and 2.4) or 32 (2.5 and later).

### NCCL_CHECKS_DISABLE[](https://docs.nvidia.com#nccl-checks-disable)

(since 2.0.5, deprecated in 2.2.12)

The `NCCL_CHECKS_DISABLE`

variable can be used to disable argument checks on each collective call.
...
#### Values accepted[](https://docs.nvidia.com#id68)

The default is 0, set to 1 to disable checks.

### NCCL_CHECK_POINTERS[](https://docs.nvidia.com#nccl-check-pointers)

(since 2.2.12, deprecated in 2.29.7)

The `NCCL_CHECK_POINTERS`

variable enables checking of the CUDA memory pointers on each collective call.
Checks are useful during development but can increase the latency.
Replaced by `NCCL_CHECK_MODE=DEBUG_LOCAL`

and may be removed in future versions.

#### Values accepted[](https://docs.nvidia.com#id69)

The default is 0, set to 1 to enable checking.

Setting to 1 restores the original behavior of NCCL prior to 2.2.12.

### NCCL_CHECK_MODE[](https://docs.nvidia.com#nccl-check-mode)

(since 2.29.7)

The `NCCL_CHECK_MODE`

variable controls the mode of checking the input arguments.

#### Values accepted[](https://docs.nvidia.com#id70)
...
### NCCL_LAUNCH_MODE[](https://docs.nvidia.com#nccl-launch-mode)

(since 2.1.0)

The `NCCL_LAUNCH_MODE`

variable controls how NCCL launches CUDA kernels.

#### Values accepted[](https://docs.nvidia.com#id71)
...
### NCCL_IB_DISABLE[](https://docs.nvidia.com#nccl-ib-disable)

The `NCCL_IB_DISABLE`

variable prevents the IB/RoCE transport from being used by NCCL. On Windows, the same variable
also disables the NetworkDirect transport. NCCL will instead fall back to another available transport such as IP sockets.

#### Values accepted[](https://docs.nvidia.com#id72)

Define and set to 1 to disable the use of InfiniBand Verbs and NetworkDirect for communication (and force another method, e.g. IP sockets).

### NCCL_IB_AR_THRESHOLD[](https://docs.nvidia.com#nccl-ib-ar-threshold)

(since 2.6)

Threshold above which we send InfiniBand data in a separate message which can leverage adaptive routing.

#### Values accepted[](https://docs.nvidia.com#id73)
...
See `NCCL_IB_SPLIT_DATA_ON_QPS`

for different ways to split data on multiple QPs, as it can affect performance.
NetworkDirect reuses this variable as the QP count per physical adapter in a connection.

#### Values accepted[](https://docs.nvidia.com#id75)

Number between 1 and 128, default is 1.

### NCCL_IB_SPLIT_DATA_ON_QPS[](https://docs.nvidia.com#nccl-ib-split-data-on-qps)
...
#### Values accepted[](https://docs.nvidia.com#id76)

0 or 1. Default is 0 (since NCCL 2.20). Setting it to 1 will enable split mode (default in 2.18 and 2.19).

### NCCL_IB_CUDA_SUPPORT[](https://docs.nvidia.com#nccl-ib-cuda-support)

(removed in 2.4.0, see NCCL_NET_GDR_LEVEL)

The `NCCL_IB_CUDA_SUPPORT`

variable is used to force or disable the usage of GPU Direct RDMA.
...
#### Values accepted[](https://docs.nvidia.com#id77)

Define and set to 0 to disable GPU Direct RDMA.

Define and set to 1 to force the usage of GPU Direct RDMA.

### NCCL_IB_PCI_RELAXED_ORDERING[](https://docs.nvidia.com#nccl-ib-pci-relaxed-ordering)
...
#### Values accepted[](https://docs.nvidia.com#id78)
...
### NCCL_IB_ADAPTIVE_ROUTING[](https://docs.nvidia.com#nccl-ib-adaptive-routing)
...
#### Values accepted[](https://docs.nvidia.com#id79)
...
### NCCL_IB_ECE_ENABLE[](https://docs.nvidia.com#nccl-ib-ece-enable)
...
#### Values accepted[](https://docs.nvidia.com#id80)
...
### NCCL_MEM_SYNC_DOMAIN[](https://docs.nvidia.com#nccl-mem-sync-domain)
...
#### Values accepted[](https://docs.nvidia.com#id81)

Default value is `cudaLaunchMemSyncDomainRemote`

(1). Currently supported values are 0 and 1.

### NCCL_CUMEM_ENABLE[](https://docs.nvidia.com#nccl-cumem-enable)

(since 2.18)

Use CUDA cuMem* functions to allocate memory in NCCL.

#### Values accepted[](https://docs.nvidia.com#id82)
...
### NCCL_CUMEM_HOST_ENABLE[](https://docs.nvidia.com#nccl-cumem-host-enable)

(since 2.23)

Use CUDA cuMem* functions to allocate host memory in NCCL. See [Shared memory](https://docs.nvidia.com/troubleshooting/runtime_and_mpi_issues.html#cumem-host-allocations) for more information.

#### Values accepted[](https://docs.nvidia.com#id83)
...
### NCCL_NET_GDR_LEVEL (formerly NCCL_IB_GDR_LEVEL)[](https://docs.nvidia.com#nccl-net-gdr-level-formerly-nccl-ib-gdr-level)

(since 2.3.4. In 2.4.0, NCCL_IB_GDR_LEVEL was renamed to NCCL_NET_GDR_LEVEL)

The `NCCL_NET_GDR_LEVEL`

variable allows the user to finely control when to use GPU Direct RDMA between a NIC and a GPU.
...
#### Values accepted[](https://docs.nvidia.com#id84)
...
#### Integer Values (Legacy)[](https://docs.nvidia.com#id85)

There is also the option to declare `NCCL_NET_GDR_LEVEL`
...
### NCCL_NET_GDR_C2C[](https://docs.nvidia.com#nccl-net-gdr-c2c)

(since 2.26)

The `NCCL_NET_GDR_C2C`

variable enables GPU Direct RDMA when sending data via a NIC attached to a CPU (i.e. distance PHB) where the CPU is connected to the GPU via a C2C interconnect. This effectively overrides the `NCCL_NET_GDR_LEVEL`

setting for this particular NIC.

#### Values accepted[](https://docs.nvidia.com#id86)
...
### NCCL_NET_GDR_READ[](https://docs.nvidia.com#nccl-net-gdr-read)

The `NCCL_NET_GDR_READ`

variable enables GPU Direct RDMA when sending data as long as the GPU-NIC distance is within the distance specified by `NCCL_NET_GDR_LEVEL`

. Before 2.4.2, GDR read is disabled by default, i.e. when sending data, the data is first stored in CPU memory, then goes to the InfiniBand card. Since 2.4.2, GDR read is enabled by default for NVLink-based platforms.
...
#### Values accepted[](https://docs.nvidia.com#id87)
...
### NCCL_GDRCOPY_ENABLE[](https://docs.nvidia.com#nccl-gdrcopy-enable)

The `NCCL_GDRCOPY_ENABLE`

variable enables GDRCopy support for CPU-accessible CUDA memory used by NCCL internal control structures.

When enabled, NCCL first tries to load `libgdrapi.so`

. If the library is not available or cannot initialize, NCCL can use its internal Linux CUDA DMA-BUF mmap backend when the CUDA driver and GPU support it.

#### Values accepted[](https://docs.nvidia.com#id88)

0 or 1. Default value is 0 (disabled).

### NCCL_GDRCOPY_FIFO_ENABLE[](https://docs.nvidia.com#nccl-gdrcopy-fifo-enable)

The `NCCL_GDRCOPY_FIFO_ENABLE`

variable controls whether the communicator work FIFO is allocated in GDRCopy-mapped CUDA memory when GDRCopy support is enabled.

When disabled, NCCL allocates the work FIFO in CUDA host memory instead.

#### Values accepted[](https://docs.nvidia.com#id89)

0 or 1. Default value is 1 (enabled).

### NCCL_GDRCOPY_SYNC_ENABLE[](https://docs.nvidia.com#nccl-gdrcopy-sync-enable)

The `NCCL_GDRCOPY_SYNC_ENABLE`

variable controls whether NCCL uses GDRCopy-mapped CUDA memory for network proxy synchronization words such as connection head and tail pointers.
...
#### Values accepted[](https://docs.nvidia.com#id90)

0 or 1. Default value is 1 (enabled).

### NCCL_GDRCOPY_FLUSH_ENABLE[](https://docs.nvidia.com#nccl-gdrcopy-flush-enable)

The `NCCL_GDRCOPY_FLUSH_ENABLE`

variable controls whether NCCL uses a GDRCopy-mapped CUDA memory read as the receive-side GDRDMA visibility flush.

When disabled, NCCL uses the network transport `iflush`

callback for receive buffers that require a flush. When enabled, NCCL uses a CPU read from GDRCopy-mapped CUDA memory to force PCIe write visibility.

#### Values accepted[](https://docs.nvidia.com#id91)

0 or 1. Default value is 0 (disabled).

### NCCL_IGNORE_NET_MISMATCH[](https://docs.nvidia.com#nccl-ignore-net-mismatch)

(since 2.30)

The `NCCL_IGNORE_NET_MISMATCH`

variable controls whether NCCL should fail initialization when it detects mismatched Net (network) device counts across ranks. By default, NCCL will ignore Net device count mismatches, but setting this to 0 will cause NCCL to fail initialization if some ranks have access to fewer Net devices than others.

#### Values accepted[](https://docs.nvidia.com#id94)

Default is 1 (ignore mismatch). Set to 0 to fail on Net device count mismatches.

### NCCL_IGNORE_COLLNET_MISMATCH[](https://docs.nvidia.com#nccl-ignore-collnet-mismatch)

(since 2.30)

The `NCCL_IGNORE_COLLNET_MISMATCH`

variable controls whether NCCL should fail initialization when it detects mismatched CollNet (collective network) device counts across ranks. By default, NCCL will fail with a system error if some ranks have access to fewer CollNet devices than others, as this typically indicates a misconfiguration that can lead to failures.

#### Values accepted[](https://docs.nvidia.com#id95)

Default is 0 (fail on mismatch). Set to 1 to ignore CollNet device count mismatches and continue with initialization.

### NCCL_SINGLE_RING_THRESHOLD[](https://docs.nvidia.com#nccl-single-ring-threshold)

(since 2.1, removed in 2.3)

The `NCCL_SINGLE_RING_THRESHOLD`

variable sets the limit under which NCCL will only use one ring.
This will limit bandwidth but improve latency.

#### Values accepted[](https://docs.nvidia.com#id96)
...
### NCCL_LL_THRESHOLD[](https://docs.nvidia.com#nccl-ll-threshold)

(since 2.1, removed in 2.5)

The `NCCL_LL_THRESHOLD`

variable sets the size limit under which NCCL uses low-latency algorithms.

#### Values accepted[](https://docs.nvidia.com#id97)

The default is 16384 (up to 2.2) or is dependent on the number of ranks (2.3 and later).

Values are integers, in bytes.

### NCCL_TREE_THRESHOLD[](https://docs.nvidia.com#nccl-tree-threshold)

(since 2.4, removed in 2.5)

The `NCCL_TREE_THRESHOLD`

variable sets the size limit under which NCCL uses tree algorithms instead of rings.

#### Values accepted[](https://docs.nvidia.com#id98)

The default is dependent on the number of ranks.

Values are integers, in bytes.

### NCCL_ALGO[](https://docs.nvidia.com#nccl-algo)
...
#### Values accepted[](https://docs.nvidia.com#id99)
...
if there is no other valid algorithm for the function. Instead,
it will fail.
...
), then all the selections are inverted.
...
And, `NCCL_ALGO=allreduce:^tree`

will allow the default (all algorithms
...
### NCCL_PROTO[](https://docs.nvidia.com#nccl-proto)

(since 2.5)

The `NCCL_PROTO`

variable defines which protocol(s) NCCL will be allowed
to use.
...
#### Values accepted[](https://docs.nvidia.com#id100)
...
. To specify protocols to exclude
...
on platforms which support LL128, and `LL,Simple`

otherwise.

(since 2.24) The accepted values are expanded to allow more flexibility,
just as decribed for `NCCL_ALGO`

above, allowing the user to specify
protocols for each function.

### NCCL_NVB_DISABLE[](https://docs.nvidia.com#nccl-nvb-disable)

(since 2.11)

Disable intra-node communication through NVLink via an intermediate GPU.

#### Value accepted[](https://docs.nvidia.com#id101)

Default is 0, set to 1 to disable this mechanism.

### NCCL_PXN_DISABLE[](https://docs.nvidia.com#nccl-pxn-disable)

(since 2.12)

Disable inter-node communication using a non-local NIC, using NVLink and an intermediate GPU.

#### Value accepted[](https://docs.nvidia.com#id102)

Default is 0, set to 1 to disable this mechanism.

### NCCL_P2P_PXN_LEVEL[](https://docs.nvidia.com#nccl-p2p-pxn-level)

(since 2.12)

Control in which cases PXN is used for send/receive operations.

#### Value accepted[](https://docs.nvidia.com#id103)
...
Using multiple GPUs per process with CUDA graph capture may result in deadlocks under certain conditions. See [Using NCCL with CUDA Graphs](https://docs.nvidia.com/usage/cudagraph.html#using-nccl-with-cuda-graphs) for details.
...
#### Value accepted[](https://docs.nvidia.com#id107)

0 or 1. Default value is 1 (enabled).

### NCCL_LOCAL_REGISTER[](https://docs.nvidia.com#nccl-local-register)

(since 2.19)

Enable user local buffer registration when users explicitly call *ncclCommRegister*.

#### Value accepted[](https://docs.nvidia.com#id108)

0 or 1. Default value is 1 (enabled).

### NCCL_LEGACY_CUDA_REGISTER[](https://docs.nvidia.com#nccl-legacy-cuda-register)
...
#### Value accepted[](https://docs.nvidia.com#id109)

0 or 1. Default value is 0 (disabled).

### NCCL_WIN_ENABLE[](https://docs.nvidia.com#nccl-win-enable)

(since 2.27)

Enable window memory registration.

#### Value accepted[](https://docs.nvidia.com#id110)

0 or 1. Default value is 1 (enabled).

### NCCL_SET_STACK_SIZE[](https://docs.nvidia.com#nccl-set-stack-size)
...
#### Value accepted[](https://docs.nvidia.com#id111)

0 or 1. Default value is 0 (disabled).

### NCCL_GRAPH_MIXING_SUPPORT[](https://docs.nvidia.com#nccl-graph-mixing-support)
...
Using a NCCL communicator (or split-shared communicators) from parallel graph launches, where parallel means on different streams without dependencies that would serialize their execution.

Launching a non-captured NCCL collective during an outstanding graph launch that uses the same communicator (or split-shared communicators), regardless of stream ordering.
...
#### Value accepted[](https://docs.nvidia.com#id113)

0 or 1. Default is 1 (enabled).

### NCCL_GRAPH_STREAM_ORDERING[](https://docs.nvidia.com#nccl-graph-stream-ordering)
...
together with **graph mixing** (communicator
`graphUsageMode=2`

; see [ncclConfig_t](https://docs.nvidia.com/api/types.html#ncclconfig)) is **not supported**. If stream
...
(and note that [NCCL_GRAPH_MIXING_SUPPORT](https://docs.nvidia.com#nccl-graph-mixing-support)
`1`

forces `graphUsageMode=2`

at init, overriding an explicit lower mode).
Workloads that require mixing must keep the default `1`

. The same rule applies
to per-communicator `graphStreamOrdering`
...
field in [ncclConfig_t](https://docs.nvidia.com/api/types.html#ncclconfig). When that
field is `0`

or `1`

, it overrides `NCCL_GRAPH_STREAM_ORDERING`
...
for a communicator—see `graphStreamOrdering`

for details.

Application responsibilities

With `NCCL_GRAPH_STREAM_ORDERING=0`
...
), NCCL stops enforcing
device-side serialization of communication kernels on the graph-capture
path. The application must then guarantee:

**Serialization on the GPU.**NCCL operations must not overlap: at most one NCCL operation may execute on a given GPU at a time.**Scope.**The rule applies across communicators, across different captured graphs, and between captured and uncaptured NCCL work. The ordering must hold at replay / execution, not only as expressed at capture time.**How to satisfy it.**The simplest approach is to enqueue**all**NCCL operations on the**same CUDA stream**. Equivalent serialization can be achieved with device-wide synchronization and/or CUDA event dependencies between streams.
...
#### Value accepted[](https://docs.nvidia.com#id115)

0 or 1. Default is 1 (enabled).

### NCCL_RMA_EAGER_INIT[](https://docs.nvidia.com#nccl-rma-eager-init)

(since 2.31)

Controls when the collective one-sided RMA signal setup is initialized. When set
to 0 (default), it is initialized at the first [ ncclCommWindowRegister()](https://docs.nvidia.com/api/comms.html#c.ncclCommWindowRegister).
Set to 1 to initialize it at communicator-init time instead; this is required if
the communicator issues

[or](https://docs.nvidia.com/api/p2p.html#c.ncclSignal)

`ncclSignal()`

[without first registering a window, which otherwise returns](https://docs.nvidia.com/api/p2p.html#c.ncclWaitSignal)

`ncclWaitSignal()`

`ncclInvalidUsage`

.This can also be set per communicator with the `rmaEagerInit`

field in
[ncclConfig_t](https://docs.nvidia.com/api/types.html#ncclconfig); the environment variable takes precedence when set.

#### Value accepted[](https://docs.nvidia.com#id117)

0 or 1. Default is 0 (init at first window registration).

### NCCL_DMABUF_ENABLE[](https://docs.nvidia.com#nccl-dmabuf-enable)
...
#### Value accepted[](https://docs.nvidia.com#id118)
...
### NCCL_P2P_NET_CHUNKSIZE[](https://docs.nvidia.com#nccl-p2p-net-chunksize)

(since 2.14)

The `NCCL_P2P_NET_CHUNKSIZE`

controls the size of messages sent through the network for ncclSend/ncclRecv operations.

#### Values accepted[](https://docs.nvidia.com#id119)
...
### NCCL_P2P_LL_THRESHOLD[](https://docs.nvidia.com#nccl-p2p-ll-threshold)

(since 2.14)

The `NCCL_P2P_LL_THRESHOLD`

is the maximum message size that NCCL will use the LL protocol for P2P operations.

#### Values accepted[](https://docs.nvidia.com#id120)

Decimal number. Default is 16384.

### NCCL_ALLOC_P2P_NET_LL_BUFFERS[](https://docs.nvidia.com#nccl-alloc-p2p-net-ll-buffers)

(since 2.14)

`NCCL_ALLOC_P2P_NET_LL_BUFFERS`

instructs communicators to allocate dedicated LL buffers for all P2P network connections. This enables all ranks to use the LL protocol for latency-bound send and receive operations below `NCCL_P2P_LL_THRESHOLD`
...
#### Values accepted[](https://docs.nvidia.com#id121)

0 or 1. Default value is 0 (disabled).

### NCCL_COMM_BLOCKING[](https://docs.nvidia.com#nccl-comm-blocking)

(since 2.14)

The `NCCL_COMM_BLOCKING`

variable controls whether NCCL calls are allowed to block or not. This includes all calls to NCCL, including init/finalize functions, as well as communication functions which may also block due to the lazy initialization of connections for send/receive calls. Setting this environment variable will override the `blocking`

configuration in all communicators (see [ncclConfig_t](https://docs.nvidia.com/api/types.html#ncclconfig)); if not set (undefined), communicator behavior will be determined by the configuration; if not passing configuration, communicators are blocking.

#### Values accepted[](https://docs.nvidia.com#id122)

0 or 1. 1 indicates blocking communicators, and 0 indicates nonblocking communicators. The default value is undefined.

### NCCL_CGA_CLUSTER_SIZE[](https://docs.nvidia.com#nccl-cga-cluster-size)
...
variable will override the `cgaClusterSize`

configuration in all communicators (see [ncclConfig_t](https://docs.nvidia.com/api/types.html#ncclconfig)); if not set
...
#### Values accepted[](https://docs.nvidia.com#id124)

0 to 8. Default value is undefined.

### NCCL_MAX_CTAS[](https://docs.nvidia.com#nccl-max-ctas)

(since 2.17)

Set the maximum number of CTAs that NCCL should use. Setting this environment
variable will override the `maxCTAs`

configuration in all communicators (see
[ncclConfig_t](https://docs.nvidia.com/api/types.html#ncclconfig)); if not set (undefined), maximum CTAs will be determined by
...
. If both are set then the most restrictive
value (i.e., the lower value) is used.

#### Values accepted[](https://docs.nvidia.com#id126)

Set to a positive integer value up to 64 (32 prior to 2.25). Default value is undefined.

### NCCL_MIN_CTAS[](https://docs.nvidia.com#nccl-min-ctas)

(since 2.17)

Set the minimum number of CTAs that NCCL should use. Setting this environment
variable will override the `minCTAs`

configuration in all communicators (see
[ncclConfig_t](https://docs.nvidia.com/api/types.html#ncclconfig)); if not set (undefined), minimum CTAs will be determined by
the configuration; if not passing configuration, NCCL will automatically choose
the best value.
...
#### Values accepted[](https://docs.nvidia.com#id128)

Set to a positive integer value up to 64 (32 prior to 2.25). Default value is undefined.

### NCCL_NVLS_ENABLE[](https://docs.nvidia.com#nccl-nvls-enable)

(since 2.17)

Enable the use of NVLink SHARP (NVLS). NVLink SHARP is available in third-generation NVSwitch systems (NVLink4) with Hopper and later GPU architectures, allowing collectives such as `ncclAllReduce`

to be offloaded to the NVSwitch domain.
The default value is 2.

#### Values accepted[](https://docs.nvidia.com#id129)
...
### NCCL_IB_MERGE_NICS[](https://docs.nvidia.com#nccl-ib-merge-nics)
...
#### Values accepted[](https://docs.nvidia.com#id130)

Default is 1 (enabled), define and set to 0 to disable NIC merging

### NCCL_NET_MERGE_POLICY[](https://docs.nvidia.com#nccl-net-merge-policy)
...
#### Values accepted[](https://docs.nvidia.com#id131)

`ALL`

: (Default) Merge all the devices at the given merge level (see NCCL_NET_MERGE_LEVEL) regardless of their rail assignment.

`RAIL`

: Merge only the devices at the given merge level (see NCCL_NET_MERGE_LEVEL) that share the same rail assignment. All devices with undefined rail assignment (`railId == -1`

) will not be merged.

### NCCL_MNNVL_ENABLE[](https://docs.nvidia.com#nccl-mnnvl-enable)

(since 2.21)

Enable NCCL to use Multi-Node NVLink (MNNVL) when available. If the system or driver are not Multi-Node NVLink capable then MNNVL will automatically be disabled. This feature also requires NCCL CUMEM support (`NCCL_CUMEM_ENABLE`
...
#### Values accepted[](https://docs.nvidia.com#id132)
...
### NCCL_MNNVL_UUID[](https://docs.nvidia.com#nccl-mnnvl-uuid)
...
#### Values accepted[](https://docs.nvidia.com#id133)

64-bit integer value.

### NCCL_MNNVL_CLIQUE_ID[](https://docs.nvidia.com#nccl-mnnvl-clique-id)
...
#### Values accepted[](https://docs.nvidia.com#id134)

32-bit integer value.

### NCCL_RAS_ENABLE[](https://docs.nvidia.com#nccl-ras-enable)

(since 2.24)

Enable NCCL’s reliability, availability, and serviceability (RAS) subsystem, which can be used to query the health of
NCCL jobs during execution (see [RAS](https://docs.nvidia.com/troubleshooting/ras.html)).

#### Values accepted[](https://docs.nvidia.com#id135)

Default is 1 (enabled); define and set to 0 to disable RAS.

### NCCL_RAS_ADDR[](https://docs.nvidia.com#nccl-ras-addr)
...
cases, each job should be started with a different value (e.g., `localhost:12345`
...
is normally used, only those with access to the nodes where the job is running can connect to the socket.
...
#### Values accepted[](https://docs.nvidia.com#id136)

Default is `localhost:28028`

. Either a host name or an IP address can be used for the first part; an IPv6 address
needs to be enclosed in square brackets (e.g., `[::1]`

).

### NCCL_RAS_TIMEOUT_FACTOR[](https://docs.nvidia.com#nccl-ras-timeout-factor)
...
the timeouts and are primarily intended for testing. The `ncclras`

client applies the same factor to its default
socket timeouts.

#### Values accepted[](https://docs.nvidia.com#id137)

Default is 1. A positive floating-point number. Values less than or equal to 0 are ignored and replaced with 1.

### NCCL_LAUNCH_ORDER_IMPLICIT[](https://docs.nvidia.com#nccl-launch-order-implicit)
...
For per-communicator configuration, see the `launchOrderImplicit`

field in [ncclConfig_t](https://docs.nvidia.com/api/types.html#ncclconfig). If this environment variable is set, it overrides
that communicator config field before initialization.

#### Values accepted[](https://docs.nvidia.com#id139)

Default is 0 (disabled); set to 1 to enable.

### NCCL_LAUNCH_RACE_FATAL[](https://docs.nvidia.com#nccl-launch-race-fatal)
...
#### Values accepted[](https://docs.nvidia.com#id140)

Default is 1 (enabled); set to 0 to disable.

### NCCL_IPC_USE_ABSTRACT_SOCKET[](https://docs.nvidia.com#nccl-ipc-use-abstract-socket)
...
#### Values accepted[](https://docs.nvidia.com#id141)

Default is 1 (enabled); set to 0 to disable.

### NCCL_SYM_GIN_KERNELS_ENABLE[](https://docs.nvidia.com#nccl-sym-gin-kernels-enable)

(since 2.29U1)

Enable use of symmetric kernels that use GIN for network communication.

#### Values accepted[](https://docs.nvidia.com#id142)
