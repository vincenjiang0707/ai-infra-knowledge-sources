...
## Logging Environment Variables[](https://docs.nvidia.com#logging-environment-variables)
...
Description |
|---|---|
Sets verbosity: VERSION, WARN, ATTN, INFO, TRACE |
|
Adds individual logging levels |
|
...
## Logging Levels[](https://docs.nvidia.com#logging-levels)
...
Verify installation |
WARN |
Errors returned by NCCL |
Production baseline |
ATTN |
Noteworthy messages that are not errors |
...
when diagnosing production jobs that need noteworthy
operational notices without the volume of `INFO`

output. It includes all
`WARN`

messages.

### Setting the Logging Level[](https://docs.nvidia.com#setting-the-logging-level)
...
### Adding Logging Levels[](https://docs.nvidia.com#adding-logging-levels)

`NCCL_DEBUG_LEVELS`

adds selected levels to the inclusive `NCCL_DEBUG`
...
releases need to retain a `WARN`
...
Here, `NCCL_DEBUG=WARN`

selects VERSION and WARN, while
`NCCL_DEBUG_LEVELS=ATTN`

selects ATTN. Newer releases therefore emit
...
### Example Output[](https://docs.nvidia.com#example-output)
...
## Filtering by Subsystem[](https://docs.nvidia.com#filtering-by-subsystem)

When using `NCCL_DEBUG=INFO`

or `NCCL_DEBUG=TRACE`

, output can be filtered to include specific
subsystems using `NCCL_DEBUG_SUBSYS`

. This helps focus on relevant information without
being overwhelmed by unrelated messages.

### Basic Usage[](https://docs.nvidia.com#basic-usage)
...
### Available Subsystems[](https://docs.nvidia.com#available-subsystems)
...
The default subsystems (when `NCCL_DEBUG_SUBSYS`

is not set) are `INIT,BOOTSTRAP,ENV`

.

### Example Output by Subsystem[](https://docs.nvidia.com#example-output-by-subsystem)
...
## Logging to Files[](https://docs.nvidia.com#logging-to-files)

For multi-process jobs, logging to the terminal can be overwhelming. It is advisable to use `NCCL_DEBUG_FILE`

to write logs to separate files per process and hostname, making it easier to isolate and analyze issues on specific ranks:
...
- Replaced with the hostname`%p`

- Replaced with the process ID (PID)

This creates files like `/tmp/nccl_node-01_12345.log`

for each process.

Note: Ensure the filename pattern is unique across all processes to avoid file corruption.

## Timestamp Configuration[](https://docs.nvidia.com#timestamp-configuration)

NCCL log messages can include timestamps for timing analysis.

### Timestamp Format[](https://docs.nvidia.com#timestamp-format)

Use `NCCL_DEBUG_TIMESTAMP_FORMAT`

to customize the timestamp format (uses strftime syntax):
...
### Timestamp Levels[](https://docs.nvidia.com#timestamp-levels)
...
## Common Debugging Scenarios[](https://docs.nvidia.com#common-debugging-scenarios)

### Diagnosing Initialization Hangs[](https://docs.nvidia.com#diagnosing-initialization-hangs)
...
### Investigating Network Issues[](https://docs.nvidia.com#investigating-network-issues)
...
### Understanding Topology Detection[](https://docs.nvidia.com#understanding-topology-detection)
...
### Debugging Performance Issues[](https://docs.nvidia.com#debugging-performance-issues)
...
### Tracing NCCL API Calls[](https://docs.nvidia.com#tracing-nccl-api-calls)
...
### Full Debugging Session[](https://docs.nvidia.com#full-debugging-session)
