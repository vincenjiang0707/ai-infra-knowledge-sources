...
# Networking Troubleshooting[](https://docs.nvidia.com#networking-troubleshooting)

## Networking issues[](https://docs.nvidia.com#networking-issues)

### IP network interfaces[](https://docs.nvidia.com#ip-network-interfaces)
...
### IP ports[](https://docs.nvidia.com#ip-ports)
...
### NIC-level diagnostics[](https://docs.nvidia.com#nic-level-diagnostics)
...
configuration (link layer, link rate, and active state). The `ibstat`

and
`ibstatus`

tools can be used for a quick sanity check:
...
)Physical state indicates link up (often

`LinkUp`

)The expected link layer is in use (for example,

`InfiniBand`

versus`Ethernet`

for RoCE setups) and is consistent across all NICs used for transportThe reported

`Rate`

matches what you expect for your fabric, with no unexpected downshift

#### Bandwidth testing with `ib_write_bw`

[](https://docs.nvidia.com#bandwidth-testing-with-ib-write-bw)

If no obvious network issues are present, `ib_write_bw`
...
with the HCA device name (for example, `mlx5_0`
...
results with GPU-memory results. Depending on how `perftest`
...
builds. Check `ib_write_bw --help`

on your system for the exact
GPU-memory options supported by that build before comparing results.

#### Latency testing with `ib_write_lat`

[](https://docs.nvidia.com#latency-testing-with-ib-write-lat)
...
#### RDMA protocol statistics[](https://docs.nvidia.com#rdma-protocol-statistics)

`rdma statistic`

can help identify retransmissions or packet-loss-like
...
Look for counters such as `rnr_nak_retry_err`
...
, or `local_ack_timeout_err`

increasing while a test
...
[](https://docs.nvidia.com#link-health-with-mlxlink)
...
### InfiniBand[](https://docs.nvidia.com#infiniband)

#### Subnet Manager[](https://docs.nvidia.com#subnet-manager)
...
fails or shows no SM, ensure the SM is running on at least one
node (for example, the `opensm`

service).

#### Port error counters[](https://docs.nvidia.com#port-error-counters)

Use `perfquery`

to check for link errors, symbol errors, or packet discards
...
, may indicate hardware
or cabling problems.

#### Connectivity testing[](https://docs.nvidia.com#connectivity-testing)

If `ibstat`

and `ibstatus`

do not indicate issues but a connectivity
problem is suspected, `ibping`

can be used to test connectivity between
nodes. `ibping`

requires running a server on the remote node first:
...
#### Comprehensive diagnostics[](https://docs.nvidia.com#comprehensive-diagnostics)
...
one specific rail, run `ibdiagnet`

and inspect the generated report for
...
#### Troubleshooting NCCL errors[](https://docs.nvidia.com#troubleshooting-nccl-errors)
...
configuration file or equivalent on your
Linux distribution. Note that changes to `limits.conf`
...
### RDMA over Converged Ethernet (RoCE)[](https://docs.nvidia.com#rdma-over-converged-ethernet-roce)

RoCE fabrics require different diagnostic tools than InfiniBand to diagnose inter-node problems.

#### Port error counters[](https://docs.nvidia.com#id1)

You can use `ethtool`

to check port counters:
...
#### Congestion indicators[](https://docs.nvidia.com#congestion-indicators)
...
#### Connectivity testing[](https://docs.nvidia.com#id2)

If port-level checks do not indicate any issues but a connectivity problem is
still suspected, `rping`

can be used to test connectivity between nodes. Note
that `rping`

requires a server to be running on the remote node first:
...
#### Troubleshooting NCCL errors[](https://docs.nvidia.com#id3)
...
and then set `NCCL_IB_GID_INDEX`

to the GID index for the RoCE v2 GID. With
NCCL 2.21 and later releases, this environment variable should *not* be set.

Users may also need to set `NCCL_IB_TC`

when using RoCE-based fabrics. Refer
