source: https://docs.nvidia.com/dynamo/zh-CN/dev/knowledge-base/modular-components/router/standalone-indexer
...
counter includes
both device- and lower-tier events. See [KV Indexer Metrics](https://docs.nvidia.com/dynamo/dev/reference/observability/metrics-catalog#kv-indexer-metrics).
...
for an existing `instance_id`

and `dp_rank`

returns an error;
it does not replace the listener or reset its sequence watermark. For worker restarts,
see [Worker Restarts](https://docs.nvidia.com/dynamo/dev/knowledge-base/modular-components/router/standalone-indexer#worker-restarts).
...
: per-instance, per-tier breakdown aligned with[Mooncake RFC #1403](https://github.com/kvcache-ai/Mooncake/issues/1403). See[Per-instance tier breakdown](https://docs.nvidia.com/dynamo/dev/knowledge-base/modular-components/router/standalone-indexer#per-instance-tier-breakdown)below.
...
A ZMQ reconnect does not identify a new worker or cache-event publisher lifetime. If the publisher restarts with its sequence reset, an existing listener retains its watermark and discards batches at or below it. Previously indexed ownership can also remain stale.

On a worker or publisher restart, call `/unregister`

and wait for the response before
calling `/register`

with the current worker metadata. For a whole-worker restart,
omit `dp_rank`

from `/unregister`

to remove all ranks. Apply this lifecycle change to
...
against in-flight old events. Registration success and listener status `active`
...
[Gap Detection and Replay](https://docs.nvidia.com/dynamo/dev/knowledge-base/modular-components/router/standalone-indexer#gap-detection-and-replay).

The [standalone selector](https://docs.nvidia.com/dynamo/dev/knowledge-base/modular-components/router/standalone-selection#worker-restarts) has a different
...
address needs `DELETE /workers/{worker_id}`

before the new `POST /workers`
