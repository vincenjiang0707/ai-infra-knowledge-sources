source: https://docs.nvidia.com/dynamo/zh-CN/v1.2.1/user-guides/agents/priority-scheduling
...
does not propagate to backend
engine scheduling; use `priority`
...
controls when the router starts holding requests. A
...
, which uses the priority value as a positive
...
in client code for vLLM. If a test
...
, GPU preemption, or a hard admission
...
values on
individual requests. For AIPerf, use a version with per-request `extra`

payload
support. Older AIPerf versions may only support global `--extra-inputs`
