source: https://docs.nvidia.com/dynamo/zh-CN/v1.2.1/backends/sg-lang/reference-guide
...
on the frontend. See [SGLang Chat Processor](https://docs.nvidia.com/dynamo/v1.2.1/backends/sg-lang/frontend-processor-fallback) for architecture and usage.
...
For details on the cancellation architecture, see [Request Cancellation](https://docs.nvidia.com/dynamo/v1.2.1/user-guides/fault-tolerance/request-cancellation).
...
Health checks are registered with the Dynamo runtime and called by the frontend or Kubernetes liveness probes. See [Health Checks](https://docs.nvidia.com/dynamo/v1.2.1/user-guides/observability-local/health-checks) for the broader health check architecture.
...
For metric details, see [SGLang Observability](https://docs.nvidia.com/dynamo/v1.2.1/backends/sg-lang/observability). For visualization setup, see [Prometheus + Grafana](https://docs.nvidia.com/dynamo/v1.2.1/user-guides/observability-local/prometheus-grafana-setup).

### KV Events

When configured with `--kv-events-config`

, workers publish KV cache events (block creation/deletion) for the [KV-aware router](https://docs.nvidia.com/dynamo/v1.2.1/components/router). Events are published via ZMQ from SGLang’s scheduler and relayed through Dynamo’s event plane.
...
: All deployment patterns[Examples](https://docs.nvidia.com/dynamo/v1.2.1/backends/sg-lang/examples): P/D architecture and KV transfer[Disaggregation](https://docs.nvidia.com/dynamo/v1.2.1/backends/sg-lang/disaggregation): LLM, image, and video diffusion models[Diffusion](https://docs.nvidia.com/dynamo/v1.2.1/backends/sg-lang/diffusion): KV-aware routing configuration[Configuration and Tuning](https://docs.nvidia.com/dynamo/v1.2.1/components/router/configuration-and-tuning)