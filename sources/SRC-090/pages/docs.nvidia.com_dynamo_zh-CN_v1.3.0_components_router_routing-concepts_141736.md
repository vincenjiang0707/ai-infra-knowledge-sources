source: https://docs.nvidia.com/dynamo/zh-CN/v1.3.0/components/router/routing-concepts
lastmod: 2026-09-23T23:30:39.914Z

# Routing Concepts

This page explains how the Dynamo router evaluates workers, chooses a target, and fits into the request path. For CLI flags and tuning knobs, see [Configuration and Tuning](https://docs.nvidia.com/dynamo/v1.3.0/components/router/configuration-and-tuning).
...
can reduce the device-local portion when a cache-rich worker has excess active prefill load. The router selects the lowest-cost eligible worker. For exact tuning behavior, see [Configuration and Tuning](https://docs.nvidia.com/dynamo/v1.3.0/components/router/configuration-and-tuning#tuning-guidelines).
...
For the flags that enable these models, see [Configuration and Tuning](https://docs.nvidia.com/dynamo/v1.3.0/components/router/configuration-and-tuning).
...
Before scoring, the router filters candidates by request allow-lists, exact pins, DP-rank bounds, required taints, and busy-threshold overload state. For those hard eligibility rules, see [Router Filtering](https://docs.nvidia.com/dynamo/v1.3.0/components/router/router-filtering.md).

When requests wait in policy-class queues, weighted
[Deficit Round Robin Queue Scheduling](https://docs.nvidia.com/dynamo/v1.3.0/components/router/deficit-round-robin-queue-scheduling) selects the
...
For detailed CLI arguments and advanced configuration options, see [Configuration and Tuning](https://docs.nvidia.com/dynamo/v1.3.0/components/router/configuration-and-tuning).
...
For benchmarking KV router performance, see the [KV Router A/B Benchmarking Guide](https://docs.nvidia.com/dynamo/v1.3.0/benchmarks/kv-router-ab-testing).
For custom routing logic and advanced patterns, see [Routing Patterns](https://docs.nvidia.com/dynamo/v1.3.0/components/router/router-examples#routing-patterns).
