source: https://docs.nvidia.com/dynamo/zh-CN/knowledge-base/modular-components/router/deficit-round-robin
...
- If none of its lane heads can currently dispatch, retain its deficit but add no credit.
- If existing deficit covers the selected candidate’s cost, dispatch it without adding another quantum.
- Otherwise, add one quantum and dispatch if the candidate is now affordable.
- If the candidate remains unaffordable, continue to the next class.
...
configured classes and `L`

exact-worker lane heads that must be
inspected or rechecked, arbitration is `O(C + L log L)`
...
[Configuration and Tuning](https://docs.nvidia.com/dynamo/knowledge-base/modular-components/router/configuration-and-tuning#policy-class-queues). See
the tested [sample policy](https://github.com/ai-dynamo/dynamo/blob/v1.4.2/examples/router/policy-class-queues.yaml)
...
admits four 512-token requests before sustained
prefill pressure causes later requests to enter the policy queues. While both
queues remain backlogged, the `512:128`

quantum ratio gives premium four times
...
Send the requested policy family in the `x-dynamo-meta-policy-class`
...
[Policy-Class Queues](https://docs.nvidia.com/dynamo/knowledge-base/modular-components/router/configuration-and-tuning#policy-class-queues) for the
...
. Across four fresh CPU Mocker runs, the test observed:
...
split. Equal request costs make the queued token-service ratio visible
directly in request completions; with varied request sizes, `quantum`
