source: https://docs.nvidia.com/dynamo/zh-CN/v1.2.1/user-guides/observability-local/metrics
...
**For creating custom metrics**, see the [Metrics Developer Guide](https://docs.nvidia.com/dynamo/v1.2.1/user-guides/observability-local/metrics-developer-guide).
...
) - Component-specific metrics**Engine Metrics**(Pass-through) - Backend engines expose their own metrics:[vLLM](https://docs.nvidia.com/dynamo/v1.2.1/backends/v-llm/observability)(`vllm:*`

),[SGLang](https://docs.nvidia.com/dynamo/v1.2.1/backends/sg-lang/observability)(`sglang:*`

),[TensorRT-LLM](https://docs.nvidia.com/dynamo/v1.2.1/backends/tensor-rt-llm/observability)(`trtllm_*`
...
). See[Component Error Types](https://docs.nvidia.com/dynamo/v1.2.1/user-guides/observability-local/metrics#component-error-types).`dynamo_component_response_bytes_total`
...
**Important:** The frontend and backend workers are separate components that expose metrics on different ports. See [Backend Component Metrics](https://docs.nvidia.com/dynamo/v1.2.1/user-guides/observability-local/metrics#backend-component-metrics) for backend metrics.
...
). See[Stage and phase labels](https://docs.nvidia.com/dynamo/v1.2.1/user-guides/observability-local/metrics#stage-and-phase-labels)below.`dynamo_frontend_inflight_requests`
...
For router deployment modes, see the [Router Guide](https://docs.nvidia.com/dynamo/v1.2.1/user-guides/kv-cache-aware-routing). For router flags and tuning, see [Configuration and Tuning](https://docs.nvidia.com/dynamo/v1.2.1/components/router/configuration-and-tuning).
...
Tracks KV cache events applied to the router’s radix tree index. Only appears when `--router-kv-overlap-score-credit`
...
For the full list of metrics, configuration options, and architecture details, see the upstream [NIXL Telemetry documentation](https://github.com/ai-dynamo/nixl/blob/main/docs/telemetry.md) and [Prometheus exporter README](https://github.com/ai-dynamo/nixl/blob/main/src/plugins/telemetry/prometheus/README.md). For Kubernetes, see [Enable NIXL Telemetry](https://docs.nvidia.com/dynamo/v1.2.1/kubernetes-deployment/operate/observability/metrics#enable-nixl-telemetry-optional).

## Related Documentation

[Distributed Runtime Architecture](https://docs.nvidia.com/dynamo/v1.2.1/design-docs/distributed-runtime)[Dynamo Architecture Overview](https://docs.nvidia.com/dynamo/v1.2.1/design-docs/overall-architecture)[Backend Guide](https://docs.nvidia.com/dynamo/v1.2.1/backends/custom-backend/python-workers-lower-level)[Forward Pass Metrics (SGLang)](https://docs.nvidia.com/dynamo/v1.2.1/backends/sg-lang/observability#forward-pass-metrics-fpm)— Per-iteration scheduler telemetry via ZMQ/NATS for planner-driven scaling (intended architecture; not available in the 1.2.0 SGLang runtime image)[Forward Pass Metrics RFC](https://docs.nvidia.com/dynamo/v1.2.1/user-guides/proposals/vllm-rfc-forward-pass-metrics.md)- Design rationale for per-iteration metrics