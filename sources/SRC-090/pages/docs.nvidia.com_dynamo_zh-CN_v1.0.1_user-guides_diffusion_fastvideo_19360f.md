source: https://docs.nvidia.com/dynamo/zh-CN/v1.0.1/user-guides/diffusion/fastvideo
...
Dynamo also supports diffusion through built-in backends: [SGLang Diffusion](https://docs.nvidia.com/dynamo/v1.0.1/user-guides/diffusion/sg-lang-diffusion) (LLM diffusion, image, video), [vLLM-Omni](https://docs.nvidia.com/dynamo/v1.0.1/user-guides/diffusion/v-llm-omni) (text-to-image, text-to-video), and [TRT-LLM Video Diffusion](https://docs.nvidia.com/dynamo/v1.0.1/user-guides/diffusion/trt-llm-diffusion). See the [Diffusion Overview](https://docs.nvidia.com/dynamo/v1.0.1/user-guides/diffusion) for the full support matrix.
...
are enabled by default for maximum throughput.**Response format:**Returns one complete MP4 payload per request as`data[0].b64_json`
...
This example is optimized for **NVIDIA B200/B300** GPUs (CUDA arch 10.0) with FP4 quantization and flash-attention. It can run on other GPUs (H100, A100, etc.) by passing `--disable-optimizations`

to `worker.py`

, which disables FP4 quantization, `torch.compile`

, and switches the attention backend from FLASH_ATTN to TORCH_SDPA. Expect lower performance but broader compatibility.
...
When using Kubernetes, mount a shared Hugging Face cache PVC (see [Kubernetes Deployment](https://docs.nvidia.com/dynamo/v1.0.1/user-guides/diffusion/fastvideo#kubernetes-deployment)) so model weights are downloaded once and reused across pod restarts.
...
. See the [Docker Image Build](https://docs.nvidia.com/dynamo/v1.0.1/user-guides/diffusion/fastvideo#docker-image-build) section for build time expectations.
...
flag (not a `dynamo.frontend`

flag), so pass it through `WORKER_EXTRA_ARGS`
...
If this is the first request after startup, expect it to take longer while warmup completes. See [Warmup Time](https://docs.nvidia.com/dynamo/v1.0.1/user-guides/diffusion/fastvideo#warmup-time) for details.
...
[vLLM-Omni Text-to-Video](https://docs.nvidia.com/dynamo/v1.0.1/user-guides/diffusion/v-llm-omni#text-to-video)— vLLM-Omni video generation via`/v1/videos`

[vLLM-Omni Text-to-Image](https://docs.nvidia.com/dynamo/v1.0.1/user-guides/diffusion/v-llm-omni#text-to-image)— vLLM-Omni image generation[SGLang Video Generation](https://docs.nvidia.com/dynamo/v1.0.1/user-guides/diffusion/sg-lang-diffusion#video-generation)— SGLang video generation worker[SGLang Image Diffusion](https://docs.nvidia.com/dynamo/v1.0.1/user-guides/diffusion/sg-lang-diffusion#image-diffusion)— SGLang image diffusion worker[TRT-LLM Video Diffusion](https://docs.nvidia.com/dynamo/v1.0.1/user-guides/diffusion/trt-llm-diffusion#quick-start)— TensorRT-LLM video diffusion quick start[Diffusion Overview](https://docs.nvidia.com/dynamo/v1.0.1/user-guides/diffusion)— Full backend support matrix