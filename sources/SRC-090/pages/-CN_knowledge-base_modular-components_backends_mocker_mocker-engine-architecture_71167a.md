source: https://docs.nvidia.com/dynamo/zh-CN/knowledge-base/modular-components/backends/mocker/mocker-engine-architecture
lastmod: 2026-09-23T23:30:39.914Z

# Mocker Engine Architecture

The mocker is organized into several cooperating components that mirror the internal architecture of production LLM inference engines. The scheduler (vLLM-style and SGLang-style variants) and KV block manager live inside the engine core. Multi-engine behavior — KV transfer/offloading simulation, KV router simulation, planner simulation — is added by the DynoSim run harness on top of multiple engine cores; see [DynoSim Architecture](https://docs.nvidia.com/dynamo/knowledge-base/concepts/simulation/dyno-sim-architecture) for the component-level design and [offline replay internals](https://github.com/ai-dynamo/dynamo/blob/v1.4.2/lib/mocker/src/replay/offline/README.md) for implementation details.

For task-oriented instructions, see [Simulate a Kubernetes Deployment](https://docs.nvidia.com/dynamo/kubernetes/operations/dynosim/live-simulation-with-mocker) or [Simulate a Local Deployment](https://docs.nvidia.com/dynamo/cli/operations/dynosim/mocker-live-simulation); for the command-line flags referenced throughout this page, see the [Mocker CLI Reference](https://docs.nvidia.com/dynamo/reference/components/mocker-cli-reference).
...
The mocker’s KV block manager is built on [ kvbm-logical::BlockManager<G1>](https://github.com/ai-dynamo/dynamo/tree/v1.4.2/lib/kvbm-logical), the same logical block manager the real Dynamo runtime uses. The mocker wraps it in

[and translates its own](https://github.com/ai-dynamo/dynamo/blob/release/1.4.1/lib/mocker/src/kv_manager/kvbm_backend.rs)
...
in [ lib/mocker/src/common/protocols.rs](https://github.com/ai-dynamo/dynamo/blob/v1.4.2/lib/mocker/src/common/protocols.rs):

(default) — parent-chain aware: evicts leaf blocks first, preserving shared prefix chains. Subsumes the preemption-priority behavior the hand-rolled`Lineage`
...
All three give the same “suffix blocks evicted before shared prefixes” outcome that the previous evictor was designed to produce; `Lineage`
