source: https://docs.nvidia.com/dynamo/zh-CN/parsing/troubleshooting-tool-calls
...
Recipe applies to non-streaming requests against Dynamo’s OpenAI `/v1/chat/completions`

endpoint. For multi-channel reasoning models (`harmony`
...
), the recipe recovers only the assistant-content channel; the reasoning channel is not surfaced in `logprobs.content`

. If the worker is the SGLang backend, `logprobs: true`

is rejected by default because SGLang’s tokenizer manager detokenizes top-k tokens serially, causing latency degradation. Set `DYN_SGL_ALLOW_TOP_LOGPROBS=1`

in the SGLang worker’s `env:`

to opt in, then remove it and re-apply once the repro is captured. Tracked at [sgl-project/sglang#24447](https://github.com/sgl-project/sglang/pull/24447).

## The request

Add `"logprobs": true`

to your failing request. Port-forward the Frontend Service first (`kubectl port-forward svc/<deployment-name>-frontend 8000:8000 -n ${NAMESPACE}`
...
[Tool Call Parsing (Dynamo)](https://docs.nvidia.com/dynamo/parsing/tool-call-parsing)— Dynamo-native parser names and request examples[Chat Processors](https://docs.nvidia.com/dynamo/parsing/chat-processors)—`--dyn-chat-processor`

and the engine-fallback path to vLLM and SGLang parsers[Frontend Configuration Reference](https://docs.nvidia.com/dynamo/reference/components/frontend-configuration)— full CLI flag reference for the frontend and worker