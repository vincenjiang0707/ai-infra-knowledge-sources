source: https://docs.nvidia.com/dynamo/zh-CN/v-0-9-1/design-docs/router-design
...
balances cache hit optimization against load distribution- Higher weights favor cache reuse (improving TTFT), while lower weights prioritize even load distribution (improving ITL)
...
**Worker 2: cost = 1.0 * 5 + 5 = 10**(selected - lowest cost)- Worker 3: cost = 1.0 * 2 + 9 = 11
...
**Request tokenization**: The incoming prompt is converted into tokens**Block partitioning**: The token sequence is divided into fixed-size blocks (e.g., 16 or 64 tokens per block)**Block hashing**: Each block of tokens is hashed to create a unique identifier**Cache lookup**:- For each block, the system checks if a matching block already exists in the KV cache
...
**NATS Core with Local Indexer**(`--enable-local-indexer`

on workers): Fire-and-forget pub/sub where workers maintain local radix trees. Router rebuilds state by querying workers on startup. Lower latency, simpler setup.
...
**Best for**: Production deployments requiring durability and multi-replica router consistency**Tradeoffs**: Requires JetStream setup; slightly higher latency due to persistence guarantees

### NATS Core with Local Indexer

When workers are started with `--enable-local-indexer`

, each worker maintains its own local radix tree (local indexer) and publishes events over NATS Core (fire-and-forget pub/sub) instead of JetStream. Each worker assigns monotonically increasing event IDs to its events. The router detects gaps in event sequences and recovers missed events by querying the worker’s local indexer directly.

**Best for**: Lower-latency setups; simpler deployments without JetStream; single-router scenarios**Tradeoffs**: State persists on workers (not centralized); recovery depends on workers being available**Enable with**:`--enable-local-indexer`

flag on workers (vLLM, mocker)
...
[!Note] The router automatically selects the transport mode based on worker configuration. If all connected workers have

`enable_local_indexer=true`

, the router uses NATS Core mode. Otherwise, it uses JetStream mode.
...
: Quick start guide for the KV Router[Router README](https://docs.nvidia.com/dynamo/v-0-9-1/components/router): Configuration, tuning, and production setup[Router Guide](https://docs.nvidia.com/dynamo/v-0-9-1/user-guides/kv-cache-aware-routing): Python API usage and custom routing patterns[Router Examples](https://docs.nvidia.com/dynamo/v-0-9-1/components/router/router-examples): Integrate custom inference engines with KV-aware routing[KV Event Publishing for Custom Engines](https://docs.nvidia.com/dynamo/v-0-9-1/integrations/kv-events-for-custom-engines)