source: https://docs.nvidia.com/dynamo/zh-CN/v1.1.1/user-guides/observability-local/metrics
...
**For visualization setup instructions**, see the [Prometheus and Grafana Setup Guide](https://docs.nvidia.com/dynamo/v1.1.1/user-guides/observability-local/prometheus-grafana-setup).

**For creating custom metrics**, see the [Metrics Developer Guide](https://docs.nvidia.com/dynamo/v1.1.1/user-guides/observability-local/metrics-developer-guide).
...
For visualizing metrics with Prometheus and Grafana, start the observability stack. See [Observability Getting Started](https://docs.nvidia.com/dynamo/v1.1.1/user-guides/observability-local#getting-started-quickly) for instructions.
...
) - Component-specific metrics**Engine Metrics**(Pass-through) - Backend engines expose their own metrics:[vLLM](https://docs.nvidia.com/dynamo/v1.1.1/backends/v-llm/observability)(`vllm:*`

),[SGLang](https://docs.nvidia.com/dynamo/v1.1.1/backends/sg-lang/observability)(`sglang:*`

),[TensorRT-LLM](https://docs.nvidia.com/dynamo/v1.1.1/backends/tensor-rt-llm/observability)(`trtllm_*`
...
). See[Component Error Types](https://docs.nvidia.com/dynamo/v1.1.1/user-guides/observability-local/metrics#component-error-types).`dynamo_component_response_bytes_total`
...
**Important:** The frontend and backend workers are separate components that expose metrics on different ports. See [Backend Component Metrics](https://docs.nvidia.com/dynamo/v1.1.1/user-guides/observability-local/metrics#backend-component-metrics) for backend metrics.
...
). See[Stage and phase labels](https://docs.nvidia.com/dynamo/v1.1.1/user-guides/observability-local/metrics#stage-and-phase-labels)below.`dynamo_frontend_inflight_requests`
...
: Number of cached tokens (prefix cache hits) per request (histogram)`dynamo_frontend_inter_token_latency_seconds`
...
For router deployment modes, see the [Router Guide](https://docs.nvidia.com/dynamo/v1.1.1/user-guides/kv-cache-aware-routing). For router flags and tuning, see [Configuration and Tuning](https://docs.nvidia.com/dynamo/v1.1.1/components/router/configuration-and-tuning).
...
Gauge tracking the number of requests pending in the router’s scheduler queue. Only registered when `--router-queue-threshold`

is set. Labeled by `worker_type`

to distinguish prefill vs. decode queues in disaggregated mode.
...
Tracks KV cache events applied to the router’s radix tree index. Only appears when `--router-kv-overlap-score-weight`

is greater than 0 (default) and workers are publishing KV events. Will not appear if `--router-kv-overlap-score-weight 0`
