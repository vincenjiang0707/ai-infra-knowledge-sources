source: https://docs.nvidia.com/dynamo/zh-CN/v1.3.0/user-guides/dynosim/runs
...
, which prints an AIPerf-style summary table, writes the full report JSON
to disk, and exposes `offline|online`
...
The command keeps the existing `replay`

name for now. The docs use “DynoSim run” for the product
concept: one workload, one simulated configuration, one report.

Unlike normal `dynamo.mocker`

usage, offline mode does not launch workers, register endpoints, or
...
with vLLM, SGLang, or TRT-LLM; multi-engine simulation (`MES`

) covers aggregated multi-worker runs, disaggregated prefill/decode runs, and KV-router runs. The trace collector produces the AIPerf-style summary table, the JSON report, and the per-request timing fields consumed by downstream analysis.
...
See [ lib/mocker/src/replay/offline/README.md](https://docs.nvidia.com/dynamo/v1.3.0/lib/mocker/src/replay/offline/README.md) for offline-harness internals (logical clock, event queue, worker model) and

[Live Simulation with Mocker](https://docs.nvidia.com/dynamo/v1.3.0/user-guides/dynosim/mocker)for engine-core details (scheduler, KV block manager).
...
prints an AIPerf-style summary table to stdout and writes the full
report JSON to disk.
...
trace formats were added after the v1.2.1
release. The `ai-dynamo`

1.2.x wheels on PyPI accept only `--trace-format mooncake`
...
(signed soft-priority hint) - optional
`strict_priority`

(unsigned queue tier; larger values run first) - optional
`policy_class`

(requested policy family or explicit class)
...
affect only KV-router pending-queue ordering when
`--router-mode kv_router`

is active and requests are actually queued. Negative `priority`
...
DynoSim runs also support multi-turn sessions. Use the same `session_id`
...
reads one or more original
`dynamo.request.trace.v1`

JSONL or JSONL.GZ shards directly. Replay derives the
block size from the records and builds the standard or agentic in-memory model
based on `agent_context`

. It does not create an intermediate Mooncake file.

### Agentic Mooncake

`--trace-format agentic_mooncake`

simulates request-level workflow dependencies in addition to the
Mooncake request fields. Each row should contain the normal Mooncake fields plus a stable
`request_id`

. Dependency fields are optional. This is a separate input format for externally
authored Mooncake-compatible traces, not an intermediate format for Dynamo request traces.

Rows with no `wait_for`

use `timestamp`

as their start time. Rows with dependencies wait for every
listed request to complete, then wait `delay + tool_wait_ms`

before dispatch. `branches`

records
child requests spawned by this row, and `prefix_reset`

marks the first row in a session.
...
: the block size used by the simulated engine and router when they re-chunk the synthesized tokens into sequence hashes

Public Mooncake/toolagent traces use `512`

tokens per `hash_id`

, so DynoSim runs should normally
...
is omitted and rejects an explicit mismatch.
...
- either positional trace files (
`--trace-format dynamo`

accepts one or more; other formats require exactly one), or all of`--input-tokens`
...
(release availability differs; see the note in[Input Format](https://docs.nvidia.com/dynamo/v1.3.0/user-guides/dynosim/runs#input-format))`--trace-block-size`
...
(JSON string)`--router-policy-config`

(policy-family/cache-bucket queue YAML path;`DYN_ROUTER_POLICY_CONFIG`

fallback)`--model-name`

(selects an exact model profile from the policy YAML)`--aic-backend`
...
, not as top-level DynoSim CLI flags. `--trace-block-size`
...
sets the startup-only policy YAML path and overrides a
`router_policy_config`

value embedded in `--router-config`

. Requests may provide an optional
`policy_class`

field in Mooncake JSONL rows. A recognized family combines with the
...
profile’s `default_policy_family`

. Use `--model-name`

to select an exact model profile,
...
setting `aic_moe_tp_size`

to the same value as `aic_tp_size`

, with `aic_moe_ep_size=1`
...
For offline disaggregated simulation, the staged JSON must set `worker_type`
...
are simulated as a closed-loop session, where each later turn waits for
...
, not a dedicated top-level CLI flag. In offline mode:
...
, DynoSim predicts one expected prefill duration per admitted request and decays only the oldest active prefill request on each worker
...
For offline disaggregated simulation, the same top-level `--aic-*`
...
works with both trace-file and synthetic workloads- mocker compute-speed knobs such as
`speedup_ratio`

still affect simulated timing when passed via the engine-args JSON for the chosen mode `--arrival-speedup-ratio`
...
only applies to synthetic workloads`--turns-per-session`
...
only apply to synthetic workloads`--extra-engine-args`
...
are JSON strings on the standalone DynoSim CLI- top-level
`--aic-*`

flags are used only for router-side prompt-load modeling; engine timing AIC still belongs in the engine-args JSON - offline mode does not need planner runtime setup, router registration, or external event transport
- trace-file workloads can use different values for
`--trace-block-size`
...
- on Apple Silicon, run the
`aarch64`

wheel natively (`docker run --platform linux/arm64 ...`

); amd64 emulation under Rosetta hits an Apple translation defect that segfaults multi-worker multi-turn runs ([issue #11228](https://github.com/ai-dynamo/dynamo/issues/11228)). To run`linux/amd64`

anyway, disable Docker Desktop’s Rosetta option to fall back to QEMU. Bare`docker run`

defaults to the native arm64 image but silently reuses a previously pulled amd64 image under the same tag; pass`--platform linux/arm64`

explicitly after running amd64 variants
...
Use [Dynamo Benchmarking](https://docs.nvidia.com/dynamo/v1.3.0/user-guides/benchmarking) when:
