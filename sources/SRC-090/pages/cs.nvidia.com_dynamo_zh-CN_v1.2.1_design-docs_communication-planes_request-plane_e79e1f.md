source: https://docs.nvidia.com/dynamo/zh-CN/v1.2.1/design-docs/communication-planes/request-plane
...
) both provide KV routing without NATS, with approximate mode using predicted cache state. - Need for message replay and persistence features
...
See [ examples/backends/vllm/launch/agg_request_planes.sh](https://github.com/ai-dynamo/dynamo/tree/v1.2.1/examples/backends/vllm/launch/agg_request_planes.sh) for a complete working example that demonstrates launching Dynamo with TCP or NATS request planes.
...
**TCP**: Lowest latency due to direct connections and binary serialization**NATS**: Moderate latency due to nats jet stream persistence
