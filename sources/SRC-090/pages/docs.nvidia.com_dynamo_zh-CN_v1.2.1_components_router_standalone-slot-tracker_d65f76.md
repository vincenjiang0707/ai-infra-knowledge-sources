source: https://docs.nvidia.com/dynamo/zh-CN/v1.2.1/components/router/standalone-slot-tracker
...
parameter. `--replica-sync-advertise`

is the externally reachable form of the local
endpoint and prevents exact self-registration. `--replica-sync-peers`
...
for an unknown `(model_name, tenant_id)`
...
The same body is accepted by `POST /deregister_peer`
...
overflow. Workers in the same `(model_name, tenant_id)`
...
. Free is idempotent while the model/tenant tracker exists, including for
