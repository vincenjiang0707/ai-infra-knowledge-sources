source: https://docs.nvidia.com/dynamo/zh-CN/dev/knowledge-base/concepts/simulation/ai-simulate/sweeper/planner-goodput-per-gpu-sweep
...
[router experiment](https://docs.nvidia.com/dynamo/dev/knowledge-base/concepts/simulation/ai-simulate/sweeper/router-end-to-end-latency-sweep), then used Sweeper’s smart sweep to find the
...
The experiment used `Sweeper.run`

with `goal.target = goodput_per_gpu`

. It fixed the deployment,
router, workload, and SLA, and swept only the planner settings using the full open-loop trace. The
configuration below documents the retired `aisimulate==0.1.0.dev1`

API and is not accepted by
AISimulate 0.12.0:
...
policies don’t use`fpm_sampling`
...
The original planner path required the `aic-forward-pass`

binding; a per-throughput-interval
load-predictor sub-sweep ran first (forecast-loss winner pinned per interval). Static and
default-planner baselines ran via `dynamo.replay.run_trace_replay`
...
[Router End-to-End Latency Sweep](https://docs.nvidia.com/dynamo/dev/knowledge-base/concepts/simulation/ai-simulate/sweeper/router-end-to-end-latency-sweep).