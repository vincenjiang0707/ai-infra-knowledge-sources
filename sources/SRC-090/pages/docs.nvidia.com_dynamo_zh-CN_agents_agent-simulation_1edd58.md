source: https://docs.nvidia.com/dynamo/zh-CN/agents/agent-simulation
...
Prompts, responses, and tool arguments are not stored. Enable [ request_payload records](https://docs.nvidia.com/dynamo/reference/observability/request-traces#request_payload) alongside tracing if you need them. Optional tool spans record where time was spent, but replay uses the captured request schedule rather than calling those tools again.
...
files. Stop the frontend, or otherwise allow tracing to flush, before using the files. See [Agent Tracing](https://docs.nvidia.com/dynamo/agents/agent-tracing) for configuration, the record schema, Perfetto conversion, and optional tool-event ingestion.
