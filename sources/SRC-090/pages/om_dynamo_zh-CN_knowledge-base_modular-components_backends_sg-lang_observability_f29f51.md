source: https://docs.nvidia.com/dynamo/zh-CN/knowledge-base/modular-components/backends/sg-lang/observability
...
endpoint or refer to the [official documentation](https://docs.sglang.io/docs/references/production_metrics) for the current list.
...
**Note:** Specific metrics are subject to change between SGLang versions. Always refer to the [official documentation](https://docs.sglang.io/docs/references/production_metrics) or inspect the `/metrics`
...
). These landed inSGLang v0.5.13and ship in the current Dynamo runtime (`sglang==0.5.15`

), so setting`DYN_FORWARDPASS_METRIC_PORT`

enables SGLang-side FPM emission to the NATS event plane via`FpmEventRelay`

. The wire-format contract is guarded by`dynamo/sglang/tests/test_fpm_contract.py`

. On runtimes older than v0.5.13 the module is absent: the Dynamo-side relay still starts and the worker serves normally, but no SGLang-side FPM payloads are emitted.

Forward Pass Metrics provide **per-iteration scheduler telemetry** pushed over ZMQ, giving the [Planner](https://docs.nvidia.com/dynamo/knowledge-base/modular-components/planner/overview) real-time visibility into batch composition, queue depth, and GPU forward pass duration. Unlike Prometheus metrics (which are scraped asynchronously and reflect only the latest gauge value), FPM emits a structured message after every scheduler iteration with the exact batch state.
...
The Planner subscribes to FPM via the NATS event plane. See the [Planner Guide](https://docs.nvidia.com/dynamo/knowledge-base/modular-components/planner/planner-guide) for configuration (`load_adjustment_interval`
...
For the full motivation and design rationale, see the [Forward Pass Metrics RFC](https://docs.nvidia.com/dynamo/knowledge-base/concepts/observability/forward-pass-metrics-rfc.md).
...
For more details on the Tempo/Grafana tracing infrastructure, see the [Local Observability Guide](https://docs.nvidia.com/dynamo/cli/operations/observability#inspect-traces-and-exported-logs).
...
[Metrics Catalog](https://docs.nvidia.com/dynamo/reference/observability/metrics-catalog)- Dynamo runtime metric definitions[Local Observability Guide](https://docs.nvidia.com/dynamo/cli/operations/observability#inspect-traces-and-exported-logs)- Distributed tracing with OpenTelemetry and Tempo[Metrics and Dashboards](https://docs.nvidia.com/dynamo/cli/operations/observability#view-metrics-and-dashboards)- Visualization walkthrough- Dynamo runtime metrics (prefixed with
`dynamo_*`
