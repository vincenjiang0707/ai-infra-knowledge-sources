source: https://docs.nvidia.com/dynamo/zh-CN/knowledge-base/modular-components/profiler/profiler-guide
lastmod: 2026-09-23T23:30:39.914Z

# Profiler Guide

Runs the AI Configurator-driven profiling pipeline that turns a model, SLA targets, and backend into a ready-to-deploy DGD.
...
), SLA targets verified, and gate checks applied (see[Gate Checks](https://docs.nvidia.com/dynamo/knowledge-base/modular-components/profiler/profiler-guide#gate-checks-and-constraints)). -
...
-
**Picking**: The profiler selects the best configuration using one of three modes, determined automatically from the DGDR spec (see[Picking Modes](https://docs.nvidia.com/dynamo/knowledge-base/modular-components/profiler/profiler-guide#picking-modes)). -
...
- Falls back to a naive config (memory-fit TP calculation) only after DGDR accepts the GPU SKU. Fallback sizing depends on AIC system metadata and does not add support for additional GPU SKUs.
...
is unaffected (easy-mode keeps load scaling enabled). See the [Planner Guide](https://docs.nvidia.com/dynamo/knowledge-base/modular-components/planner/planner-guide#optimization-target) for the full explanation of each `optimization_target`
...
See the [Planner Guide](https://docs.nvidia.com/dynamo/knowledge-base/modular-components/planner/planner-guide) for the full `PlannerConfig`
...
The recommended deployment method is through DGDRs. See [Profiler Examples](https://docs.nvidia.com/dynamo/knowledge-base/modular-components/profiler/profiler-examples) for complete DGDR YAML examples covering rapid, thorough, MoE, custom SLA, and override use cases.

#### Container Images

The DGDR `image`

field selects the container image for the profiling job. The
image must contain the profiler code and dependencies. If `image`
...
explicit component value in `spec.overrides.dgd`

takes precedence over the
DGDR-level default.

[Profiler Image Version Compatibility](https://docs.nvidia.com/dynamo/reference/api/kubernetes/dynamo-graph-deployment-request#profiler-image-version-compatibility)
for details.
...
The operator supplies `dgd-apply-overrides`

to Kubernetes profiling jobs when
`overrides.dgd`

is present. For a local run, put the matching binary on `PATH`
...
writes the binary to `GOBIN`

, or to `$(go env GOPATH)/bin`

when
`GOBIN`

is unset. Add that directory to `PATH`

if needed. Alternatively, set
`DYNAMO_DGD_APPLY_OVERRIDES_BIN`

to the binary’s absolute path. The profiler
does not download the binary at runtime. Runs without `overrides.dgd`
...
[Generated DGD Overrides](https://docs.nvidia.com/dynamo/kubernetes/auto-deployment/dgdr-walkthrough#generated-dgd-overrides).

Registry credentials are namespace-scoped. The operator chart’s
`imagePullSecrets`

pull the operator Pod only. A profiling Job that needs
credentials for the operator image must receive them from its ServiceAccount or
from `overrides.profilingJob.template.spec.imagePullSecrets`

in the DGDR namespace.
...
field names consumed by the planner service. See [SLA Planner documentation](https://docs.nvidia.com/dynamo/knowledge-base/modular-components/planner/planner-guide) for full list.
...
, the profiler merges it into each generated
...
and, when thorough profile data is needed,`planner-profile-data-XXXX`

, where the suffix is generated) - Generates optimized DGD configurations
...
With thorough sweeping, profiling still runs against the real backend to collect performance data and, when a consumer needs it, stores it in a generated `planner-profile-data-XXXX`

ConfigMap. With rapid sweeping, the mocker uses AIC performance-model flags instead of a profile-data ConfigMap. Useful for large-scale experiments, testing Planner behavior, and validating configurations.
...
: Generated DGD configuration`planner-profile-data-XXXX`

: Profiling data for Planner and mocker consumers (JSON), with a generated suffix. Only created for thorough sweeping when profile data is needed.
...
analysis. SGLang accepts `output_dir`
...
; vLLM
accepts the optional `profile_prefix`
...
, profiling fails when all AIC experiments return no SLA-feasible configuration. The profiler does not relax the SLA or generate a DGD. - With
`searchStrategy: thorough`

, the profiler selects the closest measured configuration and logs a warning when the sweep produces results but none meet the SLA. Profiling fails if the sweep produces no usable results.
...
[Profiler README](https://docs.nvidia.com/dynamo/knowledge-base/modular-components/profiler/overview)— Quick overview and feature matrix[Profiler Examples](https://docs.nvidia.com/dynamo/knowledge-base/modular-components/profiler/profiler-examples)— Complete DGDR YAML examples[Planner Guide](https://docs.nvidia.com/dynamo/knowledge-base/modular-components/planner/planner-guide)— PlannerConfig reference and scaling modes[DGDR API Reference](https://docs.nvidia.com/dynamo/reference/api/kubernetes/full-api-reference)— Full DGDR specification