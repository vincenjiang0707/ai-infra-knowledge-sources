source: https://docs.nvidia.com/dynamo/zh-CN/dev/cli/operations/observability
lastmod: 2026-09-23T23:30:39.914Z

# Observe a Local Deployment

Use the stack from [Install Observability](https://docs.nvidia.com/dynamo/dev/cli/installation/observability) to inspect a local Dynamo
...
For metric definitions and labels, see the [Metrics Catalog](https://docs.nvidia.com/dynamo/dev/reference/observability/metrics-catalog)
and [Metric Labels](https://docs.nvidia.com/dynamo/dev/reference/observability/metric-labels).
...
[Environment Variables](https://docs.nvidia.com/dynamo/dev/reference/observability/environment-variables). For log formats and
fields, see [Logging Reference](https://docs.nvidia.com/dynamo/dev/reference/observability/logging). For the signal pipeline and
span relationships, see [Observability Architecture](https://docs.nvidia.com/dynamo/dev/knowledge-base/concepts/observability-architecture).
...
behavior, see [Forward Pass Metrics Trace Reference](https://docs.nvidia.com/dynamo/dev/reference/observability/forward-pass-metrics-traces).
For the publication and persistence flow, see
[Observability Architecture](https://docs.nvidia.com/dynamo/dev/knowledge-base/concepts/observability-architecture#forward-pass-metrics-persistence).
...
Save `/tmp/request-trace-prediction.yaml`

with the `dynamo.request.trace.v1`

JSONL or JSONL.GZ
shards:

Add every shard to `traffic.source.paths`

, then run the offline prediction:

No Mooncake conversion is required. AISimulate derives and validates the trace block size. It preserves session dependencies and tool waits when every request has session context and rejects mixed session-aware and context-free trace sets.

For all variables, compatibility aliases, sinks, record schemas, and supported request shapes, see
[Request Trace Reference](https://docs.nvidia.com/dynamo/dev/reference/observability/request-traces).
...
[Health Check Reference](https://docs.nvidia.com/dynamo/dev/reference/observability/health-checks). For the canary lifecycle, see
[Observability Architecture](https://docs.nvidia.com/dynamo/dev/knowledge-base/concepts/observability-architecture#active-worker-health-checks).
