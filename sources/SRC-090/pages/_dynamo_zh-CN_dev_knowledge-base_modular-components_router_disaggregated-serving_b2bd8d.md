source: https://docs.nvidia.com/dynamo/zh-CN/dev/knowledge-base/modular-components/router/disaggregated-serving
...
For the high-level deployment matrix, see [Router Guide](https://docs.nvidia.com/dynamo/dev/knowledge-base/modular-components/router/router-guide). For the router flags used in this setup, see [Configuration and Tuning](https://docs.nvidia.com/dynamo/dev/knowledge-base/modular-components/router/configuration-and-tuning).

If prefill and decode workers span topology domains such as zones or racks, use [Topology-Aware KV Transfer](https://docs.nvidia.com/dynamo/dev/knowledge-base/modular-components/router/topology-aware-kv-transfer) to constrain or bias decode routing toward workers in the selected prefill worker’s transfer domain.
...
returned by prefill. For an advanced reference, see the [Global Router](https://github.com/ai-dynamo/dynamo/tree/main/components/src/dynamo/global_router), which composes local prefill and decode router pools explicitly.
