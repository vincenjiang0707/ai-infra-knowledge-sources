source: https://docs.nvidia.com/dynamo/zh-CN/knowledge-base/concepts/fault-tolerance/request-migration-architecture
lastmod: 2026-09-24T19:58:16.636Z

# Request Migration Architecture

Preserve token state and reissue in-flight requests on healthy workers after a worker failure.
...
The migration limit and max sequence length are configured at the **frontend** level and apply globally to all models served by that frontend. See [Enable migration on the Frontend](https://docs.nvidia.com/dynamo/kubernetes/fault-tolerance/request-migration) in the use-case guide for the flags and environment variables.
...
**Response Tracking**: As each response arrives from the worker, the migration system extracts the newly generated tokens and appends them to the request’s token sequence. This accumulates all tokens that have been generated. -
...
[Request Migration](https://docs.nvidia.com/dynamo/kubernetes/fault-tolerance/request-migration)- How to enable, tune, and reason about migration (use-case guide)[Request Cancellation Architecture](https://docs.nvidia.com/dynamo/knowledge-base/concepts/fault-tolerance/request-cancellation-architecture)- Canceling in-flight requests[Distributed Runtime](https://docs.nvidia.com/dynamo/knowledge-base/concepts/system-architecture/distributed-runtime)- Service discovery architecture that underpins failure detection[Metrics Catalog](https://docs.nvidia.com/dynamo/reference/observability/metrics-catalog#migration)- Migration metrics