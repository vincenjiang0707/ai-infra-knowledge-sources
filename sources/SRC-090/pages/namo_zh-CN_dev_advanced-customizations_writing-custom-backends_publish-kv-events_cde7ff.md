source: https://docs.nvidia.com/dynamo/zh-CN/dev/advanced-customizations/writing-custom-backends/publish-kv-events
lastmod: 2026-09-23T23:30:39.914Z

# Publish KV Events from a Custom Backend

Publish KV events from a custom inference engine so Dynamo’s KV router can track which workers hold each cached prefix. This guide covers direct publishing and relaying an engine’s existing ZeroMQ (ZMQ) event stream.
...
[KV event publishing for unified backends](https://docs.nvidia.com/dynamo/dev/advanced-customizations/writing-custom-backends/writing-unified-backends#python-kv-event-publishing-optional).
...
Events are published over the **Dynamo event plane**, a transport-agnostic pub/sub layer that supports both NATS and ZMQ backends (see [Event Plane](https://docs.nvidia.com/dynamo/dev/knowledge-base/concepts/architecture#event-plane) for details). The `KvEventPublisher`
