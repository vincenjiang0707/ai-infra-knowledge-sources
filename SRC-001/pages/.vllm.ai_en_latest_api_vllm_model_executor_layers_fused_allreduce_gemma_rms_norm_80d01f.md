source: https://docs.vllm.ai/en/latest/api/vllm/model_executor/layers/fused_allreduce_gemma_rms_norm/
lastmod: 2026-09-23

Manual fusion of tensor-parallel all-reduce with the following GemmaRMSNorm.

Under tensor parallelism a `RowParallelLinear`

(e.g. attention `o_proj`

) produces a per-rank partial sum that is all-reduced, and the result is then fed into a `GemmaRMSNorm`

that adds the residual and normalizes. flashinfer ships a kernel that fuses all-reduce + residual-add + RMSNorm into a single launch; this helper drives it directly (no torch.compile pass) for models that run eager.

Scope: attention output only, no quantization. When the flashinfer fast path is not applicable (TP==1, flashinfer/NVSwitch unavailable, unsupported dtype, or an oversize batch) it falls back to `all_reduce`
...
is the per-rank *partial* (un-reduced) output of a row-parallel linear; `norm`

is the GemmaRMSNorm applied right after. Returns `(normed_output, new_residual)`
