...
, and other models using Qwen3 ViT). - Dynamic scaling is not compatible with ViT full CUDA graphs.
...
- On ROCm, packed variable-length image and video batches are supported by AITER's native varlen FP8 attention.
...
support on gfx942 (MI300 series) or gfx950 (MI350 series).

## Usage[¶](https://docs.vllm.ai#usage)

Enable FP8 ViT attention by passing `--mm-encoder-attn-dtype fp8`
...
## Calibrate-Once, Reuse Workflow (Recommended)[¶](https://docs.vllm.ai#calibrate-once-reuse-workflow-recommended)
...
) to leave headroom against activation outliers not present in the calibration set. The default has been validated to generalize across datasets (e.g. VisionArena-Chat calibration maintains BF16 accuracy on ChartQA).
...
**Core cuDNN attention kernel** (PyTorch profiler, `cudnn_generated_fort_native_sdpa_sm100_flash_fprop`
...
**Complete AITER attention call on MI300X** (BF16 input, 16 heads, head_dim=72; FP8 includes Q/K/V quantization):
