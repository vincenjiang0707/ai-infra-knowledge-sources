source: https://docs.vllm.ai/en/latest/api/vllm/lora/request/
lastmod: 2026-09-23
...
-
[__eq__](#vllm.lora.request.LoRARequest.__eq__)

– Overrides the equality method to compare LoRARequest

-
[__hash__](#vllm.lora.request.LoRARequest.__hash__)
...
Whether this adapter's MoE weights are stored in the 3D fused `gate_up_proj`

/ `down_proj`

layout (one fused tensor per layer) or the 2D per-expert split layout (separate `gate_proj`

/ `up_proj`

/ `down_proj`

tensors per expert). Only consulted when the engine is started with `enable_mixed_moe_lora_format=True`

; otherwise it is ignored and the on-disk format is inferred from the base model.
