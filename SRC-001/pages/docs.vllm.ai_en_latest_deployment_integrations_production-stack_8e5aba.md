...
**Upstream vLLM compatibility**– It wraps around upstream vLLM without modifying its code.**Ease of use**– Simplified deployment via Helm charts and observability through Grafana dashboards.**High performance**– Optimized for LLM workloads with features like multimodel support, model-aware and prefix-aware routing, fast vLLM bootstrapping, and KV cache offloading with[LMCache](https://github.com/LMCache/LMCache)(wired up in vLLM via`--kv-offloading-backend lmcache`

; see the[LMCache examples](https://github.com/vllm-project/vllm/tree/main/examples/disaggregated/lmcache)and[docs.lmcache.ai](https://docs.lmcache.ai)), among others.
...
## Pre-requisite[¶](https://docs.vllm.ai#pre-requisite)
...
## Deployment using vLLM production stack[¶](https://docs.vllm.ai#deployment-using-vllm-production-stack)
...
### Validate Installation[¶](https://docs.vllm.ai#validate-installation)
...
### Send a Query to the Stack[¶](https://docs.vllm.ai#send-a-query-to-the-stack)
...
### Uninstall[¶](https://docs.vllm.ai#uninstall)

To remove the deployment, run:

### (Advanced) Configuring vLLM production stack[¶](https://docs.vllm.ai#advanced-configuring-vllm-production-stack)
...
: Number of replicas.`replicaCount`

: Specifies the CPU and memory resource requests for the pod.`requestCPU`

and`requestMemory`

: Specifies the number of GPUs required.`requestGPU`

: Allocates persistent storage for the model.`pvcStorage`
